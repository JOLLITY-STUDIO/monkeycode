...
class Sprite {
     ...
     */
    renderSprites(dst, vram, cram) {
        const satBase = this.vdp.spriteTableAddr;
        // 收集所有可见精灵 (沿链表)
        const sprites = [];
        let link = 0;
        const maxSprites = types_1.SPRITE.MAX_COUNT;
        ...
        for (const sprite of sprites) {
            ...
            for (let ty = 0; ty < sprite.height; ty++) {
                for (let tx = 0; tx < sprite.width; tx++) {
                    ...
                    const dx = sx + tx * 8;
                    const dy = sy + ty * 8;
                    this.decoder.decodeToImageData(tileData, cram, sprite.palette ? 1 : 0, // FIXME: use sprite.palette directly
                    true, // Sprite: color 0 = 透明
                    dst, dx, dy, sprite.hFlip, sprite.vFlip);
                }
            }
        }
    }
