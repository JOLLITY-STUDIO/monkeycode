...
exports.RomDisplayQueue = void 0;
const RomProgram_1 = require("./RomProgram");
...
// RomDisplayQueue
// ============================================================
class RomDisplayQueue extends RomProgram_1.RomProgram {
    /** 处理过的命令计数 (调试) */
    cmdCount = 0;
    ...
    _cmdVramFill(readPtr) {
        ...
        for (let i = 0; i < countWords; i++) {
            ...
            else if (srcPtr >= types_1.ADDRESS_SPACE.RAM.start) {
    ...
    _cmdVramCopy(readPtr) {
        ...
        if (srcPtr >= types_1.ADDRESS_SPACE.RAM.start) {
    ...
    _cmdCramDma(readPtr) {
        ...
        if (srcPtr >= types_1.ADDRESS_SPACE.RAM.start && lenWords > 0) {
    ...
    //   writePtr[6] = D1;
    // ================================================================
    pushCramDmaCommand(srcRam, cramDest, countWords, extraParam) {
        let wPtr = this.ram.read16(DQ_WRITE);
        if (wPtr + 14 > DQ_LIMIT) {
            this.processDisplayQueue();
            wPtr = this.ram.read16(DQ_WRITE);
    ...
    // 在 TS 中, 我们使用回调数组。
    // ================================================================
    iterateTaskList(offset, stride, count, callbacks) {
    ...
    //   根据标志切换两个调色板源之间交替
    // ================================================================
    paletteFadeDriver(stateRam, // 指向 {flags, delay, ...} 结构的 RAM 地址
    palA, // 调色板 A 地址
    palB, // 调色板 B 地址
    slotIndex, // 显示列表槽位
    delayInit, // 延迟初始值
    ...
    //   } while (sVar1 != -1);
    // ================================================================
    sequenceMatch(romDataPtr, ramPatternAddr, count) {
...
exports.RomDisplayQueue = RomDisplayQueue;
