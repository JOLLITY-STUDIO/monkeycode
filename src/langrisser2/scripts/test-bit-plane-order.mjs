...
function decodeTileOrder1(tileIdx) {
...
function decodeTileOrder2(tileIdx) {
  const offset = tileIdx * 32;
  const pixels = new Uint8Array(64);
  for (let y = 0; y < 8; y++) {
    const rowOffset = offset + y * 4;
    ...
    for (let x = 0; x < 8; x++) {
      ...
      const b2 = (p2 >> bit) & 1;
      const b3 = (p3 >> bit) & 1;
      pixels[y * 8 + x] = b3 | (b2 << 1) | (b1 << 2) | (b0 << 3);
    }
  }
  return pixels;
}

function decodeTileOrder3(tileIdx) {
  const offset = tileIdx * 32;
  const pixels = new Uint8Array(64);
  for (let y = 0; y < 8; y++) {
    const rowOffset = offset + y * 4;
...
function decodeTileOrder4(tileIdx) {
  const offset = tileIdx * 32;
  const pixels = new Uint8Array(64);
  for (let y = 0; y < 8; y++) {
    const rowOffset = offset + y * 4;
...
function renderWithDecoder(decodeFunc, label) {
...
console.log('=== 测试不同位平面顺序 ===');
console.log('');

const results = [];
results.push({ name: 'Order1: b0+b1*2+b2*4+b3*8, bit=7-x', colors: renderWithDecoder(decodeTileOrder1, 'order1') });
results.push({ name: 'Order2: b3+b2*2+b1*4+b0*8, bit=7-x', colors: renderWithDecoder(decodeTileOrder2, 'order2') });
results.push({ name: 'Order3: b0+b1*2+b2*4+b3*8, bit=x', colors: renderWithDecoder(decodeTileOrder3, 'order3') });
results.push({ name: 'Order4: b3+b2*2+b1*4+b0*8, bit=x', colors: renderWithDecoder(decodeTileOrder4, 'order4') });
...
for (const r of results) {
  console.log(`${r.name.split(':')[0]} | rgb(${r.colors.r},${r.colors.g},${r.colors.b})`);
}
