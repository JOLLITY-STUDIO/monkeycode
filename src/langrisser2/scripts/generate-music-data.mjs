...
const OUTPUT_DIR = path.join(ROOT, 'src/langrisser2/game/data/music');
...
function findMusicEnd(rom, startAddr) {
  let offset = startAddr;
  const maxSearch = 0x20000;
  
  let fdCount = 0;
  
  while (offset < startAddr + maxSearch && offset < rom.length) {
    if (offset + 1 < rom.length && rom[offset] === 0xFF && rom[offset + 1] === 0xFD) {
      fdCount++;
      if (fdCount >= 3) {
        return offset + 2;
      }
    } else if (offset + 1 < rom.length && rom[offset] === 0xFF && rom[offset + 1] === 0xFF) {
      if (offset + 3 < rom.length && rom[offset + 2] === 0xFF && rom[offset + 3] === 0xFF) {
        if (offset + 5 < rom.length && rom[offset + 4] === 0xFF && rom[offset + 5] === 0xFF) {
          return offset;
        }
      }
    }
  ...
  return startAddr + maxSearch;
}

const musicEntries = [];
for (let i = 0; i < MUSIC_COUNT; i++) {
  const tableOffset = MUSIC_TABLE_BASE + i * 2;
  if (tableOffset + 2 > rom.length) break;
  
  const offset = (rom[tableOffset + 1] << 8) | rom[tableOffset];
  
  if (offset === 0 || offset >= 0x80000) continue;
  
  const actualAddr = MUSIC_TABLE_BASE + offset;
  if (actualAddr >= rom.length) continue;
  
  const endAddr = findMusicEnd(rom, actualAddr);
  const size = Math.min(endAddr - actualAddr, 0x20000);
  
  if (size <= 0) continue;
  
  const data = rom.slice(actualAddr, actualAddr + size);
...
console.log(`Found ${musicEntries.length} valid music entries\n`);

const musicIndex = [];

for (const entry of musicEntries) {
  const fileName = `Music${entry.id.toString().padStart(2, '0')}Data.ts`;
  const filePath = path.join(OUTPUT_DIR, fileName);
  
  const base64Data = entry.data.toString('base64');
  
  const content = `/**
 * Music${entry.id.toString().padStart(2, '0')}Data.ts — BGM ${entry.id} 硬编码数据
 *
...
/** Music data size */
export const MUSIC_${entry.id.toString().padStart(2, '0')}_SIZE = ${entry.data.length};

/** Music data (YM2612 command stream, Base64 encoded) */
export const MUSIC_${entry.id.toString().padStart(2, '0')}_DATA_BASE64 = '${base64Data}';

/** Decode Base64 to Uint8Array */
export function decodeMusic${entry.id.toString().padStart(2, '0')}Data(): Uint8Array {
  const binary = atob(MUSIC_${entry.id.toString().padStart(2, '0')}_DATA_BASE64);
...
`;
  ...
  const compressedSize = Buffer.byteLength(base64Data, 'utf8');
  const ratio = ((compressedSize / entry.data.length) * 100).toFixed(1);
  console.log(`Music ${entry.id.toString().padStart(2, '0')}: ${entry.data.length} bytes (addr: 0x${entry.actualAddr.toString(16)}) → ${compressedSize} bytes (${ratio}%)`);
}
...
fs.writeFileSync(path.join(OUTPUT_DIR, '../MusicIndex.ts'), indexContent);

const barrelContent = `${musicIndex.map(e => `export * from './${e.file.replace('.ts', '')}';`).join('\n')}
`;

fs.writeFileSync(path.join(OUTPUT_DIR, 'index.ts'), barrelContent);

console.log(`\nGenerated ${musicEntries.length} music files + MusicIndex.ts (Base64 format)`);
