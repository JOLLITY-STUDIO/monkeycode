...
function decodeCRAM_BGR(i) {
  const word = (cram[i * 2 + 1] << 8) | cram[i * 2];
  const b = (word & 0x07) * 36;
  const g = ((word >> 3) & 0x07) * 36;
  const r = ((word >> 6) & 0x07) * 36;
  return { r, g, b };
}

function decodeCRAM_RGB_SCALE(i, scale) {
  const word = (cram[i * 2 + 1] << 8) | cram[i * 2];
...
function decodeCRAM_BGR_SCALE(i, scale) {
  const word = (cram[i * 2 + 1] << 8) | cram[i * 2];
  const b = (word & 0x07) * scale;
  const g = ((word >> 3) & 0x07) * scale;
  const r = ((word >> 6) & 0x07) * scale;
  return { r, g, b };
}
...
async function renderWithSettings(decodeFn, paletteNum, scale) {
  const palette = [];
  for (let i = 0; i < 64; i++) {
    palette.push(decodeFn(i, scale));
  }
  ...
  const modeName = decodeFn.name.includes('BGR') ? 'BGR' : 'RGB';
  const outputPath = join(DATA_DIR, 'output', `title-${modeName}-p${paletteNum}-s${scale}.png`);
  ...
  if (titleColors.length > 0) {
    ...
    return { mode: modeName, paletteNum, scale, avgR, avgG, avgB };
  }
  return { mode: modeName, paletteNum, scale, avgR: bgColor.r, avgG: bgColor.g, avgB: bgColor.b };
}

console.log('=== 测试颜色通道顺序 ===');
console.log('');
console.log('截图字母区域平均颜色: rgb(39,22,62)');
console.log('');

const results = [];
for (const decodeFn of [decodeCRAM_RGB_SCALE, decodeCRAM_BGR_SCALE]) {
  for (const paletteNum of [0, 1, 2, 3]) {
    for (const scale of [28, 32, 36]) {
      const result = await renderWithSettings(decodeFn, paletteNum, scale);
      results.push(result);
      const diff = Math.abs(result.avgR - 39) + Math.abs(result.avgG - 22) + Math.abs(result.avgB - 62);
      console.log(`${result.mode}, 调色板${result.paletteNum}, 缩放${result.scale}: rgb(${result.avgR},${result.avgG},${result.avgB}) 差异: ${diff}`);
    }
  }
}
...
console.log(`${best.mode}, 调色板${best.paletteNum}, 缩放${best.scale}: rgb(${best.avgR},${best.avgG},${best.avgB})`);
