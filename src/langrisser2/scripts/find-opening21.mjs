/**
 * 分析 0xC92C (默认任务函数) 如何触发开场动画
 * 同时完整反汇编 0x1593C (0xFF 命令处理器)
 */
import fs from 'fs';
...
function disasm(addr, length, label) {
  ...
  while (pc < end) {
    ...
    else if ((w & 0xF1F8) === 0x0C00) { mn = `CMPI.B #0x${(rw(pc+2)&0xFF).toString(16)}, D${(w>>9)&7}`; size = 4; }
    ...
    else if ((w & 0xF1C0) === 0x2070) { mn = `MOVEA.L (A${w&7}, D${(w>>9)&7}.W), A${(w>>9)&7}`; }
    else if ((w & 0xF1C0) === 0xD0C0) { mn = `ADD.W D${w&7}, D${(w>>9)&7}`; }
...
// 1. 反汇编 0xC92C (默认任务函数)
disasm(0xC92C, 128, '0xC92C 默认任务函数');

// 2. 反汇编 0x1593C (0xFF 命令处理器)
disasm(0x1593C, 96, '0x1593C (0xFF 命令处理器)');

// 3. 反汇编 0xCA00 附近 (开场动画可能的入口)
disasm(0xC9A0, 128, '0xC9A0');

// 4. 查看 0x184308 处的脚本数据 (scenarioIndex=1)
console.log('\n--- 0x184308 脚本数据 (scenarioIndex=1) ---');
for (let i = 0; i < 128; i += 16) {
  let hex = '';
  for (let j = 0; j < 16; j++) hex += rb(0x184308 + i + j).toString(16).padStart(2, '0') + ' ';
  console.log(`  0x${(0x184308 + i).toString(16)}: ${hex}`);
}
