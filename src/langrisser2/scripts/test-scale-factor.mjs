...
async function renderWithScale(scale) {
  ...
  const outputPath = join(DATA_DIR, 'output', `title-scale${scale}.png`);
...
const results = [];
for (const scale of [28, 32, 36, 40, 42]) {
  ...
  console.log(`缩放${scale}: rgb(${result.avgR},${result.avgG},${result.avgB}) 差异: ${diff}`);
}
