...
lctx.fillText('Langrisser II - Title Screen (Plane A Only)', 10, 20);
lctx.drawImage(canvas, 10, 60);

writeFileSync(join(DATA_DIR, 'output/title-plane-a-only.png'), labeled.toBuffer('image/png'));
console.log('✅ 输出: title-plane-a-only.png');
...
console.log('');
console.log(`Plane A渲染字母区域: rgb(${Math.round(r/count)},${Math.round(g/count)},${Math.round(b/count)})`);
