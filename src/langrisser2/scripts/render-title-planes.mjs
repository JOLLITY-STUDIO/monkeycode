...
const OUT_PATH = join(ROOT, 'src/langrisser2/20260713/output/title-planes.png');
...
function readNameTable(planeBase, row, col, swapBytes) {
  const offset = planeBase + (row * SCREEN_WIDTH + col) * 2;
  if (offset + 2 > vram.length) return 0;
  if (swapBytes) return (vram[offset + 1] << 8) | vram[offset];
...
function renderPlane(planeBase, swapBytes, canvasX, canvasY) {
  for (let row = 0; row < SCREEN_HEIGHT; row++) {
    for (let col = 0; col < SCREEN_WIDTH; col++) {
      const word = readNameTable(planeBase, row, col, swapBytes);
...
// Top-left: Plane A normal
renderPlane(PLANE_A_BASE, false, 0, 0);
// Top-right: Plane A byte-swapped
renderPlane(PLANE_A_BASE, true, SCREEN_WIDTH * 8, 0);
// Bottom-left: Plane B normal
renderPlane(PLANE_B_BASE, false, 0, SCREEN_HEIGHT * 8);
// Bottom-right: Plane B byte-swapped
renderPlane(PLANE_B_BASE, true, SCREEN_WIDTH * 8, SCREEN_HEIGHT * 8);

ctx.fillStyle = 'yellow';
ctx.font = '12px monospace';
ctx.fillText('Plane A normal', 10, 20);
ctx.fillText('Plane A swapped', SCREEN_WIDTH * 8 * SCALE + 10, 20);
ctx.fillText('Plane B normal', 10, SCREEN_HEIGHT * 8 * SCALE + 20);
ctx.fillText('Plane B swapped', SCREEN_WIDTH * 8 * SCALE + 10, SCREEN_HEIGHT * 8 * SCALE + 20);

import('fs').then(fs => fs.writeFileSync(OUT_PATH, canvas.toBuffer('image/png')));
console.log('Plane renders saved to:', OUT_PATH);
