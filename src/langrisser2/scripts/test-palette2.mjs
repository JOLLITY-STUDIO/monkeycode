...
function renderPlane(ntBase, label, forcePalette = null) {
  let entryCount = 0;
  for (let row = 0; row < ROWS; row++) {
    for (let col = 0; col < COLUMNS; col++) {
      const ntOffset = ntBase + row * 64 + col * 2;
      ...
      if (tileIdx !== 0) {
        entryCount++;
        const paletteIdx = forcePalette !== null ? forcePalette : (entry >> 13) & 3;
...
renderPlane(PLANE_B_BASE, 'Plane B (强制调色板2)', 2);

ctx.putImageData(imgData, 0, 0);

const outputPath = join(DATA_DIR, 'output', 'title-plane-b-palette2.png');
