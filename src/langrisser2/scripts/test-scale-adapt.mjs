...
async function renderWithScale(scale) {
  ...
  console.log(`缩放${scale}: 背景色 rgb(${bgColor.r},${bgColor.g},${bgColor.b})`);
  ...
  const outputPath = join(DATA_DIR, 'output', `title-be-scale${scale}.png`);
  ...
  return { scale, avgR, avgG, avgB };
}

console.log('=== 测试不同缩放因子 (大端序VRAM) ===');
console.log('');
...
const results = [];
for (const scale of [14, 18, 22, 26, 28, 30, 32]) {
  const result = await renderWithScale(scale);
  results.push(result);
}
...
for (const result of results) {
  ...
  console.log(`缩放${result.scale}: rgb(${result.avgR},${result.avgG},${result.avgB}) 差异: ${diff}`);
}
...
console.log(`缩放${best.scale}: rgb(${best.avgR},${best.avgG},${best.avgB})`);
