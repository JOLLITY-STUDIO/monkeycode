...
drawTile(tileRowMajor, 5, 5, 'Row Major (8B/plane)');
drawTile(tileRowAlt, tileSize + 10, 5, 'Row Alt (4B/row)');

const OUT_PATH = join(DATA_DIR, 'output/tile-decode-compare2.png');
writeFileSync(OUT_PATH, canvas.toBuffer('image/png'));
console.log(`输出: ${OUT_PATH}`);

console.log('');
console.log(`=== Tile ${testTileIdx} 原始数据 ===`);
...
console.log('');
console.log('=== 两种解码方式的像素值对比 ===');
console.log('位置 | Row Major | Row Alt');
for (let y = 0; y < 8; y++) {
  let rowStr = '';
  for (let x = 0; x < 8; x++) {
    const idx = y * 8 + x;
    const rm = tileRowMajor[idx];
    const ra = tileRowAlt[idx];
    if (rm !== 0 || ra !== 0) {
      rowStr += `(${x},${y}):${rm}/${ra} `;
    }
  }
