...
async function renderWithEndian(bigEndian) {
  const bgColor = palette[4];
  console.log(`${bigEndian ? '大端序' : '小端序'}: 背景色 rgb(${bgColor.r},${bgColor.g},${bgColor.b})`);
  ...
  for (let row = 0; row < ROWS; row++) {
    for (let col = 0; col < COLUMNS; col++) {
      ...
      const entry = bigEndian ? (lo << 8) | hi : (hi << 8) | lo;
  ...
  const outputPath = join(DATA_DIR, 'output', `title-vram-${bigEndian ? 'be' : 'le'}.png`);
  ...
  return { bigEndian, avgR, avgG, avgB };
}

console.log('=== 测试VRAM字节序 ===');
console.log('');
console.log('截图字母区域平均颜色: rgb(39,22,62)');
console.log('');

const leResult = await renderWithEndian(false);
const beResult = await renderWithEndian(true);

console.log('');
console.log(`小端序: rgb(${leResult.avgR},${leResult.avgG},${leResult.avgB})`);
console.log(`大端序: rgb(${beResult.avgR},${beResult.avgG},${beResult.avgB})`);
