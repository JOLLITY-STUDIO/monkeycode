...
function decodeCRAM_4bit(i) {
  const word = (cram[i * 2 + 1] << 8) | cram[i * 2];
  const r = ((word >> 0) & 0x0F) * 17;
  const g = ((word >> 4) & 0x0F) * 17;
  const b = ((word >> 8) & 0x0F) * 17;
  return { r, g, b };
}

function decodeCRAM_4bit_shifted(i) {
  const word = (cram[i * 2 + 1] << 8) | cram[i * 2];
...
console.log(`  3位: rgb(${decodeCRAM_3bit(4).r},${decodeCRAM_3bit(4).g},${decodeCRAM_3bit(4).b})`);
console.log(`  4位: rgb(${decodeCRAM_4bit(4).r},${decodeCRAM_4bit(4).g},${decodeCRAM_4bit(4).b})`);
console.log(`  4位shifted: rgb(${decodeCRAM_4bit_shifted(4).r},${decodeCRAM_4bit_shifted(4).g},${decodeCRAM_4bit_shifted(4).b})`);

console.log('');
console.log('调色板3 (索引48-63):');
for (let i = 0; i < 16; i++) {
  const idx = 48 + i;
  const c3 = decodeCRAM_3bit(idx);
  const c4 = decodeCRAM_4bit(idx);
  const c4s = decodeCRAM_4bit_shifted(idx);
  console.log(`  [${i}]: 3bit=rgb(${c3.r},${c3.g},${c3.b}) | 4bit=rgb(${c4.r},${c4.g},${c4.b}) | 4bitS=rgb(${c4s.r},${c4s.g},${c4s.b})`);
}

console.log('');
console.log('=== 使用不同CRAM解码方式渲染对比 ===');

function renderScreen(cramDecoder, label) {
  const palette = [];
  for (let i = 0; i < 64; i++) {
    palette.push(cramDecoder(i));
  }
...
renderScreen(decodeCRAM_3bit, 'cram-3bit');
renderScreen(decodeCRAM_4bit, 'cram-4bit');
renderScreen(decodeCRAM_4bit_shifted, 'cram-4bit-shifted');
