...
import { RomProgram } from './game/rom/RomProgram';
import { TITLE_CRAM, TITLE_NAMETABLE_A, TITLE_NAMETABLE_B } from './game/data/TitleScreenData';
import { DISPLAY, RESOURCES, ROM_REGIONS } from './game/core/types';
...
function hexDump(label: string, bytes: Uint8Array, offset: number = 0, len: number = 64): void {
  let out = '';
...
function loadRomBuffer(): ArrayBuffer {
...
function investigatePointerTable(rom: RomLoader): void {
  const TABLE = RESOURCES.POINTER_TABLE_ROM; // 0x0B0000
...
function loadTitleData(rom: RomLoader, vdp: VdpChip): void {
  const ram = new Ram();
  const prog = new RomProgram(rom.getData(), ram, vdp);
  ...
  const colors: number[] = [];
  for (let i = 0; i < 64; i++) {
    const lo = TITLE_CRAM[i * 2];
    const hi = TITLE_CRAM[i * 2 + 1];
  ...
  for (const resId of TITLE_RESOURCES) {
    ...
    if (size > 0) {
      ...
      let nz = 0; for (let i = 0; i < Math.min(100, bytes.length); i++) if (bytes[i] !== 0) nz++;
  ...
  for (const resId of TITLE_RESOURCES) {
    const index = resId & 0x0FFF;
    const offset16 = rom.read16(RESOURCES.POINTER_TABLE_ROM + index * 2);
    const resourceAddr = (RESOURCES.POINTER_TABLE_ROM + offset16) & 0x1FFFFF;
  ...
  // 加载 nametable
  vdp.vram.writeBytes(0xC000, TITLE_NAMETABLE_A);
  vdp.vram.writeBytes(0xE000, TITLE_NAMETABLE_B);
}

function dumpAndSave(vdp: VdpChip, renderer: Renderer): void {
  console.log('\n=== VDP 状态 ===');
  ...
  for (let i = 0; i < fb.length; i += 4) {
    if (fb[i] !== bg.r || fb[i + 1] !== bg.g || fb[i + 2] !== bg.b) nonBg++;
  }
...
async function main() {
  const rom = new RomLoader(loadRomBuffer());
  console.log('ROM 加载:', rom.size, 'bytes');

  const vdp = new VdpChip();
  const renderer = new Renderer(vdp);

  loadTitleData(rom, vdp);

  // 渲染
  renderer.renderFrame(fakeCtx as unknown as CanvasRenderingContext2D);
