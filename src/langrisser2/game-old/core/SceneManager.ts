...
export class SceneManager {
  ...
  /** 排队切换时是否保留 VDP 状态 */
  private _nextNoReset: boolean = false;
   ...
   * @param scene   目标场景实例
   * @param noReset 保留 VDP 状态 (VRAM/CRAM/VSRAM/寄存器)不重置
   */
  switchTo(phase: GamePhase, scene: Scene, noReset: boolean = false): void {
    // 销毁当前场景
    if (this._current) {
    ...
    console.log(`[SceneManager] → ${GamePhase[phase]} (${scene.name})${noReset ? ' [preserve VDP]' : ''}`);
  }
   ...
   */
  scheduleSwitch(phase: GamePhase, scene: Scene, noReset: boolean = false): void {
    this._next = scene;
    this._nextPhase = phase;
    this._nextNoReset = noReset;
  }
  ...
  applyScheduled(): boolean {
    ...
    const noReset = this._nextNoReset;
    this._next = null;
    this._nextNoReset = false;
    this.switchTo(this._nextPhase, this._next, noReset);
    return true;
  }
