 ...
 *
 * Gens/Kega CRAM dump 格式 (与 MD 硬件端口一致):
 *   byte0 = B2 B1 B0 G2 G1 G0 R2 R1
 *   byte1 = R0 + 7位填充 (通常为 0)
 *   R = (byte0 & 0x03) | ((byte1 >> 7) & 1) << 2
 *   G = (byte0 >> 2) & 0x07
 *   B = (byte0 >> 5) & 0x07
 *
 * 作为 readCRAM 返回的 16-bit word:
 *   word = byte0 | (byte1 << 8)
...
export function cramToRGB(cramValue: number): { r: number; g: number; b: number } {
  const r = (((cramValue & 0x03) | ((cramValue >> 13) & 0x04)) & 0x07) * 36;
  const g = ((cramValue >> 2) & 0x07) * 36;
  const b = ((cramValue >> 5) & 0x07) * 36;
  return { r, g, b };
}
...
export class VDP {
  ...
  writeCRAM(index: number, value: number): void {
    ...
    const g = (value >> 2) & 7;
    const r01 = value & 3;
    const r2 = (value & 4) << 5;  // R bit2 -> byte1 bit7
    this.cram[off] = (b << 5) | (g << 2) | r01;
    this.cram[off + 1] = r2 & 0x80;
  }
