...
export class Plane {
   ...
   */
  static parseEntry(word: number): PlaneEntry {
    return {
  ...
  private renderPlane(
    ...
    const tilesPerRow = Math.ceil((DISPLAY.WIDTH + startPixelX) / 8) + 1;
    const tilesPerCol = Math.ceil((DISPLAY.HEIGHT + startPixelY) / 8) + 1;
