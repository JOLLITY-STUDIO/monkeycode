...
import {
  ROM_RANGE, ROM_REGIONS, InterruptVectors,
  VDP_VRAM_LAYOUT,
} from './types';
...
export class RomLoader {
  ...
  load(buffer: ArrayBuffer): void {
    if (buffer.byteLength !== ROM_RANGE.size) {
      throw new Error(
        `ROM 大小不正确: 期望 ${ROM_RANGE.size} (${ROM_RANGE.size/1024}KB), ` +
  ...
  /** 中断向量表 ($000-$0FF) */
  readInterruptVectors(): InterruptVectors {
    return {
   ...
   */
  readVdpInitData(): Array<{ isReg: boolean; reg?: number; value: number }> {
    const bytes = this.readVdpInitRegs();
    const result: Array<{ isReg: boolean; reg?: number; value: number }> = [];
   ...
   */
  readResourcePointer(resourceId: number): number {
    const ptrAddr = ROM_REGIONS.resourcePtr.start + resourceId * 2;
    const offset = this.read16(ptrAddr);
    return ROM_REGIONS.resourcePtr.start + offset;
  }
