...
import { VdpPorts } from '../vdp/VdpPorts';
import { Lzss } from '../resource/Lzss';
import { NibbleRle } from '../resource/NibbleRle';
import { Bitplane } from '../resource/Bitplane';
import { DISPLAY, VDP_PORTS, VDP_REG, ADDRESS_SPACE, RESOURCES, CompressType } from '../core/types';
...
export interface CpuRegs {
  ...
  sr: number;   // 状态寄存器 (高字节 = 系统字节, 低字节 = CCR)
  pc: number;   // 程序计数器 (仅调试用)
}
...
export class RomProgram {
  ...
  constructor(romData: Uint8Array, ram: Ram, vdp: VdpChip) {
    ...
    this.lzss = new Lzss();
    this.nibbleRle = new NibbleRle();
    this.bitplane = new Bitplane();
  }
  ...
  /** 获取 CCR 字节 (SR 低字节) */
  get ccr(): number { return this.reg.sr & 0xFF; }

  /** 设置完整 CCR */
  set ccr(v: number) { this.reg.sr = (this.reg.sr & 0xFF00) | (v & 0xFF); }

  /** 获取/设置单个标志 */
  flagC(): boolean { return ((this.reg.sr >> CCR.C) & 1) !== 0; }
  flagV(): boolean { return ((this.reg.sr >> CCR.V) & 1) !== 0; }
  flagZ(): boolean { return ((this.reg.sr >> CCR.Z) & 1) !== 0; }
  flagN(): boolean { return ((this.reg.sr >> CCR.N) & 1) !== 0; }
  flagX(): boolean { return ((this.reg.sr >> CCR.X) & 1) !== 0; }

  setC(v: boolean) { this.setFlag(CCR.C, v); }
  setV(v: boolean) { this.setFlag(CCR.V, v); }
  setZ(v: boolean) { this.setFlag(CCR.Z, v); }
  setN(v: boolean) { this.setFlag(CCR.N, v); }
  setX(v: boolean) { this.setFlag(CCR.X, v); }

  private setFlag(bit: number, v: boolean) {
    if (v) this.reg.sr |= (1 << bit);
    else this.reg.sr &= ~(1 << bit);
  }

  /** 根据值设置 NZ 标志 (负/零) */
  setNZ8(v: number): void { this.setZ((v & 0xFF) === 0); this.setN((v & 0x80) !== 0); }
  setNZ16(v: number): void { this.setZ((v & 0xFFFF) === 0); this.setN((v & 0x8000) !== 0); }
  setNZ32(v: number): void { this.setZ(v === 0); this.setN((v & 0x80000000) !== 0); }

  /** 根据值设置 C/X 标志 */
  setCX8(v: number, carry: boolean): void { this.setC(carry); this.setX(carry); }
  setCX16(v: number, carry: boolean): void { this.setC(carry); this.setX(carry); }
  ...
  private _vdpStatus: number = 0;
  /** VDP 寄存器 1 缓存 (0xFF81A9) */
  private _vdpReg1Cache: number = 0;

  /** 写 VDP 寄存器 */
  vdpWriteReg(regNum: number, value: number): void {
    this.vdp.writeRegister(regNum & 0x1F, value & 0xFF);
    if ((regNum & 0x1F) === 1) this._vdpReg1Cache = value & 0xFF;
  }

  /** 设置 VRAM 写地址 (CD=01, 通过两阶段 CTRL 写) */
  vdpSetVramWrite(addr: number): void {
    this.vdp.ports.write(VDP_PORTS.CTRL, 0x4000 | (addr & 0x3FFF));
    this.vdp.ports.write(VDP_PORTS.CTRL, (addr >> 14) & 0x07);
  }

  /** 设置 VRAM 读地址 (CD=00) */
  vdpSetVramRead(addr: number): void {
    this.vdp.ports.write(VDP_PORTS.CTRL, addr & 0x3FFF);
    this.vdp.ports.write(VDP_PORTS.CTRL, (addr >> 14) & 0x07);
  }

  /** 写 VRAM Word (通过 DATA 端口, 自动递增) */
  vdpWriteVram(value: number): void {
    this.vdp.ports.write(VDP_PORTS.DATA, value & 0xFFFF);
  }

  /** 读 VRAM Word (通过 DATA 端口, 自动递增) */
  vdpReadVram(): number {
    return this.vdp.ports.read(VDP_PORTS.DATA);
  }

  /** 写 VRAM Word 到指定地址 (两步: 设地址 + 写数据) */
  vdpWriteVramAt(addr: number, value: number): void {
    this.vdpSetVramWrite(addr);
    this.vdpWriteVram(value);
  }

  /** 批量写 VRAM 从 RAM 缓冲区 (对应 0xFFF5/0xFFF6 命令) */
  vdpWriteVramBlock(ramSrc: number, countWords: number): void {
    const offset = ramSrc - ADDRESS_SPACE.RAM.start;
  ...
  vdpReadVramBlock(ramDst: number, countWords: number): void {
    for (let i = 0; i < countWords; i++) {
  ...
  vdpSetCramWrite(addr: number): void {
    this.vdp.ports.write(VDP_PORTS.CTRL, 0xC000 | (addr & 0x7E));
    this.vdp.ports.write(VDP_PORTS.CTRL, (addr >> 8) & 0xFF);
  }

  /** 写 CRAM Word */
  vdpWriteCram(value: number): void {
    this.vdp.ports.write(VDP_PORTS.DATA, value & 0xFFFF);
  }

  /** 批量写 CRAM 从 RAM 缓冲区 (对应 0xFFFA 命令) */
  vdpWriteCramBlock(ramSrc: number, countWords: number): void {
    for (let i = 0; i < countWords; i++) {
      const word = this.ram.read16(ramSrc + i * 2);
  ...
  vdpSetVsramWrite(addr: number): void {
    this.vdp.ports.write(VDP_PORTS.CTRL, 0x4000 | 0x0010 | (addr & 0x7F));
    this.vdp.ports.write(VDP_PORTS.CTRL, (addr >> 8) & 0xFF);
  }

  /** 写 VSRAM Word */
  vdpWriteVsram(value: number): void {
    this.vdp.ports.write(VDP_PORTS.DATA, value & 0xFFFF);
  }

  /** 设置 VRAM Fill 模式 (DMA fill, 对应 0xFFFC/0xFFFD 命令中的 $8F00 系列地址写) */
  vdpStartVramFill(value: number, countWords: number): void {
  ...
  vdpDmaRomToVram(romSrc: number, lenWords: number, vramDest: number): void {
  ...
  vdpDmaRamToVram(ramSrc: number, lenWords: number, vramDest: number): void {
    this.vdpSetVramWrite(vramDest);
    for (let i = 0; i < lenWords; i++) {
  ...
  vdpDmaRamToCram(ramSrc: number, countWords: number, cramDest: number): void {
  ...
  vdpDmaRamToVsram(ramSrc: number, countWords: number): void {
    for (let i = 0; i < countWords; i++) {
   ...
   */
  vdpWriteVramBytes(vramAddr: number, bytes: Uint8Array): void {
   ...
   */
  vdpFlushDecompressBuffer(decompressedSize: number, vramDest: number): void {
    this.vdpSetVramWrite(vramDest);
    const offset = RESOURCES.DECOMPRESS_RAM - ADDRESS_SPACE.RAM.start;
    for (let i = 0; i < decompressedSize; i += 2) {
      const word = this.ram.read16(RESOURCES.DECOMPRESS_RAM + i);
      this.vdpWriteVram(word);
    }
  }
  ...
  /** 获取缓存的 VDP 寄存器 1 值 */
  get vdpReg1Cache(): number { return this._vdpReg1Cache; }
  set vdpReg1Cache(v: number) { this._vdpReg1Cache = v & 0xFF; }
   ...
   * @returns 解压后的字节数
   */
  decompressResource(resId: number, destAddr: number = RESOURCES.DECOMPRESS_RAM): number {
    // $8001 等资源 ID 的高位是资源类型标记 (如 $8000=图形, $A000=音频)
    // 真实索引 = resId & $0FFF (12-bit, 最多支持 4096 个资源条目)
    const index = resId & 0x0FFF;
    const ptr = this.rom32(RESOURCES.POINTER_TABLE_ROM + index * 4);
    if (ptr === 0) {
    ...
    const type = typeByte as CompressType;
    ...
    const romReader = {
      read8: (addr: number) => this.romData[addr & 0x1FFFFF] ?? 0,
      read16: (addr: number) => {
        const a = addr & 0x1FFFFF;
        return ((this.romData[a] ?? 0) << 8) | (this.romData[a + 1] ?? 0);
      },
    ...
    switch (type) {
      case CompressType.LZSS:
        ...
        outSize = Lzss.decompress(romReader as any, streamAddr, this.ram.data, ramOffset, decompSize);
        break;
      case CompressType.NIBBLE_RLE:
        outSize = NibbleRle.decompress(romReader as any, streamAddr, this.ram.data, ramOffset);
        break;
      case CompressType.BITPLANE:
        outSize = Bitplane.decompress(romReader as any, streamAddr, this.ram.data, ramOffset);
        break;
    }
    return outSize;
  }
  ...
  /** 访问命名 RAM 变量 (自动处理大端序) */
  readRamVar(addr: number, size: 1 | 2 | 4 = 2): number {
    switch (size) {
  ...
  writeRamVar(addr: number, value: number, size: 1 | 2 | 4 = 2): void {
    switch (size) {
      case 1: this.ram.write8(addr, value); break;
      case 2: this.ram.write16(addr, value); break;
      case 4: this.ram.write32(addr, value); break;
    }
  }
