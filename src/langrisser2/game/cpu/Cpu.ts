/**
 * 68K CPU 解释器 — 骨架 + Boot 必须指令
 *
 * 当前状态: 第一步 — 寄存器、取指循环、基础指令
 */
...
export class Cpu {
  ...
  private _sr: number = SR_S; // 从 Supervisor 模式启动
  ...
  // SR / CCR 访问
  // ============================================================
  get sr(): number { return this._sr; }
  set sr(v: number) { this._sr = v & 0xFFFF; }
  get ccr(): number { return this._sr & 0x1F; }
  set ccr(v: number) { this._sr = (this._sr & 0xFFE0) | (v & 0x1F); }
  /** Supervisor 模式? */
  get supervisor(): boolean { return !!(this._sr & SR_S); }
  // 标志位读写
  getFlag(bit: number): boolean { return !!(this._sr & (1 << bit)); }
  setFlag(bit: number, val: boolean): void {
    if (val) this._sr |= (1 << bit); else this._sr &= ~(1 << bit);
  }
  // NZ 便捷设置
  private setNZ(v: number, size: number): void {
    const mask = (1 << (size * 8)) - 1;
    v &= mask;
    this.setFlag(FLAG.N, (v >> (size * 8 - 1)) !== 0);
    this.setFlag(FLAG.Z, v === 0);
  }
  private setVCAdd(dst: number, src: number, result: number, size: number): void {
    const mask = (1 << (size * 8)) - 1;
    dst &= mask; src &= mask; result &= mask;
    const bit = size * 8 - 1;
    // V: overflow when both operands have same sign and result has opposite sign
    this.setFlag(FLAG.V, (((~(dst ^ src)) & (dst ^ result)) >>> bit) & 1 ? true : false);
    // C/X: carry out of MSB (unsigned overflow)
    this.setFlag(FLAG.C, (dst + src) > mask);
    this.setFlag(FLAG.X, this.getFlag(FLAG.C));
  }
  private setVCSub(dst: number, src: number, result: number, size: number): void {
    const mask = (1 << (size * 8)) - 1;
    dst &= mask; src &= mask; result &= mask;
    const bit = size * 8 - 1;
    ...
    this.setFlag(FLAG.C, dst < src);
    this.setFlag(FLAG.X, this.getFlag(FLAG.C));
  }
  // ============================================================
  // 存储器读
  // ============================================================
  private read8(addr: number): number { return this.mem.read8(addr & 0xFFFFFF); }
  private read16(addr: number): number { return this.mem.read16(addr & 0xFFFFFF); }
  private read32(addr: number): number { return this.mem.read32(addr & 0xFFFFFF); }
  private write8(addr: number, v: number): void { this.mem.write8(addr & 0xFFFFFF, v); }
  private write16(addr: number, v: number): void { this.mem.write16(addr & 0xFFFFFF, v); }
  private write32(addr: number, v: number): void { this.mem.write32(addr & 0xFFFFFF, v); }
  // ============================================================
  // 取指
  // ============================================================
  ...
  // 符号扩展
  // ============================================================
  private signExt8(v: number): number {
    return (v & 0x80) ? (v | 0xFFFFFF00) : (v & 0xFF);
  }
  private signExt16(v: number): number {
  ...
  private evalCondition(cc: number): boolean {
    switch (cc) {
      ...
      case 0x2: return !this.getFlag(FLAG.C) && !this.getFlag(FLAG.Z);
      case 0x3: return this.getFlag(FLAG.C) || this.getFlag(FLAG.Z);
      case 0x4: return !this.getFlag(FLAG.C);
      case 0x5: return this.getFlag(FLAG.C);
      case 0x6: return !this.getFlag(FLAG.Z);
      case 0x7: return this.getFlag(FLAG.Z);
      case 0x8: return !this.getFlag(FLAG.V);
      case 0x9: return this.getFlag(FLAG.V);
      case 0xA: return !this.getFlag(FLAG.N);
      case 0xB: return this.getFlag(FLAG.N);
      case 0xC: return this.getFlag(FLAG.N) === this.getFlag(FLAG.V);
      case 0xD: return this.getFlag(FLAG.N) !== this.getFlag(FLAG.V);
      case 0xE: return !this.getFlag(FLAG.Z) && (this.getFlag(FLAG.N) === this.getFlag(FLAG.V));
      case 0xF: return this.getFlag(FLAG.Z) || (this.getFlag(FLAG.N) !== this.getFlag(FLAG.V));
  ...
  // 逻辑指令标志
  // ============================================================
  private setVCLogic(_result: number, _size: number): void {
  ...
  // ============================================================
  private exec0xxx(opcode: number): void {
    const op = (opcode >> 9) & 0x7;
    const size = (opcode >> 6) & 0x3; // 0=byte, 1=word, 2=long
    const eaMode = opcode & 0x3F;
    switch (op) {
      case 0: this.immOp(eaMode, size, 'ORI'); break;
      case 1: this.immOp(eaMode, size, 'ANDI'); break;
      case 2: this.immOp(eaMode, size, 'SUBI'); break;
      case 3: this.immOp(eaMode, size, 'ADDI'); break;
      case 4:
        if ((opcode & 0x0100) === 0) this.bitOp(opcode, 'BTST');
        else this.execMOVEP(opcode);
        break;
      case 5: this.immOp(eaMode, size, 'EORI'); break;
      case 6: this.immOp(eaMode, size, 'CMPI'); break;
      case 7:
        this.bitOp(opcode, 'BSET'); break;
    }
  }
  private immOp(eaMode: number, size: number, op: string): void {
    let imm: number;
    if (size === 2) {
      imm = this.fetchLong();
  ...
  private updateImmFlags(eaMode: number, size: number, val: number, imm: number, r: number, op: string): void {
  ...
  private immEval(val: number, imm: number, op: string): number {
    switch (op) {
  ...
  private bitOp(opcode: number, op: string): void {
    ...
    const mask = 1 << bitNum;
    if (op === 'BTST') {
      this.setFlag(FLAG.Z, (val & mask) === 0);
    } else {
      let result: number;
      switch (op) {
        case 'BSET': result = val | mask; break;
        case 'BCLR': result = val & ~mask; break;
        case 'BCHG': result = val ^ mask; break;
        default: result = val;
      }
      if (isStatic) {
        this.setFlag(FLAG.Z, (val & mask) === 0);
  ...
  // ============================================================
  private execMOVE(opcode: number): void {
  ...
  // ============================================================
  private exec4xxx(opcode: number): void {
    // Special 4EXX single-word ops
    if ((opcode & 0xFFF0) === 0x4E70) {
      if (opcode === 0x4E71) { this.cycles = 4; return; } // NOP
      if (opcode === 0x4E73) { this.execRTE(); return; }
      if (opcode === 0x4E75) { this.execRTS(); return; }
      if (opcode === 0x4E76) { this.execTRAPV(); return; }
      if (opcode === 0x4E77) { this.execRTR(); return; }
      // 4E70 (RESET), 4E72 (STOP), 4E74 (RTD), etc.
      this.unimplemented('4E7x-other', opcode); return;
    }
    // JMP: 0x4EC0-0x4EFF, JSR: 0x4E80-0x4EBF
    if ((opcode & 0xFFC0) === 0x4EC0) { this.execJMP(opcode); return; }
    if ((opcode & 0xFFC0) === 0x4E80) { this.execJSR(opcode); return; }
    // LEA: 0100 RRR 111 MMM RRR → bits 8-6 = 111, bit 5 = 0
    if ((opcode & 0xF1C0) === 0x41C0) { this.execLEA(opcode); return; }
    // MOVEM: 0100_1D00_1S... 按 0xF780 掩码 (bit 11=D, bit 10=0, bits 9-8=0, bit 7=1)
    if ((opcode & 0xF780) === 0x4080) { this.execMOVEM(opcode); return; }
    // TRAP: 0100 1110 0100 xxxx
    if ((opcode & 0xFFF0) === 0x4E40) { this.execTRAP(opcode); return; }
    // LINK: 0100 1110 0101 0nnn
    if ((opcode & 0xFFF8) === 0x4E50) { this.execLINK(opcode); return; }
    // UNLK: 0100 1110 0101 1nnn
    if ((opcode & 0xFFF8) === 0x4E58) { this.execUNLK(opcode); return; }
    // MOVE An,USP / MOVE USP,An
    if ((opcode & 0xFFF0) === 0x4E60) { this.execMOVE_USP(opcode); return; }
    // CHK: 0100 RRR 110 ... (stub)
    if ((opcode & 0xF1C0) === 0x4180) { this.unimplemented('CHK', opcode); return; }

    // Fallthrough for sub=6, sub=7: also try PEA (0x4850-0x485F)
    if ((opcode & 0xFFC0) === 0x4850) { this.unimplemented('PEA-abs', opcode); return; }

    // CLR, NEG, NOT, etc. — sub = bits 11-9
    const sub = (opcode >> 9) & 0x7;
    const eaMode = opcode & 0x3F;
    switch (sub) {
      case 0: this.execNEGX(this.szFrom4x(opcode), eaMode); break;
      case 1: this.execCLR(this.szFrom4x(opcode), eaMode); break;
      case 2: this.execNEG(this.szFrom4x(opcode), eaMode); break;
      case 3: this.execNOT(this.szFrom4x(opcode), eaMode); break;
      case 4: // EXT / SWAP / NBCD / PEA
        if ((opcode & 0x01C0) === 0x0100) this.execEXT(opcode);
        else if ((opcode & 0xFFF8) === 0x4840) this.execSWAP(opcode);
        else this.unimplemented('4xxx-NBCD/PEA', opcode);
        break;
      case 5: // TST
        this.execTST(this.szFrom4x(opcode), eaMode); break;
      default:
  ...
  // 5xxx — ADDQ / SUBQ / Scc / DBcc
  // ============================================================
  private execADDQ_SUBQ(opcode: number): void {
  ...
  private addqByte(data: number, isSub: boolean, eaMode: number): void {
    const val = this.readEaByte(eaMode);
  ...
  private addqWord(data: number, isSub: boolean, eaMode: number): void {
    const val = this.readEaWord(eaMode);
    const result = isSub ? val - data : val + data;
  ...
  private addqLong(data: number, isSub: boolean, eaMode: number): void {
    const val = this.readEaLong(eaMode);
    const result = isSub ? val - data : val + data;
    this.writeEaLong(eaMode, result);
    this.setNZ(result, 4);
  ...
  // ADDQ/SUBQ 直接寄存器优化
  private addqReg(sz: number, reg: number, data: number, isSub: boolean): void {
    const mask = (1 << (sz * 8)) - 1;
  ...
  // Scc
  // ============================================================
  private execScc(opcode: number): void {
  ...
  // DBcc
  // ============================================================
  private execDBcc(opcode: number): void {
    const cc = (opcode >> 8) & 0x0F;
  ...
  // 6xxx — Bcc / BSR / BRA
  // ============================================================
  private execBcc(opcode: number): void {
    const cc = (opcode >> 8) & 0x0F;
    let disp: number;
    if (cc === 0) {
      // BRA (always)
      disp = opcode & 0xFF;
      if (disp === 0) disp = this.fetchWord();
      else disp = this.signExt8(disp);
  ...
  // 7xxx — MOVEQ
  // ============================================================
  private execMOVEQ(opcode: number): void {
  ...
  // 8xxx — OR
  // ============================================================
  private execOR(opcode: number): void {
    this.dyadicOp(opcode, 'OR');
  }
  // ============================================================
  // 9xxx — SUB / SUBA
  // ============================================================
  private execSUB(opcode: number): void {
    this.dyadicOp(opcode, 'SUB');
  }
  private execSUBA(opcode: number): void {
    this.dyadicAOp(opcode, 'SUB');
  }
  // ============================================================
  // Bxxx — CMP / EOR
  // ============================================================
  private execCMP_EOR(opcode: number): void {
    // bit 8: 0=CMP, 1=EOR
    if ((opcode >> 8) & 0x1) {
  ...
  private execCMPA(opcode: number): void {
    this.dyadicAOp(opcode, 'CMP');
  }
  // ============================================================
  // Cxxx — AND / EXG
  // ============================================================
  private execAND_EXG(opcode: number): void {
  ...
  // Dxxx — ADD / ADDA
  // ============================================================
  private execADD(opcode: number): void {
    this.dyadicOp(opcode, 'ADD');
  }
  private execADDA(opcode: number): void {
    this.dyadicAOp(opcode, 'ADD');
  }
  // ============================================================
  // 8xxx — DIVS (stub)
  // ============================================================
  private execDIVS(opcode: number): void {
  ...
  // 通用双操作数指令
  // ============================================================
  private dyadicOp(opcode: number, op: string): void {
  ...
  private dyadicAOp(opcode: number, op: string): void {
    const eaMode = opcode & 0x3F;
    const reg = (opcode >> 9) & 0x7;
  ...
  private dyadicEval(a: number, b: number, op: string, size: number): number {
    const mask = (1 << (size * 8)) - 1;
    a &= mask; b &= mask;
    let result: number;
    switch (op) {
  ...
  // ============================================================
  private execExxx(opcode: number): void {
  ...
  // MOVEP (stub)
  // ============================================================
  private execMOVEP(opcode: number): void {
  ...
  private execNEG(size: number, eaMode: number): void {
    ...
    this.writeEa(sizeEnum, eaMode, result);
    this.setNZ(result, sizeEnum);
    this.setFlag(FLAG.C, val !== 0);
    const msb = 1 << (sizeEnum * 8 - 1);
    this.setFlag(FLAG.V, (val & mask) === msb);
    this.setFlag(FLAG.X, this.getFlag(FLAG.C));
  }

  private execNOT(size: number, eaMode: number): void {
    const sizeEnum = size === 0 ? 1 : size === 1 ? 2 : 4;
    const mask = (1 << (sizeEnum * 8)) - 1;
    const val = this.readEa(sizeEnum, eaMode);
  ...
  private execEXT(opcode: number): void {
  ...
  private execTST(size: number, eaMode: number): void {
    const sizeEnum = size === 0 ? 1 : size === 1 ? 2 : 4;

    // [DEBUG] 从指令流窥探有效地址（不消费 PC，因为 readEa 后续会消费）
    const mode = this.eaMode(eaMode);
    const reg = this.eaReg(eaMode);
    let dbgAddr: number | null = null;
    if (mode === 7) {
      if (reg === 0) dbgAddr = this.read16(this.pc);          // (xxx).W
      else if (reg === 1) dbgAddr = this.read32(this.pc);     // (xxx).L
      else if (reg === 2) dbgAddr = (this.pc + this.signExt16(this.read16(this.pc))) & 0xFFFFFF; // (d16,PC)
      else if (reg === 4) dbgAddr = this.read32(this.pc);      // #imm
    }

    const val = this.readEa(sizeEnum, eaMode);
    if (dbgAddr !== null) {
      console.warn(
        `[Cpu] TST.${sizeEnum === 1 ? 'B' : sizeEnum === 2 ? 'W' : 'L'} @ $${
          (dbgAddr & 0xFFFFFF).toString(16)
        } = $${val.toString(16)} → Z=${val === 0 ? 1 : 0}`
      );
    }
  ...
  private execSWAP(opcode: number): void {
    const reg = opcode & 0x7;
  ...
  // MOVEM  — 寄存器批量加载/存储
  // ============================================================
  private execMOVEM(opcode: number): void {
      ...
      if (mode === 3) {
        ...
        for (let i = 0; i < 16; i++) {
          if (mask & (1 << i)) {
            ...
            if (i < 8) {
              if (sz) { this.d[i] = val; }
              else { this.d[i] = (this.d[i] & ~0xFFFF) | (val & 0xFFFF); }
            } else {
  ...
  // LINK / UNLK  — 栈帧操作
  // ============================================================
  private execLINK(opcode: number): void {
    const reg = opcode & 0x7;
  ...
  private execUNLK(opcode: number): void {
    const reg = opcode & 0x7;
    this.a[7] = this.a[reg];
  ...
  // TRAP  — 软中断
  // ============================================================
  private execTRAP(opcode: number): void {
  ...
  // ============================================================
  private execMOVE_USP(opcode: number): void {
    const reg = opcode & 0x7;
    if (opcode & 0x8) {
  ...
  // 按尺寸读/写内存辅助 (用于 MOVEM)
  // ============================================================
  private readSized(addr: number, size: number): number {
    return size === 4 ? this.read32(addr) : this.read16(addr);
  }
  private writeSized(addr: number, value: number, size: number): void {
    if (size === 4) { this.write32(addr, value); }
    else { this.write16(addr, value & 0xFFFF); }
  }

  /** szFrom4x: 从 4xxx 指令解码操作数尺寸 */
  private szFrom4x(opcode: number): number {
  ...
  // ============================================================
  private exception(vectorNum: number): void {
    ...
    console.warn(
      `[Cpu] 异常 #$${vectorNum.toString(16)} @ PC=$${faultPC.toString(16)} ` +
    ...
    if (handler === 0 || handler === 0xFFFFFFFF) {
      console.error(
        `[Cpu] 异常向量 $${vectorNum.toString(16).padStart(2, '0')} ` +
  ...
  // 异常/未实现处理
  // ============================================================
  private exceptionLineA(): void {
    this.exception(0x0A); // Line 1010 Emulator
  }
  private exceptionLineF(): void {
    this.exception(0x0B); // Line 1111 Emulator
  }
  private exceptionIllegal(): void {
    this.exception(0x04); // Illegal Instruction
  }
  private unimplemented(name: string, opcode: number): void {
  ...
  // EA 辅助: 解码 mode/reg
  // ============================================================
  private eaMode(ea: number): number { return (ea >> 3) & 0x7; }
  private eaReg(ea: number): number { return ea & 0x7; }

  // ============================================================
  // EA 大小分发 (readEa / writeEa)
  // ============================================================
  private readEa(size: number, ea: number): number {
    if (size === 1) return this.readEaByte(ea);
    if (size === 2) return this.readEaWord(ea);
    return this.readEaLong(ea);
  }
  private writeEa(size: number, ea: number, value: number): void {
    if (size === 1) this.writeEaByte(ea, value);
    else if (size === 2) this.writeEaWord(ea, value);
    else this.writeEaLong(ea, value);
  }

  // ============================================================
  // EA 字节读写 (readEaByte / writeEaByte)
  // ============================================================
  private readEaByte(ea: number): number {
    const mode = this.eaMode(ea);
    const reg = this.eaReg(ea);
    switch (mode) {
  ...
  private writeEaByte(ea: number, value: number): void {
    const mode = this.eaMode(ea);
    const reg = this.eaReg(ea);
    const v = value & 0xFF;
    switch (mode) {
      case 0: this.d[reg] = (this.d[reg] & ~0xFF) | v; break;
      case 2: this.write8(this.a[reg], v); break;
      case 3: this.write8(this.a[reg], v); this.a[reg] = (reg === 7 ? this.a[reg] + 2 : this.a[reg] + 1) >>> 0; break;
      case 4: this.a[reg] = (reg === 7 ? this.a[reg] - 2 : this.a[reg] - 1) >>> 0; this.write8(this.a[reg], v); break;
      case 5: {
        const disp = this.fetchWord();
  ...
  // ============================================================
  private readEaWord(ea: number): number {
    const mode = this.eaMode(ea);
    const reg = this.eaReg(ea);
    switch (mode) {
      ...
      case 7:
        switch (reg) {
          case 0: { const addr = this.fetchWord(); return this.read16(addr); }
          case 1: { const addr = this.fetchLong(); return this.read16(addr); }
          case 2: {
            const disp = this.fetchWord();
  ...
  private writeEaWord(ea: number, value: number): void {
    const mode = this.eaMode(ea);
    const reg = this.eaReg(ea);
    const v = value & 0xFFFF;
    switch (mode) {
      case 0: this.d[reg] = (this.d[reg] & ~0xFFFF) | v; break;
      case 1: this.a[reg] = (this.a[reg] & ~0xFFFF) | v; break;
      case 2: this.write16(this.a[reg], v); break;
      case 3: this.write16(this.a[reg], v); this.a[reg] += 2; break;
      case 4: this.a[reg] -= 2; this.write16(this.a[reg], v); break;
      case 5: {
        const disp = this.fetchWord();
      ...
      case 7:
        if (reg === 0) { const addr = this.fetchWord(); this.write16(addr, v); }
        else if (reg === 1) { const addr = this.fetchLong(); this.write16(addr, v); }
        break;
    }
  }
  ...
  // ============================================================
  private readEaLong(ea: number): number {
    const mode = this.eaMode(ea);
    const reg = this.eaReg(ea);
    switch (mode) {
      ...
      case 3: { const v = this.read32(this.a[reg]); this.a[reg] += 4; return v; }
      case 4: { this.a[reg] -= 4; return this.read32(this.a[reg]); }
      case 5: {
        const disp = this.fetchWord();
      ...
      case 7:
        switch (reg) {
          case 0: { const addr = this.fetchWord(); return this.read32(addr); }
          case 1: { const addr = this.fetchLong(); return this.read32(addr); }
          case 2: {
            const disp = this.fetchWord();
  ...
  private writeEaLong(ea: number, value: number): void {
    const mode = this.eaMode(ea);
    const reg = this.eaReg(ea);
    const v = value >>> 0;
    switch (mode) {
      case 0: this.d[reg] = v; break;
      case 1: this.a[reg] = v; break;
      case 2: this.write32(this.a[reg], v); break;
      case 3: this.write32(this.a[reg], v); this.a[reg] += 4; break;
      case 4: this.a[reg] -= 4; this.write32(this.a[reg], v); break;
      case 5: {
        const disp = this.fetchWord();
      ...
      case 7:
        if (reg === 0) { const addr = this.fetchWord(); this.write32(addr, v); }
        else if (reg === 1) { const addr = this.fetchLong(); this.write32(addr, v); }
        break;
    }
  }
  ...
  // EA 地址计算 (仅计算地址，不读写)
  // ============================================================
  private eaAddrLong(ea: number): number {
    const mode = this.eaMode(ea);
    const reg = this.eaReg(ea);
    switch (mode) {
  ...
  // MOVE 源操作数读取
  // ============================================================
  private readSrcByte(mode: number, reg: number): number {
    return this.readEaByte((mode << 3) | reg);
  }
  private readSrcWord(mode: number, reg: number): number {
    return this.readEaWord((mode << 3) | reg);
  }
  private readSrcLong(mode: number, reg: number): number {
  ...
  // MOVE 目标操作数写入
  // ============================================================
  private writeDstByte(mode: number, reg: number, value: number): void {
    this.writeEaByte((mode << 3) | reg, value);
  }
  private writeDstWord(mode: number, reg: number, value: number): void {
    this.writeEaWord((mode << 3) | reg, value);
  }
  private writeDstLong(mode: number, reg: number, value: number): void {
