...
/**
 * ROM静态单位模板 (0x1E=30字节, 位于关卡配置中)
 */
export const ROM_UNIT_TEMPLATE_FIELDS = [
 ...
 */
export function parseLevelUnits(data, levelIndex) {
  const ptrTableAddr = 0x18005E;
  if (ptrTableAddr + levelIndex * 4 + 3 > data.length) return [];

  const view = new DataView(data.buffer, data.byteOffset, data.byteLength);
  const configPtr = view.getUint32(ptrTableAddr + (levelIndex - 1) * 4, false) & 0xFFFFFF;
  if (configPtr < 0x200 || configPtr + 0x10 > data.length) return [];

  const unitListPtr = view.getUint32(configPtr + 0x0C, false) & 0xFFFFFF;
  if (unitListPtr < 0x200 || unitListPtr > data.length) return [];

  const units = [];
  let off = 0;
  while (unitListPtr + off + 0x12 <= data.length) {
    const addr = unitListPtr + off;
    const typeIndex = data[addr + 0x1B];
    const commanderId = data[addr + 0x1A];
    if (typeIndex === 0xFF || commanderId === 0xFF) break;
    if (typeIndex === 0 && commanderId === 0) break;

    const x = data[addr + 0x18];
    const y = data[addr + 0x19];

    units.push({
      index: units.length,
      romAddr: addr,
      typeIndex, commanderId,
      x, y,
      raw: new Uint8Array(data.buffer, data.byteOffset + addr, 0x1E),
    });
    off += 0x12;
  }
  return units;
...
export function parseLevelList(data) {
  ...
  for (let i = 0; i < 32; i++) {
    const ptr = view.getUint32(ptrTableAddr + i * 4, false) & 0xFFFFFF;
    if (ptr < 0x200 || ptr > data.length) continue;
    const w = view.getUint16(ptr, false);
    const h = view.getUint16(ptr + 2, false);
...
export function parseLevelMap(data, levelIndex) {
  const ptrTableAddr = 0x61CB0;
  const view = new DataView(data.buffer, data.byteOffset, data.byteLength);
  const ptr = view.getUint32(ptrTableAddr + levelIndex * 4, false) & 0xFFFFFF;
  if (ptr < 0x200 || ptr + 4 > data.length) return null;

  const width  = view.getUint16(ptr, false);
  const height = view.getUint16(ptr + 2, false);
  if (width > 128 || height > 128) return null;

  const tileData = ptr + 4;
  const tiles = [];
  for (let i = 0; i < width * height && tileData + i < data.length; i++) {
    tiles.push(data[tileData + i]);
  }
