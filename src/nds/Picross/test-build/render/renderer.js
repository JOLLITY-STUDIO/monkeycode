...
export class PicrossRenderer {
    ...
    drawTile(idx, x, y, w, h) {
        if (!this.tiles)
            return;
        const ctx = this.ctx;
        ctx.imageSmoothingEnabled = false;
        ctx.drawImage(this.tiles, idx * this.tileW, 0, this.tileW, this.tileH, x, y, w, h);
        ctx.imageSmoothingEnabled = true;
    }
    /** 绘制带立体倒角的矩形（NDS 风格：左上亮、右下暗） */
    drawBeveledRect(x, y, w, h, main, light, dark, bevel) {
        const ctx = this.ctx;
        // 主体
        ctx.fillStyle = main;
    ...
    layout(state) {
        ...
        const cell = Math.floor(Math.min((NDS_SCREEN_W - maxRow) / width, (NDS_SCREEN_H - maxCol) / height, this.opts.maxCell));
    ...
    drawCell(x, y, mark, state) {
        ...
        if (mark === "crossed")
            this.drawCross(px, py, cs);
    }
    redrawRowHint(nums, row, satisfied, state) {
        const ctx = this.ctx;
    ...
    redrawColHint(nums, col, satisfied, state) {
        const ctx = this.ctx;
