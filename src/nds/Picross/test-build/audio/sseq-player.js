...
const wavesIndex = wavesIndexData;
export class SseqPlayer {
    constructor() {
        this.ctx = null;
        this.master = null;
        this.buffers = new Map();
        this.wavesBytes = null;
        this.playing = false;
        this.playName = null;
        this.muted = false;
        this.volume = 0.5;
        ...
        this.onEnd = null;
        this.scheduledUntil = 0; // 当前已调度到的绝对时间（秒）
        this.startTime = 0;
        this.currentSong = null;
        this.noteIdx = []; // 每个轨道下一个待播放 note 的索引
    ...
    getBuffer(wkey) {
        if (!this.ctx || !this.wavesBytes)
            return null;
        if (this.buffers.has(wkey))
            return this.buffers.get(wkey);
    ...
    pickRegion(regs, key) {
        ...
        if (regs.length === 0)
            return null;
        // Range 类型（按音高 +i）
        if ('firstPitch' in regs[0] || regs[0].key !== undefined) {
            // Range
            for (const r of regs) {
                if (r.key === key)
                    return r;
        ...
        let last = -1;
        for (const r of regs) {
            const top = (_a = r.topKey) !== null && _a !== void 0 ? _a : 127;
            if (key <= top)
                return r;
            last = top;
        }
    ...
    /** 调度一个音符 */
    scheduleNote(track, note, startSec) {
        var _a, _b;
        ...
        src.buffer = buf;
        src.loop = ((_a = wavesIndex[reg.wkey]) === null || _a === void 0 ? void 0 : _a.loop) || false;
        ...
        const dur = (note.d * 60) / (((_b = this.currentSong) === null || _b === void 0 ? void 0 : _b.tempo) || 120) / 48;
    ...
    /** 调度一个时间窗口内的所有 note */
    scheduleWindow(fromTick, toTick, startSec) {
        if (!this.currentSong)
            return;
    ...
    async play(name, opts) {
        ...
        this.playName = name;
        this.playing = true;
        this.onEnd = (opts === null || opts === void 0 ? void 0 : opts.onEnd) || null;
        ...
        const tick = () => {
            if (!this.playing || !this.currentSong || !this.ctx)
                return;
            if (this.ctx.currentTime >= this.startTime + songDur) {
                if ((opts === null || opts === void 0 ? void 0 : opts.loop) === false) {
                    this.playing = false;
                    if (this.onEnd)
                        this.onEnd();
                    return;
    ...
    getPlayingName() { return this.playName; }
}
/** 全局单例 */
