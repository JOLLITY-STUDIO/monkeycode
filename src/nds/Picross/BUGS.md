...
| BUG-019 | SSEQ 旧解析 varint 方向 + Tempo 字节宽度 + Jump/Call 字节宽度错误 | ✅ | 旧版 _s88_decode.py 把 varint 当大端累积、把 Tempo 当 varint、把 Jump/Call 当 u32 → 旧 title.json 中 prog 实际是 Pan（0xC0）、tempo=256、jump 偏移全错。新版 sseq_decode3.py 按 ndspy 完整命令表修正：varint 小端（首个 7 位为低 7 位）、Tempo u16 LE（E1 xx yy）、Jump/Call u24 LE（4B 总长）、DefineTracks 3B（FE + u16 bitfield）；ndspy 反向对照 title m0 头 5 音完全一致（首音 t=48 k=40 E vel 127 dur 12 ✓）|
