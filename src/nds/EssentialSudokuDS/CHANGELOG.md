...
#### 背景 (用户诉求)
- 图画谜题 (绘逻辑) 规则全靠玩家自己猜 — 行列提示怎么读、怎么涂色、如何判定完成都没教
- 教程类别 `numclo_tu.data` 之前只显示一张 120px 模糊 ROM 图, 学不到任何东西
- 方案: A. 自绘图文教学弹层 (5 页) + B. 首次进入玩法的高亮引导 (3 步, 一次性)

#### A. 图文教程弹层 (`tutorial-strip` + `tut-mask`)
- `numclo_tu.data` 时显示紫色「📖 玩法图文教程」按钮 (ds-btn 双态) + 提示文字; 未读过带红点
- 弹层: 紫渐变 header (页数 1/5) + scroll-view 内容 + 底部 上一步/跳过教程/下一步 (最后页=开始涂色)
- 5 页内容, 每页**纯 CSS 迷你示意图** + 要点:
  1. 认识对局界面 (顶列/左行彩色提示条 + 15×15 画板示意)
  2. 提示怎么读 (红块 3+黄块 2 → 行内各色格数, 按色计数不看位置)
  3. 涂色操作 (先选色再点格 / 色下数字=剩余格数 / 同色再点=擦除 / 擦除键)
  4. 常用工具 (撤销/重做/清空/显示答案 — 看答案不扣分)
  5. 完成一幅画 (全对自动通关 → 记录用时步数 → 图库打勾 → 自动存档)
- 已读标记 storage `esds_pic_tut_seen` (按钮红点消失)
- 旧 `tutorial-bar`/`tutorial-image`/`tutorial-close` 模糊图样式整段移除

#### B. 首次进入高亮引导 (`guide-mask`, 3 步一次性)
- 首次进入 picture-scene 350ms 后弹出; 完成/跳过写 storage `esds_pic_guide_done` 不再弹
- 高亮目标: 1) `.puzzle-area` 认识提示区 → 2) `.palette` 选色→涂色 → 3) `.tool-row` 常用工具
- 实现: `createSelectorQuery` 取目标 rect → 4 块 dim 挖洞 + 高亮框呼吸动画 +
  箭头 (目标在屏幕上方→框下 ▲ 指上, 下方→框上 ▼ 指下) + 底部步骤卡 (圆点进度/标题/说明/
  上一步/下一步/跳过)
- rect 定位失败 fallback → 整屏遮罩仅底部卡 (功能不丢)
- 遮罩层 z-index 110 > 庆祝层; 挖洞处 pointer-events 穿透可点; detached 时清理 timer + 关蒙版

#### 文件
- `picture-scene.wxml` / `.wxss` / `.ts` (3 文件)
- lint 0 诊断
