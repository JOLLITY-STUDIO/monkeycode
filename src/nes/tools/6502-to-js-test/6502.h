...
#define LDA_IDX(v)      CYCLE(6); GET_INDX(A, v);
#define LDA_IDY(v)      CYCLE(5); GET_INDY(A, v);
...
#define STX_ABS(v)      CYCLE(4); _ST(X, v);
#define STX_ABS_Y(v)    CYCLE(4); _ST(X, ABSY(v));
#define STY_ABS(v)      CYCLE(4); _ST(Y, v);
...
#define SBC_IDX(M)      CYCLE(6); { uint8_t v; GET_INDX(v, M); _SBC_VAL(v); }
#define SBC_IDY(M)      CYCLE(5); { uint8_t v; GET_INDY(v, M); _SBC_VAL(v); }
...
#define LSR_ZP(M)       CYCLE(5); _LSR_M( M );
#define LSR_ZP_X(M)     CYCLE(6); _LSR_M( ZPX(M) );
#define LSR_ABS(M)      CYCLE(6); _LSR_M( M );
#define LSR_ABS_X(M)    CYCLE(7); _LSR_M( ABSX(M) );
