...
function dispatch(ctx: GameContext, rom: RomReader): void {
  ...
  console.log('[bank_00] dispatch: idx=%d → target=$%s',
  ...
  if (targetFn) {
    console.log('[bank_00] dispatch → executing %s', key);
    targetFn(ctx, rom);
  } else {
    console.warn('[bank_00] dispatch: no fn at %s', key);
  }
}
...
function sceneLoopEntry(ctx: GameContext, rom: RomReader): void {
  ...
  if (sceneState === 0) {
    console.log('[bank_00] sceneLoopEntry: entering opening scene init ($804C)');
    // 调用场景初始化 (开场动画入口)
  ...
  if (idx >= JUMP_TABLE.length) {
    ...
    console.log('[bank_00] sceneLoopEntry: reset jump idx %d → 0', idx);
  }
...
function dispListEntry(ctx: GameContext, ppuReg: number, dataLo: number, dataHi: number): number {
  ...
  let cursor: number;
  // 等待 display list 有空位 (spin)
  do {
    cursor = u8(ctx.ram.u8(0x0628) + ppuRegMasked);
  } while ((ctx.ram.u8(0x0629) & 0x40) !== 0 || cursor >= 0x3D);
