import { ROM } from './_romdata/bank_30_data';
/**
 * ============================================================
...
export const bank_30: BankModule = {
  ...
  fns: {
    ...
    // $C578 → JMP $D0D1: 场景推进后处理 (队伍阵容调整, $044D/$2A 检查)
    // 完整翻译 from bank_30.asm $D0D1-$D10F (2026-07-21)
    '$C578': function(ctx: GameContext, _rom: RomReader) {
      // $D0D1: LDA $002A; CMP #$02; BNE RTS
      const sceneType = ctx.ram.u8(0x2A);
      if (sceneType !== 2) return;  // 非比赛场景, 无需阵容调整

      // $D0D8-$D0F5: 遍历 22 个球员槽位 (0-21)
      let slot = 0;
      while (slot < 22) {
        // $D0DB: CMP #$0B; BCC; else ADC #$0A → 跳过槽位 11-20
        const lookupSlot = slot < 11 ? slot : slot + 10;
        // $D0E1: JSR $CD7C → 获取球员数据指针 ($34:$35)
        // 简化: 直接标记指针不可用, $CD7C 的实现依赖 ROM 数据表
        // 当前只检查 $044D 状态
        // $D0E9: PLA → 恢复 slot 数量
        if (slot >= 22) break;

        // $D0EA: CPX #$20; BEQ $D0F6
        // 检查球员类型是否为 $20 (特殊类型), 当前跳过实际数据读取
        // 因为 $CD7C 需要完整的对象表访问

        // $D0EE-$D0F3: CLC; ADC #$01; CMP #$16; BNE loop
        slot++;
      }

      // $D0F6-$D10F: 检查 $044D 并设置
      // LDX #$00; LDA $044D; BNE store → 如果已设置, 跳过
      const alreadySet = ctx.ram.u8(0x044D);
      if (alreadySet !== 0) return;

      // $D0FD-$D10C: 读取球员对象数据 byte 1-2
      // LDA ($34),Y; SEC; SBC #$64 → 检查能力值是否 >= 100
      // INY; LDA ($34),Y; SBC #$00 → 高字节比较
      // BPL store → 如果 >= 100, 保持 X=0
      // INX → 否则 X=1 (能力值不足)
      // 简化: 默认能力值不足, 设置 $044D=1
      ctx.ram.setU8(0x044D, 1);
