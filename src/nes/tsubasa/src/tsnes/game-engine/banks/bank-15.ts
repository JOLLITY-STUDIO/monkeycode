 ...
 *   ✅ 注册 — registerBankRom(15)
 *
 * 原始 hex: tsubasa-hex2asm/prg_banks/prg_bank_15_data.ts
 */

import { registerBankRom } from './system-state';
import { PRG_BANK_15 } from './bank-15-data';

// 注册 ROM 数据
registerBankRom(15, new Uint8Array(PRG_BANK_15));

/** ROM 数据直接访问 */
export function rom15(offset: number): number {
  return PRG_BANK_15[offset & 0x1FFF] ?? 0;
}

/** 读取 16-bit 指针 (little-endian) */
export function rom15Ptr16(offset: number): number {
  const lo = PRG_BANK_15[offset & 0x1FFF] ?? 0;
  const hi = PRG_BANK_15[(offset + 1) & 0x1FFF] ?? 0;
  return (hi << 8) | lo;
}
