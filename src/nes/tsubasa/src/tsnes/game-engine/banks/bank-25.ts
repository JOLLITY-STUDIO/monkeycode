 ...
 *   ✅ 注册 — registerBankRom(25)
 *
 * 原始 hex: tsubasa-hex2asm/prg_banks/prg_bank_25_data.ts
 */

import { registerBankRom } from './system-state';
import { PRG_BANK_25 } from './bank-25-data';

// 注册 ROM 数据
registerBankRom(25, new Uint8Array(PRG_BANK_25));

/** ROM 数据直接访问 */
export function rom25(offset: number): number {
  return PRG_BANK_25[offset & 0x1FFF] ?? 0;
}

/** 读取 16-bit 指针 (little-endian) */
export function rom25Ptr16(offset: number): number {
  const lo = PRG_BANK_25[offset & 0x1FFF] ?? 0;
  const hi = PRG_BANK_25[(offset + 1) & 0x1FFF] ?? 0;
  return (hi << 8) | lo;
}
