 ...
 *   ✅ 注册 — registerBankRom(18)
 *
 * 原始 hex: tsubasa-hex2asm/prg_banks/prg_bank_18_data.ts
 */

import { registerBankRom } from './system-state';
import { PRG_BANK_18 } from './bank-18-data';

// 注册 ROM 数据
registerBankRom(18, new Uint8Array(PRG_BANK_18));

/** ROM 数据直接访问 */
export function rom18(offset: number): number {
  return PRG_BANK_18[offset & 0x1FFF] ?? 0;
}

/** 读取 16-bit 指针 (little-endian) */
export function rom18Ptr16(offset: number): number {
  const lo = PRG_BANK_18[offset & 0x1FFF] ?? 0;
  const hi = PRG_BANK_18[(offset + 1) & 0x1FFF] ?? 0;
  return (hi << 8) | lo;
}
