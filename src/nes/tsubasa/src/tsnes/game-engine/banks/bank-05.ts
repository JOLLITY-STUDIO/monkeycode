 ...
 *   ✅ 注册 — registerBankRom(5)
 *
 * 原始 hex: tsubasa-hex2asm/prg_banks/prg_bank_05_data.ts
 */

import { registerBankRom } from './system-state';
import { PRG_BANK_05 } from './bank-05-data';

// 注册 ROM 数据
registerBankRom(5, new Uint8Array(PRG_BANK_05));

/** ROM 数据直接访问 */
export function rom05(offset: number): number {
  return PRG_BANK_05[offset & 0x1FFF] ?? 0;
}

/** 读取 16-bit 指针 (little-endian) */
export function rom05Ptr16(offset: number): number {
  const lo = PRG_BANK_05[offset & 0x1FFF] ?? 0;
  const hi = PRG_BANK_05[(offset + 1) & 0x1FFF] ?? 0;
  return (hi << 8) | lo;
}
