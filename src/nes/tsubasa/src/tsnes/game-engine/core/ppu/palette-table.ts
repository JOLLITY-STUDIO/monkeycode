class PaletteTable {
  ...
  getEntry(yiq: number): number { return this.curTable[yiq]; }
  getRed(rgb: number): number { return (rgb >> 16) & 0xff; }
  getGreen(rgb: number): number { return (rgb >> 8) & 0xff; }
  getBlue(rgb: number): number { return rgb & 0xff; }
  getRgb(r: number, g: number, b: number): number { return (r << 16) | (g << 8) | b; }
