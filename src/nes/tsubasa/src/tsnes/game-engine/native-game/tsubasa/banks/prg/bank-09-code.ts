...
import { PRG_BANK_09_DATA } from './bank-09-data-only';

// 注册 ROM 数据
/** ROM 数据直接访问 */
export function rom09(offset: number): number {
  return PRG_BANK_09_DATA[offset & 0x1FFF] ?? 0;
}

/** 读取 16-bit 指针 (little-endian) */
export function rom09Ptr16(offset: number): number {
  const lo = PRG_BANK_09_DATA[offset & 0x1FFF] ?? 0;
  const hi = PRG_BANK_09_DATA[(offset + 1) & 0x1FFF] ?? 0;
  return (hi << 8) | lo;
}

/** 读取整个 bank 数据 */
export function getBank09Data(): readonly number[] {
  return PRG_BANK_09_DATA;
}
