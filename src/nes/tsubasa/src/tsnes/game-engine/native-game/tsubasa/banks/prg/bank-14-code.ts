...
import { PRG_BANK_14_DATA } from './bank-14-data-only';

// 注册 ROM 数据
/** ROM 数据直接访问 */
export function rom14(offset: number): number {
  return PRG_BANK_14_DATA[offset & 0x1FFF] ?? 0;
}

/** 读取 16-bit 指针 (little-endian) */
export function rom14Ptr16(offset: number): number {
  const lo = PRG_BANK_14_DATA[offset & 0x1FFF] ?? 0;
  const hi = PRG_BANK_14_DATA[(offset + 1) & 0x1FFF] ?? 0;
  return (hi << 8) | lo;
}

/** 读取整个 bank 数据 */
export function getBank14Data(): readonly number[] {
  return PRG_BANK_14_DATA;
}
