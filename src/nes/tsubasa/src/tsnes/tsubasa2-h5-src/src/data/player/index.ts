...
/** 球员位置类型 */
export type PlayerRole = 'GK' | 'field';

/** 球员目录条目 */
export interface PlayerEntry {
  id: number;       // 球员编号 (0x01-0x75)
  nameEn: string;   // 英文名
  nameJa: string;   // 日文名
  nameZh: string;   // 中文名
  role: PlayerRole; // GK=门将, field=场上球员
}
...
/** 球员编号 → 条目 (快速查找) */
export const PLAYER_MAP: ReadonlyMap<number, PlayerEntry> = new Map(
  PLAYERS.map(p => [p.id, p]),
);

/** 按编号获取球员 */
export function getPlayer(id: number): PlayerEntry | undefined {
