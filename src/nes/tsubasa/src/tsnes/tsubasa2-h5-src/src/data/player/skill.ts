...
export const SHOT_SKILLS: readonly { id: number; name: string }[] = [
  { id: 0x00, name: 'Normal' },
  { id: 0x01, name: 'Volley' },
  { id: 0x02, name: 'Head' },
...
/** 射门 ID → 名称 (快速查找) */
export const SHOT_SKILL_MAP: ReadonlyMap<number, string> = new Map(
  SHOT_SKILLS.map(s => [s.id, s.name]),
);
...
export const DRIBBLE_SKILLS: readonly { id: number; name: string }[] = [
  { id: 0x00, name: 'Normal' },
...
/** 盘带 ID → 名称 (快速查找) */
export const DRIBBLE_SKILL_MAP: ReadonlyMap<number, string> = new Map(
  DRIBBLE_SKILLS.map(s => [s.id, s.name]),
);
...
export const PASS_SKILLS: readonly { id: number; name: string }[] = [
  { id: 0x00, name: 'Normal' },
...
/** 传球 ID → 名称 (快速查找) */
export const PASS_SKILL_MAP: ReadonlyMap<number, string> = new Map(
  PASS_SKILLS.map(s => [s.id, s.name]),
);
...
export const COMBO_SKILLS: readonly { romAddr: number; name: string }[] = [
...
export const TACKLE_SKILLS: readonly { romAddr: number; name: string }[] = [
...
export const BLOCK_SKILLS: readonly { romAddr: number; name: string }[] = [
...
/** 技能类别 → 技能表 (仅含编号类技能) */
export const SKILL_TABLES: Readonly<Record<SkillCategory, readonly { id: number; name: string }[]>> = {
  [SKILL_CATEGORIES.SHOT]: SHOT_SKILLS,
  [SKILL_CATEGORIES.DRIBBLE]: DRIBBLE_SKILLS,
  [SKILL_CATEGORIES.PASS]: PASS_SKILLS,
};
