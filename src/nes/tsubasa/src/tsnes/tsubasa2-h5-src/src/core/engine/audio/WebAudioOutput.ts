...
interface ChanState {
  ...
  /** 方波 duty 模式 (0-3) */
  duty: number;
  /** 当前波形相位 (0-7 for square, 0-31 for triangle) */
  phase: number;
  /** 采样计数器 (用于音频率转换) */
  sampleAcc: number;
}
...
export class WebAudioOutput implements IAudioOutput {
  private _ctx: AudioContext | null = null;
  private _node: ScriptProcessorNode | null = null;
  private _started = false;

  /** 三个 NES 音频通道状态 */
  private _sq1: ChanState;
  private _sq2: ChanState;
  private _tri: ChanState;
  private _noise: ChanState;

  /** NES CPU 频率 (NTSC) */
  private _cpuFreq = 1789773;
  /** frame counter ticks per second (~240Hz) */
  private _frameCounterHz = 240;
  ...
  writeApu(events: ApuWriteEvent[]): void {
    ...
    // 确保 AudioContext 已启动
    if (!this._started) this._ensureAudioStarted();
  ...
  private _dispatchWrite(addr: number, value: number): void {
    // Square 1 ($4000-$4003)
  ...
  private _ensureAudioStarted(): void {
    ...
    try {
      ...
      this._node = (this._ctx as any).createScriptProcessor(4096, 0, 1);
    }
  ...
  private _onAudioProcess = (event: AudioProcessingEvent): void => {
    ...
    for (let i = 0; i < len; i++) {
      out[i] = (
        this._genSquare(this._sq1, sampleRate)
        + this._genSquare(this._sq2, sampleRate)
        + this._genTriangle(this._tri, sampleRate)
      ) * 0.15; // 衰减防止爆音
    }
  };

  /** 生成方波采样 (SQ1/SQ2) */
  private _genSquare(chan: ChanState, sampleRate: number): number {
    if (!chan.enabled || chan.timer < 8 || chan.volume === 0) return 0;
  ...
  /** 生成三角波采样 */
  private _genTriangle(chan: ChanState, sampleRate: number): number {
    if (!chan.enabled || chan.timer < 8) return 0;
    ...
    chan.phase += incr;
    if (chan.phase >= 32) chan.phase -= 32;
  ...
  /** 频率 (Hz) → NES 定时器周期值 */
  private _freqToTimer(freq: number): number {
