...
let _shotCount = 0;
const _shots: Array<{ name: string; dataUrl: string; ts: string }> = [];
 ...
 * @param name 截图名称
 */
export function screenshot(name: string): string | null {
  ...
  try {
    ...
    log(`截图失败 [${name}]: ${(e as Error).message}`, 'fail');
    return null;
  }
}

/** 设置进度 (0~100) */
export function progress(pct: number): void {
...
/** 简单断言 */
export function assert(cond: boolean, msg: string): boolean {
  if (cond) {
    log('✓ ' + msg, 'pass');
  } else {
    log('✗ ' + msg, 'fail');
