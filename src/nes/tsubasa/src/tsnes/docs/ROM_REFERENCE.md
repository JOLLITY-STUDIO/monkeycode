...
|------|---------|------------|------------|--------|------|---------|
| 0 | $8000 (SWITCHABLE) | 7274 (89%) | 427 (5%) | 491 (6%) | 🔧 代码密集型 | 系统初始化 & 标题/菜单主循环 — 通过 $9FA8 切换 PRG bank |
| 1 | $8000 (SWITCHABLE) | 4239 (52%) | 3556 (43%) | 397 (5%) | 🔧 代码密集型 | 数据查询服务（球员/队伍/赛事数据检索）→ 调用 Bank 02 $A72C |
| 2 | $8000 (SWITCHABLE) | 1828 (22%) | 245 (3%) | 6119 (75%) | 📭 低利用率 | 场景/密码/选择界面 & 数据加载引擎（$A72C 关卡数据解包器） |
| 3 | $8000 (SWITCHABLE) | 0 (0%) | 8186 (100%) | 6 (0%) | 📦 数据密集型 | 关卡地图数据 (Part 1) — 被 Bank 02 ($A72C) 解包到 RAM $0460 |
| 4 | $8000 (SWITCHABLE) | 0 (0%) | 8158 (100%) | 34 (0%) | 📦 数据密集型 | 关卡地图数据 (Part 2) — 同 Bank 03 格式，同为 Bank 02 所加载 |
| 5 | $8000 (SWITCHABLE) | 0 (0%) | 8157 (100%) | 35 (0%) | 📦 数据密集型 | 队伍阵型/策略数据 — 纯数据 |
| 6 | $8000 (SWITCHABLE) | 0 (0%) | 3345 (41%) | 4847 (59%) | 📦 数据密集型 | 剧情/脚本数据块 (Part 1) |
| 7 | $8000 (SWITCHABLE) | 0 (0%) | 3908 (48%) | 4284 (52%) | 📦 数据密集型 | 剧情/脚本数据块 (Part 2) — 被 Bank 02 ($A72C, EB=$07) 加载 |
...
| 30 | $C000 (FIXED) | 6350 (78%) | 1495 (18%) | 347 (4%) | 🔧 代码密集型 | 核心系统库（PPU/APU/控制器/数学）FIXED @ $C000 — 被 Bank 00, 31 调用 |
| 31 | $E000 (FIXED) | 3951 (48%) | 3387 (41%) | 854 (10%) | 🔧 代码密集型 | 中断向量 & 通用工具 FIXED @ $E000 — 依赖 Bank 30, 被 Boot 调用 |
...
该函数是游戏的数据加载中枢：
1. 接收 `ram_00EB` = bank 编号 (如 `#$03`)
2. 每轮迭代推进 16-bit 指针 (`ram_04E4`–`04E7`)
3. 写入 4 字节解包数据到 `ram_0468`–`04EF` 工作区
4. 调用 Bank 00 的 `$9FA8` 切换 MMC3 PRG bank（寄存器 6 → `$8000`-`$9FFF`）
...
- 被 Bank 02 的 `$A72C` 关卡数据解包器以 `ram_00EB=$03` 加载
- 数据解包后写入 `ram_0460`–`04EF` 工作缓冲区

#### 内容特点

- 📏 ROM 区域: 0x006010-0x00800F

**类型:** 纯数据 bank — 关卡地图压缩数据

---

### Bank 04 — 关卡地图数据 (Part 2) — 纯数据
...
- 与 Bank 03 格式相同，同为 Bank 02 的 `$A72C` 所加载
- 解包后写入同一 `ram_0460`–`04EF` 工作缓冲区
...
- 被 Bank 02 的 `$A72C` 以 `ram_00EB=$07` 加载为剧情脚本数据
- 数据解包后写入 `ram_0460`–`04EF` 工作缓冲区
...
### Bank 03–04: 关卡地图数据
- 8KB 数据块 (Bank 03: 8186B, Bank 04: 8158B)，压缩格式
- 被 Bank 02 的 `$A72C` 关卡数据解包器按需加载
- `ram_00EB=$03` 选择 Bank 03，解包到 RAM `$0460`–`$04EF` 工作区
- Bank 04 格式相同，同为 Bank 02 所加载
...
- Bank 07 被 Bank 02 的 `$A72C` (ram_00EB=$07) 直接加载
...
| **02** | 关卡数据加载引擎 ($A72C) | 03, 04, 07 | 00, 01 |
| **03** | 关卡地图 Part 1 | — | 02 |
| **04** | 关卡地图 Part 2 | — | 02 |
| **05** | 队伍阵型/策略 | — | 01 |
| **07** | 剧情脚本 Part 2 | — | 02 |
...
| 2 | 1828B | 245B | 6119B | 🔧 逻辑模块 — 关卡数据加载引擎 ($A72C) |

## 附录 B: 纯数据 Bank 速查（按数据量排序）

| Bank | 数据量 | 未访问 | 数据密度 | 推测内容 |
|------|--------|--------|---------|---------|
| 3 | 8186B | 6B | 100% | 关卡地图数据 (Part 1) — 被 Bank 02 ($A72C) 解包 |
| 14 | 8177B | 15B | 100% | 动画/演出数据 (Part 2) — 纯数据 |
| 13 | 8176B | 16B | 100% | 动画/过场帧数据 (Part 1) — 纯数据 |
| 4 | 8158B | 34B | 100% | 关卡地图数据 (Part 2) — 被 Bank 02 加载 |
...
| 7 | 3908B | 4284B | 48% | 剧情/脚本数据块 (Part 2) — 被 Bank 02 加载 |
