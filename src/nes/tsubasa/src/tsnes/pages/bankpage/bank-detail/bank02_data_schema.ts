...
export interface DataField {
  offset: number;    // within record
  size: number;      // bytes
  name: string;
  desc: string;
  decode?: (v: number|number[]) => string;  // optional ─ used at render time
}

export interface DataTableDef {
  name: string;           // display name
  addr: number;           // PRG file offset
  length: number;         // total bytes
  recordSize: number;     // bytes per record (0 = flat/single-record)
  fields: DataField[];
  note?: string;
}
...
/** 有符号 8-bit */
export const S8 = (v: number) => (v >= 128 ? v - 256 : v);

/** unsigned 8-bit */
export const U8 = (v: number) => v;

/** 16-bit little-endian (lo, hi) */
export const U16LE = (lo: number, hi: number) => lo + (hi << 8);

/** 16-bit signed LE */
export const S16LE = (lo: number, hi: number) => {
  const v = lo + (hi << 8);
