...
const data = {
  ...
  stats: {
    ...
    subroutineCount: 40,
    dataTableCount: 12,
    note: "Bank 00 是游戏核心系统服务层 + 主循环引擎。Bank 30 在 RESET 后将其映射到 $8000-$9FFF。被 Bank 02 密集调用。提供: NMI/IRQ 帧循环、PPU Buffer 写入($9B28/$9B5E)、Bank 切换($9FA8)、VRAM 操作($98A0 清零/$98E8 写入)、场景描述读取($8AF7)、VRAM地址设置($890C)、调色板初始化($8297/$9085)等。",
    note2: "✅ Bank 00 不直接引用 Bank 10。所有 bank 切换记录: JSR $9FA8→Bank 01(最常见)/02/04/FE; JSR $C4B9→Bank 01/02/06/07/08。$98A0 是 Nametable 清零工具,不是场景加载。场景数据通过 $8AF7 从 Bank 02 $A0xx 区域读取。",
  },
  ...
  sceneInitFlow: {
    ...
    steps: [
      { step: 1, addr: "$801F", action: "等待 VBlank / 清 PPU Buffer" },
      { step: 2, addr: "$8027", action: "JSR $9FA8(Bank $01) 等待 ram_001E bit4" },
      { step: 3, addr: "$8053", action: "JSR $9B11(NT+属性清零)" },
      { step: 4, addr: "$8058", action: "JSR $9FA8(Bank $02) 映射场景 bank" },
      { step: 5, addr: "$805B", action: "JSR $9B7F(未知初始化)" },
      { step: 6, addr: "$805E", action: "JSR $98A0(NT 清零: 禁用渲染→写 $2006=$2000→2048B 清零→恢复渲染)" },
      { step: 7, addr: "$8063", action: "JSR $8297(调色板初始化: 参数A=0x0D → $9085)" },
      { step: 8, addr: "$806C", action: "JSR $8AF7(scene=0x17: 场景描述读取 → 从 Bank 02 $A0xx 指针表读布局数据)" },
      { step: 9, addr: "$8071", action: "JSR $890C(index=0x30: VRAM 地址/滚动设置)" },
      { step: 10, addr: "$8074", action: "JSR $88FB(未知 PPU 操作)" },
      { step: 11, addr: "$8077", action: "JSR $9A35(主循环初始化)" },
      { step: 12, addr: "$808D", action: "ram_00ED=0x0A → 进入输入循环" },
    ],
  },
  ...
  subroutines: [
    ...
    },
    {
      ...
      note: "场景初始化子链: $9B11→Bank02切换→$9B7F→$98A0→$8297→$8AF7→$890C→$88FB→$9A35",
    },
    {
      bankAddr: "$8053",
      name: "场景初始化调用链执行区",
      length: "~65B ($8053-$809D)",
      desc: "$9B11(NT清零) → 切Bank02 → $9B7F → $98A0(NT清零) → $8297(调色板,A=0x0D) → $8AF7(scene=0x17) → $890C(index=0x30) → $88FB → $9A35 → $8930 → 进入主输入循环($8091)",
    },
    {
      bankAddr: "$8091",
      name: "主输入循环",
    ...
    },
    {
      ...
      note: "调用时: A=0x0D(标题画面调色板)。$9085 从内置数据读调色板值写入 $05E8 缓冲区。完成检测: $82A9 循环切换 Bank01 等待 $004D/4E=0。",
      crossRefs: [
        { from: "$8063", bankAddr: "00:8063", op: "JSR", desc: "场景初始化: A=0x0D (标题调色板)" },
        { from: "$80EE", bankAddr: "00:80EE", op: "JSR", desc: "场景切换: A=0x0D" },
      ],
    },
    {
      bankAddr: "$82A9",
      ...
      desc: "LDA #$01 → JSR $9FA8(切Bank01) → 检查 $004D/4E≠0 则循环等待 → RTS。用于等待 Bank 01 处理完成。",
    },
    {
      bankAddr: "$82B5",
      name: "场景准备/重置 ($82B5)",
      length: "~40B ($82B5-$82EC)",
      desc: "LDA #$01→$9FA8 切Bank01 → 等待$004D/4E清零 或 $001E.bit5→清零状态变量 → ram_0700=1 → $9BA0等待VBlank → 清零$44/$45/$7A/$7B → RTS",
      note: "场景切换前的前置清理。Bank 01 在 $Axxx 窗口处理数据。",
    },
    {
      bankAddr: "$8285",
      name: "PPU 缓冲完成等待 ($8285)",
      length: "~12B ($8285-$8290)",
      desc: "检查 $004D/4E=0 则RTS → 否则 LDA#$01→$9FA8切Bank01 等待 → 无操作",
    },
    {
      bankAddr: "$82ED",
      name: "PPU 缓冲处理循环 ($82ED)",
      length: "~20B ($82ED-$8308)",
      desc: "等待 ram_004C bit7 → ASL A→TAX → LDA $B800,X(查Bank02数据表?) → 分支处理 → 循环",
    },
    {
      bankAddr: "$83BA",
      name: "帧跳转表 ($83BA 数据)",
      length: "数据区",
      desc: "按 scene_index 查表,值=CMP 后分支: 0=跳转, 1=类型1, 2=类型2, 3=类型3",
    },
    {
      bankAddr: "$83DC",
      name: "帧跳转表2 ($83DC 数据)",
      length: "数据区",
      desc: "按 scene_index 查表,分支条件",
    },
    {
      bankAddr: "$83FE",
      name: "帧跳转表3 ($83FE 数据)",
    ...
    },
    {
      ...
      desc: "index(0x30 etc.) → 从 Bank 30 数据表读取滚动偏移 → 设置 PPU $2006($2005) 寄存器",
      note: "调用时: A=0x30(Bank 30 索引), 读两个 Bank 30 偏移值设置 VRAM 地址",
    },
    {
      bankAddr: "$8920",
      name: "PPU 配置写入 ($8920)",
    ...
    },
    {
      ...
      desc: "A=scene_id(0x17 etc) → $ED存储 → 清零$09/$0A/$0D/$0E → 保存$5B → LDX#$07→$C4B9切Bank07 → 清零$0552-$FFFF? → scene_id×2→指针$0063/64 = $A000+index×2(指向Bank02场景指针表) → 读指针表→$0063/64=实际场景数据地址 → 读场景描述(75/76=屏幕坐标,48=metatile数,5E/5F=?,5C/5D=tile指针) → 继续解析",
      note: "⚠ 关键发现: 场景指针表地址 $A000+index*2 位于 Bank 02 映射窗口($A000-$BFFF),不是 Bank 10！Bank 00 不引用 Bank 10。",
      crossRefs: [
        { from: "$806C", bankAddr: "00:806C", op: "JSR", desc: "场景初始化: scene=0x17" },
      ],
    },
    {
      bankAddr: "$9085",
      ...
      crossRefs: [
        { from: "$82A5", bankAddr: "00:82A5", op: "JSR", desc: "调色板初始化链" },
      ],
    },
    {
      bankAddr: "$98A0",
      ...
      desc: "禁用 NMI(AND #$7F on $2000) → 禁用渲染(AND #$E7 on $2001) → 设PPUADDR=$2000 → 向$2007写2048个$00(8×256) → 恢复渲染(ORA #$18) → 恢复NMI → RTS",
      note: "⚠ 这是一个纯粹的 VRAM 清零函数,不涉及任何 bank 切换或 Bank 10 数据。之前误描述为'场景布局加载'是错误的。",
    },
    {
      bankAddr: "$98E8",
    ...
    },
    {
      ...
      desc: "A=标志存入$EB → 检查$004A/4B(待写入数据?) → $E8/$E9存Y/X → 计算buffer偏移 → 打包control byte+PPU地址+数据 → 写入$05E8+offset → 更新$0628指针",
    },
    {
      bankAddr: "$9A35",
      name: "主循环初始化 ($9A35)",
    ...
    },
    {
      ...
      crossRefs: [
        { from: "Bank 01 $A10D", desc: "选项屏幕初始化" },
      ],
    },
    {
      bankAddr: "$9B11",
    ...
    },
    {
      ...
      desc: "A=bank编号 → 保存到ram_0019 → 保存X/Y/ED/EC/EB/EA/E9/E8/E7/E6到栈 → TSX保存栈指针到ram_0001,X → 保存0024/0025到ram_0002/0003,X → 检查bank号(0/FF→FE) → 存入ram_0000,X → JMP $9EFB 执行实际 bank 切换",
      note: "这是 Bank 00 的 bank 切换函数,用于在所有 bank 间切换。实际切换由 $9EFB 完成(写入 MMC3 寄存器)。被 Bank 00 频繁调用。",
    },
    {
      bankAddr: "$9EFB",
      name: "MMC3 Bank 写入 ($9EFB) ",
      length: "部分在 Bank 00 结尾",
      desc: "恢复栈状态 → 向 MMC3 寄存器写入 bank 编号 → 恢复寄存器和保存的变量 → JMP 返回调用点(通过栈操作等效于返回)",
    },
  ],
