...
const data = {
  ...
  stats: {
    ...
    note3: "⚠ 注意: Tecmo Theater 是【动画分镜】(过场动画), 不是功能页面(标题画面/菜单等)。frame 295 之后还有 N 个动画分镜, 整个分镜序列尚未完整 trace。场景流转由 $0026(scene_id) + $83BA/$83DC/$83FE 跳转表驱动。",
    tecmoTheater: {
      ...
      sceneId: "0x17",
      bootChain: "Bank 31 $FFF0 → Bank 30 $C503(boot) → Bank 00 $801F(场景初始化,scene=0x17) → Bank 00 $9FA8(切 Bank 01) → Bank 01 $805D(NMI handler) → 每帧渲染 Tecmo 画面",
      note: "这是启动后的第一个动画分镜。frame 295 之后 scene_id 变更, 后续分镜序列尚未 trace。",
      keyFunctions: [
        ...
        { addr: "$805B-$8077", name: "Tecmo 画面子调用链", role: "Bank 02→$9B7F→$98A0(NT清零)→$8297(调色板 A=0x0D)→$8AF7(scene=0x17)→$890C→$88FB→$9A35" },
        { addr: "$9FA8", name: "Bank 切换核心", role: "A=0x01 → 切 Bank 01 到 $A000 作为 NMI handler" },
        { addr: "$9085", name: "调色板数据写入", role: "加载内置调色板表 → 写 PPU Buffer → NMI 期间 DMA 到 PPU" },
      ],
    },
  },
