/**
 * ============================================================================
 * tsnes 内核适配器 — 用于双 canvas 对比调试
 *
 * 直接加载原始 .nes ROM 文件，喂给 tsnes 完整 NES 仿真器。
 * 每帧输出 PPU 帧缓冲到 canvas，与手写 TS 版本并排对比。
 * ============================================================================
 */

// @ts-ignore — tsnes 是 .js 模块
...
export class TsnesKernel {
  ...
  private _startAudio(): void {
    ...
    try {
      ...
      console.log('[tsnes] audio started via WebAudioContext');
    } catch (e: any) {
      console.warn('[tsnes] WebAudioContext unavailable:', e.message);
    }
  }
   ...
   * @param mask BUTTON 位掩码 (tsnes 的 buttonDown/buttonUp 逐个触发)
   */
  ...
  setInput(mask: number): void {
    ...
    const btnMap: [number, number][] = [
      ...
      [BUTTON.B, 1],      // tsnes button 1 = B
      [BUTTON.SELECT, 2], // tsnes button 2 = Select
      [BUTTON.START, 3],  // tsnes button 3 = Start
      [BUTTON.UP, 4],     // tsnes button 4 = Up
      [BUTTON.DOWN, 5],   // tsnes button 5 = Down
      [BUTTON.LEFT, 6],   // tsnes button 6 = Left
      [BUTTON.RIGHT, 7],  // tsnes button 7 = Right
    ];

    for (const [tsBtn, tsnesBtn] of btnMap) {
      const wasPressed = (this.prevButtons & tsBtn) !== 0;
      const isPressed = (mask & tsBtn) !== 0;

      if (isPressed && !wasPressed) {
        this.nes.buttonDown(1, tsnesBtn);
      } else if (!isPressed && wasPressed) {
        this.nes.buttonUp(1, tsnesBtn);
      }
    }
  ...
  traceStart(pcLow: number, pcHigh: number): void {
    if (!this._tracer) {
      console.warn('[tsnes] tracer not ready — ROM not loaded');
      return;
    }
  ...
  traceToFile(filePath?: string): void {
    const traces = this._traceResult;
    if (!traces || traces.length === 0) {
      console.log('[tsnes] trace empty, skip write');
      return;
    }
    ...
    try {
      ...
      console.log(`[tsnes] trace → ${path} (${traces.length} ops)`);
    } catch (e: any) {
      console.error('[tsnes] trace write failed:', e.message);
