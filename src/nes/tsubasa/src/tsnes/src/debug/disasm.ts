...
// prettier-ignore
const OPCODE_TABLE: Record<number, { ins: number; mode: number; size: number; cycles: number }> = {
 ...
 */
export function disassemble(addr: number, memRead: (addr: number) => number): {
  text: string;
  size: number;
  bytes: number[];
} {
 ...
 * 格式: N-V-U-B-D-I-Z-C
 */
export function flagsToString(p: number): string {
  const n = (p & 0x80) ? 'N' : 'n';
  const v = (p & 0x40) ? 'V' : 'v';
