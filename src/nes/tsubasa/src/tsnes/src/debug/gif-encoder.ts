    ...
    if (dict.has(key)) {
      ...
      if (nextCode < maxTable) {
        ...
        if (nextCode > limit && codeSize < 12) {
          codeSize++;
          limit = (1 << codeSize) - 1;
          if (limit > maxTable) limit = maxTable;
        }
      }
      current = byte;
    }
...
function writeLE16(arr: number[], val: number): void {
...
function buildPalette(frames: GifFrame[]): {
  palette: Uint8Array;   // RGB triplet × color count
...
export function encodeGif(
  ...
  let minCodeSize = 2;
  while ((1 << minCodeSize) < colorCount) minCodeSize++;
  ...
  // ── Global Color Table ──
  for (const b of fullPal) result.push(b);

  // ── Netscape Extension (loop count) ──
  if (options?.loop !== false) {
    result.push(0x21, 0xFF, 0x0B);            // application extension
    for (const ch of 'NETSCAPE2.0') result.push(ch.charCodeAt(0));
