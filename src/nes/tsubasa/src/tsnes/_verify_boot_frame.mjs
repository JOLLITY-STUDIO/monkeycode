...
// read directly from a specific bank (cross-bank read)
function readBank(sys, bankNum, offset) {
  return prgBanks[bankNum]?.[offset & 0x1FFF] ?? 0;
}

function readBank16(sys, bankNum, offset) {
  return ((prgBanks[bankNum]?.[(offset + 1) & 0x1FFF] ?? 0) << 8)
       | (prgBanks[bankNum]?.[offset & 0x1FFF] ?? 0);
}
...
function _bytecode_setupParam(sys, param) {
  ...
  sys.ppu.writeVRAMAddress(0x23);
  sys.ppu.writeVRAMAddress(0xE0);
...
for (let f = 0; f < 200; f++) {
  ...
  if (f % 10 === 0 || f < 5 || sys.bootPhase >= 10) {
    const ntNZ = countNonZeroNT(ppu);
    log(`F#${String(f).padStart(3)} phase=${sys.bootPhase} sub=${sys.bootSubStep} ntNZ=${ntNZ} ptr=$${((sys.mem[0x4E]<<8)|sys.mem[0x4D]).toString(16)} $E9=$${(sys.mem[0xE9]??0).toString(16)}`);
  }
}
...
// 最终 dump
dumpNT(ppu, '=== Final Nametable ===');
dumpPalette(ppu, '=== Final Palette ===');

// 与参考对比
log('=== Comparison with ref (frame #96 nametable rows 12-13) ===');
const expR12 = [0x28,0x29,0x2C,0x2D,0x38,0x37,0x39,0x3C,0x3D];
const expR13 = [0x2A,0x2B,0x2E,0x2F,0x3A,0x2A,0x3B,0x3E,0x3F];

log(`Expected row 12 (col 13-21): ${expR12.map(v=>'0x'+v.toString(16)).join(' ')}`);
log(`Expected row 13 (col 13-21): ${expR13.map(v=>'0x'+v.toString(16)).join(' ')}`);

const act12 = []; const act13 = [];
for (let c=13; c<22; c++) {
  act12.push(ppu.vramMem[0x2000 + 12*32 + c]);
  act13.push(ppu.vramMem[0x2000 + 13*32 + c]);
}
log(`Actual   row 12 (col 13-21): ${act12.map(v=>'0x'+v.toString(16).padStart(2,'0')).join(' ')}`);
log(`Actual   row 13 (col 13-21): ${act13.map(v=>'0x'+v.toString(16).padStart(2,'0')).join(' ')}`);

let match12 = true, match13 = true;
for (let c=0; c<expR12.length; c++) { if (act12[c] !== expR12[c]) match12 = false; }
for (let c=0; c<expR13.length; c++) { if (act13[c] !== expR13[c]) match13 = false; }

log(`\nMatch row 12: ${match12 ? 'OK ✅' : 'MISMATCH ❌'}`);
log(`Match row 13: ${match13 ? 'OK ✅' : 'MISMATCH ❌'}`);
