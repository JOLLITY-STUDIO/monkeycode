/**
 * 反汇编 bank_00 中的指定函数，把 .byte 0xXX 转成 6502 助记符 + @label
 * 用法: node _convert_bank00_run.mjs <funcName>
 */
import fs from 'fs';
...
function disasmWithLabels(bytes, baseAddr) {
  ...
  // merge standalone labels immediately before instructions
...
function findFuncContent(content, funcName) {
  const search = 'function ' + funcName + '(';
...
// Correct addresses verified against ROM
const ADDR_MAP = {
  buildsceneLoop: 0x81E6,
  buildscriptEngine: 0x82ED,
  buildsceneTables: 0x83D4,
  buildbytecodeHandlers: 0x8840,
  buildscheduler: 0x8EED,
  buildpadding: 0x8FF6,
};

const targetFunc = process.argv[2];
if (!targetFunc) {
  console.log('Usage: node _convert_bank00_run.mjs <funcName>');
...
const filePath = path.join(import.meta.dirname, 'src', 'tsnes', 'tsubasa-hex2asm', 'prg_banks', 'prg_bank_00_dispatch_scene_engine.ts');
...
console.log(targetFunc + ': ' + bytes.length + ' bytes extracted, base=$' + H4(baseAddr));
