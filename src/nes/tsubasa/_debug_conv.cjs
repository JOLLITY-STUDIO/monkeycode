/**
 * Debug: 找到 buildbytecodeHandlers 中产生 @E9CF/@EAF6 的原始字节位置
 */
const fs = require('fs');
const path = require('path');

// Restore and re-extract
const { execSync } = require('child_process');
execSync('git checkout eccbb79 -- src/tsnes/tsubasa-hex2asm/prg_banks/prg_bank_00_dispatch_scene_engine.ts', { cwd: __dirname });
execSync('git checkout eccbb79 -- src/tsnes/tsubasa-hex2asm/prg_banks/prg_bank_00_dispatch_scene_engine.ts', { cwd: path.join(__dirname, '..') });

const src = fs.readFileSync('src/tsnes/tsubasa-hex2asm/prg_banks/prg_bank_00_dispatch_scene_engine.ts','utf8');

// Extract buildbytecodeHandlers bytes
function getBytes(fname) {
...
const bytes = getBytes('buildbytecodeHandlers');
console.log('buildbytecodeHandlers: ' + bytes.length + ' bytes');

const baseAddr = 0x8840;
const H2 = n => n.toString(16).toUpperCase().padStart(2, '0');
const H4 = n => n.toString(16).toUpperCase().padStart(4, '0');

// Full opcode table for correct traversal
const OPT = {
...
const INS = [
  ...
  'TSX','TXA','TXS','TYA',null,null,null,null,null,null,null,null,null,null,null,null,
  null,null,null,null,null,null,null,null,null,null,null,null,
];
const BR = new Set(['BCC','BCS','BEQ','BMI','BNE','BPL','BVC','BVS']);

// Scan for all branches
let pc = 0;
console.log('\nExternal branches found:');
while (pc < bytes.length) {
  ...
  const insName = INS[insIdx];
  const lo = size >= 2 ? bytes[pc + 1] : 0;
  if (insName && BR.has(insName) && mode === 1) {
    const target = baseAddr + pc + 2 + (lo < 128 ? lo : lo - 256);
    const rangeEnd = baseAddr + bytes.length;
    const inRange = target >= baseAddr && target < rangeEnd;
    if (!inRange) {
      console.log('  pc=' + pc.toString(16) + ' addr=$' + H4(baseAddr+pc) +
        ' ' + insName + ' -> $' + H4(target) +
        ' range=[$' + H4(baseAddr) + ',$' + H4(rangeEnd) + ']');
    }
  }
  pc += size;
}

// Also check the converted file (if it exists) for the offending lines
// and find the corresponding raw bytes
console.log('\nNow restoring to the converted version to check...');
