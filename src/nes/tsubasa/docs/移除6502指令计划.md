...
| # | 任务 | 状态 |
|---|------|------|
| 1.1 | 创建 `domain/player/Player.ts` + `PlayerStats.ts` + `PlayerPosition.ts` + `SpecialMove.ts` | ✅ 完成 |
| 1.2 | 创建 `domain/team/Team.ts` + `Formation.ts` + `TeamRoster.ts` | ✅ 完成 |
| 1.3 | 创建 `domain/scene/Scene.ts` 抽象基类 | ✅ 完成 — SceneState 枚举 + 生命周期 enter/update/exit |
| 1.4 | 创建 `domain/progress/ProgressState.ts` + `PasswordCodec.ts` | ✅ 完成 — Stage 枚举 + 密码 stub |
| 1.5 | 创建 `domain/script/BytecodeEngine.ts` + `BytecodeOp.ts` | ✅ 完成 — 完整操作码表 + 单步/全量解释器 |
| 1.6 | 单元测试: Player 升级经验查表正确 | ✅ 完成 — 20/20 全部通过 |
