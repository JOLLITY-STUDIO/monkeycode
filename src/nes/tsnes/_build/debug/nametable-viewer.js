...
exports.renderNameTable = renderNameTable;
exports.renderAllNameTables = renderAllNameTables;
/** 滚动指示线颜色 (FCEUX 风格: 青绿色虚线) */
const SCROLL_LINE_COLOR = 4278255615;
/** 缺失 CHR tile 标记色 (品红棋盘) */
const MISSING_TILE_COLOR1 = 4294902015; // 品红
const MISSING_TILE_COLOR2 = 4278190080; // 黑
/**
 * 渲染单个 NameTable
 *
 * 策略:
 ...
 * 3. 两者都不通 → 品红棋盘标记。
 */
function renderNameTable(nes, ntIndex, scrollX, scrollY) {
    var _a;
    const ppu = nes.ppu;
                ...
                for (let py = 0; py < 8; py++) {
                    for (let px = 0; px < 8; px++) {
                        ...
                        if (colIdx === 0) {
                            ...
                            buf[(baseY + py) * 256 + (baseX + px)] = (_a = pal[colIdx + attrVal]) !== null && _a !== void 0 ? _a : backdropColor;
                        }
                    }
                }
            }
...
function renderAllNameTables(nes) {
    ...
    const drawScrollLine = (frame, sx, sy) => {
        ...
        if (sy < 240) {
            for (let x = 0; x < 256; x++) {
                if ((x & 3) === 0)
                    frame.data[sy * 256 + x] = SCROLL_LINE_COLOR;
            }
        }
        // 竖直线
        if (sx < 256) {
            for (let y = 0; y < 240; y++) {
                if ((y & 3) === 0)
                    frame.data[y * 256 + sx] = SCROLL_LINE_COLOR;
            }
        }
    };
    const ntFrames = [
