                ...
                for (let py = 0; py < 8; py++) {
                    for (let px = 0; px < 8; px++) {
                        ...
                        buf[(baseY + py) * 128 + (baseX + px)] =
                            colIdx === 0 ? backdrop : ((_a = pal[colIdx + offset]) !== null && _a !== void 0 ? _a : backdrop);
                    }
                }
            }
            else {
...
function generatePTDataText(nes, frameCount) {
    ...
    lines.push(`══════════════════════════════════════════════════════════════`);
    lines.push(`Frame: #${frameCount !== null && frameCount !== void 0 ? frameCount : '?'}  |  Pattern Tables  (BG PT=${bgAddr}  SP PT=${spAddr})`);
    // ── MMC1 CHR bank 映射 ──
    if (isMmc1) {
        ...
        lines.push(`  Table0 ($0000-$0FFF) → 4KB CHR bank #${String((_a = mapper.chrBank4k_0000) !== null && _a !== void 0 ? _a : '?').padStart(2)}  [Reg 1: $0000]`);
        lines.push(`  Table1 ($1000-$1FFF) → 4KB CHR bank #${String((_b = mapper.chrBank4k_1000) !== null && _b !== void 0 ? _b : '?').padStart(2)}  [Reg 2: $1000]`);
    }
    else if (chrBanks) {
        // MMC3 风格: 8 个 1KB slot
