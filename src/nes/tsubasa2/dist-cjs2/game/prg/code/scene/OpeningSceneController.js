...
exports.OpeningSceneController = exports.OPENING_SCENE_ID = void 0;
/**
...
const OpeningFrameTable_1 = require("../../data/scene/OpeningFrameTable");
const OpeningScreenTable_1 = require("../../data/scene/OpeningScreenTable");
const SceneIds_1 = require("./SceneIds");
/** OpeningScene 特殊场景号（定义见 SceneIds.ts，避免跨控制器成环） */
var SceneIds_2 = require("./SceneIds");
...
class OpeningSceneController extends SceneController_1.SceneController {
    constructor(store, input) {
        super(store, input);
        this.sceneId = SceneIds_1.OPENING_SCENE_ID;
        this.audio = null;
    ...
    applyEnterRenderState() {
        ...
        for (let addr = 0x2000; addr <= 0x2fff; addr++)
            this.store.writeByte(addr, 0);
        for (let i = 0x200; i < 0x300; i++)
            this.store.writeByte(i, 0xf8);
    ...
    onUpdate(_frame) {
        ...
        if (this.input.isPressed(1, 8 /* Button.Start */)) {
            this.skipped = true;
            return SceneIds_1.TITLE_MENU_SCENE_ID;
        }
        // 片头推进(逐帧走 GT 数据)
    ...
    applyNtToPpu(ppu) {
        ...
        try {
            ...
            if (!this._applyNtLogged) {
                console.warn('[OpeningScene.applyNtToPpu] 内部异常已 catch, 后续帧继续. err=', e.message);
                this._applyNtLogged = true;
            }
        }
    }
    /** applyNtToPpu 真实实现 (V0.7+ try/catch 包裹) */
    _applyNtToPpuImpl(ppu) {
    ...
    static loadStoryCupOam(store) {
        const screen = OpeningScreenTable_1.OPENING_SCREENS[STORY_CUP_SCREEN_INDEX];
        if (!screen)
            return;
