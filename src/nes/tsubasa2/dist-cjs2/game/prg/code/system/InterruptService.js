...
const scene_end_bank_table_1 = require("../../data/tables/scene-end-bank-table");
...
class InterruptService {
    ...
    nmi(frame) {
        ...
        if (frame < 80 || frame % 60 === 0)
            console.log(`[NMI f${frame}] scheduler=${this.scheduler ? 'attached' : 'null'}`);
    ...
    flushNtBuffer(ppu) {
        const store = this.store;
        if (store.renderQueue.ntBufferBusy)
            return;
        const entries = (0, RenderQueues_1.consumeNtBuffer)(store.renderQueue);
        if (entries.length === 0)
            return;
    ...
    flushPalette(ppu) {
        ...
        for (let i = 0; i < 0x10; i++)
            ppu.writeMem(0x3f00 + i, this.fadeLookup(store.palette.bg[i], fadeA));
        for (let i = 0; i < 0x10; i++)
            ppu.writeMem(0x3f10 + i, this.fadeLookup(store.palette.spr[i], fadeB));
    }
    /**
    ...
    applySceneEndBankOverride(ppu, frame) {
        ...
        if (sceneId === 100)
            return;
        const entry = scene_end_bank_table_1.SCENE_END_BANK_TABLE.find((e) => frame >= e.fromFrame);
        if (!entry)
            return;
