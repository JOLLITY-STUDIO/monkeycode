...
const RenderQueues_1 = require("../../data/store/RenderQueues");
const opening_data_1 = require("../../data/scene/opening-data");
const bank7_streams_1 = require("../../data/scene/bank7-streams");
...
class RenderingPrimitivesService {
    ...
    ntBufferAppend(entry) {
        return (0, RenderQueues_1.appendNtBuffer)(this.store.renderQueue, entry);
    }
    // ──────────────────────────── 调色板原语 ────────────────────────────
    ...
    loadBootPalette() {
        ...
        const bg = new Array(16).fill(0x0f);
        const spr = new Array(16).fill(0x0f);
        for (let group = 0; group < 4; group++) {
            // 4 个 group × 4 组：bg 用 PALETTE_TABLE[0..3]，spr 用 PALETTE_TABLE[4..7]
            const bgItem = palette_table_1.PALETTE_TABLE[group] ?? palette_table_1.PALETTE_TABLE[0];
            const sprItem = palette_table_1.PALETTE_TABLE[group + 4] ?? palette_table_1.PALETTE_TABLE[0];
    ...
    loadScene0Palettes() {
        const store = this.store;
        const bg = opening_data_1.OPENING_BG_PALETTES[1] ?? opening_data_1.OPENING_BG_PALETTES[0];
        const spr = (0, palette_table_1.loadPalette)(21);
        store.palette.loadBg(bg);
        store.palette.loadSpr(spr);
        store.fade.bg = 0;
        store.fade.spr = 0;
    ...
    loadChrConfig(configId) {
        ...
        const cfg = opening_data_1.OPENING_CHR_CONFIGS[configId] ?? opening_data_1.OPENING_CHR_CONFIGS[0];
        ...
        const ptr = opening_data_1.OPENING_CHR_POINTER_TABLE[configId] ?? opening_data_1.OPENING_CHR_POINTER_TABLE[0];
        ...
        const stream = bank7_streams_1.OPENING_TILE_STREAMS[configId] ?? [];
     ...
     */
    queueScene0LogoNt(step) {
        for (const r of opening_data_1.OPENING_SCENE0_LOGO_ROWS) {
    ...
    buildSceneRows(indices) {
        ...
        for (const rawIdx of indices) {
            ...
            let rel;
            if (idx >= 0x6d) {
                table = scene_bank02_tables_1.SCENE14_ROW_PTR_TABLE_B10;
                rel = idx - 0x6d;
            }
            else {
                table = scene_bank02_tables_1.SCENE14_ROW_PTR_TABLE_B9;
                rel = idx;
            }
            const ptr = table[rel] ?? 0xa000;
            // 复制 32 字节模板到行（ROM $90E6: LDA $978B,Y / STA ($0094),Y）
            for (let y = 0; y < 32; y++) {
                store.writeByte(dest + y, scene_bank02_tables_1.SCENE14_ROW_TEMPLATE[y]);
            }
            ...
            const block = scene_bank02_tables_1.SCENE14_ROW_BLOCKS[ptr.toString(16)] ?? null;
    ...
    copyScene16Blobs(branch) {
        const store = this.store;
        scene_bank02_tables_1.SCENE16_A677_BLOB.forEach((b, i) => store.writeByte(0x03e8 + i, b));
        if (branch === 2) {
            scene_bank02_tables_1.SCENE16_A67B_BLOB.forEach((b, i) => store.writeByte(0x0460 + i, b));
        }
    }
    /**
     ...
     */
    writeSingleTileToNt(tile, addrHi, addrLo) {
        ...
        const mapped = scene_bank02_tables_1.TILE_MAP_HIGH[(t - 0xa0) & 0xff] ?? 0x00;
