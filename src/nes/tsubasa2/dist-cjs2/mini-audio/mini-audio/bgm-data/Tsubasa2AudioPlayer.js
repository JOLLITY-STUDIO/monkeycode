...
exports.Tsubasa2AudioPlayer = void 0;
// @ts-nocheck
/**
 ...
 */
const index_1 = __importDefault(require("../../../src/core/papu/index"));
...
const _timing_data_1 = require("./_timing_data");
// Bank 12 原始数据 — SE 音效 header 表 ($8BDA) 与音效轨道数据 ($8000-$9FFF)
// 直接消费结构化数据，无需 MMC3 bank 切换/内存窗口模拟。
const prg_bank_12_1 = __importDefault(require("../rom-data/prg-bank-12"));
// ════════════════════════════════════════════════
// 常量 (Bank 12 ROM)
// ════════════════════════════════════════════════
...
class Tsubasa2AudioPlayer {
    constructor(sampleRate = 48000, onSample) {
        ...
        this.frameCount = 0;
        this.totalChannels = 4; // SQ1=ch4, SQ2=ch5, TRI=ch6, NOISE=ch7
        ...
        this.bgmNesBase = 0; // NES base address of sharedData (raw[0] 对应的 NES 地址 = RAW_START)
        this.bgmHeaderOffset = 0; // header 在 sharedData 内的偏移 (initPtr - RAW_START)
        ...
        const self = this;
        this.papu = new index_1.default({
            opts: { sampleRate, onAudioSample: null },
            cpu: {
                requestIrq(_type) { },
                dataBus: 0,
                haltCycles(_n) { },
                ...
                load(addr) {
                    ...
                    if (addr >= 0xC000 && addr < 0xE000) {
                        if (self._prgRom) {
                            ...
                            if (offset < self._prgRom.length)
                                return self._prgRom[offset];
                        }
                        return 0;
                    }
        ...
        this.seBankData = new Uint8Array(prg_bank_12_1.default);
    }
    ...
    start() {
        ...
        let en4015 = 0x0F;
        if (this.channelMuteMask & 8)
            en4015 &= ~0x01; // SQ1
        if (this.channelMuteMask & 4)
            en4015 &= ~0x02; // SQ2
        if (this.channelMuteMask & 2)
            en4015 &= ~0x04; // TRI
        if (this.channelMuteMask & 1)
            en4015 &= ~0x08; // NOISE
        ...
        for (let ch = 0; ch < NUM_CHANNELS; ch++) {
            ...
            if (nd === 0) {
                ...
                if (blk.timingHi === 0xFF) {
                    const subTable = _timing_data_1.TIMING_SUB_TABLES[blk.timingLo];
    ...
    _sequencerTick(ch, blk, track) {
        ...
        while (true) {
            ...
            if (b >= 0xE0) {
                ...
                if (!ok)
                    return; // command ended the sequence (e.g. $FF)
                continue;
            }
    ...
    _dispatchCmd(ch, blk, data, cmdByte) {
        ...
        switch (cmdIdx) {
            ...
            case 0x00: {
                ...
                const subTable = _timing_data_1.TIMING_SUB_TABLES[tblIdx];
                if (subTable && subTable.length > 0) {
    ...
    _processVolume(ch) {
        ...
        if (finalVol < 0)
            finalVol = 0; // $823B-$823D: BPL / LDA #$00
    ...
    _applyFreqModType1(ch) {
        ...
        if (si >= SEQ_MOD_TABLE_TYPE1.length)
            si = 0;
    ...
    _applyFreqModType2(ch) {
        const blk = this.blocks[ch];
        let si = this.w.seqIdx[ch];
        if (si >= SEQ_MOD_TABLE_TYPE2.length)
            si = 0;
        // Only apply freq modification if a note has been played (baseFreq is valid)
        if (this.w.notePlayed[ch]) {
    ...
    _writeApuReg(ch, group) {
        ...
        if (this.channelMuteMask & (1 << group))
            return;
        const blk = this.blocks[ch];
        const isTri = group === 1;
...
exports.Tsubasa2AudioPlayer = Tsubasa2AudioPlayer;
...
Tsubasa2AudioPlayer.APU_GROUP_BASE = [0x0C, 0x08, 0x04, 0x00];
