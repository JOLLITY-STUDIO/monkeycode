...
class SpriteFrameService {
    ...
    loadSpriteFrame(frameId, baseAddr) {
        const frame = sprite_frame_table_1.BANK19_SPRITE_FRAMES.find(f => f.frameId === frameId);
        if (!frame)
            return;
        let offset = baseAddr;
    ...
    loadSceneTiles(sceneId) {
        return sprite_frame_table_1.BANK19_SCENE_DATA[sceneId] ?? [];
    }
    /** 查询 tile 数据：BANK19_TILE_DATA 查表 */
    getTileData(tileId) {
        return sprite_frame_table_1.BANK19_TILE_DATA[tileId] ?? 0;
    }
}
exports.SpriteFrameService = SpriteFrameService;
