...
const TitleMenuSceneController_1 = require("./TitleMenuSceneController");
/** OpeningScene 特殊场景号(BootRouter 注册表外附加) */
exports.OPENING_SCENE_ID = 100;
...
class OpeningSceneController extends SceneController_1.SceneController {
    constructor(store, input) {
        super(store, input);
        this.sceneId = exports.OPENING_SCENE_ID;
        this.audio = null;
        ...
        if (this.forceFullReset) {
            ...
            for (let ni = 0; ni < 4; ni++) {
                ...
                if (!nt)
                    continue;
                if (nt.tile)
                    for (let i = 0; i < nt.tile.length; i++)
                        nt.tile[i] = 0;
                if (nt.attrib)
                    for (let i = 0; i < nt.attrib.length; i++)
                        nt.attrib[i] = 0;
            }
