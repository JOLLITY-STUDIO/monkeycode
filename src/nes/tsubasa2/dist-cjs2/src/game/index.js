...
const MiniAudioBridge_1 = require("./prg/code/audio/MiniAudioBridge");
/**
 * Tsubasa2 �� ��ϸ������弴�ã�
 *
 * �÷���
...
class Tsubasa2 {
    constructor() {
        ...
        this._webAudio = null;
        /** ��Ƶ�������� (mini-audio PAPU ���͵���, ScriptProcessorNode ��������� WebAudio destination) */
        this._audioSamples = [];
        /** ����д��λ�� */
        ...
        void sprite;
        void spriteAnim;
        // ��Ƶ�Ž� (V0.7 �� mini-audio �Ѳ��Ե� Tsubasa2AudioPlayer ��� stub)
        //   - �ڲ� PAPU ʵ�� (mini-audio �Դ� H5 src/core/papu ��ģ��)
        //   - onSample �ص����� samples �� _audioSamples (ScriptProcessorNode ��������� WebAudio)
        //   - AudioService (F1-F7) �������뵫��ʵ���� (V0.7+ �� PRG bank 12-15 SID �������)
        //
        // V0.7 ��Ƶ���� trace: ͳ��ÿ�� _audioSamples �����ٶ� + onAudioSample call ����
        // �� _audioSamples һֱ�� �� bridge û�� sample �Ƶ� consumer �� ��·�жϡ�
        let _audioSampleCount = 0;
        let _audioLastLog = 0;
        this.audio = new MiniAudioBridge_1.MiniAudioBridge(44100, (l, r) => {
            this._audioSamples.push((l + r) / 2);
            _audioSampleCount++;
            if (_audioSampleCount - _audioLastLog >= 4410) {
                _audioLastLog = _audioSampleCount;
                console.log(`[tsubasa-audio] ${_audioSampleCount} samples @ frame=${this._frame} l=${l.toFixed(3)} r=${r.toFixed(3)}`);
            }
        });
        // ��Ƶ��������� WebAudio + ScriptProcessorNode (mini-audio PAPU sample �� destination)
        this._initAudio();
    ...
    /**
     * ��ʼ����Ƶ��������� WebAudio ������ + AudioBufferSourceNode (loop=true)
     *
     * V0.7.2 ·���л����� ScriptProcessorNode �첽 producer/consumer ������Ϊ
     * AudioBufferSourceNode ���� loop ����Ԥ��Ⱦ PCM��
     *
     * ·����
     *   1) ���� WebAudio context
     *   2) ����һ������ placeholder AudioBufferSourceNode (loop=true) �ӵ� destination
     *      �� ��֤ destination ������ source��BGM PCM ׼����ʱ���滻
     *   3) MiniAudioBridge.onPcmReady(cb) ���� �� cb(pcm, sr) ��ʱ:
     *      - ������ AudioBuffer �� pcm ���ƽ�ȥ
     *      - ͣ���� source������ source, loop=true start
     *      - Log ״̬
     *
     * ScriptProcessor ·������Ϊ fallback (V0.7.1 �ۻ� samples �� onaudioprocess)��
     *
     * AudioContext resume: ����ǰһ����user gesture ������
     */
    _initAudio() {
        try {
            // ���� WebAudio ������
            ...
            this._webAudio = wac;
            // ���� destination master gain (��������)
            const masterGain = wac.createGain ? wac.createGain() : null;
            if (masterGain) {
                masterGain.gain.value = 0.7;
                masterGain.connect(wac.destination);
            }
            // V0.7.2 ·��: AudioBufferSourceNode + loop, �� MiniAudioBridge.onPcmReady �����滻
            let _loopSrc = null;
            const installLoopBuffer = (pcm, sr) => {
                if (!pcm || pcm.length === 0) {
                    console.log('[tsubasa-audio] installLoopBuffer: pcm Ϊ�գ�����');
                    return;
                }
                try {
                    // ͣ�� source
                    ...
                    // ���� buffer + source���ֲ� const������հ�������� null ��խ���⣩
                    const buf = wac.createBuffer(1, pcm.length, sr);
                    buf.copyToChannel(pcm, 0);
                    const src = wac.createBufferSource();
                    src.buffer = buf;
                    src.loop = true;
                    src.connect(masterGain || wac.destination);
                    src.start();
                    _loopSrc = src;
                    console.log(`[tsubasa-audio] installLoopBuffer: ${pcm.length} samples @ ${sr}Hz, ctx.state=${wac.state}`);
                }
                catch (e) {
                    console.log('[tsubasa-audio] installLoopBuffer ʧ��:', e.message);
                }
            };
            // ���� MiniAudioBridge �� pre-rendered PCM
            this.audio.onPcmReady((pcm, sr) => installLoopBuffer(pcm, sr));
            // V0.7.1 ������ ScriptProcessor fallback (ͬʱ���� destination)
            // �� AudioBufferSourceNode ·��û��������·�����ṩ PCM ��
            ...
            console.log('[tsubasa] ��Ƶ��ʼ�����: WebAudio + AudioBufferSourceNode (loop) + ScriptProcessor fallback (mini-audio PAPU bridge)');
            // V0.6.5 �Զ� resume ��Ƶ������
