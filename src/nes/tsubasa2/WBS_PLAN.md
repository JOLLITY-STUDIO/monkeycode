...
| v1.0.0 | CHR 资源化 + 全量差分验证 + 优化重构 | ✅ | 2026-08-23: G21-G42 全 ✅. 开场链路完整 (MMC3保留去6502 + 调色板渐显修复 + NMI入口语义确认). 300帧验证: f=90渐显完成TECMO Theater显示 (bufNz=862), f=150 bufNz=1220, f=300 bufNz=1062 进入下一阶段 |
...
| G39 | **保留MMC3去6502: PRG bank切换缺口补完** | ✅ | 2026-08-23: 新增 PrgBankService (封装 MMC3 R6/R7 bank 切换, switchR6/switchR7 直接调 mapper.write(0x8000/0x8001) 触发 executeCommand→load8kRomBank→更新 prgBankMap, getBankOfWindow/currentR6/7/getBankAtCpuAddr 查询, RAM 同步 ram_0024/0025). 接入组合根 Tsubasa2.prgBank + frame() 首帧 attachMapper. 接通 5 处调用点: _resumeCoroutine 协程恢复切 R6/R7 (原只写RAM), sceneLoad 切 bank07+切回 (原省略), sub84C5Gen 切脚本 bank ram_0056 (原省略), bankSwitchR6/R7 ($9B07 H5等价), HardwareInitService.bootScene 初始 bank0→R6 bank2→R7 (原只写RAM). 13/13 PASS. tsc零错误 |
| G40 | **sub838A bank2/bank6 切换接通 + subA215 stub 补全** | ✅ | 2026-08-23: sub838A 封装 asm $838A (切 R7=bank2 调 $A215 切回 R7=bank6). tsnes disasm dump bank2 $A8CE 运行时反汇编确认 $A215=JMP $A8CE 本体: LDA#01 JSR 9FA8 (协程让出1帧) + LDY#0 循环64精灵 LDX 0468,Y LDA 046A,Y AND#0C BEQ用原Y/LDX#F8 TXA STA 0200,Y 复制tile/attr/X INY*4 BNE + RTS. 语义=精灵OAM刷新 影子0468→硬件0200 attr bit2-3!=0→Y=F8隐藏. 等价 ShadowOam.copyToHw(). sub838A 调 copyToHw 替换空 stub. 接通 palAnimBg ($833A/$8347) + palAnimSpr ($8375) + sub82ECGen ($82ED) 调用点. tsc零错误 |
| G41 | **画面全黑根因#3: 调色板渐显逻辑修复** | ✅ | 2026-08-23: writePalettes 用 (pal & 0x0f) | fadeBg 破坏 NES 调色板索引 (6位 0-0x3F). 0x30(白)&0x0f=0 |0x0f=0x0f(黑) → 白色变黑. tsnes disasm dump bank0 $9A7E 确认原版渐显: X=(pal&0x30)+fade; base=PALETTE_FADE_TABLE[X] (ROM $9EA2 64字节); result=(base|(pal&0x0F))&0x3F. fade=15满渐显→result=pal原值; fade=0全暗→0x0F黑. 新增 palette-fade-table.ts (PALETTE_FADE_TABLE 64字节 + fadePalette 函数). 修正 writePalettes. 验证: 修复前 f=80 PPU$3F00=0f0f0f0f bufNz=0; 修复后 f=80 PPU$3F00=0f0f1030 0f362530 bufNz=805; f=90 bufNz=862 画面正常显示 |
| G42 | **G37 B02-02: 24个NMI回调入口语义确认** | ✅ | 2026-08-23: tsnes disasm dump bank0 确认 NMI_CALLBACK_TABLE 表在 bank2 ($A491) 但表里24个地址 $A4C0-$A7FA 指向 bank0 代码 (运行时 $A000 窗口映射 bank0). 之前误认入口在 bank2, bank2 偏移 $24C0 是数据区 (反汇编出非法指令 LAX/SHA). 逐入口确认语义: idx0=脚本装载初始化, idx1=数据区, idx2=推进指针2字节, idx3=调色板刷新, idx4-7=NT填充$2000/2400/2800/2C00, idx8-9=标志辅助, idx10-13=阵容装载, idx14=精灵滚动, idx15=密码续关, idx16=比赛阵容, idx17=JSR$8895, idx18=等待OAM拷贝, idx19=精灵属性bit3, idx20=读脚本字节写$004C, idx21=读脚本让出1帧, idx22=推进指针1字节, idx23=切bank2+读脚本. 更新 NmiCallbackIndex 枚举名+注释. tsc零错误 |
