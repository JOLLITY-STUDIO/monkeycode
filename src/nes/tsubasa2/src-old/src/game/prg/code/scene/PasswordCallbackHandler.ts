/**
 * PasswordCallbackHandler �?密码界面回调处理�? * @bank 02 ($A000-$BFFF 窗口)
 *
 * 职责: 密码界面渲染 ($84C1-$8559, NMI 回调 idx 0),
 *       密码→数据解码校�?($82E8-$8335, NMI 回调 idx 23)�? *
 * �?Bank02Service.resetEntry �?idx 0/23 时调�? 不独立分发�? */
...
export class PasswordCallbackHandler {
  ...
  /**
   * $9FA8 waitCounter �?等待 vblank 帧边�?(bank00 $9FA8)�?   * asm: STA $0019 (存帧数到 ram_0019); 压栈寄存�? 挂起协程 �?帧调度恢复�?   * 翻译�? 帧同步由外部帧循环驱�? 此处�?ram_0019 (帧数, 语义占位)�?   * @param frames 等待帧数 (A 寄存�? asm 传入)
   */
  ...
  /**
   * $9A35 renderRefresh �?渲染刷新 + 渐隐初始�?(bank00 $9A35)�?   * asm: JSR $9B07 (NT 刷新); JSR $9AB8 (OAM 刷新); JSR $9ADA (调色板刷�?;
   *      LDX $00E9; JSR $C4B9 (�?bank); LDA #$0F; STA $004A; STA $004B; JMP $9A71 (渐隐)
   * 翻译�? NT/OAM/调色板刷新由 PpuSync 驱动, 此处设渐隐计数器�?   */
  ...
  /**
   * $9A0D clearScreen �?清屏 (bank00 $9A0D)�?   * 翻译�? �?OAM + NT�?   */
  ...
  /**
   * $890C spriteBlink �?精灵闪烁 (bank02 $890C)�?   * asm: 切换精灵可见�?(ram_001B bit0 交替)�?   * @param val 闪烁�?(A 寄存�?
   */
  ...
  /**
   * $8920 drawFrame �?画帧 (bank02 $8920)�?   * asm: 两次 ppuFill ($2000/$2400 �? + OAM 清除�?   * @param _frameId 帧编�?(A 寄存�? 未被子程使用)
   */
  ...
  /**
   * $98EA ppuFillByte �?用指定值填 NT �?(bank00 $98EA)�?   * @param fill 填充�?(A 寄存�?
  ...
  /**
   * $99F0 / $98A0 / $9B7F �?清屏/OAM 清除 (bank00)�?   * 翻译�? 委托�?clearScreen�?   */
  protected clearAll(): void {
    this.clearScreen();
  }

  /**
   * $8AF7 sceneLoad �?场景装载 (bank00 $8AF7)�?   * 翻译�? �?ram_00ED + 清状�?(�?service, 此处仅写场景索引)�?   * @param sceneId 场景 ID (A 寄存�?
   */
  ...
  render(): number {
    ...
    // $84C9-$84D6: 48 次循�?(LDA #$01; JSR $9FA8; JSR $890C; DEY; BNE)
    ...
    // $84F9: JSR $9A35 (渲染刷新); JSR $88FB (OAM 拷贝)
    // $9A35: NT/OAM/调色板刷�?+ 渐隐初始�?(bank00)
    this.renderRefresh();
    this.oamCopy();

    // $84FF-$8513: 循环 (ram_0044 >= 3 �?
  ...
  /**
   * $88FB oamCopy �?OAM 拷贝 (bank02 $88D0/$88FB)�?   * 把工作精灵表 $0468 拷贝�?OAM $0200�?   */
  ...
  // ════════════════════════════════════════════════════════════════
  // $82E8-$8335 密码→数据解�?(check)
  // ════════════════════════════════════════════════════════════════

  /**
   * 密码→数据解�?(对应原始 $82E8-$8335)�?   *
   * asm 流程:
   *   $82E8: LDA $57; BMI $8338 (bit7=1 跳到密码分支)
   *   $82EC: STA $00ED (�?ram_0057 值到 ram_00ED)
   *   $82EE: LDA #$00; LDY #$FA; 循环 STA $FFEC,Y (�?$05E6-$05EB �?6 字节)
   *   $82FA: LDA #$01; JSR $9FA8 (�?1 �?
   *   $82FD: LDY $00ED
   *   $82FF-$8333: 密码解码循环 (5 �? ram_00EC �?0 步进 3 �?0x0F):
   *     LDA #$00; STA $00EC; TYA; AND #$0F; LSR; TAX
   *     LDA $AADF,Y; CLC; ADC $00E6,X; STA $00E6,X (低字节累�? 增量=tbl[Y])
   *     LDX $00EC; LDA $AAE0,Y; ADC $007A,X; STA $007A,X (高字节累�? 增量=tbl[Y+1], 表重�?1 字节)
   *     LDA $AAE0,Y; BPL $8322 (bit7=0 跳过符号扩展)
   *     LDA #$FF; BNE $8324 (bit7=1 符号扩展 $FF)
   *     $8322: LDA #$00; ADC $007B,X; STA $007B,X (进位累加)
   *     INY; INY; ram_00EC += 3; CMP #$0F; BNE $8303 (循环 5 �?
   *   $8335: JMP $A2F8 (跳到后续处理)
   *
   * 位置增量�?$AADF (16 字节, 2026-08 已校�?:
   *   $10,$00,$10,$00,$40,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00
   * Y=0/2/4/6/8 �?LO=tbl[Y]=$10/$10/$40/$00/$00, HI=tbl[Y+1]=$00 (全零, 无符号扩�?�?   *
   * 密码输入通过 OAM 精灵选择假名 (由其�?NMI 回调处理), 不是字符串�?   * 此方法读 ram_0057 (密码种子) 做解�? 不接收字符串参数�?   */
  check(): boolean {
    // $82E8: LDA $0057; BMI $8338
    const r57 = this.rd(0x0057);
    if ((r57 & 0x80) !== 0) {
      // bit7=1: 走密码分�?($8338), 不做解码
      return false;
    }
    ...
    // $82EE-$82F6: �?$05E6-$05EB �?(6 字节, LDY #$FA; STA $FFEC,Y; INY; BNE)
    for (let i = 0; i < 6; i++) {
      this.wr(0x05E6 + i, 0);
    }
    ...
    // $82FF-$8333: 密码解码循环 (ram_00EC �?0 步进 3 �?0x0F, �?5 �?
    let ec = 0;
    while (true) {
      this.wr(0x00EC, ec);
      ...
      // LDA $AADF,Y; CLC; ADC $00E6,X; STA $00E6,X (低字节累�?
      const loInc = PASSWORD_POS_INC_TABLE[y] ?? 0;
      const e6Old = this.rd(0x00E6 + x);
      const e6Sum = e6Old + loInc;
      this.wr(0x00E6 + x, e6Sum & 0xff);

      // LDX $00EC; LDA $AAE0,Y; ADC $007A,X; STA $007A,X (高字节累�? 带进�?
      // $AAE0 = $AADF+1, 高字节增�?= 同一表的下一个字�?      const hiInc = PASSWORD_POS_INC_TABLE[y + 1] ?? 0;
      ...
      // ADC $007B,X; STA $007B,X (进位累加到更高字�?
      ...
      y = (y + 2) & 0xff;
      ec = (ec + 3) & 0xff;
      // CMP #$0F; BNE $8303 (ram_00EC != 0x0F 时继�?
      if (ec === 0x0F) break;
    }
