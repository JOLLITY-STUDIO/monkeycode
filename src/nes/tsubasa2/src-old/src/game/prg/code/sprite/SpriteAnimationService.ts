...
import {
  ...
  ANIM_FRAME_PTR_A42A,
  SPRITE_ANIM_DATA_27,
} from '../../data/tables/sprite-animation-table';
...
export class SpriteAnimationService {
  ...
  private _read27(addr: number): number {
    const off = (addr - 0xa000) & 0x1fff;
    return SPRITE_ANIM_DATA_27[off] ?? 0;
  }
  ...
  private _playerRecordPtr(idx: number): void {
  ...
  private _coordToTile(a: number): { x: number; y: number } {
    let v = a & 0xff;
    let q = 0;
    while (v >= 0x0c) {
      v -= 0x0c;
  ...
  private _coordToOamSlot(x: number, y: number): number {
    const cx = (x & 0xff) - 0x30;
    if (cx < 0 || cx >= 0xa0) return 0xff;
    const col = cx >> 3;
    const cy = (y & 0xff) - 0x50;
    if (cy < 0 || cy >= 0x60) return 0xff;
    const row = cy >> 3;
  ...
  loadAnim(returnAddrLo: number, idx: number): void {
    ...
    if (flag) {
      ...
      animPtr = (PTR_AB65[t] | (PTR_AB65[(t + 1) & 0xff] << 8)) & 0xffff;
    } else {
      // $8149-$8156: $003F = 0x25; 索引 = $002C*2; PTR_A6AD
      this.wr(0x003F, 0x25);
      const c = this.rd(0x002C + x);
      const t = (c * 2) & 0xff;
      animPtr = (PTR_A6AD[t] | (PTR_A6AD[(t + 1) & 0xff] << 8)) & 0xffff;
    }
    ...
    if (this.rd(0x0032) < this.rd(0x00E2)) return;
  ...
  update(frame: number): void {
    ...
    if ((f4 & 0x80) === 0) {
      ...
      this._animPtr = (ANIM_PTR_A292[t] | (ANIM_PTR_A292[(t + 1) & 0xff] << 8)) & 0xffff;
      this.wr(0x05F5, 0);
    }
    // $8215-$821D: JSR $C515 (等一帧); LDA $0515; BNE 重试 — H5: OAM 忙则等下一帧
    if (this._store.oam.isBusy()) return;
    ...
    for (;;) {
      ...
      if (v === 0xff) {
        ...
        if (this.rd(0x05E3) === 0) this.wr(0x05F4, 0);
        y = 0;
        continue;
      }
      ...
      const framePtr = (ANIM_FRAME_PTR_A42A[t] | (ANIM_FRAME_PTR_A42A[(t + 1) & 0xff] << 8)) & 0xffff;
