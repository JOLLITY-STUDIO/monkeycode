/**
 * Bank02Service �?bank 02 完整翻译 ($8000-$9FFF, 运行�?$A000-$BFFF 窗口)
 *
 * 职责:
 *   §1 NMI 渲染主程 ($8000-$815F): OAM DMA、NT buffer 写入、滚动寄存器、手柄读取、MMC3 bank 切换
 *   §2 滚动/CHR bank 辅助 ($8160-$81E3): 滚动设置 A/B、MMC3 CHR bank 切换
 *   §3 跳转�?+ pre-main-loop ($820C-$82E5): 4 入口跳转表、pre-main-loop 初始�? *   §4 $8484 场景分发�?($8484-$8490): LDA ram_00ED; ASL; TAX; �?PASSWORD_DISPATCH_TABLE ($A491) �?PHA/PHA/RTS 跳转
 *   §5 密码场景分支 ($8338-$83D5): 密码校验分支
 *   §6 24 入口场景帧处�?($855A-$87FA): $8484 分发表的目标子程, 每个返回下一帧分支号
 *   §7 精灵/阵容辅助 ($872C-$88FD): 精灵批量生成、精灵表拷贝、阵容装载、OAM 拷贝
 *
 *   ram_00ED = 场景索引 (0-23), �?Bank00Service.sceneLoad / 脚本 OpSceneLoad (0xFA) 写入�? *   分发表共 24 �?16 位入口地址, 索引�?ram_00ED 值�? *
 *   注意: "场景帧处理分�?�?bank0 $8AF7 sceneLoad (场景装载/初始�? 是两个不同概�?
 *   - sceneLoad(sceneId) (bank0 $8AF7): 装载场景, �?ram_00ED, 清状�? �?bank
 *   - $8484 dispatcher (bank02): 每帧�?ram_00ED 分发到场景帧处理子程
 *
 * 已在其他 service 覆盖 (不在此重�?:
 *   - $82E8 密码→数据解�?�?PasswordCallbackHandler.check
 *   - $84C1 密码界面初始�?�?PasswordCallbackHandler.render
 *
 * 命名规范: 旧名 BootRouter �?新名 Bank02Service�? */
...
/**
 * 标准 NES NTSC 64 色调色板 (0xRRGGBB)�? * �?BootBackgroundRenderer.NES_NTSC_RGB 一�? 用于 $3F00 buffer 回放:
 *   NES 调色板索�?($05E8 buffer 数据) �?RGB �?DataStore.paletteTable �?writePalettes �?PPU
 */
...
/**
 * NMI 回调索引 (对应 NMI_CALLBACK_TABLE �?24 项入�?�? *
 * G37 确认 (tsnes disasm dump bank0): NMI_CALLBACK_TABLE 表本身在 bank2 ($A491),
 * 但表里的 24 个入口地址 ($A4C0-$A7FA) 指向 bank0 的代�?(运行�?$A000 窗口映射 bank0)�? * 之前误认为入口在 bank2, 实际 bank2 偏移 $24C0 处是数据�?(反汇编出非法指令 LAX/SHA)�? *
 * 这些�?NMI 每帧�?ram_00ED 索引调用的子�? 不是游戏场景:
 *   - 渲染/NT 填充/OAM 清空/精灵属性设�? *   - 阵容数据装载�?RAM
 *   - 密码界面绘制/续关/校验
 *   - bank 切换/标志置位
 *
 * 游戏场景 (标题/会议/比赛/结果) 由脚本引�?OpSceneLoad (0xFA) 驱动,
 * �?bank00 sceneLoad �?主循环调�? 与此表无关�? */
export enum NmiCallbackIndex {
  /** idx 0 �?$A4C0 (bank0): TYA; LDX $ED; JMP $C4B9 (�?bank=ram_00ED) + 脚本装载初始�?(LDA#$08 STA$55 �? */
  CALLBACK_00_SCRIPT_LOAD_INIT = 0,
  /** idx 1 �?$A559 (bank0): 数据�?跳转�?(非可执行代码, 反汇编为非法指令) */
  CALLBACK_01_DATA_TABLE = 1,
  /** idx 2 �?$A57B (bank0): LDA#$02; JMP $8879 �?推进脚本指针 2 字节 */
  CALLBACK_02_ADVANCE_PTR_2 = 2,
  /** idx 3 �?$A581 (bank0): JSR $9FA8 (让出1�?; JSR $997E (paletteWriteAll) �?调色板刷�?*/
  ...
  /** idx 21 �?$A7CE (bank0): INY; LDA ($4D),Y; JSR $9FA8 �?读脚本字节让�?1 �?*/
  CALLBACK_21_SCRIPT_READ_YIELD = 21,
  /** idx 22 �?$A7D6 (bank0): JMP $8879 �?推进指针 (单字�? */
  CALLBACK_22_ADVANCE_PTR_1 = 22,
  /** idx 23 �?$A7FA (bank0): EOR $ED85; INY; LDA ($4D),Y; STA $EC; LDX#$02; JSR $C4B9 �?�?bank2 + 读脚�?*/
  CALLBACK_23_SCRIPT_SWITCH_BANK2 = 23,
}

/** @deprecated 旧名 TaskIndex, 等价�?NmiCallbackIndex */
...
export class Bank02Service {
  ...
  /** BOOT 开场背景已渲染标志 (只在 BOOT 初始化时渲染一�? */
  ...
  /**
   * NMI 回调分发 (resetEntry) �?依据 ram_00ED 索引分发到对应回调子程�?   * 对应原始 $8484 分发�?+ resetEntry (asm $8281/$826D �?�?   *
  ...
  resetEntry(index: number): void {
    ...
    switch (i) {
      ...
      case NmiCallbackIndex.CALLBACK_02_ADVANCE_PTR_2:
        // ram_00ED=2 = BOOT 开�?(tsnes trace 实测开场第一个画�?$ED=2)�?        // 之前 resetScene(2) �?default �?_initScene(2) 空实�?�?BOOT 背景从不渲染
        // �?nt0=0 黑屏。case 2 �?case 0 都挂 _initBoot (BOOT 开场初始化 + 背景渲染)�?        this._initBoot();
        break;
  ...
  /** $8484 分发回调 (�?update 每帧调用) */
  onDispatched(index: number, _target: number): void {
    this.resetEntry(index);
  }

  /** 密码校验 �?委托�?PasswordCallbackHandler.check (�?ram_0057 做解�? */
  verifyPassword(): boolean {
    return this._password.check();
  }

  /** idx 0 回调: BOOT 初始�?(对应原始 $821D-$8281) */
  protected _initBoot(): void {
    ...
    this.wr(0x008F, 2);
    this.wr(0x0091, 2);
    // tsnes trace 实测: 开�?NT0 几乎全零 (�?25 �?TECMO 文字 tile),
    // �?NMI 渲染主程 $05E8 buffer 循环写入, 不需�?BootBackgroundRenderer 预填充�?    // BootBackgroundRenderer �?SCENE_0x0A + bank08 metatile 填了 672 个错�?tile, 已移除�?    // 调色板也�?NMI 渲染主程 $05E8 buffer 写入 ($3F00-$3F1F), 不需要预填充�?    // NT/调色板数据由协程调度�?($9F0F) 驱动场景脚本代码填充 $05E8 buffer,
    // nmiRender 消费 $05E8 buffer 写入 PPU�?  }

  /** 通用回调处理 (其余索引�?§6 callbackNN 方法覆盖) */
  protected _initScene(_index: number): void {
    // �?NMI 回调�?§6 callbackNN 方法覆盖
  }

  /**
   * $8484 场景分发�?(对应原始 $8484:)�?   * LDA ram_00ED �?ASL �?TAX �?�?PASSWORD_DISPATCH_TABLE �?跳转�?   *
   * TS 版用 16 位数组索引替�?ASL + 字节查表 (ASL 是字节偏�? 数组索引已隐�?�?   *
   * @param index 场景索引 (ram_00ED �? 0-23)
   * @returns 被分发到的目标地址 (16 �? $A000 窗口偏移), �?-1 越界�?   */
  ...
  /**
   * 场景分发主入�?(对应原始 $8484 的调用语�?�?   * 翻译版不执行 6502 �?PHA/RTS 跳转, 直接�?resetEntry 分发�?   */
   ...
   * $8000-$815F NMI 渲染主程�?   *
   * asm 流程:
   *   $8000-$800D: OAM DMA (�?$2003/$4014, ram_0628 标志检�?
   *   $800F-$8046: NT buffer 写入 (ram_05E8 �?�?$2006/$2007)
   *   $8048-$805A: �?ram_0628 + $2006 复位
   *   $805D-$808E: 滚动寄存器写�?($2000/$2001/$2005)
   *   $8091-$80AF: MMC3 IRQ 计数器设�?+ bank 切换
   *   $80B1-$80D4: CHR bank 切换 ($8000/$8001 寄存�?
   *   $80D7-$8137: 手柄读取 (8 位循�? ram_001B �?
   *   $8116-$8137: ram_00E1/E2/E3 累加 + ram_003A 递增
   *
   * 翻译�? PPU/OAM/滚动�?core/ppu + PpuSync 驱动, 手柄�?Controller 驱动,
   *         MMC3 bank 切换�?mapper4.prgBankMap 维护�?   *         此方法翻�?asm 语义, 硬件操作通过 DataStore 缓存状态�?   */
  nmiRender(): void {
    // ── $8000-$800D: OAM DMA ──
    // LDA #$00; STA $2003 (OAM 地址=0); LDA #$02; STA $4014 (OAM DMA, �?$0200)
    // LDA $0628; BEQ $805D (�?NT buffer 更新则跳�?
    if (this.rd(0x0628) !== 0) {
      // BIT $0629; BVS $805D (bit6=1 跳过)
      if ((this.rd(0x0629) & 0x40) === 0) {
        // $8014: LDA #$00; STA $2001 (关渲�?
        // ── $800F-$8046: NT buffer 写入 ──
        // LDX #$00; 循环: LDA $05E8,X; BPL $8026; AND #$3F; LDY #$84
        //   STY $2000; TAY; LDA $05EA,X; STA $2006 (NT 地址�?;
        //   LDA $05E9,X; STA $2006 (NT 地址�?; LDA $05EB,X; STA $2007 (tile)
        ...
        while (true) {
          ...
          // $8026-$8034: LDA $05EA,X �?$2006 (地址�?; LDA $05E9,X �?$2006 (地址�?
          const vramAddr = (this.rd(0x05EA + x) << 8) | this.rd(0x05E9 + x);
          // $8036-$803E: 循环 count �? 每次 INX �?LDA $05EB,X �?STA $2007
          //   (每迭�?INX 读不同字节写 PPU, PPU 地址自动递增 �?不是重复写同一 tile!)
          // 翻译�? 直写 DataStore �?NT �?($2000-$2FFF) �?writeNT,
          //         调色板区 ($3F00-$3FFF) �?paletteTable (NES 索引 �?RGB)
          for (let i = 0; i < count; i++) {
            const val = this.rd(0x05EB + x + i);
            this.writeVramByte(vramAddr + i, val);
          }
          // INX ×count (数据循环�? + INX×3 (跳过 addrLo/addrHi) �?下一�?ctrl 位置
          x = (x + count + 3) & 0xff;
          if (this.rd(0x05E8 + x) === 0) break;
        }
        // $8048: LDA #$00; STA $0628 (�?NT buffer 标志)
        this.wr(0x0628, 0);
    ...
    // ── $805D-$808E: 滚动寄存器写�?──
    // LDA $0021; STA $2001 (渲染开�?
    // LDA $0079; BPL $8073 (bit7=0 走简单滚�?
    ...
    if ((r79 & 0x80) !== 0) {
      // bit7=1: �?$2006 (NT 选择) �?翻译版由 PpuSync 处理
    } else {
      // bit7=0: 滚动位计�?      let g20 = (this.rd(0x0020) >> 2) & 0xff; // LSR×2
    ...
    //   LDY #$08; 循环 8 �? LDA $4015,X; LSR; ROL $003F; AND #$01; ORA $003F; STA $003F; DEY; BNE
    //   CMP $0041; BEQ $8107; DEC $0040; BNE $80DF (重试)
    //   $8107: LDA $001B,X; EOR $003F; AND $003F; STA $001D,X; LDA $003F; STA $001B,X; DEX; BNE $80DD
    // 翻译�? H5 只有一个输入源 DataStore KV 'input_mask' (bit0=A,1=B,2=SEL,3=START,
    //   4=UP,5=DOWN,6=LEFT,7=RIGHT), 已由 InterruptService._readInput ($C982 翻译)
    //   维护 ram_001C (joypad1 状�? �?ram_001E (joypad1 按下�?�?    //   原版此处是对硬件手柄的第二次读取 (边沿 = 两次读取之间的新�?, �?H5 单输入源
    //   二次计算恒为 0, 且旧实现读从未写入的 controller_1 键把 $001E 每帧清零,
    //   导致 sub801E ($802C �?UP 按下�? 永远等不到输�?�?开场无法推进�?    //   故此处不再重复写 $001C/$001E/$001D/$001F (无第二手柄输入源)�?    void 0;
  ...
  /**
   * 写单�?VRAM 字节 ($2007 写语�? 原版 PPU 地址自动递增)�?   * 翻译版直�?DataStore (�?CPU �? �?PPU 寄存�?:
   *   $2000-$2FFF (NT �? �?writeNT 网格 (nt0: $2000 基址 / nt1: $2800 基址,
   *                �?src/game/index.ts writeNameTable 的物理布局一�?
   *   $3F00-$3FFF (调色板区) �?paletteTable (NES 调色板索�?�?RGB)
   *   其他地址 (图案�?滚动寄存器等) �?�?CHR/滚动管线管理, 此处忽略
   */
  protected writeVramByte(vramAddr: number, val: number): void {
    ...
    if (addr >= 0x2000 && addr <= 0x2fff) {
      ...
      if (a < 0x3c0) {
        // 0x3c0 起是属性表字节 �?翻译版属性表�?writeNameTable �?entry.palette 计算, 忽略原始属性字�?        const tx = a % 32;
        const ty = (a / 32) | 0;
        if (ty < 30) {
  ...
  /**
   * 写单个调色板字节 ($3F00-$3F1F, NES 调色板索�?�?DataStore.paletteTable RGB)�?   * $3F00-$3F0F = BG 4 �?(�?4 字节一�? 通用�?3 �?, $3F10-$3F1F = SPR 4 组�?   * 镜像�?($3F04/$3F08/$3F0C = $3F00, $3F14/$3F18/$3F1C = $3F10) 直接写同色�?
   * 与真�?PPU 镜像结果一�?(游戏整段�?$3F00 时该位本来就是通用�?�?   */
  ...
  /**
   * $8160-$81BF 滚动设置 A�?   * asm: STA $E000/$E001 (MMC3 IRQ 关闭), LDX $0078; LDA $0078,X;
   *      BPL $818D (bit7=0 走简单滚�?;
   *      bit7=1: 延迟 6 循环, LDA $0079,X (Y 滚动), LDY $007A,X (X 滚动),
   *              �?$2006 (NT 选择), $2000 &= $FC (清滚动位), $2005=0/0, JMP $A1A8;
   *      bit7=0 ($818D): 延迟 2 循环, LSR $0020, LDA $007A,X; LSR; ROL $0020,
   *              $2000=$0020, $2005=$0079,X, $2005=0�?   *
   * @param x ram_0078 索引
   */
  scrollSetupA(x: number): void {
    // STA $E000/$E001 (�?MMC3 IRQ) �?翻译�?IRQ �?mapper4 管理, 此处 no-op
    ...
    if ((flag & 0x80) !== 0) {
      ...
      // �?$2006 (NT 选择) �?翻译版由 PpuSync 处理
      void scrollY; void scrollX;
      // $2000 &= $FC (清滚动位)
      this.wr(0x0020, this.rd(0x0020) & 0xFC);
      // $2005=0/0 (X/Y 滚动归零)
      // JMP $A1A8 �?延迟子程后返�?(翻译�?no-op, 帧同步由外部驱动)
      void x;
    } else {
      // bit7=0 ($818D): 延迟 2 循环
      // LSR $0020
  ...
  /**
   * $81C0-$81CA 滚动设置 B�?   * asm: STA $E000 (�?IRQ), STA $0078 (设索�?, LDY #$18, JSR $A1CB (延迟), RTS�?   */
  scrollSetupB(): void {
    // STA $E000 (�?IRQ) �?no-op (mapper4 管理)
    // STA $0078 �?已由调用方设�?ram_0078
    // LDY #$18; JSR $A1CB (延迟 24 循环) �?翻译�?no-op (帧同�?
    void 0;
  }

  /**
   * $81CB-$81E3 MMC3 CHR bank 切换�?   * asm: LDX $0078,Y (X = ram[$0078+Y], 零页 Y 变址);
   *      LDA #$00; ORA $0022; STA $8000; STX $8001 (�?CHR bank 寄存�?0);
   *      LDX $0079,Y (X = ram[$0079+Y]);
   *      LDA #$01; ORA $0022; STA $8000; STX $8001 (�?CHR bank 寄存�?1)�?   *
   * 零页 $0078/$0079 是两个相邻的 CHR bank 值表 (每项 1 字节), Y 是索引�?   * 翻译�? �?DataStore 零页�? �?CHR bank 缓存�?mapper4 使用�?   *
   * @param y 索引 (Y 寄存器�? �?ram[$0078+Y] �?ram[$0079+Y])
   */
  ...
  // ════════════════════════════════════════════════════════════════
  // §3 跳转�?+ pre-main-loop ($820C-$82E5)
  // 对应 code_main.s:209-307
  // ════════════════════════════════════════════════════════════════

  /**
   * $820C-$8215 4 入口跳转表�?   * asm: JMP $A855 / $A86E / $A484 / $A8CE
   *
   * @param index 0-3 选择跳转目标
   * @returns 目标地址 (运行�?$A000 窗口)
   */
  ...
  /**
   * $82AC-$82E5 pre-main-loop 初始化�?   * asm: JSR $99F0 (清屏), JSR $9B7F (oamClear), �?NMI ($2000 &= $7F),
   *      �?$FF19-$FFFF �?(0xE8 字节), �?$FFE0-$FFFF �?(0x20 字节),
   *      �?ram_00EC=$68, LDA #$98/LDX #$02/LDY #$04, JSR $AA06 (调色�?,
   *      JMP $C557 (进入主循�?�?   */
  preMainLoopInit(): void {
    // $82AC: JSR $99F0 (清屏) �?翻译版由 ntClear/oamClear 替代
    this._store.clearOAM();
    // $82B2: JSR $98A0 (ntClear)
    // $82B5: JSR $9B7F (oamClear) �?已上�?clearOAM 覆盖
    // $82B8-$82BF: �?NMI + �?$2000 bit7
    this.wr(0x0020, this.rd(0x0020) & 0x7f);
    // $82C4-$82CC: �?$FF19-$FFFF �?(0xE8 字节循环)
    for (let i = 0; i < 0xE8; i++) {
      this.wr(0xFF19 + i, 0);
    }
    // $82CE-$82D6: �?$FFE0-$FFFF �?(0x20 字节循环, 与上面重叠但无害)
    for (let i = 0; i < 0x20; i++) {
      this.wr(0xFFE0 + i, 0);
    }
    // $82D8-$82E2: �?ram_00EC=$68, LDA #$98/LDX #$02/LDY #$04, JSR $AA06 (调色板装�?
    this.wr(0x00EC, 0x68);
    // $AA06: 调色板装�?+ NT 初始�?(翻译版由 DataStore.paletteTable 驱动)
    // LDA #$08; STA $2000 (开 NMI + NT 地址增量 32 + 基址 $2000)
    // LDA #$00; STA $2001; STA $2005×2 (关渲�?+ 滚动归零)
    // LDA #$00; LDY #$01; 循环 STA $01FF,Y (�?$0200-$02FF OAM �?
    ...
    // LDX #$10; LDY #$00; 循环 STA $2007 (�?16×256 = 4096 字节 NT)
    for (let y = 0; y < 30; y++) {
    ...
    // 调色板装�? �?ram_00E6/$00E7 指针, 循环 8 �?× 4 �?    // (翻译版由 paletteTable 数据驱动, 此处设默认调色板)
    // $82E5: JMP $C557 (进入主循�? �?翻译版由外部帧循环驱�?  }
  ...
  /**
   * $8338-$83D5 密码场景分支�?   * asm: CMP #$81 (ram_0057 bit7); BEQ $83A3;
   *      $833C: LDX #$67; LDA #$05; JSR $C4BD (�?bank5);
   *      $8343: �?ram_00ED; 写精灵表 $0468-$0498 (9 球员, X �?$78 步进 4 �?$FC);
   *      $8372: JSR $9FA8; 等级调节�?$AB1F/$AB21/$AB22;
   *      $83A3: ram_0568 |= $10; JSR $9FA8; ram_0044/0046=$08; ram_056D-=4;
   *             JSR $9FA8; ram_0044=0; ram_0046=$F8; ram_056D+=4; JMP $A3AB�?   *
   * @returns 下一帧分支号 (asm RTS 返回�? 此子程无 RTS �?JMP)
   */
  passwordBranch(): number {
    ...
    if ((a & 0x80) !== 0) {
      ...
      this.wr(0x0046, 0xF8);
      this.wr(0x056D, (this.rd(0x056D) + 4) & 0xff);
      // JMP $A3AB (跳到场景 14 帧处�? 返回 2)
      return 2;
    }
    // $833C: LDX #$67; LDA #$05; JSR $C4BD (�?bank5) �?翻译�?bank 切换�?ServiceLoader
    // $8343: �?ram_00ED
    this.wr(0x00ED, 0x00);
    let y = 0;
    // $8348-$8370: 写精灵表 $0468-$0498 (X �?$78 步进 4, CPX #$FC)
    ...
    // $8372: JSR $9FA8
    this.waitCounter();
    // $8377-$839E: 等级调节循环 (X �?$78 步进 4, CPX #$FC)
    for (let x = 0x78; x < 0xFC; x += 4) {
    ...
    // JMP $A372 (跳到场景帧处�? 返回 2)
    return 2;
  }

  // ════════════════════════════════════════════════════════════════
  // §6 24 入口场景帧处�?($855A-$87FA)
  // 对应 code_sub.s:204-228 + code_data.s:1-228
  // 每个方法对应 $8484 分发表的一个入�? 返回下一帧主循环分支号�?  // ════════════════════════════════════════════════════════════════

  // ── 辅助子程 stub (被场景帧处理调用, 完整实现待后续翻�? ──

  /**
   * $8895 rosterLoadMain �?装载主力阵容�?$0408 区�?   * asm $8891: LDY #$00; 循环 LDA $AA47,X; STA $0408,Y; INX; TYA; CLC; ADC #$04;
   *            TAY; CMP #$28; BCC (10 球员循环, $0408 起每 4 字节 1 球员)�?   * @param teamIdx 球队索引 (X 寄存�? 指向 ROSTER_TABLE 偏移)
   */
  ...
  /**
   * $8920 drawFrame �?画帧 (NT 填充 + OAM 清除)�?   * asm: LDA #$00; STA $00E6; LDA #$20; STA $00E7; LDY #$10; LDX #$20; JSR $98EA (ppuFill $2000 �?16×32);
   *      LDA #$00; STA $00E6; LDA #$24; STA $00E7; LDY #$20; LDX #$20; JSR $98EA (ppuFill $2400 �?32×32);
   *      JSR $9B7F (OAM 清除); RTS
   *
   * 入口 A 寄存�?(frameId) 未被子程使用 ($8920 第一条是 LDA #$00 覆盖 A)�?   */
  protected drawFrame(_frameId: number): void {
    // 第一�?ppuFill: $2000 �? �?0, 16 �?× 32 �?    this.wr(0x00E6, 0x00);
    this.wr(0x00E7, 0x20);
    this.ppuFill16x32(0x2000, 0x00);
    // 第二�?ppuFill: $2400 �? �?0, 32 �?× 32 �?    this.wr(0x00E6, 0x00);
    this.wr(0x00E7, 0x24);
  ...
  /** ppuFill 辅助: �?NT �?16 �?× 32 �?(对应 JSR $98EA, Y=0x10, X=0x20) */
  ...
  /** ppuFill 辅助: �?NT �?32 �?× 32 �?(对应 JSR $98EA, Y=0x20, X=0x20) */
  ...
  /**
   * $9B91 scene02Helper �?�?4 个状态字�?(bank00 $9B91)�?   * asm: LDA #$00; STA $0568; STA $0588; STA $05A8; STA $05C8; RTS
   */
  ...
  /**
   * $9F96 scene04Helper �?检查协程槽 $0000,X 是否 $FF (bank00 $9F96)�?   * asm: LDA $0000,X; CMP #$FF; BNE $9FA1; LDA #$01; JSR $9FA8; LDA #$00; STA $0000,X; RTS
  ...
  /**
   * $9F89 scene05Helper �?协程槽条件置�?(bank00 $9F89)�?   * asm: LDA $0001,X; BEQ $9F95; LDA $0000,X; BNE $9F95; LDA #$01; STA $0000,X; RTS
   * 入口 X=9, �?$000A=0 �?$0009�? 则跳�? 否则 $0009=1�?   */
  ...
  /**
   * $9FA8 waitCounter �?等待 vblank 帧边�?(bank00 $9FA8)�?   * asm: STA $0019; 压栈 X/Y/ED/EC/EB/EA/E9/E8/E7/E6; 挂起协程 �?帧调度恢复�?   * 翻译�? 帧同步由外部帧循环驱�? 此处�?ram_0019 标志 (语义占位)�?   */
  protected waitCounter(): void {
    this.wr(0x0019, 0);
  }

  /**
   * $88FB oamCopy88FB �?OAM 拷贝 ($88D0 的别名入�?�?   */
  protected oamCopy88FB(): void {
    this.oamCopy();
  }

  /**
   * $8976 scene14Helper �?场景 14 数据装载 (bank02 $8976, 被反汇编误标�?.byte)�?   * asm 调用: LDX #$BD; LDY #$23; JSR $8976 (参数 X=$BD, Y=$23)�?   * 功能: �?X/Y 参数装载场景数据�?RAM (具体逻辑待反汇编修复后精确翻�?�?   */
  protected scene14Helper(x: number, y: number): void {
    // $8976 被反汇编器误标为 .byte, 无法精确翻译�?    // 语义: �?X=$BD/Y=$23 装载场景 14 数据 (可能�?NT 模板或脚本指�?�?    this.wr(0x00E6, x & 0xff);
    this.wr(0x00E7, y & 0xff);
  }

  /**
   * $9A35 scene14Helper2 �?渲染刷新 + 渐隐初始�?(bank00 $9A35)�?   * asm: JSR $9B07 (NT 刷新); JSR $9AB8 (OAM 刷新); JSR $9ADA (调色板刷�?;
   *      LDX $00E9; JSR $C4B9 (�?bank); LDA #$0F; STA $004A; STA $004B; JMP $9A71 (渐隐)
   * 翻译�? 渐隐计数器设�?15, 帧循环驱动渐隐效果�?   */
  protected scene14Helper2(): void {
    // NT/OAM/调色板刷新由 PpuSync 驱动 (对应 $9B07/$9AB8/$9ADA)
    // �?bank ($C4B9) �?ServiceLoader 处理
    this.wr(0x004A, 0x0F);
    this.wr(0x004B, 0x0F);
  ...
  /**
   * $A82F scene14Sprite �?精灵属性清�?(运行�?$A82F = asm $882F = spriteAttrClear)�?   * asm: STA $00EC (结束地址); STX $00ED (起始地址); LDY=循环次数;
   *      循环: LDA $0468,X; CMP #$82; BCS; LDA $046A,X; AND #$F3; STA $046A,X;
   *      INX×4; CPX $00EC; BNE; DEY; BNE�?   * @param yCycles 循环帧数 (Y 寄存�?
   * @param xStart 起始地址 (X 寄存�?�?ram_00ED)
   * @param ecEnd 结束地址 (A 寄存�?�?ram_00EC)
   */
  ...
  /**
   * $855A-$8579 场景 idx 1 帧处�?(角度计算)�?   * asm: LDA #$00; STA $0060; LDA $00EC; LSR; ROR $0060; LSR; ROR $0060;
   *      STA $0061; BIT $0062; BMI $8579;
   *      LDA #$00; SEC; SBC $0060; STA $0060; LDA #$00; SBC $0061; STA $0061;
   *      LDA #$03; RTS
   *
   * 计算 ram_00EC >> 2 �?ram_0060/0061 (16 �?, �?ram_0062 bit7=0 则取补�?   */
  callback01(): number {
    this.wr(0x0060, 0);
    ...
    this.wr(0x0060, g0);
    this.wr(0x0061, ec);
    // BIT $0062; BMI $8579 �?�?bit7=1 跳过取补
    if ((this.rd(0x0062) & 0x80) === 0) {
      // 取补: 0 - ram_0060/0061 (16 �?
  ...
  /**
   * $85E9-$85F8 场景 idx 9 帧处�?(�?ram_000D 分支)�?   * asm: LDA $000D; BNE $85FA; LDA #$10; JSR $8895; LDA #$06; JSR $8920; LDA #$02; RTS
   */
  callback09(): number {
    if (this.rd(0x000D) !== 0) {
  ...
  /**
   * $8603-$8612 场景 idx 11 帧处�?(�?ram_000D 分支)�?   * asm: LDA $000D; BNE $8614; LDA #$30; JSR $8895; LDA #$08; JSR $8920; LDA #$02; RTS
   */
  callback11(): number {
    if (this.rd(0x000D) !== 0) {
  ...
  /**
   * $862A-$864E 场景 idx 14 帧处理�?   * asm: LDX #$BD; LDY #$23; JSR $8976; JSR $9A35; LDA #$01; JSR $9FA8;
  ...
  /**
   * $8651-$869A 场景 idx 15 帧处�?(密码续关数据装载)�?   * asm: LDA #$00; STA $00ED; LDY $00ED; LDA $AA97,Y �?ram_00EA/00EB;
   *      ram_007B &1 <<2 | ram_00EB; TAX; INY; LDA $AA97,Y �?ram_00EB; INY;
   *      LDA $AA97,Y; INY; STY $00ED; LDY $00EB; JSR $9B28; AND #$7F; STA $00EB;
   *      循环: LDA #$00; STA $05E8,X; INX; DEC $00EB; BNE (�?NT buffer);
  ...
  callback15(): number {
    ...
    while (true) {
      ...
      // BIT ram_00EA; BMI $869A (bit7=1 �?退出循�? 返回 2)
      ...
      if ((this.rd(0x00EA) & 0x40) !== 0) {
        ...
        this.waitCounter();
        return 2; // JMP $A655 跳到场景帧处�?      }
      // bit6=0: 继续循环 (BVC $8655)
    }
  }

  /**
   * $869D-$86D2 场景 idx 16 帧处�?(比赛阵容装载 A)�?   * asm: LDA $04E5; CMP #$FF; BEQ $86D4; JSR $A767;
  ...
  callback16(): number {
    ...
    // JSR $A767 (spriteTableCopy, 拷贝 PASSWORD_SPRITE_DATA �?$03E8 �?
  ...
  /**
   * $86D4-$872A 场景 idx 16b 帧处�?(比赛阵容装载 B)�?   * asm: JSR $A767; LDY #$80; LDX #$2F; ram_00EA=$02; ram_00ED=$FF; ram_00EC=$FE;
   *      ram_00EB=$07; LDA #$F7; JSR $A72C; LDX #$08; LDA #$FE; JSR $A72C;
   *      LDY #$FC; 拷贝 $A67B �?�?$0460; LDY #$B8; LDX #$1C; ram_00ED=$02;
  ...
  scene16b_frame(): number {
    ...
    // LDY #$FC; 循环: LDA $A67B,Y; STA $0460,Y; INY; BNE (拷贝 4 字节 $A67B �?�?$0460)
    // $A67B = PASSWORD_SPRITE_DATA �?4 字节 ($79,$FF,$03,$C2 �?$FC-$FF 偏移)
    for (let i = 0; i < 4; i++) {
      const y = (0xFC + i) & 0xff;
  ...
  /**
   * $87BE-$87CD 场景 idx 20 帧处理�?   * asm: LDA #$01; JSR $9FA8; LDY #$28; LDX #$64; LDA #$B0; JSR $A82F; LDA #$02; RTS
   */
  callback20(): number {
    this.waitCounter();
  ...
  /**
   * $87D7-$87F9 场景 idx 22 帧处�?(精灵属性批量设�?�?   * asm: LDY #$80; LDA #$01; JSR $9FA8; LDX #$20;
   *      循环: LDA $0468,X; BPL $87ED; LDA $046A,X; ORA #$04; STA $046A,X;
   *      INX×4; CPX #$C4; BNE; DEY; BNE; LDA #$02; RTS
   *
   * 128 �?(Y=0x80) 循环, 每帧扫描 $0468-$04C4 工作精灵�?(X �?0x20 步进 4),
   * 若精�?Y 坐标 ($0468,X) < 0x80 (BPL), 则给属性字�?($046A,X) �?bit2�?   */
  callback22(): number {
  ...
  /**
   * $8767-$8771 精灵表拷贝�?   * asm: LDY #$FC; 循环: LDA $A677,Y; STA $03E8,Y; INY; BNE
   * �?$A677 �?(4 字节: $79,$FF,$03,$C2) 拷贝�?$03E8 区�?   * Y �?$FC 开�? INY �?$00 �?BNE 失败退�? 共拷 4 字节 ($FC/$FD/$FE/$FF �?$03E4-$03E7)�?   */
  spriteTableCopy(): void {
    const table = [0x79, 0xFF, 0x03, 0xC2]; // asm $A677 表内�?    for (let i = 0; i < 4; i++) {
      const y = (0xFC + i) & 0xff;
  ...
  /**
   * $882F-$8853 精灵属性清除�?   * asm: STA $00EC; STX $00ED; LDA #$01; JSR $9FA8;
   *      循环: LDA $0468,X; CMP #$82; BCS; LDA $046A,X; AND #$F3; STA $046A,X;
   *      INX×4; CPX $00EC; BNE; DEY; BNE
   *
   * @param ec 结束地址低字�?(ram_00EC, CPX 比较)
   * @param ed 起始地址高字�?(ram_00ED, 未直接使�?
   * @param yCycles 循环次数 (Y 寄存�? 外层帧数)
   */
  ...
  /**
   * $8857-$88B5 阵容装载�?   * asm: LDA $00E4; CMP $0026; BCS $88A8;
   ...
   *
   * �?ram_0026 (球队索引) �?ROSTER_TABLE 偏移, 装载 10 球员�?$0408 区�?   */
  rosterLoad(): void {
    const e4 = this.rd(0x00E4);
    const team = this.rd(0x0026);
    if (e4 >= team) {
      // BCS $88A8: 直接走属性装�?      this.rosterAttrLoad(team);
      return;
    }
    // �?team 选偏�?    let x = 0;
    if (team === 0x00) {
    ...
    // $8891: 装载 10 球员�?$0408 (�?4 字节 1 球员)
    this.rosterLoadMain(x);
    // $88A3: 属性装�?    this.rosterAttrLoad(team);
  }

  /** $88A3 属性装�? LDA $AA75,X; STA $002A; ram_0026+3 �?ram_002B */
  ...
  /**
   * $88B7-$88CC 阵容装载 B (替补�?�?   * asm: LDA #$0B; STA $00ED; LDY #$00;
   *      循环: LDA $AA47,X; STA $0300,Y; INX; TYA; CLC; ADC #$0C; TAY;
   *             CMP #$84; BCC (11 球员循环, 0x0C 字节步长);
   *      RTS
   *
   * @param xStart 起始索引 (X 寄存�? 指向 ROSTER_TABLE 偏移)
   */
  ...
  /**
   * $88D0-$88FD OAM 拷贝�?   * asm: LDA #$01; JSR $9FA8; LDY #$00;
   ...
   *      INY×4; BNE (64 精灵循环)
   *
   * �?$0468-$0467,FF �?(工作精灵�? 拷贝�?$0200-$02FF (OAM)�?   * 隐藏条件: $046A,Y & $0C != 0 �?Y = $F8 (屏幕�?
   */
  oamCopy(): void {
    this.waitCounter();
