/**
 * MatchSceneService �?bank19 比赛场景辅助逻辑 ($8000-$9FFF, 运行�?$A000-$BFFF)
 * @bank 19
 *
 * 职责: 比赛场景切换 + 脚本引导 + 精灵属性设�?+ 比赛事件分派�? *   入口 $9000: �?$0441=9 (场景bank�?bank09), 通过 $0088/$0089 指针 ($B467) 读脚本流�? *   循环 $902D: 逐字节读脚本, A>=$E0 走高字节事件, A<$E0 走低字节精灵设置�? *   $9043: 低字节子�?(精灵属�?坐标设置, �?$04A5 �? 填精灵表)
 *   $915A: 高字节子�?�?$B160: SEC; SBC #$E0; JSR $C509 查跳转表 ($116C �?7+1 �?
 *   $9339: 指针推进 (ram_008A 加到 ram_0088, 归零 ram_008A)
 *   $9349: 比赛初始�?(计分�?精灵/调色�? �?$046F �? �?$0540 �?
 *   $9405: 精灵批量初始�?(�?$04A5 �? 设精灵基址, 循环填表)
 *
 * 事件分派�?(字节已验�? 跳转表在 bank19 off $116C):
 *   $E0 �?$B1A6 精灵批初始化 (JSR $C52D) + 参数�?C54E + �?$0011/$0012/$000D/$000E/$05D2 + �?$0557/$0558=$FF
 *   $E1 �?$B1E0 逐字节帧等待 (�?N, 每非 $01 字节 1 �? 循环减到 0)
 *   $E2 �?$B1F3 3 字节玩家写入 (X/�?阶段�? 阶段<$0B�?002A=X 否则 $002B=X; JSR $C50C; �?($0034))
 *   $E3 �?$B218 JMP $B349 比赛初始�?(matchInit9349)
 *   $E4 �?$B21B 读字节→ram_008B (精灵索引)
 *   $E5 �?$B224 读字�?0-3 �?JSR $C509 调色板子分派 ($B23E 复制 / $B246 复制+渐显 / $B2A6 渐隐 / $B2DB 渐显)
 *   $E6 �?$B235 ram_063F |= $40 (�?$90AF 精灵连续模式)
 *   $FC �?$B333 LDA #$80; STA $0515; RTS (等精灵批完成)
 *
 * 消费�? bank26 (比赛核心引擎) �?bank19 执行�? * 数据: src/game/prg/data/tables/match-scene-table.ts (脚本�?MATCH_SCENE_SCRIPT)�? * 协程: $C515 = 让出 (H5 版用帧计数模�? 每帧推进一�?�? *
...
export class MatchSceneService {
  ...
  /**
   * $902D 循环: �?bank09 逐字节读, 按值分派�?   * 协程让出�?$C515 控制, H5 版每帧推进一步�?   */
  private matchSceneLoop(): void {
    ...
    if (a >= 0xE0) {
      ...
      // $903D: JSR $9043; JMP $902D (低字节子程自行读�? 不传 a)
      this.matchAuxLow();
    }
  }

  // ══════════════════════════════════════════════════════════�?  // $9043-$90AE 低字节子�? 精灵属性设�?  // ══════════════════════════════════════════════════════════�?
  /**
   * $9043 低字节子�?(A < $E0): 精灵属�?坐标设置�?   * 检�?ram_063F bit6, bit6=1 �?$90AF, bit6=0 �?$904B�?   */
  matchAuxLow(): void {
    // $9043: BIT $063F; BVC $904B
    if ((this.rd(0x063F) & 0x40) !== 0) {
      // $9048: JMP $90AF (bit6=1); 入口字节�?$90AF 自行重读 (LDY $008A; LDA ($0088),Y)
      this.spriteSetup90AF();
      return;
    }
    // $904B: JSR $C515 (协程让出); �?ram_0515=0
    this.yieldAndWait(0x0515);
    // $9055: ram_0515=1
    this.wr(0x0515, 0x01);
    // $905A-$9063: �?$04A5-$04EC �?(X=$47�?, STA $04A5,X; DEX; BPL)
    ...
    let x = 0;
    this.wr(0x003A, x);
    // $906F: ram_008B (精灵索引) 算坐�?    const idx = this.rd(0x008B);
    // $9071: AND #$07; ORA #$88; LSR; ROR $003A; LSR; ROR $003A; STA $04A7/$04CA
    // LSR �?v 的低位经 carry 移入 $003A; ROR 是右�?(carry→bit7, bit0→carry)
    let v = (idx & 0x07) | 0x88;
  ...
  /**
   * $9092-$90A7: 循环�?bank09 数据填精灵表�?   * LDY $008A; LDA ($0088),Y; CMP #$E0; BCS $90A9 (退�?
  ...
  /**
   * $90AF-$9124: bit6=1 的精灵连续模式�?   * 循环 (每帧处理一个精�?:
   *   $90AF: LDY $008A; LDA ($0088),Y; CMP #$E0; BCC $90B8; RTS (�?E0 结束整段)
   *   $90B8: INC $008A; PHA; LDA #$01; JSR $C515 (协程让出, �?$0515=0)
   *   $90C5: $0515=1; �?$04AD/$003A/$04A5/$04A9
   *   $90D8: $008B 算坐�?(LSR/ROR �? �?$04A7/$04AB/$04A6/$04AA
   *   $90FB: PLA; JSR $C524 �?$04AC; STY $04A8; $0515=$80
   *   $910A-$911B: cnt=0,2,4,6 循环�?sub9127($B127) + 协程让出
   *   $911D: $008B += 8; $9124: JMP $B0AF (回到 $90AF 循环)
   */
  private spriteSetup90AF(): void {
    while (true) {
      // $90AF: LDY $008A; LDA ($0088),Y �?读取当前流索引下的精灵字�?      const y = this.rd(0x008A);
      const a = this.readPtrY(0x0088, 0x0089, y);
      // $90B3: CMP #$E0; BCC $90B8; RTS (�?E0 结束整段精灵)
      if (a >= 0xE0) return;   // $90B7: RTS
      // $90B8: INC $008A
      this.wr(0x008A, (y + 1) & 0xFF);
      // $90BA: PHA (a); $90BB: LDA #$01; JSR $C515; $90C0-$90C3: �?$0515=0
      this.yieldAndWait(0x0515);
      ...
      this.wr(0x0515, 0x80);
      // $910A-$911B: cnt=0,2,4,6 循环�?sub9127 + 协程让出
      let cnt = 0;
      do {
  ...
  /**
   * $9127-$9159: 精灵位置计算 (�?$02F8-$02FF �?�?   * LDA $008B; AND #$07; ASL×4; CLC; ADC #$7C; STA $02F8; ADC #$08; STA $02FC
   * LDA $008B; AND #$F8; CLC; ADC $003A; STA $02FB; STA $02FF
   */
  private sub9127(offset: number): void {
    // $9127: STA $003A (精灵行偏移写�?$003A)
    ...
    // $9159: RTS (子程 $915A 由调用者继�?
  }

  // ══════════════════════════════════════════════════════════�?  // $915A-$915D 高字节子�? 比赛事件分派
  // ══════════════════════════════════════════════════════════�?
  /**
   * $915A 高字节子�?(A >= $E0): 比赛事件分派�?   * $B15A: JSR $B160; JMP $B339 (指针推进)
   * $B160: SEC; SBC #$E0; JSR $C509 �?查跳转表 (off $116C, 7+1 �? 字节已验�?
   * 跳转�? $E0�?B1A6 / $E1�?B1E0 / $E2�?B1F3 / $E3�?B218 / $E4�?B21B /
  ...
  matchAuxHigh(a: number): void {
    switch (a) {
      ...
      case 0xE3: this.matchInit9349(); break; // $B218 JMP $B349 比赛初始�?      case 0xE4: this.event4(); break;       // $B21B 精灵索引
  ...
  /**
   * $9339 指针推进: ram_008A 加到 ram_0088/0089, ram_008A 归零�?   * LDA $008A; CLC; ADC $0088; STA $0088; BCC $9344; INC $0089
   * LDA #$00; STA $008A; RTS
   */
  ...
  // ══════════════════════════════════════════════════════════�?  // $B1A6-$B235: 7+1 路比赛事件处�?(字节已验�?
  // ══════════════════════════════════════════════════════════�?
  /**
   * 事件0 $B1A6: JSR $C52D (精灵批初始化); LDY $008A; INC; LDA($0088),Y �?JSR $C54E;
   * LDA#$01; JSR $C515; �?$0516=0; �?$0011/$0012/$000D/$000E/$05D2;
  ...
  private event0(): void {
    ...
    const a = this.readPtrY(0x0088, 0x0089, y);
    this.subC54E(a);
    // $B1B2: LDA #$01; JSR $C515; �?ram_0516=0
    this.yieldAndWait(0x0516);
    // $B1BC: �?ram_0011/0012/000D/000E/05D2
    this.wr(0x0011, 0);
    this.wr(0x0012, 0);
    this.wr(0x000D, 0);
    this.wr(0x000E, 0);
  ...
  /**
   * 事件1 $B1E0: 读字�?N; 循环 {LDA#$01; JSR $C515; SEC; SBC #$01; BNE} �?�?N 帧�?   */
  private event1(): void {
    // $B1E0: LDY $008A; INC $008A; LDA ($0088),Y; PHA (读旧索引下的 N)
    const y = this.rd(0x008A);
    this.wr(0x008A, (y + 1) & 0xFF);
    const a = this.readPtrY(0x0088, 0x0089, y);
    // $B1E7-$B1F0: 循环 让出 1 �? N-1; BNE (�?N �?
    let n = a;
    while (n !== 0) {
      this.yieldAndWait(0x0515);
      n = (n - 1) & 0xFF;
  ...
  /**
   * 事件2 $B1F3: 3 字节 (X/�?阶段�?:
   * LDY $008A; LDA→TAX; INY; LDA→PHA(�?; INY; LDA(阶段�?; INY; STY $008A;
  ...
  private event2(): void {
    ...
    const val = this.readPtrY(0x0088, 0x0089, y0 + 1);    // PHA (�?
    const phase = this.readPtrY(0x0088, 0x0089, y0 + 2);  // 阶段�?    this.wr(0x008A, (y0 + 3) & 0xFF);                     // STY $008A
    ...
    // $B20F: JSR $C50C (查玩家数据指�?�?$0034/$0035)
    this.subC50C();
  ...
  /**
   * 事件5 $B224: INC $008A; LDA ($0088),Y; JSR $C509 �?调色板子分派 (内嵌�?4 �?:
   *   0 �?$B23E: $0472=$0F; JMP $B2F7 (�?$046F �?4 字节 $0F)
   *   1 �?$B246: $0472=$30; $046F�?0408 (0x20B); JSR $B310 整理; �?$30 �? 渐显
   *   2 �?$B2A6: 渐隐
   *   3 �?$B2DB: 渐显
   */
  private event5(): void {
    // $B224: LDY $008A; INC $008A; LDA ($0088),Y (读旧索引下的调色板指�?
    const y = this.rd(0x008A);
    this.wr(0x008A, (y + 1) & 0xFF);
    const a = this.readPtrY(0x0088, 0x0089, y);
    // $B22A: JSR $C509 �?去CPU�? 直接按参数分�?    switch (a & 0xff) {
      case 0: this.subB23E(); break;   // $0472=$0F + �?$0F
      case 1: this.subB246(); break;   // 复制 + 整理 + 渐显
      case 2: this.sub92A8(); break;   // 渐隐 (�?$B2A6)
      case 3: this.sub92DD(); break;   // 渐显 (�?$B2DB)
      default: break;
    }
  }

  /**
   * 事件6 $B235: LDA $063F; ORA #$40; STA $063F (�?$90AF 精灵连续模式)�?   */
  private event6(): void {
  ...
  /**
   * $B23E: LDA #$0F; STA $0472; JMP $B2F7 (�?$046F �?4 字节 $0F, 然后刷新+让出)�?   */
  ...
  // ══════════════════════════════════════════════════════════�?  // $9240-$930F: 调色�?计分板辅�?  // ══════════════════════════════════════════════════════════�?
  /**
   * $92A8-$92DA: 渐隐调色板循环�?   *   $92A8: PHA (step); $92A9: LDA #$02; JSR $C515 (协程让出)
   *   $92AE: PLA; STA $003A (step �?$003A)
   *   $92B1: X=0�?1F: TXA; AND #$03; BEQ $92C8 (仅处�?X&3 != 0)
   *          LDA $046F,X; AND #$0F; ORA $003A; LDY $003A; BNE $92C5 (step!=0 �?
   *          LDA #$0F (step==0 �?$0F); STA $046F,X
  ...
  /**
   * $92F7-$930F: �?$046F �?(�?字节�?A, 然后协程让出)�?   * LDX #$00; STA $046F,X; INX×4; CPX #$20; BNE; JSR $C533; LDA #$01; JSR $C515; RTS
   */
  ...
  // ══════════════════════════════════════════════════════════�?  // $9310-$9338: 调色板整�?  // ══════════════════════════════════════════════════════════�?
  /**
   * $9310: 整理 $046F �?(AND #$0F; ORA #$30; CMP #$3F; �?$30�?30), JSR $C533, 协程让出�?   */
  sub9310(): void {
    for (let x = 0; x < 0x20; x++) {
  ...
  /**
   * $9335: ram_0515=$80; RTS (设等待标�?�?   */
  sub9335(): void {
    this.wr(0x0515, 0x80);
  }

  // ══════════════════════════════════════════════════════════�?  // $9349-$93FF: 比赛初始�?(计分�?精灵/调色�?
  // ══════════════════════════════════════════════════════════�?
  /**
   * $9349: 比赛初始化�?   * �?$B2A6 (sub92A8 渐隐), �?ram_046B=1, �?ram_004B/0517/053C,
   * �?ram_053A=$80, ram_004A=$24, �?$B406 (精灵渲染), �?PPU CTRL,
   * �?$C530 (NT 填充), �?$C533, �?$0494-$0497, �?$0490/$0491, �?$0540 �?
   * 协程让出, 渐显计分板�?   */
  matchInit9349(): void {
    // $9349: JSR $B2A6 (渐隐)
    this.sub92A8();
    ...
    // $9382: JSR $C533
    this.subC533();
    // $9388: �?$B402 �?(MATCH_CTRL_B402) �?$0494-$0497 (4字节)
    for (let i = 0; i < 4; i++) {
    ...
    this.wr(0x0490, 0x7C);
    this.wr(0x0491, 0x7E);
    // $939D-$93B9: �?$0540 �?($0557/$0558/$0541/$054F/$0553/$0547/$0559)
    this.wr(0x0557, 0xFF);
    this.wr(0x0558, 0xFF);
    this.wr(0x0541, 0xFF);
    this.wr(0x054F, 0xFF);
    ...
    // 连续动画循环 ($93CB-$93FF, 由协程让出驱动跨帧推�?
    this.matchInitLoop();
  }

  /**
   * $93CB-$93FF: 比赛初始化连续循环�?   *   $93C7: $008A=0; $93CB: 让出1�? $93D0: $008A += $60; $93D7: BCC $93CB (累加到进�?
   *   $93D9: DEC $054F; $93DC: DEC $004A; $93DE: BEQ $93FA (�? �?终止让出循环)
   *   $93E0-$93EE: $004A==$14�?0470=$06; $004A==$08�?0470=$16; 否则�?$93CB
   *   $93F1: JSR $C533; $93F7: JMP $B3CB (回到 $93CB)
   *   $93FA-$93FF: 终�?(让出1�? JMP $B3FA)
   */
  private matchInitLoop(): void {
    while (true) {
      ...
      // $93DE: BEQ $93FA (�? �?终�?
      if (this.rd(0x004A) === 0) {
        // $93FA: LDA #$01; JSR $C515; JMP $B3FA (终态让出循�? H5 �?update 驱动)
        this.yieldAndWait(0x0515);
        this.subB3FA();
        return;
      }
      // $93E0-$93EE: 选择 $0470 并判断是否刷�?      if (this.rd(0x004A) === 0x14) {
        this.wr(0x0470, 0x06);       // $93EE: STX $0470 (X=$06)
      } else if (this.rd(0x004A) === 0x08) {
        this.wr(0x0470, 0x16);       // $93EE: STX $0470 (X=$16)
      } else {
        continue;                     // $93EA: CMP #$08; BNE $93CB (回内层循�?
      }
      // $93F1: JSR $C533; $93F7: JMP $B3CB (回到 $93CB)
      this.subC533();
    }
  ...
  // ══════════════════════════════════════════════════════════�?  // $9405-$944D: 精灵批量初始�?  // ══════════════════════════════════════════════════════════�?
  /**
   * $9405: 精灵批量初始化�?   * PHA; �?ram_008B/008A; 协程让出; �?$04A5 �?
   * �?$04A5=$20, $04A6=ram_008A, $04A7=ram_008B|A;
  ...
  // ══════════════════════════════════════════════════════════�?
  /** $C515 协程让出 (�?ram_0515=0) �?�?Bank00Service.coroutineYield */
  ...
  // 事件分派入口 $915A (matchAuxHigh) 已用 switch 直接分派, 不再�?$C509 跳转表�?  // $B127 (sub9127) 为精灵位置子�? 已由 spriteSetup90AF 的循环调用�?  // ════════════════════════════════════════════════════════════

  /** 比赛帧推�?(�?bank26 比赛核心引擎调用) */
  update(frame: number): void {
    void frame;
    // 每帧推进 matchSceneLoop 一�?    this.matchSceneLoop();
  }
}
