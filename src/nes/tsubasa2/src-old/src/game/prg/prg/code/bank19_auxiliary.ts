 ...
 *
 * 数据经结构化提取: B19_SCENE_STREAM (数据流池) + B31_PALETTE_FBCC (调色板表), 无整 bank 数组。
 *
 * ── MMC3 窗口映射 (关键修正) ─────────────────────────────
...
import { B19_SCENE_STREAM } from '../data/bank19-scene-stream';
import { B31_PALETTE_FBCC, B31_PALETTE_ENTRY_SIZE } from '../data/bank31-palette-fbcc';

// ═══════════════════════════════════════════════════════════════
// 常量 (地址均为本 bank 数组内索引)
// ═══════════════════════════════════════════════════════════════

/** 数据流起点: 场景数据流自 $9467 开始, B19_SCENE_STREAM 数组即从该处起, 偏移 0 */
export const BANK19_STREAM_OFFSET = 0;
/** 控制码最小值 (≥ $E0 为控制码) */
...
export class Bank19Service {
  ...
  /** 数据流指针 (B19_SCENE_STREAM 数组索引, 对应 ram_0088/0089 指针) */
  ...
  private _fixedC530(x: number, a: number): void {
    const s = this._store;
    const base = (a * B31_PALETTE_ENTRY_SIZE) & 0xff; // $FBCC + A*12 表偏移
    let y = 0;
    for (let i = 0; i < 16; i++) {
      let v: number;
      if ((x & 3) === 0) {
        ...
        v = B31_PALETTE_FBCC[base + y] ?? 0x0f; // $CC30: (ram_0065),Y 连续读表
        y++;
      }
      s.write(`ram_046F+${x}`, v);
      x = (x + 1) & 0xff;
  ...
  private _readStreamByte(): number {
    const b = B19_SCENE_STREAM[this._streamPtr + this._streamPos] ?? 0xff;
  ...
  private _peekStreamByte(): number {
    return B19_SCENE_STREAM[this._streamPtr + this._streamPos] ?? 0xff;
  }
