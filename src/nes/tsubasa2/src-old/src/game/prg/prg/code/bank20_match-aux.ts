 ...
 *
 * 数据已从 `bank20-data.ts` 结构化提取为命名常量 (新架构: 无整 bank 数组,
 * 无 MMC3 切换, 无 CPU 模拟, 无地址偏移读取)。
 *
 * ── 数据池换算 (关键) ──────────────────────────
 * 运行时指针 (ram_004C/D, ram_003E/F) 是原版 CPU 地址, 按段映射到数据池:
 *   $8B48+  → MAIN_STREAM_DATA   (主状态机数据流, 索引 = CPU-$8B48)
 *   $A38A+  → B21_NAME_DATA_1    (A1B4 名字记录, $84DC 子流)
 *   $AE25+  → B21_NAME_DATA_2    (AC47 名字记录, $83D9 子流)
 * 静态表 (T_82F6/T_88A8/B21_A1B4/B31_FB4C 等) 直接索引命名常量。
 *
 * ── 功能总览 ───────────────────────────────────
 ...
 * 4. $8796: 动画偏移计数 ($87A7/$87C7 经 $C542/$C545 查 $FB4C 表)。
 *
...
import {
  T_82F6, T_83A6, T_885B, T_88A8, T_88D0, T_88DA, T_88DF, T_88E4,
  NAME_MAP_TABLE_88F0, MAIN_STREAM_PTR_TABLE_8968, MAIN_STREAM_DATA,
  B21_A1B4, B21_AC47, B21_BA87, B21_BACF, B21_B80C, B21_B767,
  B21_NAME_DATA_1, B21_NAME_DATA_2, B31_FB4C, B31_FBCC,
} from '../data/bank20-data';
...
export const NAME_RECORD_TABLE: ReadonlyArray<number> = [
...
/** 数据池基址 (子流指针 CPU 地址 → 数据池索引换算) */
const STREAM_BASE_20 = 0x8B48;   // MAIN_STREAM_DATA 池基址 (bank20)
const STREAM_BASE_B21_1 = 0xA38A; // B21_NAME_DATA_1 池基址
const STREAM_BASE_B21_2 = 0xAE25; // B21_NAME_DATA_2 池基址
...
export class Bank20Service {
  /** 主数据流指针 (MAIN_STREAM_DATA 池索引, 对应 ram_004C/004D) */
  ...
  /** 读主数据流池 (MAIN_STREAM_DATA) 当前指针 + off 处字节 */
  private _mainByte(off: number): number {
    return MAIN_STREAM_DATA[this._streamPtr + off] ?? 0xff;
  }

  /** 读主数据流池 16bit 小端 (当前指针 + off 起) */
  private _mainU16(off: number): number {
    return (this._mainByte(off) | (this._mainByte(off + 1) << 8)) & 0xffff;
  }
  ...
  private _readName(addr: number, off: number): number {
    return this._store.read(this._ramKey(addr + off)) & 0xff;
  }

  /** 写名字记录 (RAM 地址) 内偏移 off 的字节 */
  private _writeName(addr: number, off: number, v: number): void {
  ...
  private _dispatchTable(_idx: number): void {
    // 空 (各调用处直接 switch)
  }
   ...
   */
  private _fixedC536(a: number): [number, number] {
    let x = 0;
    let v = a & 0xff;
   ...
   */
  private _fixedC545(a: number): [number, number] {
    let v = (a << 1) & 0xff;
    const neg = (a & 0x80) !== 0; // ASL carry
    if (v >= 0x80) v = (~v) & 0xff; // BPL → EOR #$FF
    v &= 0x7e;
    let lo = B31_FB4C[v] ?? 0;
    let hi = B31_FB4C[v + 1] ?? 0;
    if (neg) {
      lo = (~lo) & 0xff;
      hi = (~hi) & 0xff;
      lo = (lo + 1) & 0xff;
  ...
  private _fixedC542(a: number): [number, number] {
  ...
  entry_800F(): void {
    ...
    if (a === 0) return;               // $8012 BEQ $8083 (RTS)
    ...
    const streamAddr = (MAIN_STREAM_PTR_TABLE_8968[sub * 2] | (MAIN_STREAM_PTR_TABLE_8968[sub * 2 + 1] << 8)) & 0xffff;
    this._streamPtr = (streamAddr - STREAM_BASE_20) & 0xffff; // → MAIN_STREAM_DATA 池索引
  ...
  private _mainStreamLoop(): void {
    const s = this._store;
    for (;;) {
      const byte = this._mainByte(0);
      if (byte < 0xf0) {
      ...
      if (this._stopped) { this._stopped = false; return; } // $80A2
      // $8078: JMP $806D (继续循环)
    }
  }

  /** $83CF: 数据流指针前进 A 字节 */
  private _advanceStream(a: number): void {
  ...
  private fn_8084(ctrl: number): void {
    const idx = (ctrl - 0xf0) & 0xff;
    switch (idx) {
      case 0:  this.entry_80A2(); break;   // $80A2 stop
      case 1:  this.entry_80AA(); break;   // $80AA init-scene
      case 2:  this.entry_812B(); break;   // $812B (数据误标)
      case 3:  this.entry_8138(); break;   // $8138 (数据误标)
      case 4:  this.entry_8142(); break;   // $8142 copy 4
      case 5:  this.entry_8153(); break;   // $8153 palette
      case 6:  this.entry_83AE(); break;   // $83AE 清 ram_0547+X
      case 7:  this.entry_83BD(); break;   // $83BD ram_0540/41
      case 8:  this.entry_816F(); break;   // $816F 指针重置
      case 9:  this.entry_817C(); break;   // $817C 备份指针
      case 10: this.entry_8195(); break;   // $8195 递减计数
      case 11: this.entry_81A9(); break;   // $81A9 复制 3 字节
      default: break;
    }
  }

  /** $80A2: stop — PLA PLA; ram_053A=0 (停止主状态机) */
  private entry_80A2(): void {
    this._store.write('ram_053A', 0);
    this._stopped = true;
  }

  /** $812B (数据误标): ram_053E=0; ram_053D=1; A=1 → $83CF */
  private entry_812B(): void {
   ...
   */
  private entry_80AA(): void {
    const s = this._store;
    const b5 = this._mainByte(5) & 0xff;
    const idx = (b5 & 0x1c) >> 2; // $80AE: AND #$1C; LSR → 项号 0-5 ($88E4 6 项)
    const rec = (T_88E4[idx * 2] | (T_88E4[idx * 2 + 1] << 8)) || 0x0547; // ram_003A/3B
    // 清记录区 0x15 字节
    for (let i = 0; i < 0x15; i++) s.write(this._ramKey(rec + i), 0);
    // stream+1 → bank21 $A1B4 表 2B → rec+1/+2 (asm $80D7-$80E3: rec+1=lo, rec+2=hi)
    const b1 = this._mainByte(1) & 0xff;
    this._writeName(rec, 1, B21_A1B4[b1 * 2] ?? 0);
    this._writeName(rec, 2, B21_A1B4[b1 * 2 + 1] ?? 0);
    // stream+2 → bank21 $AC47 表 2B → rec+3/+4 (asm $80F6-$8102: rec+3=lo, rec+4=hi)
    const b2 = this._mainByte(2) & 0xff;
    this._writeName(rec, 3, B21_AC47[b2 * 2] ?? 0);
    this._writeName(rec, 4, B21_AC47[b2 * 2 + 1] ?? 0);
    // stream+3 → rec+8; stream+4 → rec+$0C
    this._writeName(rec, 8, this._mainByte(3));
    this._writeName(rec, 0x0c, this._mainByte(4));
  ...
  private entry_8142(): void {
    const s = this._store;
    for (let y = 1; y < 5; y++) {
      s.write(`ram_0493+${y}`, this._mainByte(y));
    }
   ...
   */
  private entry_8153(): void {
    const a = this._mainByte(1) & 0xff;
    if ((a & 0x80) !== 0) {
      this.fn_81BA(a); // $8159
    } else {
  ...
  private entry_816F(): void {
    const addr = this._mainU16(0);
    this._streamPtr = (addr - STREAM_BASE_20) & 0xffff; // → MAIN_STREAM_DATA 池索引
  }

  /** $817C: A=stream+1 → ram_0542; 备份 ram_004E/4F=流+2; A=2 → $83CF */
  private entry_817C(): void {
    const s = this._store;
    s.write('ram_0542', this._mainByte(1));
  ...
  private entry_81A9(): void {
    const s = this._store;
    for (let y = 1; y < 4; y++) {
      s.write(`ram_0542+${y}`, this._mainByte(y));
    }
    this._advanceStream(4);
  }

  /** $83AE: A=stream+1 → X; ram_0547+X=0; A=2 → $83CF */
  private entry_83AE(): void {
    const x = this._mainByte(1) & 0xff;
    this._store.write(`ram_0547+${x}`, 0);
    this._advanceStream(2);
  }

  /** $83BD: ram_0540=stream+1; ram_0541=stream+2; A=3 → $83CF */
  private entry_83BD(): void {
    const s = this._store;
    s.write('ram_0540', this._mainByte(1));
    s.write('ram_0541', this._mainByte(2));
  ...
  private fn_81BA(a: number): void {
    const idx = (a & 0x7f) & 0xff;
    switch (idx) {
      case 0: this._subDispatch(this._store.read('ram_0441') & 0xff); break;  // $81CF
      case 1: this._subDispatch(this._store.read('ram_0442') & 0xff); break;  // $81E9
      case 2: this._subDispatch(this._store.read('ram_05FB') & 0xff); break;  // $81DB
      case 3: this._subDispatch((this._store.read('ram_05FB') ^ 0x0b) & 0xff); break; // $81E1
      case 4: this.entry_82BC(); break;                                        // $82BC 调色板填充
      case 5: this.entry_837F(); break;                                        // $837F×2
      case 6: this.entry_837F(); break;
      case 7: this._subDispatch(this._store.read('ram_05FC') & 0xff); break;  // $81D5
      default: break;
    }
  }

  /** $81EC: 名字ID → $C50C; $826A; 记录首字节判空 → $8282/ram_002B-3; 调色板 16B */
  private _subDispatch(id: number): void {
    const s = this._store;
    ...
    s.write('ram_003A', a);
    s.write('ram_003B', 0);
    // A*5 记录内偏移 + $8264 基址表选择 (x=0/4→B80C, x=2→B767)
    const baseTab = (x === 2 ? B21_B767 : B21_B80C);
    const off = (a * 5) & 0xff;
    const t0 = baseTab[off] ?? 0;
    let y = 1;
    for (let xx = 0; xx < 0x10; xx++) {
      const m = xx & 3;
      if (m === 0) continue;      // $825D: 跳过
      let v: number;
      if (m === 1) v = 0x0f;      // $8258
      else if (m === 2) { v = baseTab[off + y] ?? 0; y++; } // $8253
      else v = t0;                // $824E: PLA PHA
  ...
  private fn_826A(): void {
    ...
    const b0 = this._readName(rec, 0);
    let v = NAME_MAP_TABLE_88F0[b0] ?? 0;
    if (b0 === 0) {
  ...
  private fn_8282(a: number): number {
    this._store.write('ram_003B', a);
    let x: number;
    if (a === 1) x = 1;
    else if (a < 0x0f || a >= 0x17) x = 0;
    else x = 2;
    switch (x) {
  ...
  private entry_82BC(): void {
    ...
    for (let x = 0; x < 0x20; x++) {
      const p = T_82F6[x] ?? 0; // $82F6
      let v: number;
      if ((p & 0x80) !== 0) {
        v = B21_BACF[b2 * 16 + (p & 0x7f)] ?? 0; // 间接 (A*16+$BACF)
      } else {
        v = p; // 直接
      }
      s.write(`ram_046F+${x}`, v);
  ...
  private fn_8316(a: number): number {
    const s = this._store;
    const idx = (a & 0x7f) & 0xff;
    switch (idx) {
      ...
      case 4: // $8361: SEC 变体 (A=$2E/$07/$09)
        {
          ...
          let v: number;
          if (ram2b === 0x12 || ram2a === 1) v = 0x2e;
          else if (ram2a === 0) v = 0x07;
          else v = 0x09;
  ...
  private entry_837F(): void {
    const s = this._store;
    const fb = s.read('ram_05FB') & 0xff;
    const idx = (fb !== 0 ? 1 : 0);
    const v = (s.read(`ram_002A+${idx}`) & 0xff) << 1; // ram_002A/002B << 1
    for (let x = 0; x < 8; x++) {
      s.write(`ram_047F+${x}`, T_83A6[x] ?? 0);
    }
    s.write('ram_0481', B21_BA87[v] ?? 0);
    s.write('ram_0482', B21_BA87[v + 1] ?? 0);
  }
  ...
  private _subStreamPtr(): number {
    const s = this._store;
    return (s.read('ram_003E') & 0xff) | ((s.read('ram_003F') & 0xff) << 8);
  }

  private _setSubStream(ptr: number): void {
    this._store.write('ram_003E', ptr & 0xff);
    this._store.write('ram_003F', (ptr >> 8) & 0xff);
  }

  /** 读子流字节 (ram_003E)+off — 结构化数据池: 按 CPU 地址归属换算索引 */
  private _readSubStream(off: number): number {
    const addr = (this._subStreamPtr() + off) & 0xffff;
    if (addr >= STREAM_BASE_B21_2) return B21_NAME_DATA_2[addr - STREAM_BASE_B21_2] ?? 0xff; // $AE25+ AC47 记录
    if (addr >= STREAM_BASE_B21_1) return B21_NAME_DATA_1[addr - STREAM_BASE_B21_1] ?? 0xff; // $A38A+ A1B4 记录
    if (addr >= STREAM_BASE_20) return MAIN_STREAM_DATA[addr - STREAM_BASE_20] ?? 0xff;      // $8B48+ 主数据流
    return 0xff;
  }

  /** 读子流 16bit 小端 */
  private _readSubStream16(off: number): number {
  ...
  entry_83D9(): void {
    ...
    if (timer === 0xff) return;                 // $83DF-$83E1: BEQ $83E8 (冻结)
    if (timer !== 0) {
  ...
  private _advance83D9(rec: number): void {
    const s = this._store;
    ...
    for (;;) {
      ...
      if (this._ctrl8438(rec, b)) return;       // $8413: JSR $8438 (PLA PLA 终止型)
    }
  }
   ...
   */
  private _ctrl8438(rec: number, ctrl: number): boolean {
    const s = this._store;
    ...
    switch (idx) {
      ...
      case 5: {                                 // $8496: rec+$0D 递减, 非零恢复子流
        ...
        if (v === 0) return false;              // $849F: RTS (零则不改)
  ...
  entry_84DC(): void {
    ...
    if (timer === 0xff) return;                 // $84E2-$84E6
    if (timer !== 0) {
   ...
   */
  private _posIntegrate(rec: number): void {
    const s = this._store;
    ...
    if ((a & 0x10) === 0) return;               // $855C-$855E: BEQ $8579
   ...
   */
  private _ctrl857A(rec: number): void {
    const s = this._store;
    for (;;) {
  ...
  private _h85A9(rec: number): void {
  ...
  private _h85E7(rec: number): void {
    const s = this._store;
    const off = s.read('ram_0040') & 0xff;
   ...
   */
  private _add85F2(rec: number, x: number, y: number): void {
    const s = this._store;
   ...
   */
  private _add860D(rec: number, x: number, ys: number): void {
  ...
  entry_8624(): void {
    ...
    for (;;) {                                  // $8637
      ...
      if (i === 0 || i === 0x0b) {              // $8639/$863E: 跳过
        if (this._nextPlayer8624()) return;
        continue;
      }
      ...
      if (v < 0x34) v = 0x34;                   // CMP #$34; BCS
      if (v >= 0xcc) v = 0xcc;                  // CMP #$CC; BCC
      s.write(this._ramKey(0x0200 + x + 3), (v + T_88DA[mode]) & 0xff);
      // $866E-$868A: name+$08 clamp $54-$AC + $88DF[mode] → OAM byte0
      v = this._readName(name, 8);
      if (v < 0x54) v = 0x54;
      if (v >= 0xac) v = 0xac;
      s.write(this._ramKey(0x0200 + x), (v + T_88DF[mode]) & 0xff);
      ...
      let t = a & 0xff;
      if (t >= 0x0b) t = (t - 1) & 0xff;        // CMP #$0B; BCC; SBC #$01
      t = (t + 0x11) & 0xff;                    // CLC; ADC #$11
      if (t >= 0x20) t = (t + 0x10) & 0xff;     // CMP #$20; BCC; ADC #$0F (C=1)
  ...
  private _nextPlayer8624(): boolean {
    const s = this._store;
   ...
   */
  fn_86DB(i: number): [number, boolean] {
    this._queryNamePtr0034(i);                  // $86DB-$86DE: JSR $C50C
    const s = this._store;
    const idx = s.read('ram_062D') & 0x0f;
    switch (idx) {
      case 0: case 1: case 4:                   // $871D: SEC; RTS
        return [idx, true];
      case 2: {                                 // $871F
        if (i >= 0x0b) return [i, true];        // $8721-$8723
        if (i === (s.read('ram_0441') & 0xff)) return [i, true]; // $8725
        let x = s.read('ram_0430') & 0xff;      // $872A-$872D
        while (x !== 0) {
          if (i === (s.read(this._ramKey(0x0430 + x)) & 0xff)) return [i, true];
          x = (x - 1) & 0xff;                   // $8734-$8735
        }
        return [i, false];                      // $8737: CLC
      }
      case 3: {                                 // $873B
        if (i === (s.read('ram_0441') & 0xff)) return [i, true]; // $8740
        let x = s.read('ram_0600') & 0xff;      // $8742-$8745
        while (x !== 0) {
          if (i === (s.read(this._ramKey(0x0600 + x)) & 0xff)) return [i, true];
          x = (x - 1) & 0xff;
        }
  ...
  entry_86F2(): number {
    const s = this._store;
    const i = s.read('ram_0046') & 0xff;
    if (i !== (s.read('ram_05FD') & 0xff)) return i;  // $86F2-$86F7
   ...
   */
  private _writeOam87E7(x34: number, y54: number): void {
    const s = this._store;
  ...
  entry_8864(): void {
    ...
    for (;;) {
      ...
      s.write(this._ramKey(0x0200 + x + 1), T_88D0[row] ?? 0); // $8872-$8877
      const tv = T_88A8[xi] ?? 0;               // $887A
