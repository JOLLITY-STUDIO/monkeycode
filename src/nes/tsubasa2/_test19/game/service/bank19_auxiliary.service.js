...
exports.BANK19_STREAM_OFFSET = 0x1467;
/** 控制码最小值 (≥ $E0 为控制码) */
exports.BANK19_CTRL_MIN = 0xe0;
/**
...
class Bank19Service {
    constructor(_store) {
        ...
        this._textQueue = new Uint8Array(8);
        /** 数据流结束 (对应 $B3FA 无限等待) */
        this._done = false;
    ...
    start(streamOffset = exports.BANK19_STREAM_OFFSET) {
        const s = this._store;
        // $9000-$902B: 初始化
        s.write('ram_0490', 0);
        s.write('ram_0491', 2);
    ...
    update(_frameCount) {
        ...
        // 单精灵模式: $B0AF 每帧一个字符
        if (this._singleMode) {
            if (this._peekStreamByte() >= exports.BANK19_CTRL_MIN) {
    ...
    _dispatchControl(ctrl) {
        // $9166 跳转表 (ROM dump 验证): 仅 0-6 与 28 有效
        //   0:B1A6  1:B1E0  2:B1F3  3:B218  4:B21B  5:B224  6:B235  28:B333($FC)
        const idx = (ctrl - exports.BANK19_CTRL_MIN) & 0xff; // SEC; SBC #$E0
        switch (idx) {
            case 0:
                this._ctrl_ClearText();
                break; // $B1A6
            case 1:
                this._ctrl_Delay();
                break; // $B1E0
            case 2:
                this._ctrl_WriteName();
                break; // $B1F3
            case 3:
                this._sceneReset();
                break; // $B218 → JMP $B349
            case 4:
                this._ctrl_SetSubPos();
                break; // $B21B
            case 5:
                this._ctrl_SubDispatch();
                break; // $B224
            case 6:
                this._ctrl_SetFlag40();
                break; // $B235
            case 28:
                this._setAnimLock();
                break; // $B333 (控制码 $FC)
    ...
    _sceneResetLoop() {
        ...
        let x = 0; // $93E0-$93EC
        if (this._resetCount === 0x14)
            x = 0x06; // CMP #$14 → X=$06
        else if (this._resetCount === 0x08)
            x = 0x16; // CMP #$08 → X=$16
        else
            return; // BNE $93CB — 继续下一轮
    ...
    _fixedC530(x, a) {
        ...
        for (let i = 0; i < 16; i++) {
            ...
            if ((x & 3) === 0) {
                ...
                v = (_a = prg_bank_31_1.default[ptr + y]) !== null && _a !== void 0 ? _a : 0x0f; // $CC30: (ram_0065),Y 连续读表
                y++;
            }
            s.write(`ram_046F+${x}`, v);
            x = (x + 1) & 0xff;
    ...
    _readStreamByte() {
        var _a;
        const b = (_a = prg_bank_19_1.default[this._streamPtr + this._streamPos]) !== null && _a !== void 0 ? _a : 0xff;
    ...
    _peekStreamByte() {
        var _a;
        return (_a = prg_bank_19_1.default[this._streamPtr + this._streamPos]) !== null && _a !== void 0 ? _a : 0xff;
    }
    /**
    ...
    readByte(addr) {
        var _a;
        return (_a = prg_bank_19_1.default[addr - 0x8000]) !== null && _a !== void 0 ? _a : 0xff;
    }
    /** 读本 bank 16bit 小端 */
    readU16(addr) {
...
exports.Bank19Service = Bank19Service;
