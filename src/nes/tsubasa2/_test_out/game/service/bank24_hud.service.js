...
class Bank24HudService {
    ...
    hudLine1() {
        ...
        // $86FD: bit7=0 → 帧延迟推进
        if ((ctrl & 0x80) === 0) {
            if (s.read(KEY_0533) !== 0)
                s.write(KEY_0533, s.read(KEY_0533) - 1);
            return;
        }
        // $86FF-$8707: 新请求
        s.write(KEY_0532, ctrl & 0x7F);
        if ((ctrl & 0x7F) === 0)
            return; // $8704: ctrl==$80 → 结束
        const idx = (ctrl & 0x7F) - 1;
        this._setPtr(KEY_0079, KEY_007A, (0, bank24_tables_1.readHud1Ptr)(idx)); // $870B-$8713
    ...
    _hud1Step() {
        ...
        const byte0 = (0, bank24_tables_1.readB25)(ptr); // $8725
        ...
        if (delay !== 0) {
            ...
            for (let i = 0; i < n; i++) {
                ...
                const off = (0, bank24_tables_1.readB25)(ptr + 1 + i * 2);
                const val = (0, bank24_tables_1.readB25)(ptr + 2 + i * 2);
        ...
        if (n === 1) {
            // $876B: 跳转 (后 2 字节绝对地址)
            this._setPtr(KEY_0079, KEY_007A, (0, bank24_tables_1.readB25U16)(ptr + 1));
    ...
    hudLine2() {
        ...
        if (ctrl === 0)
            return; // $877C
        if ((ctrl & 0x80) === 0) {
            if (s.read(KEY_0535) !== 0)
                s.write(KEY_0535, s.read(KEY_0535) - 1);
            return;
        }
        ...
        if ((ctrl & 0x7F) === 0)
            return; // $8785
        const idx = (ctrl & 0x7F) - 1;
        this._setPtr(KEY_007B, KEY_007C, (0, bank24_tables_1.readHud2Ptr)(idx)); // $878C-$8794
    ...
    _hud2Step() {
        ...
        const byte0 = (0, bank24_tables_1.readB25)(ptr); // $87A6
        if (byte0 < 0xf0) {
            // $87B7: 正常处理
            s.write(KEY_0535, byte0);
            s.write(KEY_0490, (0, bank24_tables_1.readB25)(ptr + 1));
            s.write(KEY_0491, (0, bank24_tables_1.readB25)(ptr + 2));
            // $87C7-$87D1: 指针推进 3
        ...
        if (byte0 === 0xf1) {
            // $87D8: 跳转
            this._setPtr(KEY_007B, KEY_007C, (0, bank24_tables_1.readB25U16)(ptr + 1));
    ...
    hudLine3() {
        ...
        if ((ctrl & 0x80) === 0) {
            if (s.read(KEY_0537) !== 0)
                s.write(KEY_0537, s.read(KEY_0537) - 1);
            return;
        }
        ...
        const idx = (ctrl & 0x7F) - 1;
        this._setPtr(KEY_007D, KEY_007E, (0, bank24_tables_1.readHud3Ptr)(idx)); // $87F9-$8801
    ...
    _hud3Step() {
        ...
        const byte0 = (0, bank24_tables_1.readB25)(ptr); // $8817
        if (byte0 < 0xf0) {
            // $8828: 正常处理
            s.write(KEY_0537, byte0);
            s.write(KEY_0538, (0, bank24_tables_1.readB25)(ptr + 1));
            // $8832-$883C: 指针推进 2
        ...
        if (byte0 === 0xf1) {
            // $8843: 跳转
            this._setPtr(KEY_007D, KEY_007E, (0, bank24_tables_1.readB25U16)(ptr + 1));
    ...
    spriteLoad(a) {
        ...
        const ptr = (0, bank24_tables_1.readSprPtr)(arg); // $8854-$885C → ram_0050/0051
        s.write(KEY_0050, ptr & 0xff);
        s.write(KEY_0051, ptr >> 8);
        // $8862-$887C: 位段值 → ram_05C6 (段值×9)
        const seg = (0, bank24_tables_1.readSprBits)(arg); // $B3BD[arg>>2] 取第 (arg&3) 个 2bit 段
    ...
    _initScene() {
        ...
        this._setPtr(KEY_005F, KEY_0060, (0, bank24_tables_1.readScenePtr)(s.read(KEY_05EA)));
        // $8032-$803E: 清零
        s.write(KEY_05E9, 0);
        s.write(KEY_05E5, 0);
        s.write(KEY_05E4, 0);
    ...
    _matchHudStep() {
        ...
        if (this._waitingInput) {
            ...
            if (!(s.read(KEY_001C) & 0x80))
                return; // $80A8: BPL 循环 (未就绪)
            this._waitingInput = false;
    ...
    _sceneSub0() {
        ...
        const b = (0, bank24_tables_1.readSceneByte)(ptr + idx); // $8074: LDA (ram_005F),Y
        if (b >= 0xf0) {
    ...
    _sceneSub1() {
        ...
        const idx = s.read(KEY_05E5);
        // $821B: 读文本流字节
        const b = (0, bank24_tables_1.readSceneByte)(ptr + idx);
        if (b >= 0x90) {
        ...
        const hi = b >> 4;
        s.write(KEY_05E6, (0, bank24_tables_1.readB24)(0x86b8 + hi));
        const block = (0, bank24_tables_1.readB24U16)(0x8dc2 + hi * 2);
        s.write(KEY_0061, block & 0xff);
        s.write(KEY_0062, block >> 8);
        // $824F-$825E: 数据块首 3B
        const d0 = (0, bank24_tables_1.readSceneByte)(block); // byte0 → 源地址 lo
        const d1 = (0, bank24_tables_1.readSceneByte)(block + 1); // byte1 → 源地址 hi
        s.write(KEY_05E7, (0, bank24_tables_1.readSceneByte)(block + 2)); // byte2 → 组属性
        let dy = 3; // 数据流偏移 (块内)
        // $825F-$8261: 帧数 = 6
        s.write(KEY_05E8, 6);
        let tileLo = d0;
            ...
            groupLoop: while (true) {
                ...
                for (; rleGuard < 512; rleGuard++) {
                    ...
                    if (db & 0x80) {
                        ...
                        dy++;
                        for (let k = 0; k < n; k++) {
                            oam.writeByte(x, (0, bank24_tables_1.readSceneByte)(block + dy));
                            dy++;
                            x++;
                        }
                    }
        ...
        s.write(KEY_05E7, (0, bank24_tables_1.readB24)(0x86c8 + hi * 2));
        s.write(KEY_05E8, (0, bank24_tables_1.readB24)(0x86c9 + hi * 2));
        // $82EB-$82EE: 状态推进 → 状态 2
    ...
    _sceneSub2() {
        ...
        const srcLo = (0, bank24_tables_1.readB24)(0x86e8 + e7 * 2);
        const srcHi = (0, bank24_tables_1.readB24)(0x86e9 + e7 * 2);
        ...
        for (;;) {
            ...
            const b = (0, bank24_tables_1.readSceneByte)(ptr + idx); // $834C: LDA (ram_005F),Y
            if (b < 0xe0) {
            ...
            if (s.read(KEY_05E4) !== 2 || s.read(KEY_05E3) === 0)
                break;
        }
    }
    /**
    ...
    _mapCharC524(a) {
        ...
        if (v >= 0x15)
            v = (v + 0x05) & 0xff;
        if (!carryB4)
            return [v, attr]; // CBEA: PLP; BCC $CBF0
        return [(v + 0x40) & 0xff, attr]; // CBED: CLC; ADC #$40
    }
    /**
     ...
     */
    _queryTextPtr0030(idx) {
        return (0, bank24_tables_1.readB31U16)(0xf329 + ((idx << 1) & 0x1ff));
    }
    /**
    ...
    _renderText863C(idx) {
        ...
        for (;;) {
            ...
            y++;
            if (y >= 256)
                break; // $8650: INC ram_003C; BNE (保险)
        }
    }
    /**
     * $8653: 名字区渲染。
    ...
    _renderName8653(id) {
        ...
        for (let y = 0; y < 4; y++) {
            s.write(`${KEY_05EE}+${y}`, (0, bank24_tables_1.readB24)(0x8686 + x + y));
        }
    ...
    _sceneCmd83xx(cmd) {
        ...
        switch (cmd) {
            case 0: {
                ...
                if (x === 1 && (s.read(KEY_0628) & 0x80))
                    x = 0x0a;
                this._renderText863C(((s.read(KEY_043C) & 0x7f) + (0, bank24_tables_1.readB24)(0x83bf + x)) & 0xff);
                break;
            }
            case 1: {
                // $83CA: X=043D&0x1F; (043E&0x7F)+$83DC[X] → 文本流
                const x = s.read(KEY_043D) & 0x1f;
                this._renderText863C(((s.read(KEY_043E) & 0x7f) + (0, bank24_tables_1.readB24)(0x83dc + x)) & 0xff);
                break;
            }
            case 2:
                this._cmd83E2();
            ...
            case 3: {
                ...
                if (s.read(KEY_043E) & 0x80) {
                    const t = (0, bank24_tables_1.readB24)(0x8461 + s.read(KEY_043D));
                    if (t !== 0xff)
                        this._renderText863C(t);
                }
                this._renderText863C((0, bank24_tables_1.readB24)(0x83dc + (s.read(KEY_043D) & 0x3f)));
                break;
            }
            case 4:
            ...
            case 12: {
                ...
                const t = (0, bank24_tables_1.readB24)(0x84c7 + s.read(KEY_043D));
                if (t !== 0) {
                    if (s.read(KEY_043E) & 0x80)
                        this._renderText863C(0xe6);
                    this._renderText863C(t);
                }
                break;
            }
    ...
    _cmd83E2() {
        ...
        if (v43c & 0x80) {
            ...
            if (t !== 0 && b43b === 0) {
                if (t < 3)
                    this._renderText863C((0, bank24_tables_1.readB24)(0x8440 + t)); // $83F4-$83F8
            }
            else {
                ...
                if (x === 1 && (s.read(KEY_0628) & 0x80))
                    x = 0x0a;
                const t2 = (0, bank24_tables_1.readB24)(0x8435 + x);
                if (t2 !== 0xff)
                    this._renderText863C(t2);
            }
        }
        ...
        if (x === 1 && (s.read(KEY_0628) & 0x80))
            x = 0x0a;
        let idx = (0, bank24_tables_1.readB24)(0x83bf + x);
        if (idx === 0)
            idx = (s.read(KEY_043C) & 0x03) + (0, bank24_tables_1.readB24)(0x83bf + x);
    ...
    _cmdNamePair(a1, a2) {
        ...
        const block = (0, bank24_tables_1.readB24U16)(0x8589 + idx * 2); // ram_003E/003F
        ...
        if (found) {
            ...
            this._sceneData8629((0, bank24_tables_1.readB24)(0x8579 + idx * 2)); // $8568
            this._sceneData8629((0, bank24_tables_1.readB24)(0x857a + idx * 2)); // $856E
            return;
        }
    ...
    _matchName852C(a) {
        ...
        for (let x = 0; x < 8; x++) {
            if (b0 === (0, bank24_tables_1.readB24)(0x852c + x))
                return { carry: true, idx: x };
        }
    ...
    _cmd85BB() {
        ...
        const idx = s.read(KEY_05E5);
        s.write(KEY_05E5, idx + 1);
        const n = (0, bank24_tables_1.readSceneByte)(ptr + idx); // (ram_005F),Y
        let k = n & 0xff;
        do {
    ...
    _readStreamByte(addr) {
        if (addr >= 0xe000)
            return (0, bank24_tables_1.readB31)(addr);
        if (addr >= 0x8000)
            return (0, bank24_tables_1.readSceneByte)(addr);
    ...
    _sceneReadPtr() {
        ...
        const ptr = this._getPtr(KEY_005F, KEY_0060);
        const idx = s.read(KEY_05E5);
        this._setPtr(KEY_005F, KEY_0060, (0, bank24_tables_1.readSceneU16)(ptr + idx));
        s.write(KEY_05E5, 0);
    }
    /**
     * $80CB-$80E9: 命令分发。
    ...
    _sceneDispatch() {
        ...
        const ptr = this._getPtr(KEY_005F, KEY_0060);
        const idx = s.read(KEY_05E5);
        const b = (0, bank24_tables_1.readSceneByte)(ptr + idx); // $80CE: LDA (ram_005F),Y
        const x = this._querySub(b); // $80D0: JSR $80EA → $C509 → $80ED 表
        const y = idx + x * 2 + 1; // $80D3-$80D9: Y = E5+X*2+1 (SEC;ADC)
        this._setPtr(KEY_005F, KEY_0060, (0, bank24_tables_1.readSceneU16)(ptr + y)); // $80DA-$80E2
    ...
    _q81E4() {
        ...
        const v = (0, bank24_tables_1.readSceneByte)(ptr + 7); // $81F0: LDA (ram_0034),Y (Y=7)
        if (v < 0x19)
            return 0;
        if (v < 0x36)
            return 1;
    ...
    _q8156() {
        ...
        if (a === b) {
            ...
            if (v27 === 1)
                return this._q8156Eq1(flags); // $817E
     ...
     */
    displayNumber(value, ppuAddr, digitCount = 5) {
...
exports.Bank24HudService = Bank24HudService;
