...
class Bank29RosterService {
     ...
     */
    addTeamGfxBase(div = 1) {
        const teamId = this._store.read(KEY_26);
        let gfx = (0, roster_1.getTeamGfxBase)(teamId);
        if (div === 4)
            gfx = Math.floor(gfx / 4);
        this._addTo0454(gfx);
    }
    ...
    queryRosterPtr(slot) {
        ...
        const ptr = (0, roster_1.getRosterAddr)(this._store.read(KEY_26), slot);
     ...
     */
    findTeamLimitIndex(val16) {
        let x = 0x80;
        for (;;) {
            x -= 2;
            const lo = (0, roster_1.readBank29)(roster_1.ADDR_TEAM_LIMIT + x);
            const hi = (0, roster_1.readBank29)(roster_1.ADDR_TEAM_LIMIT + x + 1);
            const limit = lo | (hi << 8);
            if (val16 >= limit)
                break;
            if (x <= 0)
     ...
     */
    getTeamLimit(idx) {
        return (0, roster_1.readBank29U16)(roster_1.ADDR_TEAM_LIMIT + idx * 2);
    }
    // ──────────────────────────────────────────────
     ...
     */
    getSpecialFormationBlock(teamId) {
        if (teamId === 0x10)
            return roster_1.ADDR_TEAM_BLOCK_10;
        if (teamId === 0x0C)
            return roster_1.ADDR_TEAM_BLOCK_0C;
        if (teamId === 0x06)
            return roster_1.ADDR_TEAM_BLOCK_06;
        return -1;
    }
    // ──────────────────────────────────────────────
     ...
     * @param teamId 球队 ID
     */
    queryRosterSlot(teamId) {
        const slot = (0, roster_1.readBank29)(roster_1.ADDR_ROSTER_BY_TEAM + teamId);
        const hiIdx = (slot & 0xF0) >> 4;
        const loIdx = (slot & 0x0F) << 1;
        // $BCF3: GFX 查找表 (2B LE)
        const gfxPtr = (0, roster_1.readBank29U16)(roster_1.ADDR_GFX_LOOKUP + hiIdx * 2);
        // $BD64: bank29 该区为 0xFF → 属于窗口切换后的 Bank28 数据
        // 保守返回 0xFF 并用 gfxPtr 兜底 (待 Bank28 service 接入)
        const subPtr = (0, roster_1.readBank29U16)(0xBD64 + loIdx);
     ...
     */
    getSkillPtr(idx) {
        return (0, roster_1.readBank29U16)(roster_1.ADDR_SKILL_PTR + idx * 2);
    }
    // ──────────────────────────────────────────────
    // bank_01 $8C4D — 名字搜索
    // ──────────────────────────────────────────────
    ...
    nameSearch(id) {
        for (let x = 0; x < 64; x++) {
            const addr = roster_1.ADDR_NAME_SEARCH + x * 3;
            if ((0, roster_1.readBank29)(addr) === id) {
                return { lo: (0, roster_1.readBank29)(addr + 1), hi: (0, roster_1.readBank29)(addr + 2) };
            }
        }
        return null;
    }
     ...
     */
    getPlayerAttrPtr(idx) {
        return (0, roster_1.readBank29U16)(roster_1.ADDR_PLAYER_PTR + idx * 2);
    }
    // ──────────────────────────────────────────────
    ...
    /** 按索引取 CPU 阵容 */
    getRoster(idx) {
        return (0, roster_1.getCpuRoster)(idx);
    }
    /** 按指针表地址反查阵容 */
    getRosterByAddr(cpuAddr) {
        return (0, roster_1.getRosterByAddr)(cpuAddr);
    }
    /** 读内嵌数据字节 (Model 层透传) */
    read(cpuAddr) {
        return (0, roster_1.readBank29)(cpuAddr);
    }
    /** 读内嵌数据 16bit LE (Model 层透传) */
    read16(cpuAddr) {
        return (0, roster_1.readBank29U16)(cpuAddr);
    }
    /** 读 16bit LE / div */
    read16Div(cpuAddr, div) {
        return (0, roster_1.readBank29U16Div)(cpuAddr, div);
    }
    /** 阵容球员槽位 */
    rosterPlayers(idx) {
        const r = (0, roster_1.getCpuRoster)(idx);
        return r ? [...r.players] : [];
    }
}
exports.Bank29RosterService = Bank29RosterService;
