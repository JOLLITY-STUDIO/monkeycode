...
const SPRITE_TAIL_A677 = [0x79, 0xFF, 0x03, 0xC2];
/** $A777-$A77A — handler[16] $A6F9 循环的拷贝源 (→ ram_0460-$0463) */
const SPRITE_TAIL_A67B = [0x46, 0xF6, 0x02, 0x52];
/**
...
// ═══════════════════════════════════════════════════════════════
class Bank02Service {
    constructor(_store, _bank00, _bank30) {
        this._store = _store;
        this._bank00 = _bank00;
        this._bank30 = _bank30;
    }
    // ── 公开接口 ──
    get store() { return this._store; }
    get bank00() { return this._bank00; }
    // ──────────────────────────────────────────────
    // $A200 → $A21B: RESET 入口
    // ──────────────────────────────────────────────
    /**
    ...
    _readTable_AADF(offset) {
        if (offset < 0 || offset >= SCROLL_DELTA.length)
            return 0;
        return SCROLL_DELTA[offset];
    }
    /** $AAE0 表: 滚动 Y delta — $AADF 的奇数下标 (Y 步进 2), 越界返回 0 */
    _readTable_AAE0(offset) {
        const i = offset + 1;
        if (i < 0 || i >= SCROLL_DELTA.length)
            return 0;
    ...
    _readFieldTiles(i) {
        if (i < 0 || i >= bank02_tables_1.FIELD_TILES.length)
            return 0;
        return bank02_tables_1.FIELD_TILES[i];
    }
    /** $AA75 FIELD_KIND 表读取 (越界返回 0) */
    _readFieldKind(i) {
        if (i < 0 || i >= bank02_tables_1.FIELD_KIND.length)
            return 0;
        return bank02_tables_1.FIELD_KIND[i];
    }
    /**
    ...
    _jumpHandler_15_A650() {
        const s = this._store;
        // $A651: LDA #$00; STA ram_00ED — ED 从 0 开始
        let ed = 0;
        s.write('ram_00ED', 0);
        const table = bank02_tables_1.SCENE_SCRIPT; // $AA97
...
exports.Bank02Service = Bank02Service;
