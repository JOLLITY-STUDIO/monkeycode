...
const bank16_data_1 = require("../data/prg/bank16-data");
// ═══════════════════════════════════════════════════════════════
// RAM 语义键 (替代 NES 内存地址)
// ═══════════════════════════════════════════════════════════════
// 零页
...
class Bank16Service {
    ...
    readU16(addr) {
        return (0, bank16_data_1.readB16U16)(addr);
    }
    ...
    _fixedC548(a) {
        ...
        for (let radius = 0x08; radius < 0x100; radius += 0x08) {
            for (let i = 0; i < 0x0a; i++) {
                const id = ((a + 1 + i) & 0xff);
                if (id === s.read(KEY_0441) || id === s.read(KEY_0442))
                    continue;
                const ptr = this._queryNamePtr0034(id);
                if (this._readRamByte(ptr + 0x0a) !== 0)
                    continue;
                // 距离检查 ($CED6)
    ...
    _loadScriptPtr() {
        ...
        const ptr = idx & 0x80 ? (0, bank16_data_1.readB16TableI)(idx & 0x7f) : (0, bank16_data_1.readB16TableH)(idx & 0x7f);
    ...
    _dispatchCmdTableA(idx) {
        switch ((0, bank16_data_1.readB16CmdPtr)(idx)) {
            case 0x80cf:
                this._cmdExit(true);
                break; // $F0
            case 0x80d4:
            ...
            case 0x87e0:
                this.entry_87E2();
                break; // $F4: ram_052A=$40
            case 0x87e6:
                this._store.write(KEY_052A, 0);
                break; // $F5
            case 0x87ec:
                this._store.write(KEY_052A, this._store.read(KEY_052A) ^ 0x40);
                break; // $F6
            case 0x87f5:
                this._cmdRead052B();
                break; // $F7
    ...
    _cmdReadG() {
        ...
        switch ((0, bank16_data_1.readB16ScriptCmdPtr)(b)) {
            case 0x88fc: {
            ...
            case 0x890d:
                this.entry_8910();
                break; // $890D (含 LDA ram_05FB)
            case 0x893d:
                this._jmp812F(0);
                break; // LDX #$00; JMP $812F
            case 0x8942:
                this.entry_8944();
                break; // LDX #$00 → entry_8944
            default: break;
        }
    }
    // ──────────────────────────────────────────────
     ...
     */
    fn_816E(a) {
        switch ((0, bank16_data_1.readB16PredPtr)(a & 0x7f)) {
            ...
            case 0x8259: {
                const v = this._store.read(KEY_044E);
                return v === 0 ? 0 : (v - 1) & 0xff;
            } // LDX 044E; BEQ; DEX
            case 0x8260: return this._store.read(KEY_0616); // LDX ram_0616
            case 0x8264: {
                const v = this._store.read(KEY_0442);
                return (v === 0 || v === 0x0b) ? 1 : 0;
            }
            ...
            case 0x8275: {
                ...
                if (v !== 0 && v !== 0x0b) {
                    x = 1;
                    if (this._store.read(KEY_043D) !== 3)
                        x = 0;
                }
                return x;
            }
            case 0x828a: return (0, bank16_data_1.readB16Lookup8291)(this._store.read(KEY_043B)); // LDX $8291,Y (Y=043B)
            ...
            case 0x829f: {
                ...
                if (this._store.read(KEY_043B) === 0) {
                    if (v >= 3)
                        x = 1;
                }
                else {
            ...
            case 0x836e: {
                let x = 0;
                if (this._store.read(KEY_043B) !== 1) {
                    x = this.fn_8677() + 1;
                }
                return x;
            }
            ...
            case 0x83a8: return (0, bank16_data_1.readB16Lookup83AF)(this._store.read(KEY_043D)); // LDX $83AF,Y (Y=043D)
            case 0x83b4: return (0, bank16_data_1.readB16Lookup83BB)(this._store.read(KEY_043B)); // LDX $83BB,Y (Y=043B)
            case 0x83c2: return this._store.read(KEY_0612); // LDX ram_0612
            case 0x83c6: {
                const v = this.fn_8207(this._store.read(KEY_0441));
                return (v === 0x1c || v === 0x48) ? 0 : 1;
            }
            ...
            case 0x83f5: {
                this.fn_8211(1);
                return this._store.read(KEY_043C) & 0x7f;
            }
            case 0x8401: {
                const v = this._store.read(KEY_043C) & 0x7f;
                this.fn_8211(v);
            ...
            case 0x8546: {
                const v = this.fn_8207(this._store.read(KEY_0441));
                return (v === 0x1b || v === 0x4a) ? 0 : 1;
            }
            case 0x8556: return this.pred_8556();
            case 0x856c: return this._store.read(KEY_044E); // LDX ram_044E
            case 0x8570: return (0, bank16_data_1.readB16Lookup857A)(this._store.read(KEY_043D)); // LDX $857A,Y (Y=043D)
            case 0x8580: return this._store.read(KEY_043E) & 0x7f; // LDA 043E; AND #$7F
            case 0x8587: {
                const x = this._store.read(KEY_043E) & 0x7f;
                return x === 0 ? 0 : 1;
            }
            case 0x8592: {
                const v = this.fn_8207(this._store.read(KEY_0441));
                return (v === 0x1a || v === 0x41) ? 0 : 1;
            }
            case 0x85a2: {
                const v = this.fn_8207(this._store.read(KEY_0441));
                return (v === 0x1d || v === 0x4b) ? 0 : 1;
            }
            case 0x85b2: {
                const v = this.fn_8207(this._store.read(KEY_0441));
                return v === 0x3e ? 0 : 1;
            ...
            case 0x862e: return (0, bank16_data_1.readB16Lookup8635)(this._store.read(KEY_043D)); // LDX $8635,Y (Y=043D)
            case 0x863b: return (0, bank16_data_1.readB16Lookup8645)(this._store.read(KEY_043D) & 0x0f); // AND #$0F → $8645,Y
    ...
    fn_82FB(a) {
        switch ((0, bank16_data_1.readB16XCountPtr)(a)) {
    ...
    pred_82BA() {
        ...
        while (rnd < (0, bank16_data_1.readB16Stats8308)(y)) {
            x = (x + 1) & 0xff;
            y = (y + 1) & 0xff;
        }
        this.fn_82FB(x);
    ...
    pred_847E() {
        ...
        if ((0, bank16_data_1.readB16AnimAction)(a2) === 0)
            return 0;
        const a1 = this.fn_8207(this._store.read(KEY_0441));
        const x1 = (0, bank16_data_1.readB16AnimAction)(a1);
        this.fn_8211(a1);
        return x1;
    }
    ...
    pred_8498() {
        const a1 = this.fn_8207(this._store.read(KEY_0441));
        if ((0, bank16_data_1.readB16AnimAction)(a1) === 0)
            return 0;
        const a2 = this.fn_8207(this._store.read(KEY_0442));
        const x2 = (0, bank16_data_1.readB16AnimAction)(a2);
        this.fn_8211(a2);
        return x2;
    }
    ...
    pred_84B2() {
        ...
        if (this._store.read(KEY_043E) & 0x80) {
            ...
            x = (0, bank16_data_1.readB16AnimAction)(a);
            this.fn_8211(a);
        }
        return x;
    }
    ...
    pred_850B() {
        if ((this._store.read(KEY_043E) & 0x80) === 0)
            return 0;
    ...
    pred_8556() {
        if ((this._store.read(KEY_043E) & 0x7f) === 0)
            return 0;
        const a = this.fn_8207(this._store.read(KEY_0442));
        const x = (0, bank16_data_1.readB16AnimHigh)(a);
        this.fn_8211(a);
        return x;
    }
    ...
    pred_8602() {
        ...
        const x = (0, bank16_data_1.readB16AnimAction)(a);
        this.fn_8211(a);
        return x;
    }
    ...
    pred_8610() {
        const v = this.fn_8207(this._store.read(KEY_05FB) ^ 0x0b);
        for (let x = 4; x >= 1; x--) {
            if (v === (0, bank16_data_1.readB16Lookup8622)(x))
                return x;
        }
        return 0;
    }
    ...
    pred_864A() {
        if (this._store.read(KEY_0612) >= 3)
            return 0;
    ...
    pred_868A() {
        ...
        let y = 0;
        for (; y < 14; y += 2) {
            if (v === (0, bank16_data_1.readB16Pair86A6)(y))
                break;
        }
        const x = (0, bank16_data_1.readB16Pair86A6)(y + 1);
        this.fn_8211(1);
        return x;
    }
    ...
    pred_86B6() {
        const v = this._store.read(KEY_043C) & 0x7f;
        for (let x = 0; x < 4; x++) {
            if (v === (0, bank16_data_1.readB16Lookup86C8)(x))
                return x;
        }
        return 4;
    }
     ...
     */
    entry_83B7() {
        return (0, bank16_data_1.readB16Lookup83BB)(this._store.read(KEY_043B));
    }
    // ──────────────────────────────────────────────
    // $83C5: 入口
    // ──────────────────────────────────────────────
     ...
     */
    entry_863E() {
        return (0, bank16_data_1.readB16Lookup8645)(this._store.read(KEY_043D) & 0x0f);
    }
    // ──────────────────────────────────────────────
    // $86B8: 入口
    // ──────────────────────────────────────────────
     ...
     */
    fn_886A(a) {
        switch ((0, bank16_data_1.readB16ScriptBytePtr)(a)) {
    ...
    entry_8879() {
        ...
        if (y < this._readRamByte(np + 0x08))
            c++; // $888A-$888F
        if (x < this._readRamByte(np + 0x06))
            c += 2; // $8894-$889B
    ...
    entry_8944() {
        ...
        if (this._store.read(KEY_05FB) === 0) {
            ...
            for (let id = 1; id <= 0x0a; id++) {
                if (this._readRamByte(this._queryNamePtr0034(id)) === 0x1a) {
                    hit = true;
                    break;
                }
            }
