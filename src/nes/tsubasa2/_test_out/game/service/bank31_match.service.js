...
exports.Bank31MatchService = exports.SPR_MODE_TABLE_F00F = exports.SPR_ATTR_F10E = exports.STEP_TABLE_E8ED = exports.SPEED_TABLE_E6CF = void 0;
const bank31_data_ptrs_1 = require("../data/prg/bank31-data-ptrs");
const bank31_data_sprites_1 = require("../data/prg/bank31-data-sprites");
const bank31_data_scripts_1 = require("../data/prg/bank31-data-scripts");
// ═══════════════════════════════════════════════════════════════
// RAM 语义键 (替代 NES 内存地址)
// ═══════════════════════════════════════════════════════════════
...
class Bank31MatchService {
    ...
    _hudPlayerMove(idx) {
        ...
        for (let i = 0; i < count; i++) {
            ...
            if (st === 0xF0 && zone === this._r(KEY_05FE))
                break;
    ...
    movePlayers() {
        ...
        for (let idx = 0; idx < 0x0B; idx++) {
            ...
            if (this._pf(this._ptrBase, 0x00) === 0x20) {
                ...
                if ((this._pf(this._ptrBase, 0x01) | this._pf(this._ptrBase, 0x02)) === 0)
                    continue;
            }
    ...
    handleInput() {
        ...
        // ── 共用段 $E3CA-$E406 ──
        // 玩家控球但无方向 → 直接返回
        if (this._r(KEY_05FB) === 0 && (this._r(KEY_001C) & 0x0F) === 0)
            return;
    ...
    dispatchActions() {
        ...
        for (let idx = 0; idx < PLAYER_MAX; idx++) {
            ...
            if (this._r(KEY_05FB) !== 0 && idx === this._r(KEY_05FD))
                continue;
            // $E427-$E42E: 玩家控球球员跳过
            if (idx === this._r(KEY_0441) && idx < 0x0B)
                continue;
            ...
            if (this._pf(this._ptrBase, PLAYER_STATE) === 0xF0)
                pal = 0x1F;
    ...
    handleAttack() {
        ...
        for (let src = 0; src < n; src++) {
            // $E55A: 状态 == $05 才处理
            if (this._r(ramKey(0x060B + src)) !== 0x05)
                continue;
            const pid = this._r(ramKey(0x0601 + src));
            // $E561-$E568: 跳过 0/$0B
            if (pid === 0x00 || pid === 0x0B)
                continue;
            // $E56A: 压缩写入
    ...
    switchSide() {
        ...
        let dir = 0;
        if ((this._r(KEY_0635) & 0x80) !== 0)
            dir |= 0x01; // BIT $0635; BPL
        if ((this._r(KEY_0637) & 0x80) !== 0)
            dir |= 0x02; // BIT $0637; BPL
        this._w(KEY_003A, dir);
        // $E69A: 随机方向 (0-7) → 速度表
        const sp = exports.SPEED_TABLE_E6CF[this._r(KEY_00E2) & 0x07];
        let sx = sp[0]; // $E6CF,X
        let sy = sp[1]; // $E6D0,X
    ...
    moveBallToward() {
        ...
        if (this._r(KEY_05FE) === this._r(KEY_0638))
            return;
        ...
        for (;;) {
            ...
            if (zone === this._r(KEY_05FE))
                continue;
            // $E7AC: 新区域
            this._w(KEY_05FE, zone);
            // $E7AF: 到达目标 → 收尾 (BEQ $E7C0)
            if (zone === this._r(KEY_0638))
                break;
     ...
     */
    writeSprites(index, row) {
        // $E940-$E94C: 等待 $0515==0 后置 1 (帧同步)
        this._rt.syncFrame(1);
        if (this._r(KEY_0515) !== 0)
            return;
        ...
        const layout = (0, bank31_data_ptrs_1.readPtrLE16)(bank31_data_ptrs_1.PTR_TABLE_E9DA, index) - 0xEA1C;
        // $E96C-$E97A: 布局[0]=NTaddr lo 偏移, 布局[1]=NTaddr hi 偏移
        let ntLo = (this._r(KEY_003E) + (layout < bank31_data_ptrs_1.LAYOUT_DATA_EA1C.length ? bank31_data_ptrs_1.LAYOUT_DATA_EA1C[layout] : 0)) & 0xFF;
        let ntHi = (this._r(KEY_003F) + (layout + 1 < bank31_data_ptrs_1.LAYOUT_DATA_EA1C.length ? bank31_data_ptrs_1.LAYOUT_DATA_EA1C[layout + 1] : 0)) & 0xFF;
        // $E97D-$E987: 控制字节 (布局[2])
        const ctrl = layout + 2 < bank31_data_ptrs_1.LAYOUT_DATA_EA1C.length ? bank31_data_ptrs_1.LAYOUT_DATA_EA1C[layout + 2] : 0;
        ...
        while (rows !== 0) {
            ...
            while (n !== 0) {
                if (!zeroFill) {
                    ...
                    const tile = off < bank31_data_ptrs_1.LAYOUT_DATA_EA1C.length ? bank31_data_ptrs_1.LAYOUT_DATA_EA1C[off] : 0;
                    off++;
                    if (tile === 0xFE) {
                        zeroFill = true;
                    }
    ...
    renderSceneSprites() {
        ...
        for (let i = 0; i < active; i++) {
            ...
            this._w(KEY_003A, bank31_data_sprites_1.SCENE_LAYOUT_PTR_F206[idx]);
            this._w(KEY_003B, bank31_data_sprites_1.SCENE_LAYOUT_PTR_F206[idx + 1]);
    ...
    _sceneSpriteMarkers() {
        ...
        let mode = exports.SPR_MODE_TABLE_F00F[modeIdx] ?? 0;
        if (modeIdx === 0x00 && this._r(KEY_0600) === 0x00)
            mode = 0x02;
        ...
        for (let i = 0; i < 6; i++) {
            ...
            const y = (mode << 2) & 0xFF;
            this._w(KEY_003C, bank31_data_sprites_1.SPRITE_NT_TABLE_F15A[y]);
            this._w(KEY_003D, bank31_data_sprites_1.SPRITE_NT_TABLE_F15A[y + 1]);
            // $F037-$F03F: $04A6 = 属性基址 + 精灵属性偏移
            this._w(KEY_04A6, (bank31_data_sprites_1.SPRITE_NT_TABLE_F15A[y + 2] + exports.SPR_ATTR_F10E[i]) & 0xFF);
            // $F042-$F064: $04A7 = 属性 hi (非模式3 合并 $05CE)
            if (mode !== 0x03) {
                this._w(KEY_04A6, (this._r(KEY_04A6) | (this._r(KEY_05CE) & 0x20)) & 0xFF);
                this._w(KEY_04A7, ((this._r(KEY_05CE) >>> 4) | bank31_data_sprites_1.SPRITE_NT_TABLE_F15A[y + 3]) & 0xFF);
            }
            else {
                this._w(KEY_04A7, bank31_data_sprites_1.SPRITE_NT_TABLE_F15A[y + 3]);
            }
            // $F067-$F069: $04A5 = 1 (行计数)
            this._w(KEY_04A5, 0x01);
            // $F06C-$F07E: $04A8 = tile ($F16A 表, 索引 = i + 模式×6)
            this._w(KEY_04A8, bank31_data_sprites_1.SPRITE_TILE_TABLE_F16A[((mode * 6) + i) & 0xFF]);
            // $F081-$F08D: 精灵布局指针 (i<<1 → $F182 表) → $003A/$003B
            const lx = (i << 1) & 0xFF;
            this._w(KEY_003A, bank31_data_sprites_1.SPRITE_PTR_TABLE_F15A[lx]);
            this._w(KEY_003B, bank31_data_sprites_1.SPRITE_PTR_TABLE_F15A[lx + 1]);
        ...
        const x = (mode << 2) & 0xFF;
        const tLo = bank31_data_sprites_1.SPRITE_NT_TABLE_F15A[x];
        const tHi = bank31_data_sprites_1.SPRITE_NT_TABLE_F15A[x + 1];
    ...
    _writeLayout(x) {
        ...
        let off;
        if (addr >= 0xF212) {
            data = bank31_data_sprites_1.NT_LAYOUT_DATA;
            off = addr - 0xF212;
        }
        else {
            data = bank31_data_sprites_1.SPRITE_DATA_F16A;
            off = addr - 0xF18E;
        }
        const read8 = () => {
    ...
    readPtr(index) {
        ...
        const table = bank31_data_scripts_1.DIALOG_PTR_TABLE_F329;
...
exports.Bank31MatchService = Bank31MatchService;
