...
exports.decodeScriptText = decodeScriptText;
// ── 单 tile 字符 ($00-$9F) ──
// 基于 chr-bank-00 tile 渲染分析
exports.CHAR_MAP_SINGLE = {
    // 空白和特殊
    0x00: ' ', // 空白
    0x01: '·', // 特殊符号 (箭头/标记)
    // 数字 0-9 (ASCII 直接映射)
...
function decodeScriptText(bytes) {
    ...
    for (const b of bytes) {
        if (b < 0xA0) {
            // 单 tile 字符
            result += exports.CHAR_MAP_SINGLE[b] ?? `[${b.toString(16).padStart(2, '0').toUpperCase()}]`;
        }
        else if (b <= 0xD7) {
            // 双 tile 字符 (日文假名)
            const entry = exports.CHAR_MAP_DOUBLE[b];
