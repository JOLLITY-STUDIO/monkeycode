...
const TEAM_BLOCK_BY_SCENE = {
    0x06: bank01_more_tables_1.TEAM_BLOCK_06,
    0x0C: bank01_more_tables_1.TEAM_BLOCK_0C,
    0x10: bank01_more_tables_1.TEAM_BLOCK_10,
};
class DataQueryService {
    ...
    entry0_PlayerData() {
        ...
        for (let i = 0; i < 10; i++) {
            ...
            let delta = 0;
            if (e7 < 63)
                delta = (this._lookupValue16(e7 + 1) - tableVal) & 0xFFFF;
            delta >>= 2;
        ...
        for (let ed = 0; ed < 0x12; ed++) {
            ...
            let x = 0;
            while (x < bank01_more_tables_1.OPTION_FLAG_B255.length && bank01_more_tables_1.OPTION_FLAG_B255[x] !== field)
                x++;
            if (x >= bank01_more_tables_1.OPTION_FLAG_B255.length)
                x = 0; // 防御: 正常流程必然命中
            ...
            const ch = bank01_more_tables_1.PLAYER_DATA_BC6E[x % bank01_more_tables_1.PLAYER_DATA_BC6E.length];
            this._charDisplay(ch, (bank01_more_tables_1.OPTION_SCREEN_Y[ed] + 0x80) & 0xFF, 0x22);
    ...
    entry6_VramBuf1() {
        ...
        const base = this._read16t(bank01_more_tables_1.TEAM_GFX_BASE, scene * 2);
    ...
    entry7_VramBuf2() {
        const s = this._store;
        const scene = s.read('ram_0026');
        const base = this._read16t(bank01_more_tables_1.TEAM_GFX_BASE, scene * 2) >> 2;
        this._add16ToQuery(base);
    }
    ...
    entry9_TeamDataInit() {
        ...
        if (scene >= 0x10)
            this._teamDataInitLoop(0x16, 0x0A);
    }
    /** $A3B4: EA 起始索引 + EB 次数 */
    _teamDataInitLoop(start, count) {
        const s = this._store;
        let ea = start;
        let eb = count;
        while (eb > 0) {
    ...
    // ── 内部: 跳转分发 ──
    _dispatchEntry(entryIndex) {
        switch (entryIndex) {
            case 0:
                this.entry0_PlayerData();
                break;
            case 1:
                this.entry1_OptionScreenInit();
                break;
            case 2:
                this.entry2_PpuGraphics();
                break;
            case 3:
                this.entry3_ScreenDraw();
                break;
            case 4:
                this.entry4_AttrBlock();
                break;
            case 5:
                this.entry5_CharDecode();
                break;
            case 6:
                this.entry6_VramBuf1();
                break;
            case 7:
                this.entry7_VramBuf2();
                break;
            case 8:
                this.entry8_DataLoad();
                break;
            case 9:
                this.entry9_TeamDataInit();
                break;
        }
    }
    ...
    _level2Confirm() {
        ...
        if (sel === index_1.TeamDataMenu.FORMATION) {
        ...
        else if (sel === index_1.TeamDataMenu.CHANGE) {
            // ChangeMenu: 0=ポジション, 1=メンバー, 2=もどる
            if (cur === index_1.ChangeMenu.BACK) {
                this._returnToSubMenu();
            }
            else {
            ...
            if (cur === index_1.LevelMenu.BACK) {
                ...
                this._optionScreen.level3Cursor = 0;
                this._optionScreen.level3Max = 11;
                this._optionScreen.levelDetailMode = cur === index_1.LevelMenu.SPECIAL_DETAIL ? 1 : 0;
            }
        }
    }
    /** 返回チームデータ子菜单 (二级→一级) */
    ...
    _level3Update() {
        const btn = this._store.read(KEY_1C);
        if (btn & 0x10)
            this._level3CursorUp();
        if (btn & 0x20)
            this._level3CursorDown();
        if (btn & 0x01)
            this._level3Confirm();
        if (btn & 0x02) {
            // B 返回二级菜单
    ...
    _level3Confirm() {
        ...
        if (sel === index_1.TeamDataMenu.CHANGE) {
            // 换人: 两步选择
     ...
     */
    _decodePlayerStats(teamId, playerIdx) {
        const raw = [];
        for (let i = 0; i < 10; i++)
            raw.push(this._r8(0x0656 + i));
    ...
    _lookupValue16(index) {
        const i = Math.max(0, Math.min(63, index | 0)) * 2;
        return (bank01_more_tables_1.LOOKUP_16BIT[i] ?? 0) | ((bank01_more_tables_1.LOOKUP_16BIT[i + 1] ?? 0) << 8);
    }
    /** 对应 bank01 $B02E: 扫描 LOOKUP_16BIT 返回最大 ≤ value 的表项索引 */
    _lookupIndex16(value) {
        for (let i = 63; i >= 0; i--) {
            if (value >= this._lookupValue16(i))
                return i;
        }
        return 0;
    }
     ...
     */
    _extractStatField(x) {
        const y = bank01_more_tables_1.POS_TABLE_AD8A[x % bank01_more_tables_1.POS_TABLE_AD8A.length] ?? 0;
     ...
     */
    _ppuBlockFill(rows, bytesPerRow, startAddr, fill) {
