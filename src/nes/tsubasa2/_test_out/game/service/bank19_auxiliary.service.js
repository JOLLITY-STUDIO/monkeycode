...
class Bank19Service {
    ...
    _fixedC530(x, a) {
        ...
        for (let i = 0; i < 16; i++) {
            ...
            if ((x & 3) === 0) {
                ...
                v = prg_bank_31_1.default[ptr + y] ?? 0x0f; // $CC30: (ram_0065),Y 连续读表
                y++;
            }
            s.write(`ram_046F+${x}`, v);
            x = (x + 1) & 0xff;
    ...
    _readStreamByte() {
        const b = prg_bank_19_1.default[this._streamPtr + this._streamPos] ?? 0xff;
        this._streamPos = (this._streamPos + 1) & 0xff;
        return b;
    }
    /** 读数据流字节 (不推进) */
    _peekStreamByte() {
        return prg_bank_19_1.default[this._streamPtr + this._streamPos] ?? 0xff;
    }
    /**
