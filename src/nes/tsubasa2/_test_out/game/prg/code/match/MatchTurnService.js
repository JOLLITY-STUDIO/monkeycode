...
class MatchTurnService {
    ...
    // ════════════════════════════════════════════════
    /** $800C → $8083: 水平滚动状态机 */
    horizontalScroll() { this.scrollStateMachine(true); }
    /** $8003 → $8083: 垂直滚动状态机 */
    verticalScroll() { this.scrollStateMachine(false); }
    /** $8006 → $84A1: 精灵组写入 */
    spriteGroupWrite() { this.sub84A1(); }
    /** $8009 → $814C: 脚本处理 */
    scriptProcess() { this.sub814C(); }
    // ════════════════════════════════════════════════
    // $800C / $8083 滚动状态机
    ...
    sub8525(a, y) {
        ...
        if (spriteId & 0x80)
            this.wr(0x0055, (this.rd(0x0055) + 1) & 0xFF);
