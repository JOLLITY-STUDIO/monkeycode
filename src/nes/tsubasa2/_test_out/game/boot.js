...
class BootService {
    ...
    init() {
        ...
        this._levelup = this._levelupSvc ?? new levelup_service_1.LevelUpService(this._store);
    ...
    getMeetingDisplayState() {
        if (this.getRoot() !== index_1.SceneRoot.MEETING)
            return null;
     ...
     */
    getLevelUpService() {
        if (this.getRoot() !== index_1.SceneRoot.LEVELUP)
            return null;
    ...
    *_passwordCoroutine() {
        ...
        for (;;) {
            ...
            if (r === 'success') {
                ...
                this._store.write(exports.BOOT_KEYS.STORY_TO_MEETING, 1); // 续关剧情→MEETING
        ...
        for (;;) {
            ...
            if (selected !== null) {
                ...
                if (selected === index_1.TitleMenu.KICKOFF) {
                    this._store.write(exports.BOOT_KEYS.STORY_TO_MEETING, 1); // 开场剧情→MEETING
                    return index_1.SceneRoot.STORY;
                }
    ...
    *_meetingCoroutine() {
        ...
        for (;;) {
            ...
            if (confirmed === index_1.MeetingMenu.KICKOFF) {
                this._store.write('ram_00ED', 0);
                this._store.write(exports.BOOT_KEYS.STORY_TO_MEETING, 0); // 比赛前剧情→MATCH
                return index_1.SceneRoot.STORY;
            }
        }
    }
    ...
    *_storyCoroutine() {
        ...
        const toMeeting = this._store.read(exports.BOOT_KEYS.STORY_TO_MEETING) !== 0;
        const nextScene = toMeeting ? index_1.SceneRoot.MEETING : index_1.SceneRoot.MATCH;
        for (;;) {
            const buttons = yield;
            if ((buttons & types_1.BUTTON.SELECT) !== 0) {
                this._bank18.skip();
                return nextScene;
            }
    ...
    *_matchCoroutine() {
        ...
        const cfg = (0, match_config_1.getMatchConfig)(matchIdx); // 一级配置字节
        void cfg;
        ...
        this._store.write(exports.BOOT_KEYS.MATCH_STORY_DONE, 0);
        // ── 阶段 1: 上半场 ──
        this._store.write(exports.BOOT_KEYS.MATCH_PHASE, index_1.MatchPhase.KICKOFF);
        ...
        this._store.write(exports.BOOT_KEYS.MATCH_PHASE, index_1.MatchPhase.HALF_TIME);
        yield* this._runEmbeddedStory(bank18_story_service_1.StoryChapter.HALF_TIME);
        // ── 阶段 3: 下半场 ──
        this._store.write(exports.BOOT_KEYS.MATCH_PHASE, index_1.MatchPhase.KICKOFF);
        this._store.write('ram_005E', HALF_DURATION);
        yield* this._runHalfPhase(false);
        // ── 阶段 4: 终场检测 — 平局 → 加时/PK, 分胜负 → RESULT ──
        const scoreA = this._store.read('ram_0028');
        const scoreB = this._store.read('ram_0029');
        if (scoreA === scoreB) {
            // 平局 → 加时赛 (可选, 由比赛配置决定; 当前默认有加时)
            // FIXME: 真实 ROM 由 ram_0062 bit 标志或比赛配置字段决定是否有加时
            this._store.write(exports.BOOT_KEYS.MATCH_PHASE, index_1.MatchPhase.EXTRA_TIME);
            // 加时赛开场 STORY
            yield* this._runEmbeddedStory(bank18_story_service_1.StoryChapter.EXTRA_TIME);
            this._store.write(exports.BOOT_KEYS.MATCH_PHASE, index_1.MatchPhase.KICKOFF);
            this._store.write('ram_005E', EXTRA_DURATION);
            yield* this._runHalfPhase(true);
            // 加时赛后仍平局 → PK
            const sa2 = this._store.read('ram_0028');
            const sb2 = this._store.read('ram_0029');
            if (sa2 === sb2) {
                // PK 阶段
                this._store.write(exports.BOOT_KEYS.MATCH_PHASE, index_1.MatchPhase.PK_SHOOTOUT);
    ...
    *_runHalfPhase(isExtra) {
        ...
        for (;;) {
            ...
            if (r5E === 0 || (r62 & 0x20) !== 0 || frame >= guard)
                return;
        }
    }
    /**
    ...
    *_runEmbeddedStory(chapter) {
        ...
        for (;;) {
            const buttons = yield;
            frame++;
            if ((buttons & (types_1.BUTTON.SELECT | types_1.BUTTON.A)) !== 0 || frame >= STORY_GUARD) {
                this._bank18.skip();
                return;
            }
            const done = this._bank18.update(0);
    ...
    *_runPkPhase() {
        ...
        for (;;) {
            ...
            this._hud?.matchFrameTick();
            frame++;
            const sa = this._store.read('ram_0028');
            const sb = this._store.read('ram_0029');
            if (sa !== sb || frame >= PK_GUARD) {
                // 帧守卫超时且仍平局 → 强制分胜负 (主队+1)
                if (sa === sb)
                    this._store.write('ram_0028', sa + 1);
                return;
            }
        }
    }
    ...
    *_levelupCoroutine() {
        ...
        for (;;) {
            ...
            if ((buttons & (types_1.BUTTON.A | types_1.BUTTON.START)) !== 0) {
                ...
                this._store.write(exports.BOOT_KEYS.STORY_TO_MEETING, 0); // 下一场赛前剧情→MATCH
                return index_1.SceneRoot.STORY;
            }
        }
    }
