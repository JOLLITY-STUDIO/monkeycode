...
exports.CHARACTER_BY_ID = exports.CHARACTER_LIST = void 0;
exports.getCharacterName = getCharacterName;
exports.getCharacterNameCn = getCharacterNameCn;
...
/** ID→角色定义映射 (快速查找) */
exports.CHARACTER_BY_ID = new Map(exports.CHARACTER_LIST.map(c => [c.id, c]));
/** 按 ID 获取角色名 (英) */
function getCharacterName(id) {
    return exports.CHARACTER_BY_ID.get(id)?.name ?? `Unknown_${id.toString(16)}`;
}
/** 按 ID 获取角色名 (中) */
function getCharacterNameCn(id) {
    return exports.CHARACTER_BY_ID.get(id)?.nameCn ?? `未知_${id.toString(16)}`;
}
/** 按 ID 判断是否门将 */
function isGoalkeeper(id) {
    return exports.CHARACTER_BY_ID.get(id)?.isGk ?? false;
}
