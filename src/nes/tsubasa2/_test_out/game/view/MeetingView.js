...
const PLAYER_TEAM = team_roster_1.PLAYER_TEAMS[0]; // Sao Paulo
const PLAYER_ITEMS = PLAYER_TEAM.starters.map((id, idx) => {
    const name = (0, character_list_1.getCharacterName)(id);
    const gk = (0, character_list_1.isGoalkeeper)(id) ? 'GK' : '';
    return `${(idx + 1).toString().padStart(2, '0')} ${name}${gk ? ' ' + gk : ''}`.slice(0, 12);
});
class MeetingView extends SceneView_1.SceneView {
    ...
    _renderLevel3(state) {
        ...
        for (let i = 0; i < PLAYER_ITEMS.length; i++) {
            ...
            // 换人模式下标记已选换下/换上
            let suffix = '';
            if (state.swapOutIdx === i)
                suffix = '<O';
            if (state.swapInIdx === i)
                suffix = '<I';
     ...
     */
    _renderPlayerStatsPanel(playerIdx, detailMode) {
        // 面板在屏幕右下 (X=16, Y=18-27)
        const PX = 16;
        const PY = 18;
        const playerId = PLAYER_TEAM.starters[playerIdx] ?? 0;
        const nameCn = (0, character_list_1.getCharacterNameCn)(playerId);
        const nameEn = (0, character_list_1.getCharacterName)(playerId);
        // 标题: 选手名
        ...
        if (detailMode === 0) {
            ...
            const moves = (0, special_moves_1.getSpecialMoves)(playerId);
            if (moves) {
                const fields = [
                    { key: 'shot', label: 'SHT', table: special_moves_1.SHOT_DIGITS },
                    { key: 'pass', label: 'PAS', table: special_moves_1.PASS_DIGITS },
                    { key: 'dribble', label: 'DRB', table: special_moves_1.DRIBBLE_DIGITS },
                ];
                for (let i = 0; i < fields.length; i++) {
    ...
    // ── 辅助: 写文本到 NT ──
    _drawText(text, tileX, tileY, highlight) {
        const entry = {
            tile: 0,
    ...
    _getLevel3Title(state) {
        if (state.subConfirmed === 2) {
            // 换人
            if (state.swapOutIdx < 0)
                return '> SELECT OUT';
            if (state.swapInIdx < 0)
                return '> SELECT IN';
            return '> SWAP DONE';
        }
