...
const SLOT_DX = 8;
const SLOT_DY = 16;
class PasswordView extends SceneView_1.SceneView {
    render(state) {
        // 背景 NT (Cut 0x17) 由 boot 协程委派 bank02.entryF(0) 写入, 不清屏 (否则清掉背景)
        // 1. 静态假名网格/边框/标题精灵 (真实 trace)
        const sprites = password_sprites_1.PASSWORD_SPRITES.map((spr) => ({
            active: true,
            x: spr.x,
            y: spr.y,
            tile: spr.tile,
        ...
        for (let i = 0; i < state.charCount; i++) {
            ...
            sprites.push({
                ...
                tile: (0, bank02_password_service_1.passwordCharToTile)(state.chars[i]),
