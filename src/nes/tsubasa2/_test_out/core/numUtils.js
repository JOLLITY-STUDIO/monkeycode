...
exports.DIGIT_TILE_BASE = void 0;
exports.div16 = div16;
exports.numberToTiles16 = numberToTiles16;
exports.numberToTiles8 = numberToTiles8;
/** 数字→tile ID 的基数 (0x33) */
 ...
 */
function div16(dividendLo, dividendHi, divisorLo, divisorHi) {
...
function numberToTiles16(valueLo, valueHi, digitCount = 5) {
    ...
    if (value === 0) {
        // 显示 "0" * digitCount
        return new Array(digitCount).fill(exports.DIGIT_TILE_BASE);
    }
    const tiles = [];
    let v = value;
    while (v > 0 && tiles.length < digitCount) {
        const { remainderLo } = div16(v & 0xFF, (v >> 8) & 0xFF, 0x0A, 0x00);
        tiles.push(remainderLo + exports.DIGIT_TILE_BASE); // 余数 + 0x33 = tile_id
        v = Math.floor(v / 10);
    }
    // 补前导 0
    while (tiles.length < digitCount) {
        tiles.push(exports.DIGIT_TILE_BASE);
    }
 ...
 */
function numberToTiles8(value, digitCount = 2) {
    const tiles = [];
    let v = value;
    if (v === 0) {
        return new Array(digitCount).fill(exports.DIGIT_TILE_BASE);
    ...
    while (v > 0 && tiles.length < digitCount) {
        tiles.push((v % 10) + exports.DIGIT_TILE_BASE);
        v = Math.floor(v / 10);
    }
    while (tiles.length < digitCount) {
        tiles.push(exports.DIGIT_TILE_BASE);
