...
exports.getPaletteData = getPaletteData;
exports.renderPaletteImage = renderPaletteImage;
function getPaletteData(nes) {
    const ppu = nes.ppu;
...
function renderPaletteImage(nes) {
    ...
    for (let r = 0; r < 8; r++) {
        for (let c = 0; c < 8; c++) {
            drawBlock(sysX + c * (SYS_B + SYS_G), sysY + r * (SYS_B + SYS_G), d.systemPalette[r * 8 + c], SYS_B);
        }
    }
