...
class RenderingPrimitivesService {
    ...
    loadBgPalette(index) {
        var _a;
        const pal = (_a = opening_data_1.OPENING_BG_PALETTES[index & 0x0f]) !== null && _a !== void 0 ? _a : opening_data_1.OPENING_BG_PALETTES[0];
    ...
    loadSprPalette(index) {
        var _a;
        const pal = (_a = opening_data_1.OPENING_SPR_PALETTES[index & 0x0f]) !== null && _a !== void 0 ? _a : opening_data_1.OPENING_SPR_PALETTES[0];
        for (let i = 0; i < 0x10; i++) {
    ...
    loadSceneData(sceneId) {
        var _a, _b;
        const entry = (_a = opening_data_1.OPENING_SCENE_TABLE[sceneId & 0x0f]) !== null && _a !== void 0 ? _a : opening_data_1.OPENING_SCENE_TABLE[0];
        const store = this.store;
        store.writeByte(0x0079, entry.scrollFlag);
        store.writeByte(0x007a, 0);
        for (let i = 0; i < 0x12; i++) {
            store.writeByte(0x007c + i, (_b = entry.data[i]) !== null && _b !== void 0 ? _b : 0);
        }
    }
    // ──────────────────────────── $8AF7 CHR 配置读取（配置副作用） ────────────────────────────
    /**
    ...
    loadChrConfig(configId) {
        var _a;
        const store = this.store;
        const cfg = (_a = opening_data_1.OPENING_CHR_CONFIGS[configId & 0x1f]) !== null && _a !== void 0 ? _a : opening_data_1.OPENING_CHR_CONFIGS[0];
    ...
    queueScene3NametableRows(fromRow, rows) {
        ...
        for (let r = 0; r < rows; r++) {
            ...
            for (let c = 0; c < 6; c++) {
                const patIdx = (_a = opening_data_1.OPENING_SCENE3_TILES[Math.floor(row / 4) * 6 + c]) !== null && _a !== void 0 ? _a : 0;
                const pattern = (_b = opening_data_1.OPENING_TILE_PATTERNS[patIdx]) !== null && _b !== void 0 ? _b : opening_data_1.OPENING_TILE_PATTERNS[0];
                const pr = row % 4;
