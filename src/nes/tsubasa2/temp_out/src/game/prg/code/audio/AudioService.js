...
const ApuTarget_1 = require("./ApuTarget");
...
function readFreqTable(idx) {
    return audio_rom_1.AudioRom.readBank12U16(0x870D + idx * 2);
}
// $8725 时值表（42 条，bank12 固定表）
function readDurationTable(idx) {
    return audio_rom_1.AudioRom.readBank12Byte(0x8725 + idx);
}
// $84DA 命令跳转表（32 个命令，bank12 固定表）
function readCommandAddr(cmdIdx) {
    return audio_rom_1.AudioRom.readBank12U16(0x84DA + cmdIdx * 2);
}
class AudioService {
    constructor(store) {
        this.store = store;
        this.apu = new ApuTarget_1.NullApuTarget();
    }
    attachApu(apu) { this.apu = apu; }
    // ════════════════════════════════════════════════════════════════
    // 公共 API
    // ════════════════════════════════════════════════════════════════
    update() {
    ...
    playDpcm(sample) {
        if (this.store.readByte(RAM_DPCM_FLAG) !== 0)
            return;
    ...
    startBgm(bgmId) {
        ...
        for (let ch = 0; ch < 4; ch++) {
            ...
            if (ch > 0) {
                // 从头部读取通道 ch 的数据指针
                chDataAddr = audio_rom_1.AudioRom.readBgmData(dataAddr + ch * 2);
                chDataAddr |= audio_rom_1.AudioRom.readBgmData(dataAddr + ch * 2 + 1) << 8;
                if (chDataAddr < 0x8000 || chDataAddr > 0xBFFF)
                    chDataAddr = 0;
            }
            if (chDataAddr === 0)
                continue;
    ...
    executeCommandStream(ch) {
        ...
        for (let safety = 0; safety < 256; safety++) {
            const dataByte = audio_rom_1.AudioRom.readBgmData(dataPtr + noteIdx);
            // $00 = 空数据/通道分隔符，跳过
            if (dataByte === 0x00) {
                noteIdx++;
                continue;
            ...
            const nextByte = audio_rom_1.AudioRom.readBgmData(dataPtr + noteIdx);
            noteIdx++;
            // 音名处理
    ...
    executeCommand(ch, cmdAddr, dataPtr, noteIdx) {
        ...
        if (cmdAddr === 0x8544) {
            ...
            const songNo = audio_rom_1.AudioRom.readBgmData(dataPtr + noteIdx);
            noteIdx++;
            // $8754 表在 bank12（引擎执行时 $8000-$9FFF = bank12）
            // 但 $8754 存的是地址指针，不是频率
            // 实际上 $8544 设置 offset 2-3 为 $8754[songNo] 的值
            const tablePtr = audio_rom_1.AudioRom.readBank12U16(0x8754 + songNo * 2);
        ...
        // $8641 (cmd 0xE2): 设置音量
        if (cmdAddr === 0x8641) {
            const vol = audio_rom_1.AudioRom.readBgmData(dataPtr + noteIdx);
            noteIdx++;
        ...
        if (cmdAddr === 0x8670) {
            const transpose = audio_rom_1.AudioRom.readBgmData(dataPtr + noteIdx);
            noteIdx++;
        ...
        if (cmdAddr === 0x8681) {
            const mode = audio_rom_1.AudioRom.readBgmData(dataPtr + noteIdx);
            noteIdx++;
    ...
    stopAllSeChannels() {
        ...
        for (const a of volAddrs)
            this.store.writeByte(a, VOL_STOP);
        for (const a of envAddrs)
