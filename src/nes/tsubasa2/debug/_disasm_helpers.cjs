// temp: disasm bank00 helper routines referenced by bank02 scenes
const fs = require('fs');
const rom = fs.readFileSync('src/asm/dist/tsubasa2.nes');
const prgStart = 16;
const bankSize = 8192;
const bank0 = rom.slice(prgStart, prgStart + bankSize);
...
// bank00 helper offsets (runtime addr - 0x8000)
const targets = [
  ['$8895 CHR load', 0x895, 40],
  ['$8920 sceneLoad', 0x920, 60],
  ['$890C OAM Y off', 0x90C, 25],
  ['$8976 NT attr load', 0x976, 45],
  ['$8AF7 loadChrConfig', 0xAF7, 130],
  ['$9A0D sprite helper', 0x1A0D, 50],
  ['$9A35', 0x1A35, 30],
  ['$9A43', 0x1A43, 20],
  ['$9B07', 0x1B07, 30],
  ['$9B28', 0x1B28, 30],
  ['$9B5E', 0x1B5E, 20],
  ['$9B7F hideOam', 0x1B7F, 25],
  ['$9B91 clearExt', 0x1B91, 25],
  ['$98A0', 0x1A00 - 0x60, 40],
  ['$98E8 fill', 0x18E8, 20],
  ['$98EA NT write', 0x18EA, 30],
  ['$99F0', 0x19F0, 25],
  ['$88FB OAM copy', 0x8FB, 25],
  ['$88CA', 0x8CA, 25],
  ['$9E7C', 0x1E7C, 20],
  ['$9DEE', 0x1DEE, 25],
];
for (const [name, off, n] of targets) {
  console.log(`\n===== ${name} =====`);
  console.log(disasm(bank0, off, n));
}
