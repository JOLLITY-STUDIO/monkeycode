// bank0 例程第二批：NT buffer / fade / clear
const fs = require('fs');
...
function disasm(cpuStart, cpuEnd, label) {
  console.log(`\n===== ${label} $${cpuStart.toString(16).toUpperCase()} =====`);
  let pc = cpuStart;
  let guard = 0;
  while (pc <= cpuEnd && guard++ < 250) {
    const off = b0(pc);
...
disasm(0x9B5E, 0x9BA0, 'bank0 $9B5E-$9B9F (NT commit / clear ext)');
disasm(0x9B7F, 0x9B28, 'bank0 $9B7F (hideOam)');
disasm(0x9B91, 0x9B7F, 'bank0 $9B91 (clear ext)');
disasm(0x9A0D, 0x9A35, 'bank0 $9A0D (fade BG only)');
disasm(0x9A71, 0x9AC0, 'bank0 $9A71-$9ABF (palette write)');
disasm(0x9AB8, 0x9ADA, 'bank0 $9AB8');
disasm(0x9ADA, 0x9B07, 'bank0 $9ADA');
disasm(0x9B07, 0x9B28, 'bank0 $9B07');
disasm(0x99F0, 0x9A0D, 'bank0 $99F0 (fade out?)');
disasm(0x98A0, 0x98E8, 'bank0 $98A0 (clear NT)');
disasm(0x98E8, 0x9910, 'bank0 $98E8/$98EA (NT fill)');
