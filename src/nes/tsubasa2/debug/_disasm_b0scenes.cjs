// temp: disasm bank0 scene table $8545 targets $8574-$8870
const fs = require('fs');
...
function disasm(off, count, base = 0x8000) {
  const out = [];
  let o = off;
...
// 24 entries from $8545
const table = [];
for (let i = 0; i < 24; i++) {
  const lo = bank[0x545 + i * 2];
  const hi = bank[0x546 + i * 2];
  table.push((lo | (hi << 8)) + 1);
}
const lines = [];
for (let i = 0; i < 24; i++) {
  const target = table[i];
  const next = i < 23 ? table[i + 1] : 0x8870 + 40;
  const off = target - 0x8000;
  const count = Math.min(next - target, 40);
  lines.push(`\n===== bank0 scene${i} $${target.toString(16).toUpperCase()} (${count} bytes) =====`);
  lines.push(disasm(off, count));
}
fs.writeFileSync('debug/_bank0_scenes.txt', lines.join('\n'));
console.log('written');
