// 反汇编 Scene0 真入口 + 全部 Scene1-10 真地址
const fs = require('fs');
...
const ops = {
  ...
  0xB5: ['LDA z,X', 2], 0xE8: ['INX', 1], 0xC8: ['INY', 1],
  ...
  0x25: ['AND z', 2], 0x05: ['ORA z', 2], 0x15: ['ORA z,X', 2], 0x95: ['STA z,X', 2],
  0xE1: ['SBC (z,X)', 2], 0xB1: ['LDA (z),Y', 2], 0x91: ['STA (z),Y', 2],
  0x9A: ['TXS', 1], 0xBA: ['TSX', 1],
  0x08: ['PHP', 1], 0x28: ['PLP', 1], 0x40: ['RTI', 1],
};
function disasm(bank, off, count, base = 0xA000) {
  const out = [];
  let o = off;
...
console.log('===== Scene0 真入口 $A4C0-$A55A =====');
console.log(disasm(bank2, 0x4C0, 154, 0xA000));
