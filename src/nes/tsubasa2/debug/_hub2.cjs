...
const ops = new Map([
  ...
  [0xde, 'DEC abs,X$'], [0xfe, 'INC abs,X$'], [0xb5, 'LDA $,X'], [0x95, 'STA $,X'],
  [0xd9, 'CMP abs,Y$'], [0x79, 'ADC abs,Y$'], [0x75, 'ADC $,X'], [0x7d, 'ADC abs,X$'],
  [0xf9, 'SBC abs,Y$'], [0xf5, 'SBC $,X'], [0xdd, 'CMP abs,X$'], [0xd5, 'CMP $,X'],
]);
function disasm(cpuStart, len) {
  ...
  while (p < end) {
    ...
    if (b === 0xa9 || b === 0xa0 || b === 0x09 || b === 0x29 || b === 0x49 || b === 0x69 || b === 0xe9 || b === 0xc9) {
      ...
      out.push('$' + p.toString(16).toUpperCase() + ' ' + op + prg[i + 1].toString(16).padStart(2, '0')); p += 2;
    } else if (b === 0x8d || b === 0xad || b === 0xae || b === 0x9d || b === 0xbd || b === 0xb9 || b === 0x99 || b === 0xde || b === 0xfe || b === 0xd9 || b === 0x79 || b === 0x7d || b === 0xf9 || b === 0xdd) {
      out.push('$' + p.toString(16).toUpperCase() + ' ' + op + (prg[i + 1] | (prg[i + 2] << 8)).toString(16).padStart(4, '0').toUpperCase()); p += 3;
    } else if (b === 0x81 || b === 0x91 || b === 0xb1 || b === 0xa1) {
      out.push('$' + p.toString(16).toUpperCase() + ' ' + op + prg[i + 1].toString(16).padStart(2, '0')); p += 2;
    } else if (b === 0x75 || b === 0xf5 || b === 0xd5) {
      out.push('$' + p.toString(16).toUpperCase() + ' ' + op + prg[i + 1].toString(16).padStart(2, '0')); p += 2;
    } else { out.push('$' + p.toString(16).toUpperCase() + ' ' + op); p++; }
  }
  return out.join('\n');
}
console.log('===== $A855 scene-entry A =====');
console.log(disasm(0xa855, 0x19));
console.log('\n===== $A86E scene-entry B =====');
console.log(disasm(0xa86e, 0x60));
console.log('\n===== $A8CE scene-entry C =====');
console.log(disasm(0xa8ce, 0x30));
console.log('\n===== $A8FE =====');
console.log(disasm(0xa8fe, 0x30));
