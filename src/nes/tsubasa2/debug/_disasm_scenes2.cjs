// temp: disasm bank2 scene routines A4C1+ (offset 0x4C1+)
const fs = require('fs');
...
function disasm(off, count, base = 0xA000) {
  const out = [];
  let o = off;
...
const scenes = [
  ...
  [8, 0x5C0, 'scene8 $A5C0'],
  [9, 0x5CE, 'scene9 $A5CE'],
  [10, 0x5DC, 'scene10 $A5DC'],
  [11, 0x5E9, 'scene11 $A5E9'],
  [12, 0x603, 'scene12 $A603'],
  [13, 0x61D, 'scene13 $A61D'],
  [14, 0x62A, 'scene14 $A62A'],
  [15, 0x651, 'scene15 $A651'],
  [16, 0x69D, 'scene16 $A69D'],
  [17, 0x77B, 'scene17 $A77B'],
  [18, 0x783, 'scene18 $A783'],
  [19, 0x78E, 'scene19 $A78E'],
  [20, 0x7BE, 'scene20 $A7BE'],
  [21, 0x7CF, 'scene21 $A7CF'],
  [22, 0x7D7, 'scene22 $A7D7'],
  [23, 0x7FB, 'scene23 $A7FB'],
];
const lines = [];
for (const [id, off, name] of scenes) {
  const nextOff = id < 23 ? scenes[id + 1][1] : 0x7FB + 60;
  const count = Math.min(nextOff - off, 120);
  lines.push(`\n===== ${name} (${count} bytes) =====`);
  lines.push(disasm(off, count));
}
fs.writeFileSync('debug/_scenes2.txt', lines.join('\n'));
