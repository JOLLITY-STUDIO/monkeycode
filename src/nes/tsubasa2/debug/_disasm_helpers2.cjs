// 反汇编 bank2/bank0 场景辅助例程（完整操作码表）
const fs = require('fs');
const rom = fs.readFileSync('docs/roms/Captain Tsubasa II - Super Striker (Japan).nes');
const prg = rom.slice(16);
// bank 判断：$8000-$9FFF = bank0（PRG 偏移 0）；$A000-$BFFF = bank2（PRG 偏移 0x4000）
function cpu2prg(cpu) { return cpu < 0xA000 ? cpu - 0x8000 : cpu - 0xA000 + 0x4000; }
const ops = new Map([
...
const HELPERS = [
  [0x8880, 0x80, 'bank2 $8880-$88FF (contains $8895 chr cfg, $88CA nt write, $88FB flip)'],
  [0x8976, 0x50, 'bank2 $8976 attr stream'],
  [0x9a35, 0x60, 'bank0 $9A35 palette+fade'],
  [0x9b20, 0x90, 'bank0 $9B20-$9BAF (9B28 nt alloc / 9B5E commit / 9B7F hide / 9B91 clear ext)'],
  [0x9e7c, 0x60, 'bank0 $9E7C hex convert'],
  [0x9f80, 0x60, 'bank0 $9F80-$9FDF (9F89/9F96 flags / 9FA8 wait)'],
  [0xac6d, 0x20, 'bank2 $AC6D/$AC71 hex tile'],
];
