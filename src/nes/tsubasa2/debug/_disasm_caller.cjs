// 反汇编 $A484 调用者 (bank2 $8212) 与 $A212 调用者 (bank0 $8807)
// 目的: 确认场景分发返回值的消费模型 (2=pointer-advance / 3=scene1 special)
const fs = require('fs');
const path = require('path');

const romPath = process.argv[2] || 'roms/Captain Tsubasa II - Super Striker (Japan).nes';
...
function cpu2prg(cpu) {
  // $8000-$9FFF = bank0 (PRG index 0..0x1FFF)
  // $A000-$BFFF = bank2 (PRG index 0x4000..0x5FFF)
...
const LEN = {
  0x20:3,0x4C:3,0x6C:3,0x10:2,0x30:2,0x50:2,0x70:2,0x90:2,0xB0:2,0xD0:2,0xF0:2,
};
...
function disasm(start, end, label) {
  ...
  while (cpu < end) {
    ...
    const op = prg[idx];
    const name = OPS[op];
    const isBranch = name && name.includes('B') && op >= 0x10 && (op & 0x1F) === 0x10;
    let line = `  $${cpu.toString(16).toUpperCase()}: ${name || '??'} `;
    let extra = 1;
    if (name && name.includes('#$')) { line += `#$${prg[idx+1].toString(16).padStart(2,'0').toUpperCase()}`; extra = 2; }
    else if (name && name.includes('abs')) { const a = prg[idx+1] | (prg[idx+2]<<8); line += `$${a.toString(16).padStart(4,'0').toUpperCase()}`; extra = 3; }
    else if (name && name.includes('zp')) { line += `$${prg[idx+1].toString(16).padStart(2,'0').toUpperCase()}`; extra = 2; }
    else if (name && (name.includes('JMP') || name.includes('JSR'))) { const a = prg[idx+1] | (prg[idx+2]<<8); line += `$${a.toString(16).padStart(4,'0').toUpperCase()}`; extra = 3; }
    else if (isBranch) { const off = (prg[idx+1] << 24) >> 24; const t = cpu + 2 + off; line += `$${t.toString(16).padStart(4,'0').toUpperCase()}`; extra = 2; }
    else if (name && name.includes('(zp'), name.includes(',Y)')) { line += `($${prg[idx+1].toString(16).padStart(2,'0').toUpperCase()}),Y`; extra = 2; }
    else if (name && name.includes('(zp')) { line += `($${prg[idx+1].toString(16).padStart(2,'0').toUpperCase()},X)`; extra = 2; }
    console.log(line);
    cpu += extra;
  }
}

// bank2 $8212 = JSR $A484 的调用者
disasm(0x81F0, 0x8280, 'bank2: $A484 调用者上下文');
// bank0 $8807 = JSR $A212 的调用者
disasm(0x87F0, 0x8820, 'bank0: $A212 调用者上下文');
// bank0 $80FA / $810F 调用 $A20F/$A20C 上下文
disasm(0x80E0, 0x8120, 'bank0: $A20F/$A20C 调用者上下文');
