...
Page({
  ...
  onLoad() {
    console.log('[game_compare] Game page loading');
  ...
  onUnload() {
    console.log('[game_compare] Game page unloading');
    this._unbindKeys();
    this.stopEngine();
    this.nes = null;
  },
  ...
  initCanvas() {
    ...
    query.exec((res: any) => {
      const info = res[0];
      if (!info?.node) {
        console.warn('[game_compare] canvas not ready, retry in 300ms');
      ...
      console.log('[game_compare] canvas ready (256x240)');
    ...
    try {
      ...
      this.nes = new NES({
        ...
        onFrame: (buffer: Uint32Array) => {
          if (this.slot) this.slot.frameBuf = buffer;
          this.renderSlot();
        },
      ...
      this.setData({ statusText: 'tsnes 运行中' });
      this.frameLoop();
    } catch (e: any) {
      console.error('[game_compare] NES init error:', e);
  ...
  frameLoop(): void {
    ...
    try {
      ...
      console.error('[game_compare] frame error:', e);
  ...
  _bindKeys() {
    ...
    if (!wx.onKeyDown || !wx.onKeyUp) {
      console.log('[game_compare] wx.onKeyDown/onKeyUp not available');
      return;
    }
