...
import BANK31_ANALYSIS from '../bank-detail/bank31_analysis';
import { Bank12AudioPlayer, ApuChannelState } from '../bank-detail/bank12-audio-player';
import { DATA_TABLES, S8, U16LE, S16LE } from '../bank-detail/bank02_data_schema';
...
Page({
  data: {
    ...
    // ── Record View 数据 ──
    recordMode: 'auto' as 'auto' | '4byte' | 'fc' | '8byte' | '16byte' | 'bank07',
    ...
    fieldReady: false,
    fieldImageSrc: '' as string,

    // ── Bank 02 结构分析 ──
    b02StructureReady: false,
    b02StructureSrc: '' as string,
    b02Subroutines: [] as any[],
    b02DataTables: [] as any[],
    b02SelectedFunc: null as any,

    // ── Bank 02 调试 ──
    debugMode: 'metatile' as 'metatile' | 'a72c' | 'ntxy' | 'dtable' | 'ppubuf' | 'ppufns',
    debugInput: { param: 'F7', iterations: '2F', deltaLo: 'FF', deltaHi: 'FE', mask: '07', bankId: 'F7', flags: '00', ntLo: '79', ntHi: 'C2' } as Record<string, string>,
    debugResult: '' as string,
    debugTableIdx: 0,             // 选中的数据表索引
    debugTableHex: '' as string,  // 数据表 hex 内容
    a72cPreset: 'branchA' as 'branchA' | 'branchB' | 'manual',  // A72C 预设选择
    a72cNTLayoutSrc: '' as string,  // NT 版面可视化图片
    a72cNTCanvasW: 192,
    a72cNTCanvasH: 360,

    // ── PPU Buffer 调试 ──
    ppuBufHex: '21 80 00 48 45 4C 4C 4F 00',  // 输入 hex
    ppuBufParsed: [] as any[],  // 解析后的条目
    ppuBufDiagramSrc: '' as string,  // 结构图
    ppuBufFuncPreset: 'palette_init' as string,  // 函数预设
    ppuBufFnTrace: '' as string,  // 函数追踪输出

    // ── Bank 31 结构分析 ──
    b31StructureReady: false,
    b31StructureSrc: '' as string,
    b31Subroutines: [] as any[],
    b31DataTables: [] as any[],
    b31SelectedFunc: null as any,
    b31Deps: {} as any,

    // ── Bank 31 调试 ──
    b31DebugMode: 'vector' as 'vector' | 'direction' | 'sprite_ptr' | 'zone' | 'near_player' | 'bpm_update',
    b31DebugInput: { angle: '0', animId: '0', x: '80', y: '70', px: '80', py: '70', tx: 'A0', ty: '80', threshold: '07', bpm: '00', valLo: '50', valHi: '02' } as Record<string, string>,
    b31DebugResult: '' as string,

    // ── Bank 30 核心系统库 ──
    b30StructureReady: false,
    b30StructureSrc: '' as string,
    b30Subroutines: [] as any[],
    b30DataTables: [] as any[],
    b30JumpTable: [] as any[],
    b30SelectedFunc: null as any,
    b30InterruptVectors: null as any,
    b30ResetFlow: [] as any[],
  ...
  _loadBank02Analysis() {
    const subs = (BANK02_ANALYSIS as any).subroutines || [];
    const tables = (BANK02_ANALYSIS as any).dataTables || [];
    this.setData({
      b02Subroutines: subs,
  ...
  _loadBank31Analysis() {
    const subs = (BANK31_ANALYSIS as any).subroutines || [];
    const tables = (BANK31_ANALYSIS as any).dataTables || [];
    const deps = {
      dependsOn: (BANK31_ANALYSIS as any).deps?.dependsOn || [],
      usedBy: (BANK31_ANALYSIS as any).deps?.usedBy || [],
      ramMap: (BANK31_ANALYSIS as any).ramMap || [],
      crossRefs: (BANK31_ANALYSIS as any).crossRefs || [],
    };
    this.setData({
      b31Subroutines: subs,
  ...
  _loadBank30Analysis() {
    const analysis = BANK30_ANALYSIS as any;
    const subs = analysis.subroutines || [];
    const tables = analysis.dataTables || [];
  ...
  _parseDataTables() {
    const prg02 = NES_PRG_ROM.slice(0x02 * BANK_SIZE, 0x03 * BANK_SIZE);
    const prgBase = 0x02 * BANK_SIZE; // 0x4000
    const results: any[] = [];

    for (const def of DATA_TABLES) {
      const off: number = def.addr - prgBase;
      if (off < 0 || off >= BANK_SIZE) continue;
      const raw = Array.from(prg02.slice(off, off + def.length));

      // 解析字段
      const fieldValues: { name: string; desc: string; hex: string; decoded: string }[] = [];
      for (const f of def.fields) {
        const bytes = raw.slice(f.offset, f.offset + f.size);
        const hex = bytes.map((b: number) => byteHex(b)).join(' ');
        let decoded = '';
        if (f.size === 1) {
          const s8 = S8(bytes[0]);
          const u8 = bytes[0];
          decoded = `${u8} (0x${byteHex(u8)})`;
          if (s8 !== u8) decoded += ` signed=${s8}`;
        } else if (f.size === 2) {
          const u16 = U16LE(bytes[0], bytes[1]);
          const s16 = S16LE(bytes[0], bytes[1]);
          decoded = `${u16} (0x${byteHex(bytes[1])}${byteHex(bytes[0])})`;
          if (s16 !== u16) decoded += ` signed=${s16}`;
        } else {
          decoded = `[${bytes.length}B]`;
          // For larger fields, show first few and total count
          if (bytes.length <= 16) {
            decoded = bytes.map((b: number) => byteHex(b)).join(' ');
          } else {
            const preview = bytes.slice(0, 16).map((b: number) => byteHex(b)).join(' ');
      ...
      if (def.recordSize > 0) {
        for (let i = 0; i + def.recordSize <= raw.length; i += def.recordSize) {
          ...
          const vals: string[] = [];
          for (const f of def.fields) {
            if (f.offset >= rec.length) continue;
          ...
          records.push({
            ...
            hex: rec.map((b: number) => byteHex(b)).join(' '),
            fields: vals,
          });
        }
      }
      ...
      results.push({
        ...
        fieldValues,
        records,
        rawHex: raw.map((b: number) => byteHex(b)).join(' '),
      });
    }
  ...
  onRecordModeSwitch(e: any) {
    const mode = e.currentTarget.dataset.mode as 'auto' | '4byte' | 'fc' | '8byte' | '16byte' | 'bank07';
  ...
  _parseRecords(data: number[], mode: string) {
    ...
    for (let i = 0; i < data.length - 1; i++) {
      if (data[i] === 0x00 && data[i + 1] === 0xA0) stats.$00A0count++;
    }
  ...
  _parseByBank07(data: number[]): { records: ParsedRecord[]; ptrCount: number } {
    const out: ParsedRecord[] = [];
    ...
    for (let i = 0; i < data.length - 1; i++) {
      if (data[i] === 0x00 && data[i + 1] === 0xA0) {
        ...
        // 如果遇到连续 $FF，说明到了未使用区域
        if (recStart < data.length && data[recStart] === 0xFF) break;
      }
    }
    // 尾部剩余（跳过 $FF 填充）
  ...
  _makeBank07Record(offset: number, chunk: number[], recIdx: number): ParsedRecord {
    const rec: ParsedRecord = this._makeRecord(offset, chunk);

    // 记录头分类
    let header = '';
    const h0 = chunk[0], h1 = chunk[1];
    if (chunk.length >= 2) {
      if (h0 === 0x3C && h1 === 0x3E) header = `标准区域(h=${chunk.length >= 5 ? chunk[4] : '?'}, w=${chunk.length >= 6 ? chunk[5] : '?'})`;
      else if (h0 === 0x5C && h1 === 0x5E) header = `扩展区域`;
      else if (h0 === 0x3A && h1 === 0x00) header = `小尺寸`;
      else if (h0 === 0x64 && h1 === 0x66) header = `屏幕索引(${chunk.length >= 3 ? '$' + byteHex(chunk[2]) : '?'})`;
      else if (h0 === 0x7C && h1 === 0x7E) header = `宽结构(${chunk.length >= 3 ? '$' + byteHex(chunk[2]) : '?'})`;
      else if (h0 === 0x60 && h1 === 0x62) header = `填充区域`;
      else if (h0 === 0x68 && h1 === 0x6A) header = `纯色填充`;
      else if (h0 === 0x76 && h1 === 0x7E) header = `宽体结构`;
      else if (h0 === 0x44 && h1 === 0x46) header = `次要区域`;
      else if (h0 === 0x3C && h1 === 0x4C) header = `带扩展字段`;
      else if (h0 === 0x3C && h1 === 0x66) header = `棋盘格变体`;
      else if (h0 === 0x3C && h1 === 0x6A) header = `填充变体`;
      else if (h0 === 0x3C && h1 === 0x62) header = `窄区域`;
      else if (h0 === 0x3C && h1 === 0x02) header = `小宽区域`;
      else if (h0 === 0x3C && h1 === 0x2C) header = `草地纹理`;
      else if (h0 === 0x34 && h1 === 0x36) header = `边界区域`;
      else if (h0 === 0x56 && h1 === 0x46) header = `填充扩展`;
      else if (h0 === 0x66 && h1 === 0x5E) header = `填充变体2`;
      else if (h0 === 0x00 && h1 === 0x02) header = `空填充`;
      else if (h0 === 0x06 && h1 === 0x00) header = `纯零`;
      else header = `${byteHex(h0)} ${byteHex(h1)}`;
    }

    // 统计 metatile 值分布
    const tileMap = new Map<number, number>();
    for (const b of chunk) tileMap.set(b, (tileMap.get(b) || 0) + 1);
    ...
    (rec as any)._header = header;
    (rec as any)._topTiles = topTiles;
    (rec as any)._recIdx = recIdx;
    (rec as any)._ptrLabel = '';
  ...
  _renderFieldMetatiles() {
    ...
    for (let i = 0; i < data.length - 1; i++) {
      if (data[i] === 0x00 && data[i + 1] === 0xA0) {
        ...
        rs = i + 2;
        if (rs < data.length && data[rs] === 0xFF) break;
      }
    }
    ...
    // 计算每个记录的宽高
    interface RecInfo {
      chunk: number[]; header: string; w: number; h: number; ok: boolean; label: string;
    }
    const infos: RecInfo[] = [];
    ...
    setTimeout(() => {
      ...
      query.select('#fieldCanvas')
        ...
        .exec((res: any) => {
          const canvas = res?.[0]?.node;
          if (!canvas) { setTimeout(() => that._renderFieldMetatiles(), 300); return; }
          ...
          for (let i = 0; i < infos.length; i++) {
            ...
            const ox = col * REC_W;
            if (col === 0 && row > 0) yCursor += rowHeights[row - 1] + GAP;
            const oy = yCursor;
          ...
          wx.canvasToTempFilePath({
            ...
            fail: () => {
              ...
              try {
                ...
                if (dataUrl) that.setData({ fieldImageSrc: dataUrl });
              } catch (_e) { /* ignore */ }
            },
          });
        });
  ...
  _a72cStep(
    lo: number, hi: number,
    iterations: number, deltaLo: number, deltaHi: number,
    mask: number, bankId: number, flags: number,
  ): { records: Array<[number, number, number, number]>; endLo: number; endHi: number } {
    const records: Array<[number, number, number, number]> = [];
  ...
  _parseBank07Blocks(data: readonly number[]): number[][] {
    const blocks: number[][] = [];
    let rs = 0;
    for (let i = 0; i < data.length - 1; i++) {
      if (data[i] === 0x00 && data[i + 1] === 0xA0) {
        const chunk = data.slice(rs, i);
        if (chunk.length > 0) blocks.push(chunk);
        rs = i + 2;
        if (rs < data.length && data[rs] === 0xFF) break;
      }
    }
  ...
  _ntToXY(ntLo: number, ntHi: number): { cx: number; cy: number } {
    // 构造 16-bit PPU 地址
  ...
  _renderStructureMap() {
    const that = this;
    const analysis: any = BANK02_ANALYSIS;
    ...
    query.select('#structCanvas')
      ...
      .exec((res: any) => {
        const canvas = res?.[0]?.node;
        if (!canvas) { setTimeout(() => that._renderStructureMap(), 300); return; }
        const w = 360;
        const h = 120;
        canvas.width = w;
        canvas.height = h;
        ...
        let cx = 0;
        ctx.fillStyle = '#58a6ff'; ctx.fillRect(cx, barY, codeW, barH); cx += codeW;
        ctx.fillStyle = '#3fb950'; ctx.fillRect(cx, barY, dataW, barH); cx += dataW;
        ctx.fillStyle = '#6e7681'; ctx.fillRect(cx, barY, unaccW, barH);
        ...
        for (let bankAddr = 0x8000; bankAddr <= 0xA000; bankAddr += 0x800) {
          ...
          ctx.beginPath(); ctx.moveTo(x, barY); ctx.lineTo(x, detailY + detailH); ctx.stroke();
        }
        ...
        wx.canvasToTempFilePath({
          canvas,
          success: (r: any) => { that.setData({ b02StructureSrc: r.tempFilePath, b02StructureReady: true }); },
          fail: () => {
            try { const url = (canvas as any).toDataURL?.(); if (url) that.setData({ b02StructureSrc: url, b02StructureReady: true }); } catch (_e) {}
          },
        });
      });
  },
  ...
  _expandMetatile(param: number, mtGroup: number): number[] {
  ...
  _renderFieldAssembled() {
    const that = this;
    const bank07Data = PRG_BANK_07 as readonly number[];
    ...
    const allRecords: Array<[number, number, number, number]> = [];
    ...
    // 3. 按 param 分类着色
    const paramColors: Record<number, string> = {
      0xF7: '#2d8a3e', // 草地绿
      0xFC: '#f0f0f0', // 白线
      0xFE: '#c4a84d', // 浅褐
      0xF6: '#e8e0c8', // 浅灰
    ...
    setTimeout(() => {
      ...
      query.select('#fieldCanvas')
        ...
        .exec((res: any) => {
          const canvas = res?.[0]?.node;
          if (!canvas) { setTimeout(() => that._renderFieldAssembled(), 300); return; }
          canvas.width = cvsW;
          canvas.height = cvsH;
          const ctx = canvas.getContext('2d');
          ...
          ctx.lineWidth = 1;
          ctx.beginPath(); ctx.moveTo(0, 30 * PX); ctx.lineTo(cvsW, 30 * PX); ctx.stroke();
  ...
  // ── Bank 02 调试调用接口 ──
  onDebugModeSwitch(e: any) {
    const mode = e.currentTarget.dataset.mode as 'metatile' | 'a72c' | 'ntxy' | 'dtable' | 'ppubuf' | 'ppufns';
  ...
  onDebugRun() {
    ...
    try {
      if (mode === 'metatile') {
        ...
        // 追加解读
        const defDt: Record<number, string> = {
        ...
        // 触发 NT 版面可视化
        const allRecords: Array<[number, number, number, number]> = step.records;
   ...
   * 连线显示轨迹路径
   */
  _renderA72CNTLayout(records: Array<[number, number, number, number]>) {
    const that = this;
    ...
    const cntColors: Record<string, number> = {};
    ...
    setTimeout(() => {
      ...
      query.select('#a72cNTCanvas')
        ...
        .exec((res: any) => {
          const canvas = res?.[0]?.node;
          if (!canvas) { setTimeout(() => that._renderA72CNTLayout(records), 200); return; }
          canvas.width = cvsW;
          canvas.height = cvsH;
          const ctx = canvas.getContext('2d');
          ...
          for (let tx = 0; tx <= TILES_X; tx += 4) {
            ctx.beginPath();
            ctx.moveTo(tx * PX, 0); ctx.lineTo(tx * PX, cvsH);
            ctx.stroke();
          }
          for (let ty = 0; ty <= TILES_Y; ty += 4) {
            ctx.beginPath();
            ctx.moveTo(0, ty * PX); ctx.lineTo(cvsW, ty * PX);
            ctx.stroke();
          }
          ...
          for (const [rLo, param, _flags, rHi] of records) {
            const { cx, cy } = that._ntToXY(rLo, rHi);
            if (cx < 0 || cx >= TILES_X || cy < 0 || cy >= TILES_Y) continue;
          ...
          wx.canvasToTempFilePath({
            ...
            fail: () => {
              try {
                const url = (canvas as any).toDataURL?.();
                if (url) that.setData({ a72cNTLayoutSrc: url });
              } catch (_e) { /* ignore */ }
            },
          });
        });
  ...
  _parsePpuBuf() {
    ...
    while (i < bytes.length) {
      const ctrl = bytes[i];
      if (ctrl === 0x00) { i++; entryIdx++; continue; }  // 结束标记或填充
  ...
  /** 解码 PPU 地址 → 人类可读目标 */
  _decodePpuAddr(addr: number): { name: string; color: string; desc: string } {
    const a = addr & 0x3FFF;
    ...
    // PPU registers
    if (addr === 0x2000) return { name: '⚙️ PPUCTRL', color: '#f85149', desc: 'PPU 控制寄存器' };
    if (addr === 0x2001) return { name: '⚙️ PPUMASK', color: '#f85149', desc: 'PPU 遮罩寄存器' };
    if (addr === 0x2005) return { name: '📐 PPUSCROLL', color: '#f85149', desc: '滚动寄存器' };
    if (addr === 0x2006) return { name: '📍 PPUADDR', color: '#f85149', desc: 'VRAM 地址寄存器' };
    if (addr === 0x2007) return { name: '✏️ PPUDATA', color: '#f85149', desc: 'VRAM 数据寄存器' };
    // Pattern tables
  ...
  _renderPpuBufDiagram() {
    ...
    setTimeout(() => {
      ...
      query.select('#ppuBufCanvas')
        ...
        .exec((res: any) => {
          const canvas = res?.[0]?.node;
          if (!canvas) { setTimeout(() => that._renderPpuBufDiagram(), 300); return; }
          canvas.width = cvsW;
          canvas.height = cvsH;
          const ctx = canvas.getContext('2d');
          ctx.fillStyle = '#0d1117';
          ...
          for (let ei = 0; ei < entries.length; ei++) {
            ...
            if (ent.dataLen > 0 && ent.dataLen <= 8) {
              const dataBytes = ent.dataHex.split(' ').map((s: string) => parseInt(s, 16));
          ...
          wx.canvasToTempFilePath({
            canvas,
            success: (r: any) => { that.setData({ ppuBufDiagramSrc: r.tempFilePath }); },
            fail: () => {
              try { const url = (canvas as any).toDataURL?.(); if (url) that.setData({ ppuBufDiagramSrc: url }); } catch (_e) {}
            },
          });
        });
    }, 100);
  ...
  _tracePpuBufFn(preset: string) {
    const fns: Record<string, { name: string; addr: string; desc: string; sourceBank: string; dataExplained: string; ppuTarget: string; flow: string[] }> = {
      palette_init: {
    ...
    const fn = fns[preset];
    if (!fn) { this.setData({ ppuBufFnTrace: '未知预设: ' + preset }); return; }
  ...
  // ── Bank 31 调试调用接口 ──
  onB31DebugModeSwitch(e: any) {
    const mode = e.currentTarget.dataset.mode as 'vector' | 'direction' | 'sprite_ptr' | 'zone' | 'near_player' | 'bpm_update';
    this.setData({ b31DebugMode: mode, b31DebugResult: '' });
  },
  onB31DebugInput(e: any) {
    const { field } = e.currentTarget.dataset;
    const value = e.detail.value as string;
  ...
  onB31DebugRun() {
    ...
    try {
      if (mode === 'vector') {
        ...
        result = this._b31CalcZone(
          parseInt(input.x || '80', 16),
          parseInt(input.y || '70', 16),
        );
      } else if (mode === 'near_player') {
        result = this._b31SimNearPlayer(
          parseInt(input.px || '80', 16),
          parseInt(input.py || '70', 16),
          parseInt(input.tx || 'A0', 16),
          parseInt(input.ty || '80', 16),
          parseInt(input.threshold || '07', 16),
        );
      } else if (mode === 'bpm_update') {
        result = this._b31SimBpmUpdate(
          parseInt(input.bpm || '00', 16),
          parseInt(input.valLo || '50', 16),
          parseInt(input.valHi || '02', 16),
        );
      }
    } catch (err: any) {
      result = '错误: ' + err.message;
  ...
  /** 读取中断向量表 */
  _b31ReadVectors(): string {
    const prg31 = NES_PRG_ROM.slice(0x1F * BANK_SIZE, 0x20 * BANK_SIZE);
  ...
  /** 查 8 方向速度向量表 $E6CF */
  _b31LookupDirection(angle: number): string {
    const prg31 = NES_PRG_ROM.slice(0x1F * BANK_SIZE, 0x20 * BANK_SIZE);
  ...
  /** 查精灵动画指针表 $E9DA */
  _b31LookupSpriteAnim(animId: number): string {
    const prg31 = NES_PRG_ROM.slice(0x1F * BANK_SIZE, 0x20 * BANK_SIZE);
    ...
    // 指针表中所有条目
    const allPtrs: {id: number, addr: number, bankAddr: string}[] = [];
    for (let i = 0; i <= 21; i++) {
  ...
  /** 场地 Zone 计算 */
  _b31CalcZone(x: number, y: number): string {
    // 模拟 $E6EC → $E709
  ...
  /** 近距球员筛选模拟 */
  _b31SimNearPlayer(px: number, py: number, tx: number, ty: number, threshold: number): string {
    // 曼哈顿距离: |dx| + |dy|
  ...
  /** BPM 计数器模拟 */
  _b31SimBpmUpdate(bpm: number, valLo: number, valHi: number): string {
  ...
  _renderBank30StructureMap() {
    ...
    query.select('#structCanvas')
      ...
      .exec((res: any) => {
        const canvas = res?.[0]?.node;
        if (!canvas) { setTimeout(() => that._renderBank30StructureMap(), 300); return; }
        const w = 360;
        const h = 180;
        canvas.width = w;
        canvas.height = h;
        ...
        features.forEach(f => {
          ...
          if (fw > 50) ctx.fillText(f.name, fx + 2, featY + 16);
        });
  ...
  _renderBank31StructureMap() {
    const that = this;
    const analysis: any = BANK31_ANALYSIS;
    const query = wx.createSelectorQuery();
    query.select('#structCanvas')
      .fields({ node: true, size: true })
      .exec((res: any) => {
        const canvas = res?.[0]?.node;
        if (!canvas) { setTimeout(() => that._renderBank31StructureMap(), 300); return; }
        const w = 360;
        const h = 160;
        canvas.width = w;
        canvas.height = h;
        ...
        for (const s of sections) {
          ...
          ctx.font = '8px monospace';
          if (sw > 30) ctx.fillText(s.name, sx + 2, detailY + detailH - 5);
        }
        ...
        for (let addr = 0xE000; addr <= 0x10000; addr += 0x800) {
          ...
          ctx.strokeStyle = '#21262d';
          ctx.beginPath(); ctx.moveTo(x, barY); ctx.lineTo(x, detailY + detailH + 4); ctx.stroke();
        }
        ...
        wx.canvasToTempFilePath({
          canvas,
          success: (r: any) => { that.setData({ b31StructureSrc: r.tempFilePath, b31StructureReady: true }); },
          fail: () => {
            try { const url = (canvas as any).toDataURL?.(); if (url) that.setData({ b31StructureSrc: url, b31StructureReady: true }); } catch (_e) {}
          },
        });
      });
  },
  ...
  _getName(type: string, id: number): string {
    if (type === 'CHR') {
