// 临时: dump emu f450 中下部行 ASCII + 关键像素值
const fs = require('fs');
...
const charset = ' .:-=+*#%@';
const { w, h, ct, rgba } = decodePng('output/emu-full/frame-0450/screen.png');
console.log('size', w, 'x', h, 'colorType', ct);
console.log('pixel samples: (0,0)=', [rgba[0], rgba[1], rgba[2]], ' (100,0)=', [rgba[100 * 4], rgba[100 * 4 + 1], rgba[100 * 4 + 2]], ' (40,200)=', [rgba[(200 * w + 40) * 4], rgba[(200 * w + 40) * 4 + 1], rgba[(200 * w + 40) * 4 + 2]]);
for (let y = 130; y < 240; y++) {
  ...
  for (let x = 0; x < 256; x++) {
    ...
    if (lum > 0) nonZero++;
    if (x % 2 === 0) line += charset[Math.min(9, Math.floor((lum / 256) * 10))];
  }
  console.log(line + ' nz=' + nonZero);
}
