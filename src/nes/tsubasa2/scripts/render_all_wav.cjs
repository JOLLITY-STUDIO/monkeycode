...
function writeWav(samples, sampleRate, outPath) {
  ...
  buf.writeUInt16LE(16, 34); buf.write('data', 36); buf.writeUInt32LE(samples.length * 2, 40);
...
for (let songIdx = 0; songIdx < SONG_IDS.length; songIdx++) {
  ...
  for (let i = 0; i < totalFrames; i++) { try { nes.frame(); } catch (e) { break; } }

  writeWav(samples, sampleRate, outFile);
  console.log(`[${songIdx + 1}/${SONG_IDS.length}] song-${String(songIdx + 1).padStart(3, '0')}.wav (ID $${songId.toString(16)}, ${samples.length} 采样)`);
}
