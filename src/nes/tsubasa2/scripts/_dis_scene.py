#!/usr/bin/env python3
"""反汇编 bank02 $A200-$A800 场景入口代码（8KB[2] 高半，通过 $A000 窗口访问）"""
import sys

rom = open('docs/roms/Captain Tsubasa II - Super Striker (Japan).nes', 'rb').read()
prg = rom[16:16 + 0x40000]
# 8KB[2] = ROM 0x4000-0x5FFF，映射 CPU $8000-$9FFF (R6) 与 $A000-$BFFF (R7)
BASE = 0x4000

def cpu_to_rom(a):
    return BASE + (a & 0x1FFF)

def dis(a_start, a_end):
    i = cpu_to_rom(a_start)
    end = cpu_to_rom(a_end)
    out = []
    while i < end:
        addr = 0x8000 + (i - BASE)
        b = prg[i]
        if b == 0x00:
            ...
            out.append(f'{addr:04X}: NOP abs ${prg[i+1]:02X}{prg[i+2]:02X}'); i += 3
        elif b == 0x0D:
            ...
            t = addr + 2 + rel
            out.append(f'{addr:04X}: BPL ${t:04X}'); i += 2
        elif b == 0x11:
            ...
            rel = prg[i+1] if prg[i+1] < 0x80 else prg[i+1] - 0x100
            t = addr + 2 + rel
            out.append(f'{addr:04X}: BMI ${t:04X}'); i += 2
        elif b == 0x31:
            ...
            rel = prg[i+1] if prg[i+1] < 0x80 else prg[i+1] - 0x100
            t = addr + 2 + rel
            out.append(f'{addr:04X}: BVS ${t:04X}'); i += 2
        elif b == 0x75:
            ...
            rel = prg[i+1] if prg[i+1] < 0x80 else prg[i+1] - 0x100
            t = addr + 2 + rel
            out.append(f'{addr:04X}: BCC ${t:04X}'); i += 2
        elif b == 0x91:
            ...
            rel = prg[i+1] if prg[i+1] < 0x80 else prg[i+1] - 0x100
            t = addr + 2 + rel
            out.append(f'{addr:04X}: BCS ${t:04X}'); i += 2
        elif b == 0xB1:
            ...
            rel = prg[i+1] if prg[i+1] < 0x80 else prg[i+1] - 0x100
            t = addr + 2 + rel
            out.append(f'{addr:04X}: BNE ${t:04X}'); i += 2
        elif b == 0xD1:
            ...
            rel = prg[i+1] if prg[i+1] < 0x80 else prg[i+1] - 0x100
            t = addr + 2 + rel
            out.append(f'{addr:04X}: BEQ ${t:04X}'); i += 2
        elif b == 0xF1:
