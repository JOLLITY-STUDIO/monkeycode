// 从 ROM 提取 bank12 完整 8192B 数据 + 结构化表, 生成 src/game/prg/data/audio/bank12-data.ts
const fs = require('fs');
...
const prgStart = 0x10;
const BANK = 12;
const bankOff = prgStart + BANK * 0x2000;

function cpuToOff(cpuAddr) { return bankOff + (cpuAddr - 0x8000); }

const data = [];
for (let i = 0; i < 0x2000; i++) data.push(buf[bankOff + i]);
...
function fmtBytes(arr) {
  ...
  for (let i = 0; i < arr.length; i += 16) {
    ...
    lines.push('  ' + hex + (chunk.length === 16 ? ',' : ''));
  }
  return lines.join('\n');
}
 ...
 */
export const BANK12_DATA: readonly number[] = [
${fmtBytes(data)}
];
 ...
 */
export const JUMP_TABLE_8269: readonly number[] = [
${fmtBytes(data.slice(0x269, 0x269 + 16))}
];
 ...
 */
export const JUMP_TABLE_82E4: readonly number[] = [
${fmtBytes(data.slice(0x2E4, 0x2E4 + 16))}
];
 ...
 */
export const DISPATCH_TABLE_84DA: readonly number[] = [
${fmtBytes(data.slice(0x4DA, 0x4DA + 64))}
];
 ...
 */
export const FREQ_TABLE_870D: readonly number[] = [
${fmtBytes(data.slice(0x70D, 0x70D + 24))}
];
 ...
 * 索引 0x30+ 是后续数据重叠, 不用。
 */
export const DUR_TABLE_8725: readonly number[] = [
${fmtBytes(data.slice(0x725, 0x725 + 0x30))}
];
 ...
 */
export const ENV_PTR_TABLE_8754: readonly number[] = [
${fmtBytes(data.slice(0x754, 0x754 + 0x10 * 2))}
];
 ...
 */
export const SE_PTR_TABLE_8BDA: readonly number[] = [
${fmtBytes(data.slice(0xBDA, 0xBDA + 0x6C))}
];
...
const outPath = path.join(__dirname, 'src', 'game', 'prg', 'data', 'audio', 'bank12-data.ts');
fs.writeFileSync(outPath, parts.join('\n'));
console.log('written:', outPath, 'bytes:', data.length);
