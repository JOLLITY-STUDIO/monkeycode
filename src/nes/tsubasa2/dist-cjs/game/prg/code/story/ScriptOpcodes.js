...
exports.OPCODE_HANDLERS = exports.OPCODE_INDIRECT_BASE = exports.OPCODE_DIRECT_MAX = exports.ScriptOpcode = void 0;
exports.classifyOpcode = classifyOpcode;
exports.setScriptRuntime = setScriptRuntime;
...
exports.OPCODE_DIRECT_MAX = 0x6c;
/** 间接模式起点（与 asm CMP #$6D + SEC SBC #$6D 一致） */
exports.OPCODE_INDIRECT_BASE = 0x6d;
/**
 * 计算 opcode 模式（直接 / 间接）并返回归一化索引。
 */
function classifyOpcode(opcode) {
    if (opcode <= exports.OPCODE_DIRECT_MAX)
        return { direct: true, index: opcode };
    return { direct: false, index: opcode - exports.OPCODE_INDIRECT_BASE };
}
/** 全局运行时（注入式） */
let RUNTIME = null;
/** 注册运行时（StoryController / Story 服务启动时调用一次） */
function setScriptRuntime(rt) {
    RUNTIME = rt;
}
/** opcode → handler 注册表 */
...
function registerHandler(opcode, handler) {
    exports.OPCODE_HANDLERS[opcode] = handler;
}
/** 取 opcode handler */
function getHandler(opcode) {
    return exports.OPCODE_HANDLERS[opcode] ?? null;
}
