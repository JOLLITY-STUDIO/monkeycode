...
export const MATCH_CONFIG_PTR_TABLE: readonly number[] = [
...
export const RIO_CUP_CONFIG_BYTES: readonly number[][] = [
 ...
 */
export function getMatchConfig(matchIdx: number): readonly number[] {
  if (matchIdx < 0 || matchIdx >= RIO_CUP_CONFIG_BYTES.length) return [];
  return RIO_CUP_CONFIG_BYTES[matchIdx];
}
