 ...
 */
export const PALETTE_FADE_MAX = 15;

/** 渐显：fade 0-15，0=全黑 15=原色 */
export function fadePalette(pal: number, fade: number): number {
  const f = fade & 0x0f;
  const p = pal & 0x3f;
  if (f >= PALETTE_FADE_MAX) return p;
  // TODO V0.2: 用真实 64 项查找表替换
  if (f === 0) return 0x0f; // 全暗 = 黑
  // 中间档按"索引所在色组 + fade"近似（后续用真表覆盖）
  const group = (p >> 4) & 0x03; // 0=灰组 1/2/3=色组
  const steps = Math.floor((f * 4) / PALETTE_FADE_MAX);
