...
import { PLAYER_TABLE, PLAYER_COLOR_TABLE, PLAYER_HAIR_TABLE } from './player-stats';
import { BANK19_SPRITE_FRAMES } from './sprite-frame-table';
...
const HAIR_TEMPLATES: ReadonlyArray<number> = PLAYER_HAIR_TABLE;

/** PLAYER_COLOR_TABLE 查表: 球员色 by ID */
const COLOR_BY_ID = new Map<number, { skin: number; hair: number; shirt: number; shorts: number }>();
for (const c of PLAYER_COLOR_TABLE) COLOR_BY_ID.set(c.id, c);

/** PALETTE_TABLE 中"球员 sprite 调色板"组选择: shirt 高 4 位决定组偏移 */
function shirtToPaletteGroup(shirt: number): number {
  // shirt 字节高 4 位用作 sprite palette 调色板组号 (0..7)
  return (shirt >> 4) & 0x07;
}

/** 球员 ID → hair 模板索引 */
function hairTemplateIdFor(playerIndex: number): number {
  return HAIR_TEMPLATES[playerIndex % HAIR_TEMPLATES.length] ?? 0;
}

/** 球员 ID → body 基础 tile 索引 (frame 0x00 = Tsubasa 静态) */
function bodyBaseTileFor(playerIndex: number): number {
 ...
 */
export const PLAYER_TILE_TABLE: ReadonlyArray<PlayerTileEntry> = PLAYER_TABLE.map((p, i) => {
...
/** 按球员 ID 查 tile 桥接 */
export function findPlayerTilesById(playerId: number): PlayerTileEntry | null {
