 ...
 *
 * 9 路入口跳�?
 *   entry0 $A01E 球员数据处理 (查能力�?建立阵容)
 *   entry1 $A10D 数据/选项屏幕初始�? *   entry2 $A4EB PPU 图形数据显示   (TODO)
 *   entry3 $A64C NT 屏幕内容绘制    (TODO)
 *   entry4 $A6D2 PPU 属性块写入     (TODO)
 *   entry5 $AFC2 字符数据解码/显示  (TODO)
 *   entry6 $AF79 VRAM 缓冲区写�?1
 *   entry7 $AF8A VRAM 缓冲区写�?2
 *   entry8 $B050 Bank 切换 + 数据加载
 *   entry9 $A39B 球队数据初始�? *
 * 数值显示链�? ROM 编码�?�?LOOKUP_16BIT 查表 �?真实数�? * �?$8C55 循环�?0 (16bit除法) �?余数+0x33=tile_id �?�?ram_04A8 PPU Buffer。bank01 不只是查球员，它负责整个"球员数据查询+数值显�?阵容菜单"界面�? */
...
export class PlayerQueryService {
  ...
  /**
   * 球员数据查询 (entry0 $A01E)。按 playerId 在阵容区查能力�?建立阵容数据�?   *
   * 对应原始 $A01E: 读取 $0448/$0026/$0446/$044D/$00E1 组合出队�?
   * 遍历 10 名球员建�?$0656 区阵�? 计算球员实际能力 ($0454 经验�?�?   *
   * 参数: playerId = 球员索引; field = 能力字段 (0=体力/其余查能力表)�?   * 返回: 该字段查表后的真实数值�?   */
  queryPlayer(playerId: number, field: number): number {
    // $A01E: LDA $0448; LSR; LDA $0026; ROL ... 队形标记组合
    // 这里按字段类型走查表链路�?    const enc = this._readPlayerStat(playerId, field);
    // 体力字段 (16bit) �?STAMINA_TABLE_16BIT; 其他字段用能力表�?    if (field === 0) {
      return this._lookupValue16(enc);
    }
    return enc;
  }

  /** 读阵容区 $0656+ 球员原始编码 (与原 $A438 段语义对�? �?code_main.s) */
  protected _readPlayerStat(playerId: number, field: number): number {
    // 阵容数据�?$0656 �? 每名球员 2 字节编码 (参照 $A438 读取逻辑)�?    const base = 0x0656 + playerId * 2;
    const v = this._store.read(`ram_${base.toString(16).toUpperCase().padStart(4, '0')}`);
    switch (field & 0x03) {
      case 0: // 高位能力 (A �?
        return (v >> 6) & 0x03;
      case 1: // 体力/组合�?        return (v >> 4) & 0x0f;
      case 2: // 中位
        return (v >> 2) & 0x0f;
      default: // 低位
        return v & 0x03;
  ...
  // ============================================================
  // entry1 数据/选项屏幕初始�?(�?$A10D)
  // ============================================================

  /**
   * 选项屏幕初始�?(entry1 $A10D)�?   *
   * 对应原始 $A10D: 清空 $0566-$0656 �? 建立阵容显示指针,
   * 循环调用 $88CA 字符显示写标�? 载入队伍数据, 进入选项菜单状态机�?   */
  initOptionScreen(): void {
    // $A10D: JSR $9BA0 (清屏, �?bank)
    // $8110: 清空 $0566-$0656
    for (let i = 0x0566; i < 0x0656; i++) {
      this._store.write(`ram_${i.toString(16).toUpperCase().padStart(4, '0')}`, 0);
    }
    // $811A: 设置滚动/显存控制 (�?$9B6F/$9B74)
    // $812A: 初始化文本光�?$008E/$008F
    this._store.write('ram_008E', 0);
    this._store.write('ram_008F', 0x2e);
    // $8136: JSR $8920 初始�?PPU 缓冲
    this._ppuBufAlloc();
    // $813B-$817D: 标题字符循环 (�?ram_04A8 PPU Buffer)
    this._drawTitleChars();
    // $817D: 拷贝 $B205 起始数据�?$0460 (�?bank 数据载入)
    // $818F: JSR $997A 清背�?    // $8196: 设置 PPU 写入控制 $004C / 状�?$0700
  ...
  /** 标题字符循环 (�?$813B-$817B, 5 �?× 13 字符) */
  protected _drawTitleChars(): void {
    const e6 = 0x6ebc; // 字符源指�?(�?$00E6/$00E7 = $BC6E)
    let col = 0; // $00EC
    let row = 5; // $00EB
    let dst = 0x21c4; // $00E8/$00E9 = $21C4
    while (row > 0) {
      let n = 0x0d; // $00ED 每行字符�?      while (n > 0) {
  ...
  protected _optionScreenUpdate(): void {
    // $A1A6: LDA #$01; JSR $9FA8 (等待�?; JSR $A3D0 (光标)
    // $A1AE: BIT $001E 检查按�?..
    // 状态机整体尚未完成翻译, 此处保留入口供后续逐段覆盖�?    this._optionScreenPoll();
  }

  /**
   * 按键轮询 (�?$A1AE-$A3D0 按键状态机)�?   * asm $81AE: BIT $001E; BPL→JMP $A260; BVC→JMP $A231;
   *   AND #$20; BEQ; JMP $A252; AND #$10; BEQ; JMP $A26C;
   *   AND #$0F; BEQ $81A6 (无按键→回循�?;
   *   LDY #$14; STY $00EA; LDX $00EC; LDA $B1E8,X; JSR $A4D8;
   *   �?$001C AND #$0F �?TAX; LDA $B2ED,X; 按方向调整光�?
   *   JMP $A201 (光标更新)
   */
  protected _optionScreenPoll(): void {
    const input = this._store.read('ram_001E');
    const input1C = this._store.read('ram_001C');
    // $81AE: BIT $001E; BPL $81B5 (bit7=A�?
    if ((input & 0x80) !== 0) {
      // JMP $A260 �?A键确�?      this._confirmSelection();
      return;
    }
    // $81B2: BVC $81BA (bit6=B�?
    if ((input & 0x40) === 0) {
      // JMP $A231 �?B键取�?      this._cancelSelection();
      return;
    }
    // $81BA: AND #$20 (Select)
    if ((input & 0x20) !== 0) {
  ...
  protected _moveCursor(dir: number): void {
    ...
    // LDA $B2ED,X �?方向偏移�?    let offset = 0; // ROM stub
  ...
  // ============================================================
  // 数值显示链�? LOOKUP_16BIT 查表 (�?$B045/$B02E)
  // ============================================================

  /**
   * �?LOOKUP_16BIT 表取数�?(�?$B045):
   *   ASL; TAX; LDA $BA90,X; TAY; LDA $BA91,X �?16bit(lo/hi)�?   * 表以 16bit 小端�? 字节偏移 = index*2, 故数组按 index 直接取�?   */
  ...
  /**
   * �?$0454 经验值区 16bit (�?$B016):
   *   LDA $0026 �?查表后索�? �?$0454+X 16bit; 返回 Y=lo/X=hi�?   */
  protected _query16(idx: number): number {
    const x = (idx & 0x0f) * 2;
    return this._store.read16(`ram_${(0x0454 + x).toString(16).toUpperCase().padStart(4, '0')}`);
  }

  /** 提取能力字段 (�?$8464, 阵容 16bit 编码拆位) */
  protected _extractStatField(v: number): number {
    return v & 0x3f;
  }

  // ============================================================
  // PPU 缓冲 / 字符显示辅助 (�?$997A/$8920/$88CA �?
  // ============================================================

  /**
   * PPU 缓冲分配 (�?$997A, bank00 �?bank 调用)�?   * asm $997A: �?ram_04A8 PPU Buffer �? 分配写入槽�?   * 复用 Bank00Service.ppuBufAlloc 语义�?   */
  protected _ppuBufAlloc(): void {
    // bank00 $997A = PPU buffer 分配, H5 版由 Bank00Service 管理
    // 此处�?bank01 侧入�? 实际由外部帧合成器消�?  }

  /**
   * PPU 缓冲结束 (�?$997E, bank00 �?bank 调用)�?   * asm $997E: 写终止符�?PPU Buffer�?   */
  protected _ppuBufEnd(): void {
    // bank00 $997E = PPU buffer 结束标记
  }

  /**
   * 字符显示 (�?$88CA, �?tile 字符映射)�?   * asm $88CA: �?CHAR_MAP_DOUBLE �? �?2 �?tile �?PPU buffer�?   * 每个 8×16 字符 = 2 �?8×8 tile (�?�?�?   */
  protected _charDisplay(ch: number, dst: number, bank: number): void {
    // bank00 $88CA: 查字符映射表, 写双 tile �?ram_04A8
    // H5 �? �?Bank00Service.writeNTByte �?NT
    // 简�? 直接�?tile 值到 NT
    void bank;
  ...
  /**
   * PPU 块填�?(�?$9895, bank00 �?bank 调用)�?   * asm $9895: �?tile 填充 NT 区域 (dst, count)�?   */
  protected _ppuBlockFill(tile: number, dst: number, count: number): void {
    // bank00 $9895 = NT 块填�?    // H5 �? 简化为�?NT �?    for (let i = 0; i < count; i++) {
      void (dst + i);
      void tile;
    }
  }
  ...
  /**
   * entry2 PPU 图形数据显示 (�?$A4EB = $84EB)�?   * asm $84EB: LDX #$6A; LDY #$6B; JSR $9B6F (设滚�?;
   *   LDX #$7A; LDY #$7B; JSR $9B74; JSR $9B7F (清精�?;
   *   LDY #$05; LDX #$B3; JSR $B0C0 (设CHR bank);
   *   �?$0044/$0045; 拷贝 $B271+Y �?$039C+Y (Y=$CC-$FF);
   *   �?$BCD1 �?(队伍→阵�?, �?$BCF3/$BD64 指针;
   *   JSR $9D27 (写队�?; JSR $9D50 (写阵型名);
   *   JSR $A63C (写数字到 NT); �?$00E8/$00E9 PPU 地址;
   *   循环写球员数据到屏幕
   */
  entry2_PpuGraphics(): void {
    // $84EB: LDX #$6A; LDY #$6B; JSR $9B6F (设滚�? bank00)
    this._store.write('ram_006A', 0);
    this._store.write('ram_006B', 0);
    // $84F9: JSR $9B7F (清精�? bank00)
    // $84FC: LDY #$05; LDX #$B3; JSR $B0C0 (CHR bank 切换)
    this._store.write('ram_0044', 0);
    this._store.write('ram_0045', 0);
    // $8509: 拷贝 $B271+Y �?$039C+Y (Y=$CC�? 循环�?Y=0)
    ...
    // $8532: LDY $BD64,X; LDA $BD65,X; TAX (查阵型指�?
    // $8539: LDA #$07; STA $00E8; LDA #$22; STA $00E9 (PPU 地址 $2207)
    this._store.write('ram_00E8', 0x07);
    this._store.write('ram_00E9', 0x22);
    // $8541: JSR $9D50 (写阵型名�?NT)
    // $8544: LDA $002A; LDY #$D0; LDX #$21; JSR $A63C (写数�?
    void hi; void lo;
  }

  /**
   * entry3 NT 屏幕内容绘制 (�?$A64C = $864C)�?   * asm $864C: JSR $98A0 (清NT); JSR $9B7F (清精�?;
  ...
  entry3_ScreenDraw(): void {
    ...
    // $864F: JSR $9B7F (清精�? bank00)
    // $8652: LDX $0026; LDA $B393,X (查队伍标�?
    const teamId = this._store.read('ram_0026');
    void teamId;
    // $8657: JSR $8464; JSR $82A9 (设文本指�?
  ...
  /**
   * entry4 PPU 属性块写入 (�?$A6D2 = $86D2)�?   * asm $86D2: LDA #$55; STA $0700; JSR $98A0 (清NT);
   *   JSR $9B7F (清精�?; LDX $0026; LDA $B3B5,X; JSR $8464;
   *   JMP $A6F9 (后续绘制)
   */
  entry4_AttrBlock(): void {
    ...
    // $86DA: JSR $9B7F (清精�? bank00)
    // $86DD: LDX $0026; LDA $B3B5,X; JSR $8464 (设文�?
    const teamId = this._store.read('ram_0026');
    void teamId;
  ...
  /**
   * entry5 字符数据解码/显示 (�?$AFC2 = $8FC2)�?   * asm $8FC2: STX $00EC; JSR $B023; STA $00EB;
   *   AND #$F0; LSR; CLC; ADC $00EC; TAX;
   *   LDA $BA1C,X; TAX (查表);
   *   LDA $0026; ASL; TAY; LDA $BA4D,Y; STA $00ED;
   *   LDA $BA4C,Y; ROR $00ED; LSR; ROR $00ED (16�?0位变�?;
   *   循环写球员数�?   */
  entry5_CharDecode(): void {
    ...
    const eb = 0; // stub: $B023 返回�?    this._store.write('ram_00EB', eb);
  ...
  /**
   * entry6 VRAM 缓冲区写�?1 (�?$AF79 = $8F79)�?   * asm $8F79: LDA $0026; ASL; TAX; LDA $BA4C,X; STA $00E6;
  ...
  entry6_VramBuf1(): void {
    ...
    // �?PLAYER_STAT_TABLE_16BIT (�?$BA4C, 16bit LE) 设数据指�?    const statIdx = x >> 1;  // X 是字节偏�? 表项索引 = X/2
  ...
  /**
   * entry7 VRAM 缓冲区写�?2 (�?$AF8A = $8F8A)�?   * asm $8F8A: LDA $0026; ASL; TAX; LDA $BA4C,X; STA $00E6;
  ...
  entry7_VramBuf2(): void {
    ...
    // �?PLAYER_STAT_TABLE_16BIT (�?$BA4C) �?16bit 指针, �?÷4
    const statIdx = x >> 1;
    const ptr = statIdx < PLAYER_STAT_TABLE_16BIT.length
      ? PLAYER_STAT_TABLE_16BIT[statIdx]
      : 0;
    let lo = ptr & 0xFF;
    let hi = (ptr >> 8) & 0xFF;
    // LSR hi; ROR lo; LSR hi; ROR lo (16�?÷4)
    for (let i = 0; i < 2; i++) {
      lo = ((lo >> 1) | ((hi & 1) << 7)) & 0xFF;
      hi = hi >> 1;
    }
    ...
    //   LDA #$FF; STA $0454,X; STA $0455,X (溢出�?$FF)
  ...
  /**
   * entry8 Bank 切换 + 数据加载 (�?$B050 = $9050)�?   * asm $9050: LDA $0026; CMP #$10; BEQ $906C;
  ...
  // ════════════════════════════════════════════════════════════
  // bank01 $81E0-$8466 �?�?按键状态机主体 + 球员数据循环
  // ════════════════════════════════════════════════════════════

  /**
   * $81E0: 方向键移动光标主体�?   * asm $81E0-$8216:
   *   LDA $001C; AND #$0F; TAX; LDA $B2ED,X (查方向偏移表)
   *   BMI $81F7 (负值跳)
   *   CLC; ADC $00EC (加光标位�?; CMP #$41; BCC $8201 (<$41 ok)
   *   SEC; SBC #$41 (�?41 则减 $41, 环绕); JMP $A201
   *   $81F7: 负值路�? CLC; ADC $00EC; CMP #$41; BCC $8201
   *          CLC; ADC #$41 (�?41 �?$41); $8201: STA $00EC
  ...
  protected sub81E0(): void {
    ...
    const offset = 0; // ROM $B2ED �?stub
    const ec = this._store.read('ram_00EC');
    let newPos: number;
    if ((offset & 0x80) !== 0) {
      // 负值路�?      newPos = ec + offset;
      if (newPos >= 0x41) newPos = newPos + 0x41;
    } else {
      // 正值路�?      newPos = ec + offset;
      if (newPos >= 0x41) newPos = newPos - 0x41;
    }
    this._store.write('ram_00EC', newPos & 0xFF);
    // $8203: 查菜单项�?$B1E8
    // JSR $A4D8 (菜单项显�?; JSR $9FA8 (等待); JSR $A3D0 (精灵设置)
    this.subA3D0();
    // 循环等待按键释放
    let ea = 0x01;
    while (ea > 0) {
  ...
  /**
   * $8231: A键确�?(二级菜单选择)�?   * asm $8231-$826C:
   *   LDX $00EC; LDA $B255,X; CMP #$FF; BEQ $826C (=FF�?
   *   TXA; LDY $00ED; STA $0664,Y (存选择�?$0664)
  ...
  protected sub8231(): void {
    const ec = this._store.read('ram_00EC');
    const v: number = 0; // ROM $B255 �?stub
    if (v === 0xFF) {
      this.sub826C();
      return;
    }
  ...
  /**
   * $826C: 三级菜单 (球员选择确认)�?   * asm $826C-$82B0:
  ...
  /**
   * $82BB: 确认选择 (非匹配路�?�?   * asm: LDA #$F8; STA $0558; STA $055C; LDA #$43; STA $0700;
  ...
  protected sub82BB(): void {
    ...
    // LDA #$78; JSR $9FA8 (等待 120 �?
    this._store.write('ram_007E', 0x00);
    this._store.write('ram_0700', 0x33);
  }

  /**
   * $82DD: 球员经验�?等级计算�?   * asm $82DD-$836B:
   *   LDA #$00; STA $00E6; TAX
   ...
   *   BEQ $8338 (�?); ASL $00EA; ROL $00EB; CMP #$02; BEQ $832D
  ...
  // ════════════════════════════════════════════════════════════
  // bank01 $83D0-$8466 �?�?精灵设置 + 球员位置计算
  // ════════════════════════════════════════════════════════════

  /**
   * $A3D0: 精灵设置 (光标 Y 坐标)�?   * asm $83D0-$8401:
   *   LDA $003A; AND #$04; BEQ $83F9 (bit2=0 �?
  ...
  /**
   * $8402: 球员位置计算 (遍历 11 球员, 累加位置偏移)�?   * asm $8402-$8466:
   *   LDA $0661; AND #$F0; CLC; ADC $0663; STA $00EC; LDA #$00; ADC #$00; STA $00ED
   *   LDX #$00; 循环 11 �? LDA $0656,X; CLC; ADC $00EC; STA $00EC; LDA $00ED; ADC #$00; STA $00ED; INX
   *   CPX #$0B; BNE $8415
  ...
  // ════════════════════════════════════════════════════════════
  // bank01 $8467-$86EA �?�?数值显�?+ entry2 后半�?  // ════════════════════════════════════════════════════════════

  /**
   * $8464: 读球员数�?(按索�?偏移�?$0656/$0657 �? 返回 A)�?   * asm $8464-$8473: LDY $AD8A,X; TXA; AND #$03; BEQ $846E
  ...
  /**
   * $8474: 写球员数�?(�?$8464 对应的写入版�?�?   * asm $8474-$84D7: AND #$3F; STA $00EC; LDY $AD8A,X; TXA; AND #$03; 分支
   */
  ...
  /**
   * $84D8: PPU 地址设置 + �?$9895 块填充�?   * asm $84D8-$84E8: STY $00E8; AND #$3F; CLC; ADC #$D8; TAY;
  ...
  protected sub84D8(a: number, y: number): void {
    ...
    // LDX #$23; JMP $9895 (bank00 块填�?
    void newA;
  }

  /**
   * $8509-$8610: entry2 后半�?(球员数据写入 NT)�?   * asm: 拷贝 $B271+Y �?$039C+Y (Y=$CC-$FF);
   *   �?$BCD1 �?(队伍→阵�?, �?$BCF3/$BD64 指针;
   *   JSR $9D27 (写队�?; JSR $9D50 (写阵型名);
  ...
  /**
   * $A611: 球员数据初始化循环�?   * asm $8611-$863B: STA $00EB; LDA $00ED; JSR $C50C;
  ...
  /**
   * $A63C: 数字写入 NT (�?$BDA8 �?�?   * asm $863C-$8649: STY $00E8; STX $00E9; ASL; TAX;
  ...
  // ════════════════════════════════════════════════════════════
  // bank01 $86EB-$8921 �?�?entry3/entry4 子程 + 属性块绘制
  // ════════════════════════════════════════════════════════════

  /**
   * $A6E8: entry3 后续 (清屏 + 查队伍表 + 写文�?�?   * asm $86E8-$870D: JSR $98A0; JSR $9B7F; LDX $0026; LDA $B3B5,X;
  ...
  /**
   * $A721: 子菜单绘制�?   * asm $8721-$8723: JSR $9BA0; RTS (实际代码�?$8724 �?
   * $8724: LDX #$1F; LDY #$2E; JSR $9B6F (设滚�?;
  ...
  /**
   * $A779: 子菜单精灵设置�?   * asm $8779-$878D: LDX #$B6; LDY #$AA; JSR $97AB; ... (省略详细)
   */
  ...
  /**
   * $8920: PPU buffer 分配 (bank01 侧入�?�?   * asm $8920-$8953: �?ram_008E (buffer 状�?; �?ram_04A8 �?
   *   �?ram_00E8/$00E9 PPU 地址; �?ram_008F;
   *   循环填充 ram_04A8 (Y 计数)
   */
  protected sub8920(): void {
    const bufState = this._store.read('ram_008E');
    if (bufState !== 0) return;
    // �?PPU buffer �?    this._store.write('ram_008E', 0x2E);
    // 循环填充 ram_04A8 (stub)
  }

  // ════════════════════════════════════════════════════════════
  // bank01 $8922-$8C56 �?�?子菜单绘�?+ 精灵设置 + 按键处理
  // ════════════════════════════════════════════════════════════

  /**
   * $A7CE: 子菜单绘�?(含按键循�?�?   * asm $87C5-$884B:
   *   LDA #$58; STA $0564; LDA #$94; STA $004C
   *   LDX #$AD; JSR $9C3A; LDA #$01; JSR $9FA8 (等待1�?
   *   JSR $9CC9 (按键检�?; BIT $001E; BVS $883C (B键取�?
   *   BPL $87D5 (A键确�?; LDA #$01; STA $0562; JSR $9CD3
  ...
  /**
   * $A84E: 子菜�?(阵型=2 路径)�?   * asm $884E-$889A: LDY #$51; LDX #$B4; JSR $B0C0;
  ...
  // ════════════════════════════════════════════════════════════
  // bank01 $8C55-$8F90 �?�?数值显示链路核�?+ 字符显示
  // ════════════════════════════════════════════════════════════

  /**
   * $8C55: 数据流解�?(�?$BB2E �? �?$005C/$005D 指针, 循环读数�?�?   * asm $8C2E-$8C5E:
   *   JSR $997E (PPU buffer 结束); LDA #$01; JSR $9FA8 (等待1�?
   *   BIT $001E; BVC $8C3D (B键跳); JMP $AAE5
   *   $8C3D: BPL $8C31 (A键确�?; LDY #$00; LDA ($0034),Y
   ...
   *     BPL $8C83 (正数�?; CMP #$FF; BNE $8C6D; JMP $AD23 (FF=结束)
  ...
  /**
   * $88CA: 字符显示 (�?tile 映射)�?   * asm $88CA-$891F: �?CHAR_MAP_DOUBLE �? �?2 �?tile �?PPU buffer�?   *   LDA $B8A8,Y (查字符表); STA $00E6; INY; LDA $B8A9,Y; STA $00E7
   *   LDY #$00; LDA ($00E6),Y (�?tile 高字�?; STA $003A; INY
   *   LDA ($00E6),Y (�?tile 低字�?; STA $003B
   *   �?PPU buffer (ram_04A8 �?
   */
  protected sub88CA(): void {
    // �?CHAR_MAP_DOUBLE �?(bank01 ROM $B8A8)
    // �?$00E6/$00E7 指针
    // �?2 �?tile �?ram_04A8
    // H5 �? 简�? �?_charDisplay 处理
  }

  // ════════════════════════════════════════════════════════════
  // bank01 $8F91-$91E7 �?�?entry5/6/7/8 后半�?  // ════════════════════════════════════════════════════════════

  /**
   * $8F91: entry7 VRAM 缓冲写入 2 后半�?(16位指针�? + 经验值循环加偏移)�?   * asm $8F91-$8FC1:
  ...
  protected sub8F91(): void {
    ...
    // LDA $BA4D,X; LSR; ROR $00E6; LSR; ROR $00E6; STA $00E7 (16位�?)
    // X 来自 entry7 �?teamId<<1, �?PLAYER_STAT_TABLE_16BIT 高字�?    const teamId = this._store.read('ram_0026');
  ...
  /**
   * $8FC2: entry5 字符解码后半�?(�?$BA1C �?+ 经验值加偏移)�?   * asm $8FC2-$9012:
   *   STX $00EC; JSR $B023; STA $00EB; AND #$F0; LSR; CLC; ADC $00EC; TAX
   *   LDA $BA1C,X; TAX
   *   LDA $0026; ASL; TAY; LDA $BA4D,Y; STA $00ED;
   *   LDA $BA4C,Y; ROR $00ED; LSR; ROR $00ED; LSR (16位�?)
   *   JSR $9DEE
  ...
  protected sub8FC2(): void {
    ...
    // �?PLAYER_STAT_TABLE_16BIT (�?$BA4C) �?16bit, �?÷4
    const statIdx = y >> 1;
    const ptr = statIdx < PLAYER_STAT_TABLE_16BIT.length
      ? PLAYER_STAT_TABLE_16BIT[statIdx]
      : 0;
    let edLo = ptr & 0xFF;
    let edHi = (ptr >> 8) & 0xFF;
    // ROR $00ED; LSR hi; ROR lo; LSR hi (16�?÷4)
    for (let i = 0; i < 2; i++) {
  ...
  /**
   * $9013: 查经验值表 (返回 Y:X = 16位经验�?�?   * asm $9013-$9022:
  ...
  /**
   * $9023: 查队伍偏移表 (返回 A = $B9D6,X)�?   * asm $9023-$902D:
  ...
  /**
   * $902E: 二分查找 (�?$BA90 16位表)�?   * asm $902E-$9044:
  ...
  /**
   * $9045: �?$BA90 �?(反向, 返回 Y:X)�?   * asm $9045-$904F:
  ...
  /**
   * $90A0: entry8 其他队伍处理�?   * asm $90A0-$90BF:
   *   LDX $0027; BEQ $90BF (队伍2=0 �?
   *   LDY #$C8; LDX #$B9; JSR $97B6 (精灵设置)
   *   LDY #$52; LDX #$22; LDA #$01; STA $00E9
   *   LDA $0450; EOR #$FF; CLC; ADC #$37; JSR $9895 (块填�?
   *   $90BF: RTS
   */
  ...
  /**
   * $90C0: 脚本分派�?(�?$B0D7 跳转�?�?   * asm $90C0-$90D4:
  ...
  /**
   * $A3B4: 循环设球员数�?(JSR $C50C; 读球员数�? JSR $B013; JSR $B02E; 写回)
   */
