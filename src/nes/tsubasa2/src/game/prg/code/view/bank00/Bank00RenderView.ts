 ...
 *   view (本类)     → 把状态/数据映射写入 RamStore (NT/OAM/palette)
 *   core (合成器)   → 消费 RamStore 合成像素帧
 */
...
import type { PaletteColor } from '../../../data/model-types';
...
export class Bank00RenderView {
  ...
  /** 加载镜头 0x17 (标题菜单背景) 的 NT/ATTR 到 RamStore */
  ...
  /** 对应 $890C: VRAM 地址/滚动设置 → RamStore scroll 偏移 */
  ...
  /**
   * 对应 $9AB8 (BG) + $9ADA (SPR): 调色板组加载 → RamStore.paletteTable。
  ...
  paletteLoad(bgGrp: number, sprGrp: number): void {
    ...
    // 同步到 RamStore.paletteTable，供 H5 帧合成器消费
   ...
   * H5: 把目标 VRAM 字节映射到 RamStore nt0/nt1 网格:
