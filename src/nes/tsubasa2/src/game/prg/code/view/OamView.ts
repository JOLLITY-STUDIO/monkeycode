 ...
 * OAM View — 精灵桥接层 (View)
 *
 * 把游戏各 bank 写入 RamStore 的影子 OAM 数据解析为 SpriteEntry[]，
 * 供 FrameCompositor 合成渲染。对应 NES NMI 的 OAM DMA 职责。
 *
 * 数据源 (两套影子缓冲并存, 本 View 统一桥接):
 *   1. ram_0468 影子 OAM  (bank02 场景精灵表, 64 槽 × 4B: Y/tile/attr/X)
 *      - 对应 NES $0468-$0567 区, sub_88CE 拷贝到 $0200 硬件 OAM
 *      - attr bit2-3 ≠ 0 → Y 置 $F8 (屏幕外隐藏)
 *   2. store.oam (OamManager, $04A5 3B/槽语义缓冲) — bank30/11/19/24 演出/HUD 精灵
 *
 * 分层职责 (MVC):
 *   service (业务) → 写 ram_0468 KV / store.oam 影子缓冲
 *   view (本类)    → 每帧解析影子缓冲 → RamStore.sprites
 *   core (合成器)  → 消费 RamStore.sprites 画像素
 */
...
export class OamView {
  ...
  emit(): void {
    ...
    const sceneSprites: SpriteEntry[] = [];
    for (let i = 0; i < 64; i++) {
  ...
  /** 比较两段精灵是否一致 (用于尾段校验) */
  private _sameSprites(a: SpriteEntry[], b: SpriteEntry[]): boolean {
