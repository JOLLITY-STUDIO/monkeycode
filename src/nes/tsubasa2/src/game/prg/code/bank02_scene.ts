...
export class Bank02Service {
  ...
  resetEntry(a: number = 0): void {
    trace('Bank02', `resetEntry(a=${a}) $A200/$A21B RESET 首场景入口`);
    // 对应 $821B-$826B: 通用初始化(无论A=0与否都执行)
    this._doCommonInit(a);
    trace('Bank02', 'resetEntry() 通用初始化完成');
  }
  ...
  private _doShared8292(): void {
    ...
    // 真实 ROM: JMP $9EED 后 CPU 进入主循环永不返回 (等 NMI → 推进 → 等下一帧 NMI)。
    // H5: 调 mainLoop(0) 延续控制流。buttons=0 对应真实 ROM 进入主循环时 A=0 (无输入)。
    //     RESET 链到此已完成 (通用初始化 + 场景初始化), 进入主循环是正常控制流。
    //     Tsubasa2.start() 后 RAF 每帧调 interrupt.nmi(buttons) 推进后续帧。
