...
export class Bank22Service {
  ...
  entry_8005(): void {
    ...
    a = (neg0538 + 1) & 0xff;
    if (a & 0x80) s.write(KEY_003F, (s.read(KEY_003F) - 1) & 0xff);
    s.write(KEY_003E, a);
    ...
    if (hiCarry) s.write(KEY_0043, (s.read(KEY_0043) + 1) & 0xff);
    y = (b12v << 1) & 0xff;
    ...
    for (;;) {
      ...
      if ((code & 0x07) === 0) {
        ...
        if (this.fn_80B3(code & 0x07)) break;
      }
    }
  }
   ...
   */
  private fn_80B3(a: number): boolean {
    const s = this._store;
    s.write(KEY_0044, (s.read(KEY_0044) + 1) & 0xff);

    const dst = readB22Dispatch(a & 0xff);
    switch (dst) {
  ...
  entry_80C0(): void {
    ...
    y = (y + 1) & 0xff;
    a = readB22OffX(this._ptrRead(y));
    x = 0;

    // $80D4-$80E8: A = ±偏移 (0049.bit7=1 取负), 加 0040:0041 → 0046:X 高位
    if (s.read(KEY_0049) & 0x80) a = ((~a) + 1) & 0xff;
    if (a & 0x80) x = 0xff;
    ...
    for (;;) {
      ...
      a = readB22OffY((b >> 2) & 0x3f);
      x = 0;

      // $8114-$8128: A = ±偏移 (0049.bit6=1 取负), 加 003E:003F → 0047:高位
      if (s.read(KEY_0049) & 0x40) a = ((~a) + 1) & 0xff;
      if (a & 0x80) x = 0xff;
      ...
      if (hi2 !== 0) {
        ...
        if (y !== 0) {
          ...
          s.write(KEY_0045, cnt);
          if ((cnt & 0x80) === 0) continue; // $815C BPL $810A
          s.write(KEY_0044, y);
          return;
        }
  ...
  private fn_8187(): void {
    ...
    if (v13 !== 0) {
      if (!flagZ) v13 = ((~v13) + 1) & 0xff; // $819B BEQ $81A2 不成立 → 取补
  ...
  private fn_8175(): void {
    const s = this._store;
    let a = s.read(KEY_0546);
    if (a >= 0x0c) a = (a - 0x0c) & 0xff; // $817A BCC; $817C SBC #$0C
    a = (a << 1) & 0xff;                   // $817E ASL
