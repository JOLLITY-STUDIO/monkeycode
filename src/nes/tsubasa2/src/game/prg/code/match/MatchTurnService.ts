/**
 * MatchTurnService �?bank11 回合/滚动/精灵�?($8000-$9FFF, 运行�?$A000-$BFFF)
 * @bank 11
 *
 * 职责:
 *   入口 $800C �?$8083: 水平滚动状态机 (�?$05D4/$05D7/$05D8, 算滚动量, �?$005A, 协程让出)
 *   入口 $8003 �?$8083: 垂直滚动状态机 (类似水平, �?$005A �?NT 行偏�?
 *   入口 $8006 �?$84A1: 精灵组写�?(�?data_tables.s 精灵描述符流�?OAM)
 *   入口 $8009 �?$814C: 脚本处理 (�?$0524 索引, �?$0052 指针, 执行描述符流)
 *   $810C: 取负坐标变正 (X/Y 取补)
 *   $812B: 查滚动表�?NT 偏移 ($8B64+0x64)
 *   $81BC: 精灵组描述符分派 ($81D5 �?+ $832B)
 *   $82FE: 球场坐标→NT tile 索引 (X=$0637-0x50, Y=$0635-0x30)
 *
 * RAM 关键:
 *   $0052/$0053: 脚本/数据指针 (lo/hi)
 *   $0058/$0059: 数据流指�? *   $005A: 当前滚动�?(列偏�?
 *   $005B: 滚动方向标志
 *   $005D: 当前精灵�?id
 *   $0524: 脚本索引 ($FF=�?
 *   $05D4/$05D5: 当前滚动 X/Y (有符�?16 �?
 *   $05D7: 滚动方向�?(bit7=垂直)
 *   $05D8: 目标滚动位置
 *   $05FB: 比赛阶段 ($0B=进攻方向基准)
 *
...
export class MatchTurnService {
  ...
  /** 模拟 6502 X 寄存�?(跨方法传�? �?sub82FE �?sub81CC) */
  protected _regX = 0;
  ...
  //
  // 水平 ($800C):
  //   $800E: LDA #$01; JSR $C515 (协程让出 1 �?
  //   $8011: LDA $05D4 (当前滚动 X �?
  //   $8014: BIT $05D7 (方向�?
  //   $8017: BPL $801E (正方向跳)
  //   $8019: EOR #$FF; CLC; ADC #$11 (负方向取�?0x11)
  //   $801E: AND #$E0 (取高 5 �?列对�?
  //   $8020: CMP $05D8 (与目标比)
  //   $8023: BNE $8028 (不同则继续滚�?
  //   $8025: JMP $800C (相同则循环等下一�?
  //   $8028: STA $05D8 (更新目标)
  //   $802B: JSR $810C (坐标取负变正)
  //   $802E: LDA $003B; AND #$FE; JSR $812B (查滚动表)
  //   $8035: LDA #$E0 (水平基准 $E0)
  //   $8037: BIT $05D7; BPL $803E; LDA #$A0 (垂直�?$A0)
  //   $803E: CLC; ADC $05D4; AND #$E0; LSR; LSR; ORA #$40
  //   $8048: STA $005A (滚动�?= 列号 | $40 标志)
  //   $804A: LDA #$04; PHA (外层循环 4 �?
  //   $804D: LDA #$01; JSR $C515 (每帧让出)
  //   $8052: LDA $0515; BNE $804D (�?$0515 清零)
  //   $8057: LDA #$01; STA $0515 (置滚动进行中)
  //   $805C: LDX #$00; LDA #$02; PHA (内层 2 �?
  //   $8061: LDA $005A; AND #$3F; TAY (取列�?
  //   $8066: LDA ($0058),Y (读数据流)
  //   $8068: LDY $005A; JSR $85C2 (�?NT �?
  //   $806D: INC $005A (下一�?
  //   $806F-$8073: 内层计数
  ...
  private scrollStateMachine(horizontal: boolean): void {
    // $800E/$8085: 协程让出 1 �?    this._system.coroutineYield(1);
    // $8011/$8088: 读当前滚动坐�?    const scrollLo = this.rd(0x05D4);
    const dir = this.rd(0x05D7);
    // $8014/$808B: BIT $05D7 (bit7=方向)
    let aligned: number;
    if ((dir & 0x80) !== 0) {
      // $8019: 负方向取�?      aligned = ((scrollLo ^ 0xFF) + (horizontal ? 0x11 : 0x01)) & 0xFF;
    } else {
      aligned = scrollLo;
    }
    // $801E/$8095: AND #$E0 (列对�?
    ...
    // $804A-$807E: 外层 4 �?/ 内层 2 列滚动循�?    for (let outer = 0; outer < 4; outer++) {
      // $804D: 每帧让出
      ...
      // $8057: 置滚动进行中
      this.wr(0x0515, 0x01);
      // $805C-$8073: 内层 2 �?      for (let inner = 0; inner < 2; inner++) {
  ...
  // ════════════════════════════════════════════════
  // $812B: 查滚动表�?NT tile 偏移
  // asm: TAY; LDA ($005B),Y; JSR $86D3
  //   LDX #$00; STX $0058; LSR; ROR $0058 (×3 右移=�?)
  //   LSR; ROR $0058; LSR; ROR $0058; TAX
  //   LDA $0058; CLC; ADC #$64; STA $0058 (偏移 +0x64)
  //   TXA; ADC #$8B; STA $0059 (高字�?$8B �?bank11 数据�?$8B64)
  // ════════════════════════════════════════════════
  ...
  // $814C �?脚本处理入口
  // asm $814C:
  //   $814E: LDA #$F6; STA $0052; LDA #$87; STA $0053 (指针=$87F6 脚本�?
  //   $8154: LDA $0524 (脚本索引)
  //   $8157: CMP #$FF; BEQ $819C ($FF=无脚�? 跳结�?
  //   $815B: ASL; BCC $8160; INC $0053 (×2, 进位加高字节)
  //   $8160: TAY; LDA ($0052),Y; TAX; INY; LDA ($0052),Y; STA $0053; STX $0052
  //     (查指针表得脚本入�? �?$0052/$0053)
  //   $816B: LDA #$00; STA $05D1; STA $003A
  //   $8172: LDY $003A; LDA ($0052),Y (读脚本字�?
  //   $8176: CMP #$F0; BCC $8188 (< $F0 = 普�?tile, �?NT)
  //   $817A: JSR $81BC (�?$F0 = 命令, 分派)
  //   $817D: LDA #$02; STA $05D0; STA $0525 (设标�?
  //   $8185: JMP $819C
  //   $8188: STA $0525; TYA; CLC; ADC $0052; STA $0052 (指针前进)
  //   $8195: LDY #$01; LDA ($0052),Y; JSR $81A7 (读长�? 分派)
  //   $819C: LDA $0516; ORA #$10; STA $0516; JMP $C512 (设标�? 返回)
  // ════════════════════════════════════════════════
  private sub814C(): void {
    // $814E: 指针 = $87F6 (脚本索引�?
    this.wrPtr(0x0052, 0x0053, 0x87F6);
    // $8154: 读脚本索�?    const idx = this.rd(0x0524);
    // $8157: $FF = 无脚�?    if (idx === 0xFF) {
    ...
    this.wr(0x05D1, 0);
    this.wr(0x003A, 0);
    // $8172: 读脚本字�?    const y = this.rd(0x003A);
    ...
    if (b < 0xF0) {
      ...
      // $8195: 读长度分�?      const len = this.readMemByte(adv + 1);
      this.sub81A7(len);
    } else {
      // $817A: 命令分派
      this.sub81BC(b);
      // $817D: 设标�?      this.wr(0x05D0, 0x02);
      this.wr(0x0525, 0x02);
    }
    this.endScriptProcess();
  }

  /** $819C: 脚本处理结束 (�?$0516 bit4, 返回) */
  private endScriptProcess(): void {
    const v = this.rd(0x0516) | 0x10;
    this.wr(0x0516, v);
    // $81A4: JMP $C512 (返回调用�? H5 no-op)
  }

  // ════════════════════════════════════════════════
  // $81A7: 命令分派 (�?$81A8 跳转�?
  // asm: JSR $C509; .byte $27,$83,$E7,$83,$FF,$83,$58,$83,$77,$83,$64,$83,$D2,$83,$E7,$83,$EE,$83
  //   跳转�?9 �? $8327/$83E7/$83FF/$8358/$8377/$8364/$83D2/$83E7/$83EE
  // ════════════════════════════════════════════════
  private sub81A7(a: number): void {
    // �?6502: JSR $C509 (A=cmd, $CB99 采样内联跳转�?, cmd N �?表项 N
  ...
  //   跳转�? $81CC/$8276/$824D/$82F7 (4 �?
  //   $81D5 数据�? 精灵�?tile 偏移�?  //   JMP $832B
  // ════════════════════════════════════════════════
  private sub81BC(a: number): void {
    this.wr(0x0525, 0);
    // �?6502: AND #$0F; JSR $C509 (cmd = a&0x0F, 4 �?
    const cmd = a & 0x0F;
    switch (cmd & 0x03) {
  ...
  // ════════════════════════════════════════════════
  // $84A1: 精灵组写入入�?  // asm $84A1 (code_data.s):
  ...
  //   $84CE: RTS
  //   $84CF: JSR $84F4; STX $05DE; STY $05DF; RTS (写精灵坐�?
  //   $84D8: JSR $84F4; ... (带翻转的精灵坐标)
  // ════════════════════════════════════════════════
  // ════════════════════════════════════════════════
  // $85C2: �?NT �?(滚动�?
  // asm: 读数�?+ NT 地址, 写一�?tile
  // TODO: 翻译 $85C2 完整实现
  // ════════════════════════════════════════════════
  ...
  /** $8377: 命令4 �?JSR $84CF; 读脚本[4]�?05E2; $05E1=0; 复杂精灵组设�?*/
  ...
  /** $8364: 命令5 �?JSR $84D9; LDA #$80; STA $05D1; Y=4�?; �?$8329 */
  ...
  /** $81CC: 精灵�? �?JSR $82F7; LDA $81D5,X; JMP $832B */
  ...
  /** $8276: 精灵�? �?JSR $82F7; LDA $827F,X; JMP $832B */
   ...
   *   �?$05CB=1; $05CE; $05CD; �?$05DB/$05DC/$05DD
   */
  ...
  private readMemByte(addr: number): number {
    if (addr < 0x0800) {
      // RAM �?      return this.rd(addr);
    }
    // ROM �? bank11 数据 (stub, �?import bank11 数据�?
    // 临时返回 0
    return 0;
  }
}
