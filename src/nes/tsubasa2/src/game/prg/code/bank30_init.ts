...
export class Bank30Service {
   ...
   */
  init(): void {
    trace('Bank30', 'init() $C503 RESET 硬件初始化链开始');
    ...
    this._initC400();
    trace('Bank30', 'init() RESET 硬件初始化链完成');
  }
  ...
  private _initC400(): void {
    trace('Bank30', '_initC400() $C400 PPU 最终配置 + 跳 Bank02');
    const s = this._store;
    ...
    trace('Bank30', '$C400 → bank02.resetEntry(0) ($A200 首场景入口)');
    this._bank02.resetEntry(0);
    trace('Bank30', '_initC400() 完成 (bank02 已进入首场景)');
  }
