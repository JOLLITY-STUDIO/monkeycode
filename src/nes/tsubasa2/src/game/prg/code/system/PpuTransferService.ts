 ...
 */
const SCENE_CFG_UP_BOUND: ReadonlyArray<number> = [
  0x10, 0x04, 0x20, 0x05, 0x60, 0x06, 0xff,
];
const SCENE_CFG_SUB: ReadonlyArray<number> = [
  0x00, 0x03, 0x10, 0x04, 0x20, 0x05, 0x60, 0x06, 0xff,
];
const SCENE_CFG_PARAM: ReadonlyArray<number> = [
  0x03, 0x10, 0x04, 0x20, 0x05, 0x60, 0x06, 0xff,
];
...
export class PpuTransferService {
  constructor(
    ...
    private readonly ppu: PpuTarget | null = null,
  ) {}
  ...
  resolveSceneCfg(cfgId: number): SceneLoadCfg | null {
    const s = cfgId & 0xff;
    let y = 1;
    while (y < SCENE_CFG_UP_BOUND.length && s >= SCENE_CFG_UP_BOUND[y]) y++;
    if (y >= SCENE_CFG_UP_BOUND.length) return null;
