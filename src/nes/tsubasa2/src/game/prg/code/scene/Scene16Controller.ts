...
/** 分支 1（$04E5 != $FF）压印序列 */
const BRANCH1_TASKS: readonly StampTask[] = [
...
/** 分支 2（$04E5 == $FF）压印序列 */
const BRANCH2_TASKS: readonly StampTask[] = [
...
export class Scene16Controller extends SceneController {
  ...
  private readonly branch: 1 | 2;
  private tasks: readonly StampTask[];
  private current: StampTask | null = null;
  private remaining = 0;
  private spriteIdx = 0;
  private attrLoopIdx = 0; // 分支2 末段 $046A|=2 循环游标（> $EC 表示已结束）
  private ready = false;
  ...
  onUpdate(_frame: number): number | undefined {
    ...
    if (this.current === null) {
      ...
      if (task.spriteIdx >= 0) this.spriteIdx = task.spriteIdx;
    }
    // 单次 $A72C 压印
    const c = this.current;
    this.spriteIdx = this.prim.a72cStampSprite(
      c.tile, this.spriteIdx, c.attr, c.dx, c.dy, c.mask,
    );
    this.remaining--;
    if (this.remaining <= 0) this.current = null;
