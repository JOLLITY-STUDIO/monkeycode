...
export const PHASE_GOAL = 2;
export const PHASE_CORNER = 3;
...
export class MatchEngineService {
  ...
  dispatch(index: number): void {
    switch (index) {
      case 0: this.mainLoop(); break;        // $803C
      case 1: this.kickoff(); break;         // $84F8
      case 2: this.goalkeeperAction(); break; // $86F6
      case 3: this.playerAIDefend(); break;  // $82FC
      case 4: this.playerAIAttack(); break;  // $83A2
      case 5: this.substituteHome(); break;  // $8835
      case 6: this.substituteAway(); break;  // $888D
      case 7: this.initMatchState(); break;  // $88F3
      case 8: this.matchPhaseHandler(); break; // $8978
      case 9: this.dispatchEvent(); break;   // $8BDF
      case 10: this.adjustBallPosition(); break; // $8CA4
      case 11: this.calculateValue(); break; // $8D06
      case 12: this.calculateExp(); break;   // $8EE9
      case 13: this.calculateAbility(); break; // $8F72
      case 14: this.freeKick(); break;       // $911C
      case 15: this.shootStart(); break;     // $92EE
      case 16: this.actionSelect(); break;   // $9470
      case 17: this.tacticsAdjust(); break;  // $95E1
      case 18: this.formationEdit(); break;  // $9731
    }
  }
  ...
  private _processPlayer(idx: number): void {
    ...
    // 根据状态选择行为
    switch (state & 0x07) {
      case 0: this._idleBehavior(idx); break;
      case 1: this._moveToBall(idx); break;
      case 2: this._chasePlayer(idx); break;
      case 3: this._holdAndDribble(idx); break;
      case 4: this._passBall(idx); break;
      case 5: this._shootTry(idx); break;
      default: this._idleBehavior(idx); break;
    }
  ...
  /** 控球/盘带 */
  private _holdAndDribble(idx: number): void {
    const s = this._store;
    const id = this._readArr(KEY_0601, idx);
  ...
  /** 朝球移动 */
  private _moveToBall(idx: number): void {
    const s = this._store;
    const id = this._readArr(KEY_0601, idx);
  ...
  /** 追踪指定球员 */
  private _chasePlayer(idx: number): void {
    const s = this._store;
  ...
  /** 传球 */
  private _passBall(idx: number): void {
    const s = this._store;
    const id = this._readArr(KEY_0601, idx);
    const target = s.read(KEY_05FC) & 0xFF;
  ...
  /** 射门尝试 */
  private _shootTry(idx: number): void {
    const s = this._store;
  ...
  /** 空闲行为 */
  private _idleBehavior(idx: number): void {
    // 返回阵型位置附近
    const s = this._store;
    const id = this._readArr(KEY_0601, idx);
  ...
  /** 寻找空位跑位 */
  private _moveToOpen(id: number): void {
    const s = this._store;
  ...
  /** 抢断尝试 */
  private _tackleAttempt(id: number): void {
    const s = this._store;
  ...
  /** 找空位队友 */
  private _findOpenTeammate(id: number): void {
    const s = this._store;
    const count = s.read(KEY_0600) & 0xFF;
    ...
    for (let i = 0; i < count && i < 44; i++) {
      ...
      if (this._sideOf(pid) !== side) continue;
  ...
  kickoff(): void {
    ...
    s.write(KEY_0612, 0);
    s.write(KEY_043B, PHASE_PLAY);
  }

  /** 边线掷球 ($858A 附近) */
  private _throwIn(): void {
    const s = this._store;
    const side = s.read(KEY_05FB) & 0xFF;
  ...
  /** 交换两名球员的场上槽位 */
  private _swapPlayers(a: number, b: number): void {
    const s = this._store;
    const count = s.read(KEY_0600) & 0xFF;
  ...
  private _handleGoal(): void {
    ...
    // 回中圈开球
    s.write(KEY_043B, PHASE_KICKOFF);
    s.write(KEY_0612, 0);
  }

  /** 角球处理 */
  private _handleCorner(): void {
    const s = this._store;
    // 球放到角旗区
    const side = s.read(KEY_05FB) & 0xFF;
    const dir = T_92EA[side === 0 ? 0 : 1] ?? 1;
  ...
  dispatchEvent(): void {
    ...
    switch (eventId) {
      case 1: this._eventPass(); break;
      case 2: this._eventTackle(); break;
      case 3: this._eventShot(); break;
      case 4: this._eventOut(); break;
      case 5: this._eventGoal(); break;
      default:
        // 事件后处理
  ...
  /** 出界事件 */
  private _eventOut(): void {
    const s = this._store;
    s.write(KEY_043B, PHASE_CORNER);
    s.write(KEY_0612, 0);
  }

  /** 进球事件 */
  private _eventGoal(): void {
    const s = this._store;
    s.write(KEY_043B, PHASE_GOAL);
    s.write(KEY_0612, 0);
  }

  /** 事件后处理 ($8A6F-$8B3A) */
  private _postEventHandler(): void {
    const s = this._store;
    s.write(KEY_0612, 0);
    s.write(KEY_0617, 0);
  }
  ...
  calculateValue(): void {
    ...
    switch (x) {
      case 0: this._calcTotalAbility(); break;
      case 1: this._checkPlayerState(); break;
      case 2: this._handleTouch(); break;
      default: this._calcDistance(); break;
    }
  }

  /** 能力总值计算 */
  private _calcTotalAbility(): void {
    const s = this._store;
    const id = s.read(KEY_0441) & 0xFF;
    let total = 0;
    for (let i = 0; i < 6; i++) {
  ...
  /** 球员状态检查 ($8E33) */
  private _checkPlayerState(): void {
    const s = this._store;
    const id = s.read(KEY_0441) & 0xFF;
  ...
  calculateExp(): void {
    ...
    let gain = 0;
    switch (x) {
      case 0: gain = 1; break;  // 触球
      case 1: gain = 2; break;  // 传球
      case 2: gain = 3; break;  // 抢断
      case 3: gain = 5; break;  // 射门
      case 4: gain = 10; break; // 进球
  ...
  /** 球员等级 (查 $9E4E 类似映射) */
  private _levelOf(id: number): number {
    const exp = this._playerStat(id, 5);
    if (exp >= 0x9A) return 3;
    if (exp >= 0x60) return 2;
    if (exp >= 0x30) return 1;
    return 0;
  }
  ...
  freeKick(): void {
    ...
    switch (x) {
      case 0: this._freeKickNormal(); break;
      case 1: this._freeKickIndirect(); break;
      case 2: this._penaltyKick(); break;
      default: this._throwIn(); break;
    }
  }

  /** 任意球 */
  private _freeKickNormal(): void {
    const s = this._store;
    const side = s.read(KEY_05FB) & 0xFF;
  ...
  /** 间接任意球 */
  private _freeKickIndirect(): void {
    this._freeKickNormal();
  }

  /** 点球 */
  private _penaltyKick(): void {
    const s = this._store;
    const side = s.read(KEY_05FB) & 0xFF;
    const taker = this._kickoffTaker(side);
    s.write(KEY_05FC, taker);
  ...
  /** 必杀技判定 */
  specialMoveCheck(playerId: number, moveId: number): boolean {
    const s = this._store;
    s.write(KEY_0441, playerId);

    // 体力检查
    const stamina = this._playerStat(playerId, 0);
    if ((stamina & 0x80) !== 0) return false;
  ...
  /** 防守/扑救判定 */
  defend(defenderId: number): boolean {
    const s = this._store;
  ...
  /** 射门处理 (H5 语义: 计算射门结果) */
  handleShoot(attackerId: number, power: number): void {
    const s = this._store;
  ...
  actionSelect(): void {
    ...
    if ((input & 0x0F) !== 0) {
      let next = cur;
      if ((input & 0x01) !== 0) next = (cur - 1) & 0xFF;  // 上
      if ((input & 0x02) !== 0) next = (cur + 1) & 0xFF;  // 下
      if ((input & 0x04) !== 0) next = (cur - 1) & 0xFF;  // 左
      if ((input & 0x08) !== 0) next = (cur + 1) & 0xFF;  // 右
  ...
  /** 菜单确认分发 */
  private _menuConfirm(sel: number): void {
    const s = this._store;
    switch (sel) {
      case 0: s.write(KEY_0612, 1); break; // 传球
      case 1: s.write(KEY_0612, 5); break; // 射门
      case 2: this.tacticsAdjust(); break; // 战术
      case 3: s.write(KEY_0612, 0); break; // 取消
    }
  }
  ...
  /** 应用战术 (T_9F0F, 每战术 5 项) */
  private _applyTactics(tactic: number, side: number): void {
    const s = this._store;
    const base = (tactic & 0x07) * 5;
    for (let i = 0; i < 5; i++) {
      const slot = T_9F0F[base + i] ?? 0;
      if (slot === 0) continue;
  ...
  /** 应用阵型 (T_9EB7, 每阵型 8B) */
  private _applyFormation(formation: number, side: number): void {
    const s = this._store;
    const base = (formation & 0x0F) * FORMATION_SIZE;
    for (let i = 0; i < 8; i++) {
      const slot = T_9EB7[base + i] ?? 0;
      if (slot === 0 || slot === 0xFF) continue;
      const pid = slot + (side === 0 ? 0 : 11);
      // 阵型位置: 简化映射
  ...
  /** 阵型槽位 X 坐标 (简化) */
  private _formationX(slot: number): number {
    const cols = [0x10, 0x30, 0x50, 0x70];
    return cols[slot & 0x03] ?? 0x40;
  }

  /** 阵型槽位 Y 坐标 (简化) */
  private _formationY(slot: number): number {
  ...
  private _playerDataAddr(id: number): number {
    const side = id >= 11 ? 1 : 0;
    const slot = id % 11;
    if (slot >= 8) return -1;
    const ptr = T_9FF0[slot * 2] | (T_9FF0[slot * 2 + 1] << 8);
    // Bank29 CPU 基址 $A000
    return ptr - 0xA000 + (side === 0 ? 0 : 0x4C);
  }

  /** 读球员属性 (0-6 项能力) */
  private _playerStat(id: number, statIdx: number): number {
    const base = this._playerDataAddr(id);
    if (base < 0) return 0;
  ...
  /** 球员状态 */
  private _playerState(id: number): number {
    const s = this._store;
    const idx = this._findPlayerIndex(id);
    if (idx < 0) return 0;
  ...
  /** 找球员所在槽位索引 */
  private _findPlayerIndex(id: number): number {
    const s = this._store;
    const count = s.read(KEY_0600) & 0xFF;
    for (let i = 0; i < count && i < 44; i++) {
      if ((this._readArr(KEY_0601, i) & 0xFF) === id) return i;
    }
    return -1;
  }

  /** 球员所属队伍 (0=主队, 1=客队) */
  private _sideOf(id: number): number {
    return id >= 11 ? 1 : 0;
  }

  /** 找指定队伍的传球目标 */
  private _findOpenTeammateId(id: number): number {
    const s = this._store;
    const count = s.read(KEY_0600) & 0xFF;
    const side = this._sideOf(id);
    let best = -1;
  ...
  /** 指定队伍的阵型槽位 (简化) */
  private _formationSlotFor(id: number): number {
    return id % 11;
  }

  /** 开球球员 */
  private _kickoffTaker(side: number): number {
    return side === 0 ? 10 : 21;
  }

  /** 门将 */
  private _goalkeeperOf(side: number): number {
    return side === 0 ? 1 : 12;
  }

  /** 前锋 */
  private _forwardOf(side: number): number {
    return side === 0 ? 10 : 21;
  }

  /** 到球距离 */
  private _distanceToBall(id: number): number {
    const s = this._store;
    const idx = this._findPlayerIndex(id);
  ...
  /** 坐标 → 区域编码 */
  private _coordToZone(x: number, y: number): number {
    const zx = (x >> 4) & 0x0F;
    const zy = (y >> 4) & 0x0F;
    return (zy << 4) | zx;
  }
  ...
  /** 区域 → 索引 */
  private _zoneToIndex(zone: number): number {
    return zone & 0x3F;
  }

  /** 8bit 有符号解释 */
  private _signed(v: number): number {
    const b = v & 0xFF;
    return b >= 0x80 ? b - 0x100 : b;
  }

  /** 方向表: X 增量 */
  private _dirDx(dir: number): number {
    switch (dir & 0x07) {
      case 1: case 8: return 1;   // 右
      case 2: return 1;            // 右下
      case 3: return 0;            // 下
  ...
  /** 方向表: Y 增量 */
  private _dirDy(dir: number): number {
    switch (dir & 0x07) {
  ...
  /** 位置更新 (简化) */
  private _updatePosition(idx: number): void {
    // H5: 位置由 AI 动作直接更新, 无需额外处理
    void idx;
  }

  /** 等待/同步 (对应固定区 $C515) */
  private _waitSync(_frames: number): void {
    // H5: 同步由渲染层驱动
  }
  ...
  read(cpuAddr: number): number {
    return readB26(cpuAddr);
  }

  /** 读 bank26 16bit LE */
  read16(cpuAddr: number): number {
    return readB26U16(cpuAddr);
  }
  ...
  /** 获取球员属性 (供 AI/UI 查询) */
  getPlayerStat(playerId: number, statIdx: number): number {
    return this._playerStat(playerId, statIdx);
  }

  /** 获取阵型表条目 */
  getFormationEntry(formation: number): readonly number[] {
    const base = (formation & 0x0F) * FORMATION_SIZE;
    return T_9EB7.slice(base, base + FORMATION_SIZE);
  }

  /** 获取战术表条目 */
  getTacticsEntry(tactic: number): readonly number[] {
    const base = (tactic & 0x07) * 5;
    return T_9F0F.slice(base, base + 5);
  }

  /** 获取球员数据指针 (T_9FF0) */
  getPlayerDataPtr(slot: number): number {
    const i = (slot & 0x07) * 2;
    return (T_9FF0[i] | (T_9FF0[i + 1] << 8)) - 0xA000;
  }
}
