...
/**
 * 协程槽 — 对应 ROM bank0 $9EED 主循环遍历的 ram_0001-$0018 (6槽×4字节)。
 *
 * 真实 ROM 是协作式多任务(协程)调度器:
 *   - 每槽4字节 [栈SP][bank0=R6][bank1=R7][计数]
 *   - 主循环 $9EED 遍历, 计数归零 → $9F0F 恢复栈+寄存器现场 + RTS 跳回协程上次 yield 处
 *   - 协程内 JSR $9F69 = yield: 保存现场到槽 + 设 $FF 标记 + RTS 回主循环
 *   - $FF 标记槽下次轮到 → $9F52 恢复执行 (协程续跑)
 *
 * H5 转写: 用 TS Generator 实现协程。gen = 场景协程, done=是否结束。
 * 每帧 _runCoroutineLoop 轮转所有非空槽, 调 gen.next(buttons) 推进一步。
 * 空槽: gen===null。结束槽: gen.done===true。
 */
...
/** 协程 yield 返回值: SceneRoot=请求切换到新场景, void=继续本协程 */
export type CoroutineYield = SceneRoot | void;

export class BootService {
  /** 开场每镜头持续帧数 (真实 ROM: 开场动画约 4-5 秒后才能跳过, ~280-300帧) */
  private static readonly SHOT_DURATION = 300; // ~5 秒 @60fps

  /** 当前镜头已过帧数 */
  private _shotFrame = 0;

  /** 比赛已进行帧数 (MATCH 守卫) */
  private _matchFrame = 0;
   ...
   */
  private _slots: CoroutineSlot[] = Array.from({length: COROUTINE_SLOT_COUNT}, () => ({gen: null, scene: SceneRoot.BOOT}));

  /** 上一帧按键（边沿检测，防止按键穿透场景） */
  private _prevButtons = 0;

  /** 当前关卡 (里约杯场次 1-6, 初始1) */
  private _matchRound = 1;
   ...
   */
  private _advanceRound(win: boolean): SceneRoot {
    if (win) {
      this._matchRound++;
      if (this._matchRound > 6) {
        // 通关 (决赛获胜) → CREDITS
        return SceneRoot.CREDITS;
      }
      return SceneRoot.LEVELUP;  // 赢 → 升级 → 下一场 STORY→MEETING→MATCH
    } else {
      // 输: 偶数场回前一场, 奇数场自回
      ...
      return SceneRoot.LEVELUP;  // 输也显示升级(经验) → 重打本场
    }
  }
  ...
  update(buttons: number, _frameCount: number): boolean {
    ...
    for (let i = 0; i < this._slots.length; i++) {
      ...
      if (slot.gen === null) continue;          // 空槽, 跳过 (对应 $9EF1 BEQ)
      ...
      if (r.done) {
        ...
        this._slots[i] = {gen: null, scene: SceneRoot.BOOT}; // 清空槽
      } else if (typeof r.value === 'number') {
        // 协程 yield 返回场景标签 = 请求切换 (对应 JSR $9F69 让出 + 切换)
        this._spawnCoroutine(r.value);
        this._slots[i] = {gen: null, scene: SceneRoot.BOOT}; // 终止旧协程, 新协程接管
        stateChanged = true;
      }
    }
    return stateChanged;
  ...
  private *_bootCoroutine(): Generator<number | undefined, SceneRoot, number> {
    ...
    for (;;) {
      ...
      if ((buttons & BUTTON.START) || this._shotFrame >= BootService.SHOT_DURATION) {
        return SceneRoot.TITLE; // 切换到 TITLE (Cut 0x17 标题菜单)
      }
    }
  }
