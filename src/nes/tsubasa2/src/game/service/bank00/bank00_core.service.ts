...
/** 帧状态标志 — 真实 RAM 地址键 */
const FRAME_FLAG = 'ram_001E';   // ram_001E: bit4=vblank done, bit7=精灵事件, bit6=?
const SCENE_ID   = 'ram_0026';   // ram_0026: 场景 ID
const RAM_1B     = 'ram_001B';   // ram_001B: 场景状态标志 (bit0=已初始化, bit7=?)
...
export class Bank00Service {
   ...
   * 真实 ROM 是永不返回的循环 (等待 NMI 后推进状态机), H5 改为每帧推进一次。
   * 主要工作:
   *   1. 首帧 (ram_001B bit0 == 0) 执行完整场景初始化
   *   2. 等待帧事件 → 输入解码 → 菜单状态机 (sceneId 分发)
   *
   * @param buttons 当前帧按键
   */
  private _mainInputLoop(buttons: number): void {
    ...
    // ── $8027-$8046: 等待 VBlank 后清零一组状态变量 ──
    // asm: JSR $9BA0 (waitVBlank) → 清 ram_0005/0006/0009/000A/0011/0012/000D/000E/004C/005B
    if ((s.read(FRAME_FLAG) & 0x10) === 0) {
      return; // 等待 NMI (VBlank) 标志
    }
    this._clearStateVars();

    // ── $8048-$8051: ram_0700=1; 首帧检测 ram_001B bit0 ──
    s.write('ram_0700', 1);
    ...
    // ── $8087-$808B: 清 ram_001B bit0 (允许下次场景再初始化) ──
    s.write(RAM_1B, ram1b & 0xFE);
  ...
  /** 设置 ram_001E bit4 (VBlank 完成) — 供 Bank02 NMI 设置 */
  setVBlankFlag(): void {
