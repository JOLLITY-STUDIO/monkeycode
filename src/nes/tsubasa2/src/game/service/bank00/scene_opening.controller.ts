...
export class OpeningSceneController {
  ...
  private _attrPaletteAt(tx: number, ty: number): number {
    const ax = tx >> 2;
    const ay = ty >> 2;
    const byte = BOOT_ATTR0[(ay << 3) + ax] ?? 0;
    const subX = (tx >> 1) & 1;
    const subY = (ty >> 1) & 1;
  ...
  private _applyBootPalette(step: number): void {
    const bg = BOOT_BG_PALETTE.map((c) => bootFadeByte(c, step));
    const spr = BOOT_SPR_PALETTE.map((c) => bootFadeByte(c, step));
