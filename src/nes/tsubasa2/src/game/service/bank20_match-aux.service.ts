...
export const T_88E4: ReadonlyArray<number> = [0x0547, 0x055C, 0x0571, 0x0586, 0x059B, 0x05B0];

/** $8264: 3 个 bank21 调色板基址 (16bit LE, CPU $B80C/$B6C7/$B767) */
export const T_8264: ReadonlyArray<number> = [0xB80C, 0xB6C7, 0xB767];
...
/** $83A6: 8B 精灵调色板 */
export const T_83A6: ReadonlyArray<number> = [0x0F, 0x0F, 0x00, 0x00, 0x0F, 0x0F, 0x30, 0x00];

/** $885B: 6B 动画 tile 表 */
export const T_885B: ReadonlyArray<number> = [0x36, 0x37, 0x3D, 0x3D, 0x37, 0x36];
...
/** $88D0: 10B 计分板属性表 */
export const T_88D0: ReadonlyArray<number> = [0x1C, 0x1D, 0x1E, 0x1F, 0x30, 0x31, 0x32, 0x33, 0x34, 0x35];

/** $88DA: 5B Y 偏移 */
export const T_88DA: ReadonlyArray<number> = [0x1D, 0xFD, 0xFD, 0xFD, 0xFD];

/** $88DF: 5B X 偏移 */
export const T_88DF: ReadonlyArray<number> = [0x2C, 0xC7, 0xC7, 0xC7, 0xC7];

/** 主数据流指针表位置 (标注 $8968 → 数组索引 0x1968) */
export const MAIN_STREAM_TABLE = 0x1968;
/** 名字映射表位置 (标注 $88F0 → 数组索引 0x8F0) */
export const NAME_MAP_TABLE = 0x08F0;
...
export class Bank20Service {
  ...
  private _readBank21(addr: number): number {
  ...
  private _fixedC545(a: number): [number, number] {
    ...
    let lo = PRG_BANK_31[base + v] ?? 0;
    let hi = PRG_BANK_31[base + v + 1] ?? 0;
    if (neg) {
      lo = (~lo) & 0xff;
      hi = (~hi) & 0xff;
      lo = (lo + 1) & 0xff;
  ...
  entry_800F(): void {
    ...
    const streamAddr = this.readU16(MAIN_STREAM_TABLE + sub * 2); // CPU 地址 $8xxx
  ...
  private _mainStreamLoop(): void {
    const s = this._store;
    for (;;) {
      const byte = PRG_BANK_20[this._streamPtr] ?? 0xff;
      if (byte < 0xf0) {
  ...
  private entry_80AA(): void {
    ...
    const rec = T_88E4[idx] ?? 0x0547; // ram_003A/3B
    // 清记录区 0x15 字节
  ...
  private _subDispatch(id: number): void {
    ...
    for (let xx = 0; xx < 0x10; xx++) {
      ...
      let v: number;
      if (m === 1) v = 0x0f;      // $8258
      else if (m === 2) { v = this._readBank21(ptr + y); y++; } // $8253
  ...
  private fn_826A(): void {
    ...
    const b0 = this._readName(rec, 0);
    let v = PRG_BANK_20[NAME_MAP_TABLE + b0] ?? 0;
    if (b0 === 0) {
  ...
  private entry_82BC(): void {
    ...
    for (let x = 0; x < 0x20; x++) {
      const p = PRG_BANK_20[0x2F6 + x] ?? 0; // $82F6 (数组索引 0x2F6)
      let v: number;
      if ((p & 0x80) !== 0) {
  ...
  private _readSubStream(off: number): number {
    ...
    if (addr >= 0xa000) return PRG_BANK_21[addr - 0xa000] ?? 0xff;
    return PRG_BANK_20[addr - 0x8000] ?? 0xff;
  }
