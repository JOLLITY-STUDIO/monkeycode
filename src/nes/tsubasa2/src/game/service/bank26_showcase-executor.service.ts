 ...
 *   $8036: JMP $85AC  射门演出状态机 (技能演出初始化)
 *
 * 翻译完整性核对 (agent 指南):
 *   - $8F72 能力计算: 已按 $8F72-$8FF2 逐条翻译, 含 $8FFB 名字区坐标差子程序
 *     (carry/8bit 回绕精确, 固定区 $C54B/$C521 用 _fixed 辅助占位)
 *   - $85AC 演出初始化: 已按 $85AC-$85E0 逐条翻译 (读 $0441 → $8BD4 → $85E3
 *     → $C54E → $987B → 切换 $05FB → $C50C → 清名字区字段 → $0629=4 → $C636)
 *   - $911C 演出球推进: tick() 中按 $911C 语义推进球坐标 (X=&~7+4, Y 按符号位
 *     0x4C/0xB4 跳变), 并补 $92EA 方向表查表 (BALL_DIR_TABLE)
 *
 * 分层职责:
 ...
 *   Cyclone #46 链: Bank30 $D792 → requestShowcase(0x46) + $8021 + $8036
 *
 * 固定区辅助映射 (Bank30 $C000-$DFFF):
...
} from '../data/prg/showcase-data';
import { getShowcasePalette, SHOWCASE_PALETTE_DEFAULT } from '../data/prg/showcase-palette';
...
export class Bank26ShowcaseExecutor {
  ...
  private _fixedBankSwitch(_bank: number): void {
    // H5: 无硬件 bank 切换
  }
  ...
  private _fixedC4C8(_a: number): void {
    // H5: 固定区精灵/数值展开 no-op
  }

  /** JSR $C521 — 16bit 能力运算 (Bank30 固定区, 对 $006B/$006C 运算 → $006C/$006D) */
  private _fixedC521(): void {
    // H5: 由能力计算逻辑内联近似
  }

  /** JSR $C52A / $C515 / $C52D / $C600 — 渲染同步/切场景/任务入队 */
  private _fixedRenderSync(_a: number): void {
    // H5: 同步由渲染层驱动
  }

  /** JMP $C636 / $C603 / $C60F — 主循环跳转 / 事件分发 (Bank30) */
  private _fixedJump(_entry: number): void {
  ...
  private _sub8BD4(playerId: number): void {
  ...
  private _sub904E(): void {
    const s = this._store;
    // $904E: BIT $044B; $9051: BPL $906F → 若 $044B bit7 清则返回
    if ((s.read(KEY_044B) & 0x80) === 0) return;
  ...
  tick(): void {
    ...
    if (busy) {
      ...
      let xi = 0;
      if ((s.read(KEY_0635) & 0x80) !== 0) xi++;
      if ((s.read(KEY_0637) & 0x80) !== 0) xi += 2;
      if (s.read(KEY_05FB) !== 0) xi ^= 0x03;
  ...
  getDisplayState(): ShowcaseDisplayState {
    ...
    const blockIndex = showcaseBlockIndexByType(type);
    const block = getShowcaseBlock(ram043B);
    return {
      active: busy,
      showId,
