 ...
 */
const CHAPTER_STREAM_OFFSET: Record<number, number> = {
  [StoryChapter.OPENING]: BANK19_STREAM_OFFSET,
 ...
 */
const CHAPTER_SCENE_IDX: Record<number, number> = {
...
export class Bank18Service {
   ...
   * 剧情流偏移启动渲染。
   */
  enterChapter(chapter: StoryChapter): void {
    this._chapter = chapter;
    // 章节 → bank18 场景段 (真实数据, 见 CHAPTER_SCENE_IDX / B18_SCENE_MAPS)
    const sceneIdx = CHAPTER_SCENE_IDX[chapter] ?? 0;
    this._sceneIdx = sceneIdx >= 0 && sceneIdx < B18_SCENE_COUNT ? sceneIdx : 0;
    // 章节 → Bank19 数据流偏移 (跨 bank 占位, 待 bank00/bank02 trace)
    const offset = CHAPTER_STREAM_OFFSET[chapter] ?? BANK19_STREAM_OFFSET;
    this._active = true;
  ...
  readByte(cpuAddr: number): number {
    return readB18(cpuAddr);
  }

  /** 读本 bank 内 16bit LE */
  readU16(cpuAddr: number): number {
    return readB18U16(cpuAddr);
  }

  /** 读背景 tile 图块行 (row 索引, 每行 16 字节 = 4×4 tile) */
  readTileRow(row: number): readonly number[] {
    return readB18TileRow(row);
  }
  ...
  readScene(sceneIdx: number): readonly number[] {
    return readB18Scene(sceneIdx);
  }

  /** 读指定场景段的单行 tile (16 字节 = 4×4 bg 图块) */
  readSceneRow(sceneIdx: number, row: number): readonly number[] {
    return readB18SceneRow(sceneIdx, row);
  }
}
