...
export class Bank28MatchService {
  ...
  entryFormationConfig(): void {
    ...
    if (teamIdx === 3) teamIdx = 2; // $8292: CPY #$03 → DEY
  ...
  entryRoleQuery(): void {
    ...
    for (;;) {
      ...
      a = (a + 1) & 0xFF;
      if (a === cnt) break; // $8638-$863B: TXA; CMP $0600; BNE $8618
    }
  }

  /** $863F-$8650: 角色数据处理入口 */
  private _roleQuery863F(a: number): void {
    const s = this._store;
  ...
  private _pixelToZone(y: number): number {
  ...
  private _zoneAt(pixelIdx: number): number {
    // asm: LDX #$00; CMP $8BBE,X; BEQ → LDA $8BBF,X; RTS; INX;INX; BNE 循环
    for (let i = 0; i + 1 < T_ZONE_COORD.length; i += 2) {
      if (T_ZONE_COORD[i] === pixelIdx) {
        return T_ZONE_COORD[i + 1];
      }
    }
    return 0;
  }
  ...
  private _sub868E(idx: number): void {
    const s = this._store;
  ...
  private _roleQueryTable(sel: number): void {
    switch (sel & 0xFF) {
      case 0:
  ...
  private _roleQueryTable_BA(): void {
    const s = this._store;
  ...
  private _roleQueryTable_EB(): void {
    const s = this._store;
    const v = s.read(KEY_043D) & 0xFF;
    switch (v) {
      case 0:
  ...
  private _roleQueryTable_10(): void {
    const s = this._store;
    const v = s.read(KEY_043D) & 0xFF;
    switch (v) {
      case 0:
  ...
  private _sub879C(idx: number): void {
    const s = this._store;
    // $879E: JSR $8ADE (Y=4)
    this._sub8ADE(4, idx);
  ...
  private _roleQuery875DTable(sel: number): void {
    const s = this._store;
  ...
  private _roleQuery875D_scan(): void {
    const s = this._store;
    if ((s.read(KEY_043C) & 0xFF) !== 0) return; // BNE $8849
    ...
    for (;;) {
      ...
      if ((dx & 0x80) !== 0) dx = (((~dx) & 0xFF) + 1) & 0xFF; // abs
      if (dx >= 0x14) { scene = (scene + 1) & 0xFF; continue; }
      const sceneY = this._readRamAbs(ptr.lo + (ptr.hi << 8) + 8);
      let dy = (sceneY - (s.read(KEY_0637) & 0xFF)) & 0xFF;
      if ((dy & 0x80) !== 0) dy = (((~dy) & 0xFF) + 1) & 0xFF;
      if (dy < 0x14) {
        this._writeScene05FC(scene);
        return;
      }
  ...
  private _writeScene05FC(scene: number): void {
    this._store.write(KEY_05FC, scene & 0xFF);
    this._sub8A09();
  }

  /** $8A3F: 属性+等级 → 043C 汇总 */
  private _sub8A3F(): void {
    const s = this._store;
  ...
  private _sub8A62(a: number): void {
    const s = this._store;
    // $8A62: PHA; JSR $C50C
    this._fixedC50C(a);
  ...
  private _readIndirectBank8A9D(x: number): number {
    // $8A6F: LDY $8A9D,X (读 bank28 字节)
    const y = T_ATTR_ROLE_8A9D[x & 0xFF] ?? 0;
    const ptr = this._readIndirectPtr(KEY_38, KEY_39);
    return readB28(ptr.lo + (ptr.hi << 8) + y);
  }

  /** $8AB3: 距离/方向表 ($8B9E) 查询 */
  private _sub8AB3(v: number): number {
    const s = this._store;
    ...
    let result = 0;
    for (let i = 0; i + 1 < T_DIST_DIR.length; i += 2) {
      if (T_DIST_DIR[i] === a) {
        result = T_DIST_DIR[i + 1];
        break;
      }
    }
  ...
  private _sub8ADE(y: number, a: number): void {
    const s = this._store;
    // $8ADE: STA $003E
    ...
    const v = readB28(base.lo + (base.hi << 8) + yy);
  ...
  private _sub8AEB(a: number): [number, number] {
    let val = a & 0xFF;
    let hi = 0;
  ...
  private _sub8B0B(): number {
    ...
    let v = readB28(ptr.lo + (ptr.hi << 8) + idx);
  ...
  entryScenePosition(): void {
    ...
    if ((s.read(KEY_0446) & 0xFF) === 5) return;
  ...
  private _readBAB2(idx: number): number {
  ...
  entryTeamQuery(): void {
    ...
    const ptr = this._readIndirectPtr(KEY_38, KEY_39);
    const v = readB28(ptr.lo + (ptr.hi << 8));
    ...
    const v7 = readB28(ptr.lo + (ptr.hi << 8) + 7);
  ...
  private _teamQueryBranch(sel: number): void {
    const s = this._store;
    switch (sel) {
      case 0:
        ...
        {
          const ptr = this._readIndirectPtr(KEY_38, KEY_39);
          const v = readB28(ptr.lo + (ptr.hi << 8) + 8);
          this._sub895E(v);
        }
        break;
      default:
  ...
  /** $8596-$85B4: 队伍查询收尾 */
  private _teamQueryFinish(val: number): void {
    const s = this._store;
  ...
  /** $895E: 角色数据 ×$30 → 指针读取 */
  private _sub895E(a: number): void {
    const s = this._store;
    ...
    const ptr = this._readIndirectPtr(KEY_3C, KEY_3D);
    s.write(KEY_3E, readB28(ptr.lo + (ptr.hi << 8) + y));
  }

  /** $8A09: 场景坐标写入 (JSR $C50C; 读取 (0034),6/8; JSR $C539; STA $0638) */
  private _sub8A09(): void {
    const s = this._store;
  ...
  private _sub8499(x: number, y: number): number {
    const s = this._store;
  ...
  private _queryRoleAttributes(roleType: number, paramA?: number): void {
    ...
    if (name0 === 0) {
      ...
      const y = (a - 0x0B) & 0xFF;
      const rt = T_ROLE_TYPE[y] ?? 0;
      const ptr38 = this._readIndirectPtr(KEY_38, KEY_39);
      v = readB28(ptr38.lo + (ptr38.hi << 8) + rt);
    } else {
      v = name0;
    }
    ...
    lo = (lo + T_ATTR_BASE[yReg * 2]) & 0xFF;
    hi = (hi + T_ATTR_BASE[yReg * 2 + 1]) & 0xFF;
    s.write(KEY_32, lo);
    s.write(KEY_33, hi);
  ...
  private _attributeSpecial(x: number): void {
    ...
    let result = (readB28(lo + (hi << 8)) + ((offset << 1) & 0xFF)) & 0xFF;
    if (result >= 0xC0) result = 0xBF; // $8110: STY $0032 (Y=$BF)
    s.write(KEY_32, result);
    s.write(KEY_33, 0);
  }

  /** $80F9 (x!=0) 路径: (0034),3 偏移修正 */
  private _attrSpecialC(v: number, x: number): void {
    const s = this._store;
   ...
   *   (ptr),Y+3 >> 3 → ram_0443
   */
  private _loadMatchConfigBlock(cpuAddr: number, y4: number, isHome: boolean): void {
    const s = this._store;
    const base = cpuAddr + y4;
    const b0 = readB28(base);
    const b1 = readB28(base + 1);
    const b2 = readB28(base + 2);
    const b3 = readB28(base + 3);
  ...
  private _findZone(x: number, y: number): number {
    for (let i = 0; i < T_ZONE_COORD.length; i += 2) {
      if (T_ZONE_COORD[i] === x && T_ZONE_COORD[i + 1] === y) {
        return i >> 1;
      }
    }
    return 0xFF;
  ...
  private _fixedC50C(id: number): void {
  ...
  private _fixedC539(x: number, y: number): number {
    const a = (x & 0xFF) - 0x30;
    if (a < 0) return 0xFF;
    if (a >= 0xA0) return 0xFF;
    const column = a >> 3;
    const b = (y & 0xFF) - 0x50;
    if (b < 0) return 0xFF;
    if (b >= 0x60) return 0xFF;
    const row = b >> 3;
  ...
  private _fixedC509(a: number): void {
    void a;
  }
  ...
  private _readRamAbs(addr: number): number {
    return this._store.read(`ram_${addr.toString(16).toUpperCase().padStart(4, '0')}`) & 0xFF;
  }

  /** 写 RAM 绝对地址 */
  private _writeRamAbs(addr: number, v: number): void {
