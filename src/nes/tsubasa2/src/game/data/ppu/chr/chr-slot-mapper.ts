...
export interface ChrSlotMapResult {
  /** H5 8KB bank (0-15); null = 越界 (超出 H5 注册范围) */
  bank: number | null;
  /** H5 bank 内 tile 索引 */
  tile: number;
  /** 原始 1KB chunk 号 */
 ...
 */
export function mapChrSlotToH5(
  chrBanks: readonly number[],
  tile: number,
  patternTable: 0 | 1,
): ChrSlotMapResult | null {
  if (chrBanks.length < 8) return null;
 ...
 */
export function spriteAttrToPalette(attr: number): number {
