...
export interface OamShadowSlot {
  attr: number;   // $04A5+X 精灵组属性
  tileLo: number; // $04A6+X tile 低 8 位
  tileHi: number; // $04A7+X tile 高 8 位
}

/** 忙标志值 (对应 NES ram_0515) */
export const OAM_IDLE = 0;
export const OAM_BUILDING = 0x01;
export const OAM_READY = 0x80;
...
export class OamManager {
  ...
  writeBlock(offset: number, bytes: ArrayLike<number>): void {
  ...
  private _syncEntryFields(index: number): void {
    const slot = this._shadow[index];
    if (!slot || !this._entries[index]) return;
