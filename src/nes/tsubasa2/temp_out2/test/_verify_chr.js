...
const index_2 = require("../src/game/chr/index");
...
function expected1k(bank1k) {
    const b4k = Math.floor(bank1k / 4) % 32;
    const off = (bank1k % 4) * 1024;
    const chrBank = index_2.CHR_BANKS[Math.floor(b4k / 2)];
    const base = (b4k % 2) * 4096 + off;
    const out = new Uint8Array(1024);
    for (let i = 0; i < 1024; i++)
        out[i] = chrBank[base + i] ?? 0xff;
    return out;
}
function sliceEq(actual, expected, aoff) {
    for (let i = 0; i < expected.length; i++) {
        if (actual[aoff + i] !== expected[i])
            return false;
    }
    return true;
}
