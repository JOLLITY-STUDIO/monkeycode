...
// ─────────────────────────── 输入：键盘 ───────────────────────────
const KEY_TO_BUTTON = {
    ArrowUp: controller_1.default.BUTTON_UP,
    ArrowDown: controller_1.default.BUTTON_DOWN,
    ArrowLeft: controller_1.default.BUTTON_LEFT,
    ArrowRight: controller_1.default.BUTTON_RIGHT,
    KeyZ: controller_1.default.BUTTON_A,
    KeyX: controller_1.default.BUTTON_B,
    Enter: controller_1.default.BUTTON_START,
    ShiftLeft: controller_1.default.BUTTON_SELECT,
    ShiftRight: controller_1.default.BUTTON_SELECT,
};
...
// 画布下方虚拟手柄按钮
document.querySelectorAll('.gp-btn').forEach((el) => {
...
function runIntegration() {
    ...
    for (let i = 0; i < 120; i++) {
        if (i === 60)
            runtime.setButton(1, controller_1.default.BUTTON_START, true);
        if (i === 61)
            runtime.setButton(1, controller_1.default.BUTTON_START, false);
        game.frame(runtime);
    }
    const store = game.store;
...
function runInput() {
    log('step', '— 输入测试 —');
    runtime.setButton(1, controller_1.default.BUTTON_A, true);
    game.frame(runtime);
    const store = game.store;
    ...
    runtime.setButton(1, controller_1.default.BUTTON_A, false);
    game.frame(runtime);
