...
enabledAutoRun: true
---
> **v2 新架构（2026-08，强制）**：旧路径 `src/game/service/bank28_match.service.ts`、`src/game/data/prg/bank28-tables.ts` 已废弃（prg 层已被用户清空重建，旧文件不存在）。统一走新 MVC 结构：
> - 业务逻辑 → `src/game/prg/code/match/MatchConfigService.ts`（骨架 stub 已建，逐个覆盖：比赛配置表 $8B1C 链路、getMatchConfig(idx)）
> - 数据 → `src/game/prg/data/tables/match-config-table.ts`（指针表 24 项 + 各场配置字节，从 asm/bank28/*.s 与 bank7 配置提取）
