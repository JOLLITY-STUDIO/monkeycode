...
exports.MatchSceneService = void 0;
const match_scene_table_1 = require("../../data/tables/match-scene-table");
/** 4 位大写十六进制 RAM 键 */
...
class MatchSceneService {
    ...
    readPtrY(ptrLo, ptrHi, y) {
        // 指针窗口地址 → 脚本流相对索引: 窗口基址 $A000, 脚本流起点窗口 $B467 (offset $1467)
        const base = this.rdPtr(ptrLo, ptrHi);
        const idx = base - 0xA000 - match_scene_table_1.MATCH_SCENE_SCRIPT_OFFSET + y;
        // 越界返回 $FF (与 asm 未映射窗口读 $FF 一致)
        return (0, match_scene_table_1.getMatchScriptByte)(idx);
    }
    // ═══════════════════════════════════════════════════════════
    ...
    matchAuxHigh(a) {
        switch (a) {
            ...
            case 0xE6:
                this.event6();
                break; // $B235 ram_063F |= $40
            case 0xFC:
                this.eventFC();
                break; // $B333 ram_0515 = $80
            default: break;
        }
        // $B15D: JMP $B339 (指针推进)
        this.advancePointer();
    ...
    advancePointer() {
        ...
        if (sum > 0xFF)
            this.wr(0x0089, (this.rd(0x0089) + 1) & 0xFF);
        this.wr(0x008A, 0x00);
    }
    // ═══════════════════════════════════════════════════════════
    ...
    subB246() {
        ...
        for (let i = 0; i < 0x20; i++)
            this.wr(0x0408 + i, this.rd(0x046F + i));
        this.sub9310();
    ...
    sub92A8() {
        ...
        while ((step & 0x80) === 0) {
            ...
            for (let x = 0; x < 0x20; x++) { // $92B1-$92CB
                if ((x & 0x03) === 0)
                    continue; // $92B6: BEQ $92C8 (跳过 X&3==0)
                let nv = (this.rd(0x046F + x) & 0x0F) | step; // $92B8-$92BD
                if (step === 0)
                    nv = 0x0F; // $92C3: step==0 → $0F
    ...
    sub9405(a) {
        ...
        while (this.rd(0x008B) < 4) {
            ...
            if (lo > 0xFF)
                this.wr(0x008B, (this.rd(0x008B) + 1) & 0xFF);
        }
    }
    // ═══════════════════════════════════════════════════════════
...
exports.MatchSceneService = MatchSceneService;
exports.default = MatchSceneService;
