...
const bank00_tables_2 = require("../../data/tables/bank00-tables");
const scene_loader_tables_1 = require("../../data/scene/scene-loader-tables");
const bank09_raw_1 = require("../../data/scene/bank09-raw");
const bank10_raw_1 = require("../../data/scene/bank10-raw");
const ScriptEngine_1 = require("../story/ScriptEngine");
/** 4 位大写十六进制 RAM 键 */
...
class GameSystemService {
    constructor(store) {
        ...
        this._store = store;
        this._scriptEngine = new ScriptEngine_1.ScriptEngine(store);
    ...
    *sub82ECGen() {
        ...
        const idx = c & 0x7f;
        const stream = scene_loader_tables_1.BANK6_B800_STREAMS[idx] ?? [];
        if (stream.length === 0) {
            this.wr(0x004C, 0);
            yield this.coroutineYield(1);
            return;
    ...
    sub9085() {
        ...
        for (let s = 0; s < segCount; s++) {
            ...
            const table = bank === 9 ? scene_loader_tables_1.BANK9_SCENE_PTR_TABLE : scene_loader_tables_1.BANK10_SCENE_PTR_TABLE;
            const dataPtr = table[idx] ?? 0;
            // $90E4: 复制 32B 模板 $978B 到 buffer
            for (let i = 0; i < 0x20; i++)
                this.wr(bufPtr + i, bank00_tables_2.TEXT_BUFFER_TEMPLATE_978B[i]);
