...
const palette_table_js_1 = __importDefault(require("./palette-table.js"));
const vram_store_js_1 = require("./vram-store.js");
const oam_store_js_1 = require("./oam-store.js");
const scroll_store_js_1 = require("./scroll-store.js");
const scroll_counters_js_1 = require("./scroll-counters.js");
const bit = __importStar(require("../bitfield.js"));
class PPU {
    constructor(nes) {
        // Status flags:
        ...
        this.vramStore = new vram_store_js_1.VramStore();
        // OAM / SPR-RAM (spriteMem + sramAddress)
        this.oamStore = new oam_store_js_1.OamStore();
        // 滚动寄存器 (regFV/regV/regH/regVT/regHT/regFH/regS)
        this.scrollStore = new scroll_store_js_1.ScrollStore();
        // 渲染滚动计数器 (cntFV/cntV/cntH/cntVT/cntHT)
        this.counters = new scroll_counters_js_1.ScrollCounters();
    ...
    writeVRAMAddress(address) {
        if (this.firstWrite) {
            ...
            this.scrollStore.set("v_tile", (this.scrollStore.get("v_tile") & 7) | (bit.lowBits(address, 2) << 3)); // regVT hi
        }
        else {
            this.triggerRendering();
            this.scrollStore.set("v_tile", (this.scrollStore.get("v_tile") & 24) | bit.bits(address, 5, 3)); // regVT lo
    ...
    vramLoad() {
        ...
        if (this.vramStore.addr <= 0x3eff) {
            ...
            if (this.vramStore.addr < 0x2000) {
                this.vramStore.bufferedReadValue = this.vramStore.get(this.vramStore.addr);
            }
            else {
                this.vramStore.bufferedReadValue = this.mirroredLoad(this.vramStore.addr);
            }
            // Mapper latch access:
            if (this.vramStore.addr < 0x2000) {
                this.nes.mmap.latchAccess(this.vramStore.addr);
        ...
        this.vramStore.bufferedReadValue = this.mirroredLoad(this.vramStore.addr & 0x2FFF);
        this._incrementVramAddress();
        this.cntsFromAddress();
        this.regsFromAddress();
        return tmp;
        ...
        if (scan < 240 && scan - cnt.fineV >= 0) {
            ...
            for (let tile = 0; tile < 32; tile++) {
                if (scan >= 0) {
                    ...
                    let tileIndex = nameTable[this.curNt].getTileIndex(cnt.tileH, cnt.tileV);
                    ...
                    if (this.validTileData) {
                        ...
                        if (mmap.bgTileOverride) {
                            let override = mmap.getBgTileData(baseTile, tileIndex, cnt.tileH, cnt.tileV);
                            if (override) {
                                t = override.tile;
                                tpix = t.pix;
                                att = override.attrib;
    ...
    performOAMCorruption() {
        ...
        if (!renderingEnabled)
            return;
        if (!this.oamStore.isAddrNonZero())
            return;
        ...
        for (let scan = startscan; scan < startscan + scancount; scan++) {
            ...
            for (let i = 0; i < count; i++) {
                ...
                if (this.f_spriteSize === 0) {
                    ...
                    if (fineY < 0 || fineY >= 8)
                        continue;
                    ptTile[tileIndex].render(buffer, 0, fineY, 8, fineY + 1, sprX, dy, palAdd, sprPalette, horiFlip, vertFlip, i, // priority: lower index in secondary OAM = higher priority
                    pixrendered);
    ...
    checkSprite0(scan) {
        ...
        if (this.f_spriteSize === 0) {
            ...
            if (y <= scan && y + 16 > scan && x < 256) {
                ...
                if (toffset < 8) {
                    t = mmap.getSpritePatternTile(sprTile + (vertFlip ? 1 : 0) + (bit.bit0(sprTile) ? 255 : 0));
                }
                else {
                    t = mmap.getSpritePatternTile(sprTile + (vertFlip ? 0 : 1) + (bit.bit0(sprTile) ? 255 : 0));
    ...
    updatePalettes() {
        ...
        for (i = 0; i < 16; i++) {
            if (this.f_dispType === 0) {
                this.imgPalette[i] = this.palTable.getEntry(bit.lowBits(this.vramStore.get(0x3f00 + i), 6));
            }
            else {
                this.imgPalette[i] = this.palTable.getEntry(bit.bits(this.vramStore.get(0x3f00 + i), 4, 2));
            }
        }
        for (i = 0; i < 16; i++) {
            if (this.f_dispType === 0) {
                this.sprPalette[i] = this.palTable.getEntry(bit.lowBits(this.vramStore.get(0x3f10 + i), 6));
            }
            else {
                this.sprPalette[i] = this.palTable.getEntry(bit.bits(this.vramStore.get(0x3f10 + i), 4, 2));
            }
        }
    }
    ...
    patternWrite(address, value) {
        ...
        if (leftOver < 8) {
            this.ptTile[tileIndex].setScanline(leftOver, value, this.vramStore.get(address + 8));
        }
        else {
            this.ptTile[tileIndex].setScanline(leftOver - 8, this.vramStore.get(address - 8), value);
        }
    }
