...
const palette_fade_table_1 = require("./prg/data/tables/palette-fade-table");
// ═══════════════════════════════════════════════════════════
...
function writePalettes(store, ppu, paletteTable) {
    ...
    if (ramPalNonZero) {
        ...
        for (let i = 0; i < 0x10; i++) {
            ...
            ppu.writeMem(0x3f00 + i, (0, palette_fade_table_1.fadePalette)(pal, fadeBg));
        }
        for (let i = 0; i < 0x10; i++) {
            const key = 'ram_0' + (0x63A + i).toString(16).toUpperCase().padStart(3, '0');
            const pal = store.read(key) & 0x3f;
            ppu.writeMem(0x3f10 + i, (0, palette_fade_table_1.fadePalette)(pal, fadeSpr));
        }
        return;
    }
...
class Tsubasa2 {
    ...
    frame(nes) {
        ...
        try {
            ...
            console.error('writeStoreToPpu error at frame ' + this._frame + ': ' + e.message);
            throw e;
        }
