...
class PlayerQueryService {
    ...
    _readPlayerStat(playerId, field) {
    ...
    _lookupValue16(index) {
        const i = index & 0xff;
        if (i < player_table_1.STAMINA_TABLE_16BIT.length) {
            return player_table_1.STAMINA_TABLE_16BIT[i];
        }
        return 0;
    }
    /**
    ...
    _lookupIndex16(value) {
        ...
        do {
            x -= 2;
            const i = x >> 1;
            if (i < player_table_1.STAMINA_TABLE_16BIT.length && value <= player_table_1.STAMINA_TABLE_16BIT[i]) {
                return i;
            }
        } while (x > 0);
        return 0;
    ...
    entry6_VramBuf1() {
        ...
        const ptr = statIdx < player_table_1.PLAYER_STAT_TABLE_16BIT.length
            ? player_table_1.PLAYER_STAT_TABLE_16BIT[statIdx]
            : 0;
    ...
    sub81E0() {
        ...
        let ea = 0x01;
        while (ea > 0) {
            if ((this._store.read('ram_001C') & 0x0F) === 0)
                break;
            ea = (ea - 1) & 0xFF;
        }
    }
    ...
    sub8509_PlayerDataLoop() {
        ...
        let x = 0xF0;
        while (x !== 0) {
            if ((this._store.read('ram_001E') & 0x80) !== 0)
                break;
            x = (x - 1) & 0xFF;
        }
        // JSR $99F0; JSR $98A0 (清屏)
        ...
        if (e4 >= teamId) {
            ...
            if (this._store.read('ram_00EC') === 0)
                return;
            ...
            // 队伍 6/12/16: JMP $8610
            if (teamId === 0x06 || teamId === 0x0C || teamId === 0x10)
                return;
            if (this._store.read('ram_00EC') === 0)
                return;
            this._store.write('ram_00ED', 0x00);
...
exports.default = PlayerQueryService;
