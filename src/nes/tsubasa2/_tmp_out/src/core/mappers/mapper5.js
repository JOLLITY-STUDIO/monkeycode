...
class Mapper5 extends mapper0_1.default {
    constructor(nes) {
        super(nes);
        this.prgMode = 3;
    ...
    _readPrgRom32k(bank32k, offset) {
        let bank16k = (bank32k * 2 + Math.floor(offset / 0x4000)) % this.nes.rom.romCount;
    ...
    _syncNametables() {
        let ppu = this.nes.ppu;
        let fillAttrByte = this.fillAttr |
            (this.fillAttr << 2) |
            (this.fillAttr << 4) |
            (this.fillAttr << 6);
        for (let i = 0; i < 960; i++) {
        ...
        if (this.exramMode >= 2) {
            ...
            (0, utils_1.copyArrayElements)(this.exram, 0, ppu.vramMem, 0x2800, 0x400);
        }
