...
exports.runInputTests = runInputTests;
exports.runPerformanceTests = runPerformanceTests;
exports.runEdgeTests = runEdgeTests;
const index_1 = require("../src/index");
const DataStore_1 = require("../src/game/prg/data/DataStore");
const Renderer_1 = require("../src/core/engine/render/Renderer");
const bank00_core_1 = require("../src/game/prg/code/bank00/bank00_core");
const bank12_audio_1 = require("../src/game/prg/code/bank12_audio");
const bank30_init_1 = require("../src/game/prg/code/bank30_init");
const bank02_scene_1 = require("../src/game/prg/code/bank02_scene");
const bank16_skills_1 = require("../src/game/prg/code/bank16_skills");
const DataStore_2 = require("../src/game/prg/DataStore");
...
function record(suite, name, pass, detail) {
    ...
    (0, utils_1.log)(`[${suite}] ${pass ? '✓' : '✗'} ${name}${detail ? ' — ' + detail : ''}`, pass ? 'pass' : 'fail');
}
function getResults() {
...
async function runUnitTests(ctx) {
    ...
    {
        const oam = new DataStore_2.OamManager();
        oam.reset();
    ...
    {
        const store = new DataStore_1.DataStore();
        const bank00 = new bank00_core_1.Bank00Service(store);
    ...
    {
        const store = new DataStore_1.DataStore();
        const audio = new bank12_audio_1.Bank12AudioService(store);
    ...
    // 1.6 Bank30Service / Bank02Service
    {
        const store = new DataStore_1.DataStore();
        const bank00 = new bank00_core_1.Bank00Service(store);
        const bank02 = new bank02_scene_1.Bank02Service(store, bank00);
        const bank16 = new bank16_skills_1.Bank16Service(store);
        const bank30 = new bank30_init_1.Bank30Service(store, bank00, bank02, bank16);
        let crashed = false;
        try {
            bank30.init();
        }
    ...
    // 1.7 类型常量
    {
        record('单元-常量', 'NES_WIDTH=256', types_1.NES_WIDTH === 256, `${types_1.NES_WIDTH}`);
        record('单元-常量', 'NES_HEIGHT=240', types_1.NES_HEIGHT === 240, `${types_1.NES_HEIGHT}`);
        record('单元-常量', 'BUTTON.A=1', types_1.BUTTON.A === 1, `${types_1.BUTTON.A}`);
        record('单元-常量', 'BUTTON.START=8', types_1.BUTTON.START === 8, `${types_1.BUTTON.START}`);
        record('单元-常量', 'BUTTON.RIGHT=128', types_1.BUTTON.RIGHT === 128, `${types_1.BUTTON.RIGHT}`);
        // BUTTON 枚举完整性
        const allBtns = types_1.BUTTON.A | types_1.BUTTON.B | types_1.BUTTON.SELECT | types_1.BUTTON.START | types_1.BUTTON.UP | types_1.BUTTON.DOWN | types_1.BUTTON.LEFT | types_1.BUTTON.RIGHT;
        record('单元-常量', '按键位掩码不重叠', allBtns === 0xFF, `mask=0x${allBtns.toString(16)}`);
    }
    // 1.8 ScriptVM 脚本虚拟机
    {
        const { ScriptVM } = await Promise.resolve().then(() => __importStar(require('../src/game/prg/code/bank00/script-vm')));
        const { getScriptData } = await Promise.resolve().then(() => __importStar(require('../src/game/prg/code/bank00/script-data-loader')));
        // 脚本 0x00 存在性
    ...
    (0, utils_1.log)('━━━ 单元测试完成 ━━━', 'step');
}
// ════════════════════════════════════════════════════════════
// 2. 集成测试 — 完整引擎启动与运行
// ════════════════════════════════════════════════════════════
async function runIntegrationTests(ctx) {
    (0, utils_1.log)('━━━ 集成测试开始 ━━━', 'step');
    // 2.1 引擎完整构造
    let game = null;
    let crashed = false;
    let errMsg = '';
    ...
    if (!game) {
        (0, utils_1.log)('引擎构造失败，跳过后续集成测试', 'fail');
        return;
    }
    // 2.2 启动引擎
    crashed = false;
    ...
    // 2.4 运行若干帧，验证不崩溃
    (0, utils_1.log)('运行 60 帧（~1秒）观察稳定性...', 'info');
    ...
    (0, utils_1.screenshot)('集成-开场画面(1秒)');
    await (0, utils_1.sleep)(100);
    const imgData = ctx.ctx.getImageData(0, 0, types_1.NES_WIDTH, types_1.NES_HEIGHT);
    ...
    const totalPixels = types_1.NES_WIDTH * types_1.NES_HEIGHT;
    ...
    // 2.7 场景流转 — 检查 boot_root
    {
        ...
        (0, utils_1.log)(`场景流转验证: 当前状态=${dbg2.gameStateName}, frame=${dbg2.frame}`, 'info');
        record('集成-场景', 'OPENING 状态保持', dbg2.gameStateName === 'OPENING', '开场阶段');
    }
    // 2.8 OpeningSceneController 脚本驱动模式集成测试
    {
        const { OpeningSceneController } = await Promise.resolve().then(() => __importStar(require('../src/game/prg/code/bank00/scene_opening.controller')));
        const { OpeningShot } = await Promise.resolve().then(() => __importStar(require('../src/game/prg/data/scene/index')));
        const store = new DataStore_1.DataStore();
        ...
        (0, utils_1.log)(`脚本状态: textLines=${lastState.scriptTextLines.length}, lastInstr="${lastState.scriptLastInstr}"`, 'info');
        // 开场推进测试 — 逐镜头推进直到开场完成 (进入标题由 BootService 流转)
        let guard = 0;
        while (!ctrl.complete && guard++ < 200)
            ctrl.nextShot();
        const titleState = ctrl.getDisplayState();
        record('集成-OpeningScript', '逐镜头推进至开场完成', ctrl.complete, `shot=${titleState.shot}, guard=${guard}`);
        // 截图记录脚本驱动状态
        (0, utils_1.screenshot)('集成-OpeningScript-脚本驱动状态');
    }
    (0, utils_1.log)('━━━ 集成测试完成 ━━━', 'step');
}
// ════════════════════════════════════════════════════════════
// 3. 输入测试
// ════════════════════════════════════════════════════════════
async function runInputTests(ctx) {
    (0, utils_1.log)('━━━ 输入测试开始 ━━━', 'step');
    const game = ctx.game;
    if (!game) {
        (0, utils_1.log)('引擎未初始化，跳过输入测试', 'warn');
    ...
    record('输入-按键', 'pressButton(A) 设置位', (btnA & types_1.BUTTON.A) !== 0, `buttons=0x${btnA.toString(16)}`);
    game.releaseButton('A');
    const btnA2 = game.getButtons();
    record('输入-按键', 'releaseButton(A) 清除位', (btnA2 & types_1.BUTTON.A) === 0, `buttons=0x${btnA2.toString(16)}`);
    // 3.2 多键组合
    game.pressButton('A');
    game.pressButton('B');
    game.pressButton('START');
    const combo = game.getButtons();
    const expected = types_1.BUTTON.A | types_1.BUTTON.B | types_1.BUTTON.START;
    ...
    (0, utils_1.screenshot)('输入-按START后画面');
    game.releaseButton('START');
    await (0, utils_1.sleep)(200);
    // 3.5 快速连按（防抖/中断测试）
    (0, utils_1.log)('快速连按 A 键 10 次...', 'info');
    for (let i = 0; i < 10; i++) {
        game.pressButton('A');
    ...
    (0, utils_1.screenshot)('输入-连按A后画面');
    (0, utils_1.log)('━━━ 输入测试完成 ━━━', 'step');
}
// ════════════════════════════════════════════════════════════
// 4. 性能测试
// ════════════════════════════════════════════════════════════
async function runPerformanceTests(ctx) {
    (0, utils_1.log)('━━━ 性能测试开始 ━━━', 'step');
    const game = ctx.game;
    if (!game) {
        (0, utils_1.log)('引擎未初始化，跳过性能测试', 'warn');
        return;
    }
    // 4.1 FPS 稳定性 — 采样 3 秒
    (0, utils_1.log)('采样 3 秒 FPS...', 'info');
    const samples = [];
    ...
    (0, utils_1.log)(`FPS 采样: avg=${avgFps.toFixed(1)} min=${minFps} max=${maxFps} samples=[${samples.join(',')}]`, 'info');
    ...
    if (performance.memory) {
        ...
        (0, utils_1.log)(`内存: used=${(mem.usedJSHeapSize / 1048576).toFixed(1)}MB total=${(mem.totalJSHeapSize / 1048576).toFixed(1)}MB`, 'info');
    ...
    (0, utils_1.screenshot)('性能-3秒运行后画面');
    (0, utils_1.log)('━━━ 性能测试完成 ━━━', 'step');
}
// ════════════════════════════════════════════════════════════
// 5. 边界测试
// ════════════════════════════════════════════════════════════
async function runEdgeTests(ctx) {
    (0, utils_1.log)('━━━ 边界测试开始 ━━━', 'step');
    // 5.1 重复 start() — 应被忽略
    {
        const game = new index_1.Tsubasa2(ctx.ctx);
        let crashed = false;
        try {
            game.start(ctx.canvas);
    ...
    // 5.7 长时间运行（快速 2 秒）
    {
        const game = ctx.game;
        if (game) {
            (0, utils_1.log)('额外运行 2 秒验证稳定性...', 'info');
            const f1 = game.getDebugInfo().frame;
            await (0, utils_1.sleep)(2000);
            const f2 = game.getDebugInfo().frame;
            record('边界-长时运行', '2秒后帧继续推进', f2 > f1, `${f2 - f1} 帧推进`);
            (0, utils_1.screenshot)('边界-长时运行画面');
        }
    }
    (0, utils_1.log)('━━━ 边界测试完成 ━━━', 'step');
}
