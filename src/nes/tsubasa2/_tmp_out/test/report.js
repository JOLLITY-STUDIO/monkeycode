...
exports.KNOWN_BUGS = void 0;
exports.generateReport = generateReport;
exports.exportReportMarkdown = exportReportMarkdown;
...
exports.KNOWN_BUGS = [
    {
        ...
        line: '39-58',
        description: 'tsubasa2-h5-src 被设计为独立、无外部依赖的 src，但 Tsubasa2.ts 第 39-58 行 ' +
    ...
    },
    {
        ...
        description: 'OamManager._ensure 对越界索引 (index<0 或 index>=64) 仅打印 console.warn 后直接 return，' +
    ...
    },
    {
        ...
        line: '显示状态计算',
        description: '开场控制器在计算显示文本时，若某些数值字段为 undefined，字符串拼接会产生 "undefined" 字样，' +
            '而非预期的数字。属于数据初始化时序问题。',
    ...
    },
    {
        ...
        line: 'include',
        description: 'tsconfig.check.json 的 include 包含 "../rom-data/**/*.ts"，使类型检查依赖外部目录，' +
    ...
    },
    {
        ...
        line: 'import 语句',
        description: '除 Tsubasa2.ts 外，还有至少 8 个 Bank 服务/数据文件通过 "../../../rom-data/prg-bank-XX" 引用外部 PRG Bank 原始字节。' +
    ...
    },
    {
        ...
        line: '209-211',
        description: 'DataStore.write(key, value) 会执行 value & 0xFF 截断为 8 位（NES 硬件兼容），' +
...
function generateReport() {
    const results = (0, test_suite_1.getResults)();
    const shots = (0, utils_1.getShots)();
...
<tr><td>已知 Bug</td><td>${exports.KNOWN_BUGS.length} (${exports.KNOWN_BUGS.filter(b => b.severity === 'Critical').length} Critical)</td></tr>
</table>
`;
    // 套件明细
    html += '<h3>二、测试套件明细</h3>';
    ...
    for (const b of exports.KNOWN_BUGS) {
        html += `<tr>
      <td>${b.id}</td>
    ...
    if (pane)
        pane.innerHTML = html;
    (0, utils_2.log)(`报告已生成: ${total} 用例, 通过率 ${passRate}%, ${exports.KNOWN_BUGS.length} 已知Bug`, 'step');
}
...
function exportReportMarkdown() {
    ...
    md += `| 指标 | 值 |\n|---|---|\n`;
    md += `| 测试用例总数 | ${total} |\n| 通过 | ${totalPass} |\n| 失败 | ${totalFail} |\n| 通过率 | ${passRate}% |\n`;
    md += `| 截图数量 | ${shots.length} |\n| 已知 Bug | ${exports.KNOWN_BUGS.length} |\n\n`;
    md += `## 二、测试用例明细\n\n`;
    const suites = new Map();
    for (const r of results) {
        if (!suites.has(r.suite))
    ...
    (0, utils_2.log)('报告已导出为 Markdown 文件', 'pass');
}
