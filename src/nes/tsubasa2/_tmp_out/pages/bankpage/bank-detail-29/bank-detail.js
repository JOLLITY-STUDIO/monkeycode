...
const bank29_analysis_1 = __importDefault(require("../bank-detail/bank29_analysis"));
...
Page({
    ...
    onLoad() {
        ...
        for (let i = 0; i < BANK_SIZE; i++)
            bankData.push(index_1.NES_PRG_ROM[offset + i]);
        this.setData({
    ...
    _parseRoster(data) {
        ...
        while (off + 8 < FF_FILL_OFF) {
            // 跳过前导分隔字节 (00 xx)
            while (off < FF_FILL_OFF && data[off] === 0x00)
                off++;
            if (off >= FF_FILL_OFF)
                break;
            if (data[off] === 0xFF)
    ...
    _buildStructureInfo() {
        this.setData({
            structureInfo: {
                blockCount: bank29_analysis_1.default.structure.blockCount,
                ...
                loaders: bank29_analysis_1.default.loaders,
                consumers: bank29_analysis_1.default.consumers,
            },
        });
    },
});
