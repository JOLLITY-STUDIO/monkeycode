...
for (let y = 0; y < 240; y++) {
  ...
  for (let x = 0; x < 256; x++) {
    ...
    const hv = h5[y * 256 + x] & 0xffffff;
    if (ev !== hv) { diff++; badX.push(x); }
  }
  if (diff > 0) {
    total += diff;
    lines.push(`y${y}: diff=${diff}`);
    let eLine = '  E: ';
    let hLine = '  H: ';
    let mLine = '  M: ';
    for (let x = 0; x < 64; x++) {
      ...
      const ev = (emu.data[p] << 16) | (emu.data[p + 1] << 8) | emu.data[p + 2];
      const hv = h5[y * 256 + x] & 0xffffff;
      eLine += (ev === 0 ? '....' : ev.toString(16).padStart(4, '0')) + ' ';
      hLine += (hv === 0 ? '....' : hv.toString(16).padStart(4, '0')) + ' ';
      mLine += (ev === hv ? '== ' : (ev === 0 ? 'E0 ' : hv === 0 ? 'H0 ' : 'XX '));
    }
    lines.push(mLine);
    lines.push(eLine);
    lines.push(hLine);
...
lines.push(`total diff pixels=${total}`);
fs.writeFileSync('_diag_rows_out.txt', lines.join('\n'));
console.log('done');
