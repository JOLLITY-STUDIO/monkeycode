import { findPlayerById, findPlayerNameById, PLAYER_TABLE } from '../src/game/prg/data/tables/player-table';
import { findLevelByExp, LEVEL_UP_TABLE } from '../src/game/prg/data/tables/levelup-table';
import { findTeamById, findTeamNameById, findRosterById } from '../src/game/prg/data/tables/team-table';
...
function API_PLAYERS() {
    ...
    lines.push(row({ s: 'ID', w: 6 }, { s: 'NAME (en/ja/zh)', w: 28 }, { s: 'STM', w: 4, a: 'right' }, { s: 'SHOT', w: 4, a: 'right' }, { s: 'PASS', w: 4, a: 'right' }, { s: 'DRB', w: 4, a: 'right' }, { s: 'BLK', w: 4, a: 'right' }, { s: 'TKL', w: 4, a: 'right' }, { s: 'ITC', w: 4, a: 'right' }, { s: 'CLUB', w: 5, a: 'right' }, { s: 'POS', w: 4 }));
    lines.push(sep2);
    let totalCount = 0;
    for (const p of PLAYER_TABLE) {
        ...
        lines.push(row({ s: '0x' + id.toString(16).padStart(2, '0').toUpperCase(), w: 6 }, { s: nameTrilingual, w: 28 }, { s: (p.stamina ?? 0).toString(), w: 4, a: 'right' }, { s: (p.shot ?? 0).toString(), w: 4, a: 'right' }, { s: (p.pass ?? 0).toString(), w: 4, a: 'right' }, { s: (p.dribble ?? 0).toString(), w: 4, a: 'right' }, { s: (p.block ?? 0).toString(), w: 4, a: 'right' }, { s: (p.tackle ?? 0).toString(), w: 4, a: 'right' }, { s: (p.intercept ?? 0).toString(), w: 4, a: 'right' }, { s: (p.club ?? 0).toString(), w: 5, a: 'right' }, { s: pos, w: 4 }));
    }
    lines.push(sep2);
...
function API_PLAYER_DETAIL(id) {
    ...
    if (!p)
        return `Player 0x${id.toString(16).padStart(2, '0')} NOT FOUND`;
    const lines = [];
    const pid = p.id ?? 0;
    const club = p.club ?? 0;
    const pos = (p.position ?? 0) === 1 ? 'GK' : 'FW';
...
function API_TEAM(teamId) {
    ...
    if (!team)
        return `Team 0x${teamId.toString(16).padStart(2, '0')} NOT FOUND`;
    ...
    lines.push(sep2);
    lines.push(row({ s: '#', w: 3 }, { s: 'ID', w: 6 }, { s: 'NAME', w: 14 }, { s: 'POS', w: 4 }, { s: 'SHOT', w: 4, a: 'right' }, { s: 'PASS', w: 4, a: 'right' }, { s: 'DRB', w: 4, a: 'right' }, { s: 'BLK', w: 4, a: 'right' }, { s: 'TKL', w: 4, a: 'right' }, { s: 'ITC', w: 4, a: 'right' }, { s: 'STM', w: 4, a: 'right' }));
    lines.push(sep2);
    ids.forEach((pid, i) => {
        const p = findPlayerById(pid);
        if (!p) {
            lines.push(row({ s: (i + 1).toString(), w: 3 }, { s: '0x' + pid.toString(16).padStart(2, '0').toUpperCase(), w: 6 }, { s: '???', w: 14 }, { s: '-', w: 4 }, { s: '-', w: 4, a: 'right' }, { s: '-', w: 4, a: 'right' }, { s: '-', w: 4, a: 'right' }, { s: '-', w: 4, a: 'right' }, { s: '-', w: 4, a: 'right' }, { s: '-', w: 4, a: 'right' }, { s: '-', w: 4, a: 'right' }));
            return;
        }
        const pos = (p.position ?? 0) === 1 ? 'GK' : 'FW';
        lines.push(row({ s: (i + 1).toString(), w: 3 }, { s: '0x' + pid.toString(16).padStart(2, '0').toUpperCase(), w: 6 }, { s: p.name ?? '?', w: 14 }, { s: pos, w: 4 }, { s: (p.shot ?? 0).toString(), w: 4, a: 'right' }, { s: (p.pass ?? 0).toString(), w: 4, a: 'right' }, { s: (p.dribble ?? 0).toString(), w: 4, a: 'right' }, { s: (p.block ?? 0).toString(), w: 4, a: 'right' }, { s: (p.tackle ?? 0).toString(), w: 4, a: 'right' }, { s: (p.intercept ?? 0).toString(), w: 4, a: 'right' }, { s: (p.stamina ?? 0).toString(), w: 4, a: 'right' }));
    });
    return lines.join('\n');
}
...
function API_MATCH_CONFIG(home, away) {
    ...
    lines.push(row({ s: 'H', w: 5 }, { s: 'A', w: 5 }, { s: 'TOURNAMENT', w: 14 }, { s: 'HALF', w: 5, a: 'right' }, { s: 'TOTAL', w: 6, a: 'right' }, { s: 'SUB', w: 4, a: 'right' }, { s: 'INJ', w: 4, a: 'right' }, { s: 'ET', w: 3 }));
    lines.push(sep2);
    for (const c of MATCH_CONFIG_TABLE) {
        if (!c)
            continue;
        lines.push(row({ s: '0x' + c.homeTeam.toString(16).padStart(2, '0').toUpperCase(), w: 5 }, { s: '0x' + c.awayTeam.toString(16).padStart(2, '0').toUpperCase(), w: 5 }, { s: c.tournament, w: 14 }, { s: c.halfLength.toString(), w: 5, a: 'right' }, { s: c.durationMinutes.toString(), w: 6, a: 'right' }, { s: c.maxSubstitutions.toString(), w: 4, a: 'right' }, { s: c.injuryTime.toString(), w: 4, a: 'right' }, { s: c.extraTime ? 'Y' : '-', w: 3 }));
    }
    return lines.join('\n');
}
function API_SKILLS(playerId) {
    ...
    lines.push(sep2);
    lines.push(row({ s: '#', w: 3 }, { s: 'ID', w: 6 }, { s: 'NAME', w: 22 }, { s: 'POWER', w: 7, a: 'right' }, { s: 'POWER BAR', w: 36 }));
    lines.push(sep2);
    ids.forEach((sid, i) => {
        const sk = findSkillByMoveId(sid);
        if (!sk)
        ...
        lines.push(row({ s: (i + 1).toString(), w: 3 }, { s: '0x' + sid.toString(16).padStart(2, '0').toUpperCase(), w: 6 }, { s: sk.name ?? '?', w: 22 }, { s: power.toString(), w: 7, a: 'right' }, { s: bar, w: 36 }));
    });
    lines.push(sep2);
