...
import { findPlayerById, findPlayersByTeam, PLAYER_TABLE } from '../src/game/prg/data/tables/player-table';
import { findLevelByExp, findLevelById } from '../src/game/prg/data/tables/levelup-table';
import { findTeamById, findRosterById, TEAMS_FULL } from '../src/game/prg/data/tables/team-table';
...
// ─────────────────────────── 视图 1: 球员列表 ───────────────────────────
export function renderPlayerList(ctx) {
    // 黑色背景（768x720 内部画布）
...
// ─────────────────────────── 视图 3: 球队名单 ───────────────────────────
export function renderTeamView(ctx) {
    ctx.fillStyle = BG_DARK;
    ctx.fillRect(0, 0, 256, 240);
    // 头部（绿色）
...
// ─────────────────────────── 视图 4: 等级升级 ───────────────────────────
export function renderLevelUp(ctx) {
    ctx.fillStyle = BG_DARK;
    ctx.fillRect(0, 0, 256, 240);
    // 头部（红色）
...
// ─────────────────────────── 视图 5: 比赛配置 ───────────────────────────
export function renderMatchConfig(ctx) {
    ctx.fillStyle = BG_DARK;
    ctx.fillRect(0, 0, 256, 240);
    // 头部
    ctx.fillStyle = C(0x1A);
...
if (typeof document !== 'undefined') {
    ...
    if (canvases.list)
        renderPlayerList(canvases.list.getContext('2d'));
    if (canvases.detail)
        renderPlayerDetail(canvases.detail.getContext('2d'));
    if (canvases.team)
        renderTeamView(canvases.team.getContext('2d'));
    if (canvases.levelup)
        renderLevelUp(canvases.levelup.getContext('2d'));
    if (canvases.match)
        renderMatchConfig(canvases.match.getContext('2d'));
    if (canvases.skills)
        renderSkills(canvases.skills.getContext('2d'));
