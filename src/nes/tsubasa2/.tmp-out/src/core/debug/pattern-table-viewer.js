 ...
 */
export function renderPatternTable(nes, tableIdx, paletteOffset = 0, paletteSrc) {
    const ppu = nes.ppu;
 ...
 */
export function renderBothPatternTables(nes, paletteOffset = 0, palT0, palT1) {
    return {
...
export function pushChrSwitch(rec) {
    chrSwitchLog.push(rec);
}
/** 取并清空切换日志（dump 工具用） */
export function drainChrSwitchLog() {
    const out = chrSwitchLog.slice();
    chrSwitchLog.length = 0;
    return out;
}
/** 按 scanline 范围取切换日志（不消费） */
export function getChrSwitchesInRange(scanStart, scanEnd) {
...
export function buildFinalChrBankMap(switches, initialBanks) {
 ...
 */
export function buildChrBankMapByScanline(switches, initialBanks) {
    const out = new Map();
 ...
 */
export function renderPatternTableAtScanline(nes, tableIdx, slotBanks, paletteOffset = 0) {
    const ppu = nes.ppu;
    const buf = new Uint32Array(128 * 128);
...
export function renderBothPatternTablesAtScanline(nes, slotBanks, paletteOffset = 0) {
    return {
 ...
 */
export function generatePTDataText(nes, frameCount) {
    const ppu = nes.ppu;
    if (!ppu)
        return '';
