 ...
 */
export function disassemble(addr, memRead) {
    ...
    const h2 = (b) => (b & 0xff).toString(16).toUpperCase().padStart(2, '0');
...
export function disassembleRange(startAddr, lineCount, memRead) {
    const entries = [];
 ...
 * 格式: N-V-U-B-D-I-Z-C
 */
export function flagsToString(p) {
    const n = (p & 0x80) ? 'N' : 'n';
    const v = (p & 0x40) ? 'V' : 'v';
