...
export const FCEUX_DEFAULT_OPTIONS = LOG_REGISTERS | LOG_PROCESSOR_STATUS | LOG_TO_THE_LEFT | LOG_CODE_TABBING;
/**
...
function formatInstruction(ctx, instrPC, opcode, opinfo, opbytes) {
    ...
    const disasmBase = disassemble(instrPC, (a) => readMemRaw(cpu, a));
    ...
    if (ctx.opts.format === 'fceux') {
        ...
        let line = '';
        if (opt & LOG_FRAMES_COUNT)
            line += `f${frame}`.padEnd(8);
        if (opt & LOG_CYCLES_COUNT)
            line += `c${cycle}`.padEnd(13);
        if (opt & LOG_INSTRUCTIONS_COUNT)
...
export class Tracer {
    ...
    trace(instrPC, opcode, opinfo, opbytes) {
        ...
        if (cdlOpt) {
            ...
            const newSomething = ((ctx.fceuxOptions & LOG_NEW_INSTRUCTIONS) && isNewCode) ||
                ((ctx.fceuxOptions & LOG_NEW_DATA) && isNewData);
            if (!newSomething) {
                ctx.unloggedLines++;
                return;
            }
            if (ctx.fceuxOptions & LOG_NEW_INSTRUCTIONS)
                ctx.cdlCode.add(codeKey);
            if (dataKey >= 0 && (ctx.fceuxOptions & LOG_NEW_DATA))
