...
const OP = {
  ...
  0x46: ['LSR', 'zp'], 0xc1: ['CMP', 'indx'], 0xc5: ['CMP', 'zp'],
  0xce: ['DEC', 'abs'], 0xc6: ['DEC', 'zp'], 0xe0: ['CPX', 'imm'],
  0xe4: ['CPX', 'zp'], 0xec: ['CPX', 'abs'], 0xc0: ['CPY', 'imm'],
  0x10: ['BPL', 'rel'],
};
...
function disasm(start, count, label) {
  ...
  while (n < count) {
    ...
    const entry = OP[op];
    const m = a - 0xc000;
    if (!entry) { console.log(`$${a.toString(16)}: .byte $${op.toString(16).padStart(2, '0')} ???`); a++; n++; continue; }
    ...
    if (mode === 'imm') { operand = `#$${b[a + 1].toString(16).padStart(2, '0')}`; len = 2; }
      ...
      const lo = b[a + 1], hi = b[a + 2]; const v = (hi << 8) | lo;
      operand = mode === 'absx' ? `$${v.toString(16).padStart(4, '0')},X` : mode === 'absy' ? `$${v.toString(16).padStart(4, '0')},Y` : `$${v.toString(16).padStart(4, '0')}`;
      len = 3;
    }
    else if (mode === 'zp') { operand = `$${b[a + 1].toString(16).padStart(2, '0')}`; len = 2; }
    else if (mode === 'zpx') { operand = `$${b[a + 1].toString(16).padStart(2, '0')},X`; len = 2; }
    else if (mode === 'indy') { operand = `($${b[a + 1].toString(16).padStart(2, '0')}),Y`; len = 2; }
    else if (mode === 'indx') { operand = `($${b[a + 1].toString(16).padStart(2, '0')},X)`; len = 2; }
    else if (mode === 'rel') {
      const off = b[a + 1]; const t = a + 2 + ((off & 0x80) ? off - 0x100 : off);
      operand = `$${t.toString(16).padStart(4, '0')}`; len = 2;
    }
    console.log(`$${(0xc000 + m).toString(16)}: ${name} ${operand}`);
    a += len; n++;
  }
}

disasm(0xcd7c, 20, 'CD7C (C50C 球员记录指针)');
disasm(0xcdc9, 26, 'CDC9 (C536 坐标→tile)');
disasm(0xcde2, 40, 'CDE2 (C539 坐标→OAM槽)');
