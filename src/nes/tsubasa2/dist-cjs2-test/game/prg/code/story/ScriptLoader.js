...
const bank18_data_1 = require("../../data/scene/bank18-data");
const SEGMENT_SIZE = 0x1000; // 每段 4KB
/** 预分割段：按 SEGMENT_SIZE 切分 BANK18_DATA_TABLES */
const SEGMENT_BYTES = (() => {
    const out = [];
    for (let i = 0; i < bank18_data_1.BANK18_DATA_TABLES.length; i += SEGMENT_SIZE) {
        out.push(bank18_data_1.BANK18_DATA_TABLES.slice(i, i + SEGMENT_SIZE));
    }
    return out;
})();
class ScriptLoader {
