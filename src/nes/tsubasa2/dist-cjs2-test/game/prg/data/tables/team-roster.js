...
exports.TEAM_TABLE = exports.TEAM_ROSTER_TABLE = void 0;
exports.findRosterById = findRosterById;
exports.findByEncounterLevel = findByEncounterLevel;
const roster_prg_bytes_1 = require("./roster-prg-bytes");
...
function buildTeam(meta) {
    ...
    const byteTable = isPlayer ? roster_prg_bytes_1.PLAYER_ROSTER_PRG_BYTES : roster_prg_bytes_1.CPU_ROSTER_PRG_BYTES;
    ...
    if (!meta.formation && !meta.tactic) {
        const tb = roster_prg_bytes_1.CPU_ROSTER_TACTIC_BYTES[meta.id];
        if (tb !== undefined) {
    ...
    return {
        ...
        anchorVerified: roster_prg_bytes_1.ANCHOR_VERIFIED_TEAMS.has(meta.id),
    };
}
exports.TEAM_ROSTER_TABLE = TEAM_META.map(buildTeam);
/** 按 ID 找队伍 */
function findRosterById(id) {
    for (const t of exports.TEAM_ROSTER_TABLE) {
        if (t.id === (id & 0xff))
            return t;
    }
    return null;
...
/** 按关号找该关对手 (第一支) */
function findByEncounterLevel(level) {
    for (const t of exports.TEAM_ROSTER_TABLE) {
        if (t.encounterLevels.includes(level))
            return t;
    }
    return null;
}
/** 兼容 team-table.ts 的别名 */
exports.TEAM_TABLE = exports.TEAM_ROSTER_TABLE;
